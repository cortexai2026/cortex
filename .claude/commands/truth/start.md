...
cortex-agent truth --period 7d

# Export to HTML report
cortex-agent truth --export report.html --format html
```

### Advanced Analysis
```bash
# Monitor real-time scores
cortex-agent truth --watch

# Find problematic files
cortex-agent truth --threshold 0.8

# Agent-specific metrics
cortex-agent truth --agent coder --period 24h

# JSON for processing
cortex-agent truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    cortex-agent truth --format json > truth.json
...
cortex-agent truth --format json | \
...
Set truth display preferences in `.cortex-agent/config.json`:
