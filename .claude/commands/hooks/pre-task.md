...
```bash
npx cortex-agent hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
npx cortex-agent hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
npx cortex-agent hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
npx cortex-agent hook pre-task --description "Your task here"
```
