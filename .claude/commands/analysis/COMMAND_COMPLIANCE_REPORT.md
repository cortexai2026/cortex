...
- `mcp__cortex-agent__*` tools (preferred)
- `npx cortex-agent` commands (as fallback)
...
- Updated to: `Tool: mcp__cortex-agent__token_usage` with appropriate parameters
...
**After**:
```
Tool: mcp__cortex-agent__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__cortex-agent__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__cortex-agent__*` format
...
The analysis directory is now fully compliant with the Cortex Agent command standards.