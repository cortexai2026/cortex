#!/bin/bash
# Cortex Agent V3 Development Status Line
...
V3_METRICS="${PROJECT_DIR}/.cortex-agent/metrics/v3-progress.json"
SECURITY_AUDIT="${PROJECT_DIR}/.cortex-agent/security/audit-status.json"
PERFORMANCE_METRICS="${PROJECT_DIR}/.cortex-agent/metrics/performance.json"
...
ACTIVE_PROCESSES=$(ps aux 2>/dev/null | grep -E "(agentic-flow|cortex-agent)" | grep -v grep | wc -l)

# Check for real-time activity data from swarm monitor
SWARM_ACTIVITY=".cortex-agent/metrics/swarm-activity.json"
...
CONTEXT_COLOR="${DIM}"
AUTOPILOT_STATUS=""
AUTOPILOT_STATE="${PROJECT_DIR}/.cortex-agent/data/autopilot-state.json"
...
INTEL_SCORE=0
INTEL_COLOR="${DIM}"
PATTERNS_DB="${PROJECT_DIR}/.cortex-agent/learning/patterns.db"
LEARNING_METRICS="${PROJECT_DIR}/.cortex-agent/metrics/learning.json"
ARCHIVE_DB="${PROJECT_DIR}/.cortex-agent/data/transcript-archive.db"
...
CURRENT_DIR=$(basename "$PROJECT_DIR" 2>/dev/null || echo "cortex-agent")

# Build colorful output with better formatting
OUTPUT=""

# Header Line: V3 Project + Branch + Integration Status
OUTPUT="${BOLD}${BRIGHT_PURPLE}▊ Cortex Agent V3 ${RESET}"
