...
mcp__flow-nexus__execution_stream_subscribe({
  stream_type: "cortex-agent-swarm",
...
Flow Nexus swarms integrate seamlessly with Cortex Agent hooks:

```bash
# Pre-task coordination setup
npx cortex-agent@alpha hooks pre-task --description "Initialize swarm"

# Post-task metrics export
npx cortex-agent@alpha hooks post-task --task-id "swarm-execution"
```
...
**Remember**: Flow Nexus provides cloud-based orchestration infrastructure. For local execution and coordination, use the core `cortex-agent` MCP server alongside Flow Nexus for maximum flexibility.
