...
```bash
# List all results
npx cortex-agent@v3alpha memory list --namespace {{namespace}}

# Search for specific patterns
npx cortex-agent@v3alpha memory search -q "{{filter}}" -n {{namespace}}

# Get detailed entries
{{#each results}}
npx cortex-agent@v3alpha memory get -k "{{this.key}}" -n {{namespace}}
{{/each}}
```
