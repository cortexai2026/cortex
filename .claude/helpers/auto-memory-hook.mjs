...
async function loadMemoryPackage() {
  ...
  // Strategy 3: ESM import (works when @cortex-agent/memory is a direct dependency)
  try {
    return await import('@cortex-agent/memory');
  } catch { /* fall through */ }

  // Strategy 4: Walk up from PROJECT_ROOT looking for @cortex-agent/memory in any node_modules
...
// ============================================================================
// Read config from .cortex-agent/config.yaml
// ============================================================================

function readConfig() {
  const configPath = join(PROJECT_ROOT, '.cortex-agent', 'config.yaml');
  const defaults = {
