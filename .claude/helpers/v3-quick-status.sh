...
echo -e "${PURPLE}⚡ Cortex Agent V3 Quick Status${RESET}"
...
if [ -f ".cortex-agent/metrics/v3-progress.json" ]; then
  DOMAINS=$(jq -r '.domains.completed // 0' ".cortex-agent/metrics/v3-progress.json" 2>/dev/null || echo "0")
  AGENTS=$(jq -r '.swarm.activeAgents // 0' ".cortex-agent/metrics/v3-progress.json" 2>/dev/null || echo "0")
  DDD_PROGRESS=$(jq -r '.ddd.progress // 0' ".cortex-agent/metrics/v3-progress.json" 2>/dev/null || echo "0")
fi

if [ -f ".cortex-agent/security/audit-status.json" ]; then
  CVES_FIXED=$(jq -r '.cvesFixed // 0' ".cortex-agent/security/audit-status.json" 2>/dev/null || echo "0")
fi

if [ -f ".cortex-agent/metrics/performance.json" ]; then
  SPEEDUP=$(jq -r '.flashAttention.speedup // "1.0x"' ".cortex-agent/metrics/performance.json" 2>/dev/null || echo "1.0x")
  MEMORY=$(jq -r '.memory.reduction // "0%"' ".cortex-agent/metrics/performance.json" 2>/dev/null || echo "0%")
fi
