#!/bin/bash
# Cortex Agent Plugin Installation Script
# Version: 2.5.0
...
╔═══════════════════════════════════════════════════════════╗
║                                                           ║
║          Cortex Agent Plugin Installer v2.5.0             ║
        ...
        cat > "$SETTINGS_FILE" << 'SETTINGS_EOF'
{
  "mcpServers": {
    "cortex-agent": {
      ...
      "description": "Core Cortex Agent MCP server with 40+ orchestration tools"
    }
  }
}
SETTINGS_EOF
        success "Created settings.json with Cortex Agent MCP server"
    else
...
MCP_INSTRUCTIONS
    ...
    if [ "$INSTALL_MCP" = "y" ]; then
        info "Installing cortex-agent MCP server..."
        npx cortex-agent@alpha --version 2>/dev/null || npm install -g cortex-agent@alpha
        success "Cortex Agent MCP server installed"
...
success "Cortex Agent plugin is ready to use!"
echo ""
