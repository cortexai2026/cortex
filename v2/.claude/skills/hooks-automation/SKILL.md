...
**Required:**
- Cortex Agent CLI installed (`npm install -g cortex-agent@alpha`)
- Claude Code with hooks enabled
- `.claude/settings.json` with hook configurations

**Optional:**
- MCP servers configured (cortex-agent, ruv-swarm, flow-nexus)
...
npx cortex-agent hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx cortex-agent hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx cortex-agent hook session-end --session-id "dev-session" --export-metrics
```
...
Examples:
  npx cortex-agent hook pre-edit --file "src/auth/login.js"
  npx cortex-agent hook pre-edit -f "config/db.js" --validate-syntax
  npx cortex-agent hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx cortex-agent hook pre-bash --command <cmd>
...
Examples:
  npx cortex-agent hook pre-bash -c "rm -rf /tmp/cache"
  npx cortex-agent hook pre-bash --command "docker build ." --estimate-resources
```
...
Examples:
  npx cortex-agent hook pre-task --description "Implement user authentication"
  npx cortex-agent hook pre-task -d "Continue API dev" --load-memory
  npx cortex-agent hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx cortex-agent hook pre-search --query <query>
...
Examples:
  npx cortex-agent hook pre-search -q "authentication middleware"
```
...
Examples:
  npx cortex-agent hook post-edit --file "src/components/Button.jsx"
  npx cortex-agent hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx cortex-agent hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx cortex-agent hook post-bash --command <cmd>
...
Examples:
  npx cortex-agent hook post-bash -c "npm test" --update-metrics
```
...
Examples:
  npx cortex-agent hook post-task --task-id "auth-implementation"
  npx cortex-agent hook post-task -t "api-refactor" --analyze-performance
  npx cortex-agent hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx cortex-agent hook post-search --query <query> --results <path>
...
Examples:
  npx cortex-agent hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx cortex-agent hook mcp-initialized --swarm-id <id>
...
```bash
npx cortex-agent hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx cortex-agent hook task-orchestrated --task-id <id>
...
```bash
npx cortex-agent hook neural-trained --pattern <name>
...
```bash
npx cortex-agent hook memory-sync --namespace <ns>
...
```bash
npx cortex-agent hook session-start --session-id <id>
...
```bash
npx cortex-agent hook session-restore --session-id <id>
...
Examples:
  npx cortex-agent hook session-restore --session-id "swarm-20241019"
  npx cortex-agent hook session-restore -s "feature-auth" --restore-memory
```
...
Examples:
  npx cortex-agent hook session-end --session-id "dev-session-2024"
  npx cortex-agent hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx cortex-agent hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx cortex-agent hook notify --message <msg>
...
Examples:
  npx cortex-agent hook notify -m "Task completed" --level info
  npx cortex-agent hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx cortex-agent hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx cortex-agent hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx cortex-agent hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx cortex-agent hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx cortex-agent hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx cortex-agent hook post-edit --file "api/auth.js"
...
mcp__cortex-agent__neural_train {
...
```javascript
// Hook command
npx cortex-agent hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__cortex-agent__memory_persist {
  sessionId: "dev-2024"
}
...
for FILE in $FILES; do
  ...
  npx cortex-agent hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx cortex-agent hook post-edit --file "$FILE" --auto-format
done
...
TRUTH_SCORE=$(npx cortex-agent metrics score --format json | jq -r '.truth_score')
...
npx cortex-agent hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx cortex-agent hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx cortex-agent hook post-edit \
  --file "api/auth.js" \
...
npx cortex-agent hook post-task \
  --task-id "auth-api" \
  --analyze-performance \
  --store-decisions \
  --export-learnings
...
npx cortex-agent hook session-restore \
...
npx cortex-agent hook session-start --session-id "fullstack-feature"
...
# Backend work
npx cortex-agent hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx cortex-agent hook post-edit \
  --file "api/profile.js" \
...
npx cortex-agent hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx cortex-agent hook post-edit \
...
npx cortex-agent hook session-start --session-id "debug-memory-leak"
...
npx cortex-agent hook pre-search --query "EventEmitter"
# ... search executes ...
npx cortex-agent hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx cortex-agent hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx cortex-agent hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx cortex-agent hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx cortex-agent hook post-edit \
    --file "$file" \
...
npx cortex-agent hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .cortex-agent/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx cortex-agent hook validate-config
```
...
- Ensure cortex-agent CLI is in PATH
...
- `npx cortex-agent init --hooks` - Initialize hooks system
- `npx cortex-agent hook --list` - List available hooks
- `npx cortex-agent hook --test <hook>` - Test specific hook
- `npx cortex-agent memory usage` - Manage memory
- `npx cortex-agent agent spawn` - Spawn agents
- `npx cortex-agent swarm init` - Initialize swarm
