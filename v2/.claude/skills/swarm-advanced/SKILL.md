...
### Prerequisites
```bash
# Ensure Cortex Agent is installed
...
mcp__cortex-agent__agent_spawn({ type: "researcher", name: "Agent 1" })

// 3. Orchestrate tasks
mcp__cortex-agent__task_orchestrate({ task: "...", strategy: "parallel" })
```
...
npx cortex-agent swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
npx cortex-agent swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
npx cortex-agent swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
try {
  await mcp__cortex-agent__task_orchestrate({
  ...
  // Check swarm health
  const status = await mcp__cortex-agent__swarm_status({})

  // Analyze error patterns
  await mcp__cortex-agent__error_analysis({
    "logs": [error.message]
  })
...
mcp__cortex-agent__swarm_init({ topology: "star", maxAgents: 5 })
...
mcp__cortex-agent__swarm_init({ topology: "mesh", maxAgents: 4 })
...
- [Cortex Agent Documentation](https://github.com/ruvnet/cortex-agent)
- [Swarm Orchestration Guide](https://github.com/ruvnet/cortex-agent/wiki/swarm)
- [MCP Tools Reference](https://github.com/ruvnet/cortex-agent/wiki/mcp)
- [Performance Optimization](https://github.com/ruvnet/cortex-agent/wiki/performance)
