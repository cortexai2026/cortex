# Known Issues with Cortex Agent Benchmark System

## Issue 1: Non-Interactive Mode Not Working in Cortex Agent

### Problem
The `--non-interactive` flag is not being properly handled by cortex-agent commands, particularly:
...
```bash
./cortex-agent hive-mind spawn "Task description" --count 2 --coordination majority --non-interactive
```

However, the cortex-agent implementation needs to be updated to:
