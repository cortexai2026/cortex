...
### 🎯 Mission Statement
Transform the benchmark system into a state-of-the-art performance testing suite that leverages Cortex Agent's full capabilities, including MLE-STAR ensemble learning, non-interactive automation, CLAUDE.md optimization, and advanced swarm orchestration.
...
class MLEStarEnsembleExecutor:
    ...
    async def _spawn_model_agent(self, 
        ...
        # Use cortex-agent MCP to spawn ML agent
...
```bash
npx cortex-agent@alpha swarm init \
...
```bash
npx cortex-agent@alpha swarm execute \
...
```bash
npx cortex-agent@alpha swarm report \
