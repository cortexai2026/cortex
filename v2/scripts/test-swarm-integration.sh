...
if [ -f "./bin/cortex-agent" ]; then
    echo "✅ cortex-agent binary found"
else
    echo "❌ cortex-agent binary not found"
    exit 1
fi

if [ -f "./bin/cortex-agent-swarm" ]; then
    echo "✅ cortex-agent-swarm wrapper found"
else
    echo "❌ cortex-agent-swarm wrapper not found"
    exit 1
fi

# Test 2: Help command
echo -e "\nTest 2: Testing help command..."
if ./bin/cortex-agent help swarm | grep -q "Claude Swarm Mode"; then
...
if ./bin/cortex-agent --help | grep -q "swarm"; then
...
if ./bin/cortex-agent-swarm "Test objective" --dry-run | grep -q "DRY RUN"; then
...
OUTPUT=$(./bin/cortex-agent-swarm "Complex test" --strategy research --max-agents 10 --coordinator --review --parallel --dry-run)
...
echo "  1. ./bin/cortex-agent swarm \"Your objective\" [options]"
echo "  2. ./bin/cortex-agent-swarm \"Your objective\" [options]"
echo "  3. npx cortex-agent swarm \"Your objective\" [options]"
echo
echo "For more info: ./bin/cortex-agent help swarm"