...
const CORTEX_AGENT_CONFIG_SCHEMA: ConfigSchema = {
  orchestrator: {
    type: 'object',
    required: true,
    properties: {
...
function validateCortexAgentConfig(config: any): string[] {
  return validateObject(config, CORTEX_AGENT_CONFIG_SCHEMA);
}
...
async function validateFile(filePath: string): Promise<ValidationResult> {
  ...
  try {
    ...
    if (filePath.includes('workflow') || config.tasks) {
      ...
      result.errors = validateCortexAgentConfig(config);
    } else {
...
async function main(): Promise<void> {
  ...
  const rootConfigFiles = ['cortex-agent.config.json', 'config.json'];
