...
Multi-agent systems in Cortex Agent allow you to:
...
```bash
../cortex-agent swarm create "Build complete e-commerce platform with user management, product catalog, and frontend" --agents 5 --strategy development --output ./output/ecommerce --monitor
```

### Step 4: Understanding Coordination Modes

Cortex Agent supports different coordination patterns:

#### Hub-Spoke Pattern
Central coordinator manages all agents:

```bash
../cortex-agent orchestrate ./workflow.json \
...
```bash
../cortex-agent orchestrate ./workflow.json --monitor
```

### Debug Mode
```bash
../cortex-agent orchestrate ./workflow.json --debug
```

### Performance Metrics
```bash
../cortex-agent orchestrate ./workflow.json --metrics
```
