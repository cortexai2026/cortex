# Your First Cortex Agent Swarm

This tutorial will guide you through creating your first swarm and understanding how Cortex Agent works.

## Prerequisites

- Cortex Agent installed (`npm install -g cortex-agent`)
...
```bash
cd examples
../cortex-agent swarm create "Build a hello world CLI application" \
  --name my-first-swarm \
  --output ./output/hello-world
```

### What happens:
1. Cortex Agent creates a swarm coordinator
...
2. Use configuration files:
   ```bash
   ../cortex-agent swarm create "Your task" \
     --config ../01-configurations/basic/simple-config.json
   ```

3. Monitor swarm progress:
   ```bash
   ../cortex-agent swarm create "Your task" --monitor
   ```
