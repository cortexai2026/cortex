...
Learn how to use SPARC methodology for systematic TDD with Cortex Agent.
...
- Cortex Agent with SPARC modes configured
...
```bash
cd examples
../cortex-agent sparc run spec-pseudocode \
...
```bash
../cortex-agent sparc run architect \
...
```bash
../cortex-agent sparc run integration \
...
```bash
../cortex-agent sparc run security-review \
...
```bash
../cortex-agent sparc run parallel \
...
../cortex-agent memory store auth_spec "$(cat ./output/auth-spec.md)"

# Retrieve for future work
../cortex-agent memory query auth_spec > ./restored-spec.md
```
...
### Authentication Flow
```bash
../cortex-agent sparc tdd "implement login flow" \
  --pattern "request-validate-authenticate-respond"
```

### CRUD Operations
```bash
../cortex-agent sparc tdd "implement user CRUD" \
  --pattern "rest-api" \
  --with-validation
```
...
```bash
../cortex-agent sparc tdd "implement event handlers" \
  --pattern "pub-sub" \
  --async
```
