#!/bin/bash
# SPARC TDD Test Suite for Cortex Agent Swarm CLI
# Tests all strategies and features systematically

echo "🔬 SPARC TDD Test Suite for Cortex Agent Swarm"
echo "============================================="
echo ""
...
run_test "Development Strategy" \
    "npx cortex-agent@latest swarm 'build a calculator app' --strategy development --dry-run --max-agents 4 2>&1 | grep -q 'Strategy: development'"

run_test "Research Strategy" \
    "npx cortex-agent@latest swarm 'research AI trends' --strategy research --dry-run 2>&1 | grep -q 'Strategy: research'"

run_test "Analysis Strategy" \
    "npx cortex-agent@latest swarm 'analyze codebase' --strategy analysis --dry-run 2>&1 | grep -q 'Strategy: analysis'"

run_test "Testing Strategy" \
    "npx cortex-agent@latest swarm 'create test suite' --strategy testing --dry-run 2>&1 | grep -q 'Strategy: testing'"

run_test "Optimization Strategy" \
    "npx cortex-agent@latest swarm 'optimize performance' --strategy optimization --dry-run 2>&1 | grep -q 'Strategy: optimization'"

# Mode Tests
echo -e "\n${YELLOW}🏗️  MODE TESTS${NC}"

run_test "Centralized Mode" \
    "npx cortex-agent@latest swarm 'test task' --mode centralized --dry-run 2>&1 | grep -q 'Mode: centralized'"

run_test "Distributed Mode" \
    "npx cortex-agent@latest swarm 'test task' --mode distributed --distributed --dry-run 2>&1 | grep -q 'Mode: distributed'"

run_test "Hierarchical Mode" \
    "npx cortex-agent@latest swarm 'test task' --mode hierarchical --dry-run 2>&1 | grep -q 'Mode: hierarchical'"

# Feature Tests
echo -e "\n${YELLOW}⚡ FEATURE TESTS${NC}"

run_test "Parallel Execution" \
    "npx cortex-agent@latest swarm 'parallel task' --parallel --dry-run 2>&1 | grep -q 'Parallel: true'"

run_test "Monitoring Feature" \
    "npx cortex-agent@latest swarm 'monitored task' --monitor --dry-run 2>&1 | grep -q 'Monitoring: true'"

run_test "Quality Threshold" \
    "npx cortex-agent@latest swarm 'quality task' --quality-threshold 0.95 --dry-run 2>&1 | grep -q 'Quality Threshold: 0.95'"

run_test "Memory Namespace" \
    "npx cortex-agent@latest swarm 'memory task' --memory-namespace custom-space --dry-run 2>&1 | grep -q 'Memory Namespace: custom-space'"

run_test "Max Agents Configuration" \
    "npx cortex-agent@latest swarm 'agent task' --max-agents 10 --dry-run 2>&1 | grep -q 'Max Agents: 10'"

# Advanced Configuration Tests
echo -e "\n${YELLOW}🎛️  ADVANCED CONFIGURATION TESTS${NC}"

run_test "Task Scheduling Options" \
    "npx cortex-agent@latest swarm 'scheduled task' --task-scheduling round-robin --dry-run 2>&1 | grep -q 'Task Scheduling: round-robin'"

run_test "Load Balancing Options" \
    "npx cortex-agent@latest swarm 'balanced task' --load-balancing random --dry-run 2>&1 | grep -q 'Load Balancing: random'"

# Help Test
echo -e "\n${YELLOW}📚 HELP TEST${NC}"

run_test "Help Command" \
    "npx cortex-agent@latest swarm --help 2>&1 | grep -q 'Cortex Agent Advanced Swarm System'"
...
#!/bin/bash
npx cortex-agent@latest swarm "build a todo app with database" \
...
if [ $FAILED -eq 0 ]; then
    ...
    echo ""
    echo "The Cortex Agent Swarm system successfully:"
...
echo ""
echo "  npx cortex-agent@latest swarm \"create a REST API\" --strategy development"
echo ""
