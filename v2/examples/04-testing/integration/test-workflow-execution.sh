...
echo "🔄 Cortex Agent Workflow Integration Test"
echo "======================================="
echo ""
...
if ../cortex-agent orchestrate "$WORKFLOW_FILE" --output "$OUTPUT_DIR" --quiet; then
...
../cortex-agent memory store "$MEMORY_KEY" "Workflow test data" >/dev/null 2>&1
test_step "Memory storage during workflow" "../cortex-agent memory query '$MEMORY_KEY' >/dev/null 2>&1"
...
if [[ -f "$PARALLEL_WORKFLOW" ]]; then
    ...
    ../cortex-agent orchestrate "$PARALLEL_WORKFLOW" --output "$PARALLEL_OUTPUT" --monitor --quiet &
    PID=$!
...
if ! ../cortex-agent orchestrate "$INVALID_WORKFLOW" --output "$OUTPUT_DIR/error" 2>/dev/null; then
...
sleep 2
TEMP_FILES=$(find /tmp -name "*cortex-agent*$TEST_ID*" 2>/dev/null | wc -l)
...
rm -rf "$OUTPUT_DIR"
../cortex-agent memory delete "$MEMORY_KEY" >/dev/null 2>&1
