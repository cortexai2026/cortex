# Cortex Agent Automation Examples

This document provides practical examples of using Cortex Agent's automation features.
...
cortex-agent automation mle-star \
...
```bash
cortex-agent automation run-workflow web-app-workflow.json \
  --claude \
...
cortex-agent automation run-workflow devops-workflow.json \
...
```bash
# Light workload
cortex-agent automation mle-star --max-agents 3 --claude

# Heavy workload
cortex-agent automation mle-star --max-agents 12 --claude
```
...
jobs:
  ml-training:
    ...
    steps:
      ...
      - name: Install Cortex Agent
        run: npm install -g cortex-agent@alpha
...
CMD ["cortex-agent", "automation", "mle-star", \
  ...
  template:
    spec:
      ...
      - name: ml-trainer
        ...
        command: ["cortex-agent", "automation", "mle-star"]
        args:
          - "--dataset"
          - "/data/dataset.csv"
          - "--non-interactive"
...
cortex-agent automation run-workflow my-workflow.json \
  --output-format json \
...
```bash
# Development
cortex-agent automation run-workflow workflow.json \
...
These examples demonstrate the full power and flexibility of Cortex Agent's automation system. Start with simple examples and gradually build more complex workflows as you become familiar with the system.