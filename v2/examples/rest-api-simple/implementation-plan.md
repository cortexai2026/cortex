...
**Command**:
```bash
npx cortex-agent sparc run code "Setup Express server with health endpoint for rest-api-simple" --non-interactive
```
...
**Command**:
```bash
npx cortex-agent sparc run tdd "Implement CRUD operations for tasks with full test coverage" --non-interactive
```
...
**Command**:
```bash
npx cortex-agent sparc run code "Add input validation and error handling to REST API" --non-interactive
```
...
**Command**:
```bash
npx cortex-agent sparc run docs-writer "Create comprehensive documentation for simple REST API" --non-interactive
```
...
**Command**:
```bash
npx cortex-agent sparc run security-review "Review REST API for security best practices" --non-interactive
```
...
npx cortex-agent swarm "Implement complete simple REST API based on architecture" \
