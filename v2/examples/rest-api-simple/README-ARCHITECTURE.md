...
```bash
# Phase 1: Setup
npx cortex-agent sparc run code "Setup Express server with health endpoint" --non-interactive

# Phase 2: CRUD with TDD
npx cortex-agent sparc run tdd "Implement CRUD operations for tasks" --non-interactive

# Phase 3: Validation
npx cortex-agent sparc run code "Add input validation and error handling" --non-interactive

# Phase 4: Documentation
npx cortex-agent sparc run docs-writer "Create REST API documentation" --non-interactive

# Phase 5: Security Review
npx cortex-agent sparc run security-review "Review API security" --non-interactive
```

### Option 2: Parallel Swarm Execution (~15 minutes)
```bash
# Complete implementation with parallel agents
npx cortex-agent swarm "Build complete simple REST API from architecture docs" \
