# Cortex Agent Browser Dashboard - Proof of Concept

## Overview

A browser-based real-time swarm orchestration dashboard that demonstrates WebAssembly potential for Cortex Agent. This proof of concept shows how Cortex Agent's multi-agent coordination could run entirely in the browser using WebSocket transport instead of stdio.
...
Run the WebSocket bridge server to connect to real Cortex Agent MCP tools.
...
{
  "jsonrpc": "2.0",
  "method": "mcp__cortex-agent__agents_spawn_parallel",
  "params": {
    "agents": [
...
This proof of concept demonstrates how Cortex Agent could leverage the SDK's `wasm32` compilation target:
...
To integrate with real Cortex Agent MCP tools, the server would:

1. **Import MCP Tools**:
```javascript
const { createSdkMcpServer, tool } = require('@anthropic-ai/claude-code');
const claudeFlow = require('cortex-agent');
```

2. **Create MCP Bridge**:
```javascript
const mcpServer = createSdkMcpServer({
  name: 'cortex-agent-websocket',
  tools: [
...
- [ ] Connect to actual cortex-agent MCP tools
...
MIT - Same as Cortex Agent

## Credits

Built as proof of concept for Cortex Agent v2.5.0-alpha.130+ demonstrating WebAssembly potential for browser-based multi-agent orchestration.
