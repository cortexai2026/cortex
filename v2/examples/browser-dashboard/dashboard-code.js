/**
 * Cortex Agent + Claude Code - Browser IDE
...
class ClaudeCodeDashboard {
    ...
    getDefaultCode() {
        return `// Cortex Agent + Claude Code - Browser IDE
// Write your code here and click "Run Code" to execute via MCP

console.log('Hello from Cortex Agent!');
...
`;
    ...
    connectWebSocket(url = 'ws://localhost:8080') {
        try {
            ...
            this.ws.onopen = () => {
                ...
                this.addLog('Connected to Cortex Agent MCP server', 'success');
    ...
    sendCommand(command, params = {}) {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            const message = {
                jsonrpc: '2.0',
                method: `mcp__cortex-agent__${command}`,
                params,
                id: Date.now()
            };
    ...
    executeCode() {
        ...
        this.addTerminalOutput('> Executing code via Cortex Agent MCP...\n', 'command');
