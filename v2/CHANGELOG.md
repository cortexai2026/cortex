...
- **MCP 2025-11 Specification Compliance** - Full Phase A & B implementation
  ...
  - Enable with: `npx cortex-agent mcp start --mcp2025`
...
**Issue**: Users unable to install cortex-agent due to non-existent dependency
```
...
```bash
# Build and test
docker build -f tests/docker/Dockerfile.v2.7.29-test -t cortex-agent-v2.7.29-test .
docker run --rm cortex-agent-v2.7.29-test
...
```bash
# Build test image
docker build -f tests/docker/Dockerfile.init-test -t cortex-agent-init-test:v2.7.28 .

# Run tests
docker run --rm cortex-agent-init-test:v2.7.28
```
...
- **cortex-agent**: Core orchestration - always needed
...
```bash
npm update -g cortex-agent

# Verify version
cortex-agent --version  # Should show v2.7.28
```

### 🔗 Related Documentation

- **Issue**: [#857 - Remove automatic agentic-payments installation](https://github.com/ruvnet/cortex-agent/issues/857)
...
### Summary
Fixed NPM ENOTEMPTY errors occurring during npx cortex-agent execution, particularly after recent agentic-flow module updates. Implemented automatic retry logic with exponential backoff and intelligent cache cleanup.

### 🐛 Bug Fixed

**Issue**: NPM encounters ENOTEMPTY errors when npx tries to install cortex-agent
...
#### 1. **Automatic Retry Logic** (`bin/cortex-agent:62-101`)
...
#### 2. **Cache Cleanup Function** (`bin/cortex-agent:53-59`)
...
#### 3. **NPX Optimization Flags** (`bin/cortex-agent:113`)
...
```bash
# Build test image
docker build -f tests/docker/Dockerfile.npx-test -t cortex-agent-npx-test:v2.7.27 .

# Run tests
docker run --rm cortex-agent-npx-test:v2.7.27
```

### 🔧 Technical Details

**Files Modified**:
- `bin/cortex-agent` - Added retry logic and cache cleanup
...
npx cortex-agent@latest --version  # Should show v2.7.27

# Global install users
npm update -g cortex-agent
cortex-agent --version  # Should show v2.7.27
...
- **Issue**: [#856 - NPX ENOTEMPTY error after v2.7.26 updates](https://github.com/ruvnet/cortex-agent/issues/856)
...
$ npx cortex-agent@2.7.7 mcp start
✅ Starting Cortex Agent MCP server...  # <- ON STDOUT (BAD!)
{"jsonrpc":"2.0",...}

# After v2.7.8 - stdout is clean:
$ npx cortex-agent@2.7.8 mcp start
...
- **Fixed**: Skills copier path resolution in both `bin/init/skills-copier.js` and `src/cli/simple-commands/init/skills-copier.js`
  ...
  - All 21 built-in skills properly initialize during `npx cortex-agent init`
...
- **Fixed**: Statusline script creation with proper bash variable escaping and missing imports
  ...
  - Added missing `path` and `os` module imports in `enhancedCortexAgentInit()` function
...
- **Skills Integration**: `npx cortex-agent init` now copies 21 built-in skills to `.claude/skills/`
...
- **Updated**: Integration guide for cortex-agent + agentic-flow
...
```
Claude Code
  ├─ cortex-agent Skills (21 Built-In)
...
2. `bin/cortex-agent` - Version: 2.7.0-alpha.11
...
**For Existing Users:**
1. Update to alpha.11: `npm install -g cortex-agent@alpha`
2. Run init: `npx cortex-agent init` (copies skills to `.claude/skills/`)
3. MCP server: `claude mcp add cortex-agent npx cortex-agent@alpha mcp start`
...
2. `bin/cortex-agent` - Version: 2.7.0-alpha.10
...
**Before (alpha.9):**
```bash
$ npx cortex-agent@alpha memory query "config" --namespace semantic --reasoningbank
...
npx cortex-agent@alpha memory store test "validation data" --namespace semantic --reasoningbank
npx cortex-agent@alpha memory query "validation" --namespace semantic --reasoningbank
...
- **npm Package**: [cortex-agent@2.7.0-alpha.10](https://www.npmjs.com/package/cortex-agent/v/2.7.0-alpha.10)
...
- **Sublinear-time-solver Integration**: Removed sublinear-time-solver MCP server from initialization
  ...
  - Simplified initialization to focus on core MCP servers (cortex-agent, ruv-swarm, flow-nexus)
  - Updated help documentation to remove sublinear references

### ✨ Improved
- **Cleaner Initialization**: `npx cortex-agent init` now sets up only core, stable MCP servers
...
- Updated CLAUDE.md templates with cortex-agent MCP tools instead of sublinear examples
...
- **Neural Module**: `npx cortex-agent@alpha neural init` now creates only the essential `safla-neural.md` agent file
...
- **Goal Module**: `npx cortex-agent@alpha goal init` now creates only the essential `goal-planner.md` agent file
  - Removed unnecessary `config.json` generation
  - Removed unnecessary `README.md` generation
...
- `bin/cortex-agent.js` - Version display and release notes
...
5. **Use coordination hooks**: Every agent must use cortex-agent hooks
...
### 🔗 Links
- [npm Package](https://www.npmjs.com/package/cortex-agent/v/2.0.0-alpha.90)
- [Pull Request #661](https://github.com/ruvnet/cortex-agent/pull/661)
- [Issue #660](https://github.com/ruvnet/cortex-agent/issues/660)
...
- **Full Interactive Implementation** (Fixed compilation issues):
  ...
  - Session data persistence in `.cortex-agent/sessions/pair/`
...
- **Stream Chain Command Wiki**: Created `/cortex-agent-wiki/Stream-Chain-Command.md`
  - Complete command reference with all subcommands
  - Background execution guide
  - Performance characteristics
  - Integration with other Cortex Agent features
...
- `/cortex-agent-wiki/Stream-Chain-Command.md` - Wiki documentation
...
- `/.cortex-agent/agents/profiles.json` - Updated with real execution metrics
...
- **Reduced NPM Package Size by 31%**:
  - Excluded unnecessary `bin/cortex-agent-node-pkg` binary (45MB) from npm package
...
- **Real Performance Analytics**: ALL analysis commands now use real data
  - `cortex-agent analysis performance-report` - Real task execution metrics
  - `cortex-agent analysis bottleneck-detect` - Actual system bottleneck detection
...
- **Optional Monitoring During Init**:
  - `cortex-agent init --monitoring` sets up token tracking
  - Creates `.cortex-agent/` directory with tracking configuration
...
- **Token Usage Tracking**:
  ...
  - Project-specific tracking (`.cortex-agent/token-usage.json`)
...
- **TBench Integration**: Added comprehensive Terminal Bench support
  - Created `CortexAgentInstalledAgent` implementation
...
### 🔧 Critical Fix
- **Init Template**: Fixed `cortex-agent init` creating broken settings.json with xargs quote errors
  ...
  - All new projects initialized with `cortex-agent init` now have working hooks
...
- Ensures `./cortex-agent` wrapper always uses the most recent alpha release
...
- **Advanced swarm coordination commands** with `npx cortex-agent swarm`
...
2. **Install v2.0.0**:
   ```bash
   npm install -g cortex-agent@2.0.0
   ```

3. **Verify installation**:
   ```bash
   cortex-agent --version  # Should show 2.0.0
   cortex-agent --help     # Verify all commands available
   ```
...
# Spawn agents
npx cortex-agent agent spawn researcher
npx cortex-agent agent spawn coder

# Orchestrate tasks
npx cortex-agent task orchestrate "complex development task"
```

#### GitHub Integration
```bash
# Automated PR management
npx cortex-agent github pr-manager "review and merge feature branch"

# Issue tracking
npx cortex-agent github issue-tracker "manage project issues"

# Release coordination
npx cortex-agent github release-manager "prepare v2.0.0 release"
```
