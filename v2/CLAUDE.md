...
[Single Message - Complete Swarm Spawn]:
  ...
  mcp__cortex-agent__swarm_init { topology: "hierarchical-mesh", maxAgents: 15, strategy: "adaptive" }
  mcp__cortex-agent__agent_spawn { type: "coordinator", name: "queen" }
  mcp__cortex-agent__task_orchestrate { task: "[user's task]", strategy: "adaptive", priority: "high" }
  mcp__cortex-agent__memory_usage { action: "store", namespace: "swarm", key: "session", value: "..." }
...
npx cortex-agent@alpha hooks session-start --auto-configure
npx cortex-agent@alpha memory store --key "session/config" --value '{"topology":"hierarchical-mesh","maxAgents":15}'
```
...
[Auto-Init on Complex Task]:
  ...
  mcp__cortex-agent__features_detect { component: "task-analyzer" }

  // Initialize if complex
  mcp__cortex-agent__swarm_init { topology: "hierarchical-mesh" }
  mcp__cortex-agent__memory_usage { action: "store", key: "session/active", value: "true" }
...
[During Task - Memory Coordination]:
  ...
  mcp__cortex-agent__memory_usage { action: "store", key: "task/decisions", value: "[decisions]" }

  // Track progress
  mcp__cortex-agent__task_status { taskId: "[current-task]" }

  // Neural learning from patterns
  mcp__cortex-agent__neural_patterns { action: "learn", operation: "[what was done]", outcome: "[result]" }
```
...
[Auto-Cleanup]:
  // Persist learnings
  mcp__cortex-agent__memory_persist { sessionId: "[session-id]" }

  // Export metrics
  mcp__cortex-agent__swarm_status { verbose: true }

  // Train neural patterns
  mcp__cortex-agent__neural_train { pattern_type: "coordination", training_data: "[session-summary]" }
```
