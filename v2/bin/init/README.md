...
This directory contains the modular implementation of the `cortex-agent init` command, which initializes Claude Code integration files for projects.
...
└── claude-commands/             # Claude Code slash commands
    ...
    └── cortex-agent-commands.js  # Claude-Flow specific commands
```
...
2. **Memory System**:
   ...
   - `memory/cortex-agent-data.json` - Persistence database
...
5. **Slash Commands Created**:
   ...
   - `/cortex-agent-help` - Help command
   - `/cortex-agent-memory` - Memory system command
   - `/cortex-agent-swarm` - Swarm coordination command

6. **Local Executable**:
   - `./cortex-agent` (Unix/Mac/Linux)
   - `cortex-agent.cmd` (Windows)
...
npx cortex-agent@latest init --sparc

# Minimal setup
npx cortex-agent init --minimal

# Force overwrite existing files
npx cortex-agent init --force
```
