...
### Core SPARC Commands
- `npx cortex-agent@alpha sparc modes`: List all available SPARC development modes
- `npx cortex-agent@alpha sparc run <mode> "<task>"`: Execute specific SPARC mode for a task
- `npx cortex-agent@alpha sparc tdd "<feature>"`: Run complete TDD workflow using SPARC methodology
- `npx cortex-agent@alpha sparc info <mode>`: Get detailed information about a specific mode

### Batchtools Commands (Optimized)
- `npx cortex-agent@alpha sparc batch <modes> "<task>"`: Execute multiple SPARC modes in parallel
- `npx cortex-agent@alpha sparc pipeline "<task>"`: Execute full SPARC pipeline with parallel processing
- `npx cortex-agent@alpha sparc concurrent <mode> "<tasks-file>"`: Process multiple tasks concurrently
...
npx cortex-agent@alpha sparc run spec-pseudocode "Define user authentication requirements" --parallel
```
...
npx cortex-agent@alpha sparc run spec-pseudocode "Create authentication flow pseudocode" --batch-optimize
```
...
npx cortex-agent@alpha sparc run architect "Design authentication service architecture" --parallel
```
...
npx cortex-agent@alpha sparc tdd "implement user authentication system" --batch-tdd
```
...
npx cortex-agent@alpha sparc run integration "integrate authentication with user management" --parallel
```
...
### 🧠 Cortex Agent MCP Tools ONLY Handle:
...
1. **MCP**: `mcp__cortex-agent@alpha__swarm_init` (coordination setup)
2. **MCP**: `mcp__cortex-agent@alpha__agent_spawn` (planning agents)
3. **MCP**: `mcp__cortex-agent@alpha__task_orchestrate` (task coordination)
...
7. **MCP**: `mcp__cortex-agent@alpha__memory_usage` (store results)

**❌ WRONG Workflow:**

1. **MCP**: `mcp__cortex-agent@alpha__terminal_execute` (DON'T DO THIS)
...
[Single Message with BatchTool]:
  ...
  mcp__cortex-agent@alpha__swarm_init { topology: "mesh", maxAgents: 6 }
  mcp__cortex-agent@alpha__agent_spawn { type: "researcher" }
  mcp__cortex-agent@alpha__agent_spawn { type: "coder" }
  mcp__cortex-agent@alpha__agent_spawn { type: "code-analyzer" }
  mcp__cortex-agent@alpha__agent_spawn { type: "tester" }
  mcp__cortex-agent@alpha__agent_spawn { type: "task-orchestrator" }
...
```javascript
Message 1: mcp__cortex-agent@alpha__swarm_init
...
```bash
# Add Cortex Agent MCP server to Claude Code using stdio
claude mcp add cortex-agent@alpha npx cortex-agent@alpha mcp start
```

### 2. Use MCP Tools for Coordination in Claude Code

Once configured, Cortex Agent MCP tools enhance Claude Code's coordination:

**Initialize a swarm:**

- Use the `mcp__cortex-agent@alpha__swarm_init` tool to set up coordination topology
- Choose: mesh, hierarchical, ring, or star
- This creates a coordination framework for Claude Code's work

**Spawn agents:**

- Use `mcp__cortex-agent@alpha__agent_spawn` tool to create specialized coordinators
- Agent types represent different thinking patterns, not actual coders
- They help Claude Code approach problems from different angles

**Orchestrate tasks:**

- Use `mcp__cortex-agent@alpha__task_orchestrate` tool to coordinate complex workflows
...
- `mcp__cortex-agent@alpha__swarm_init` - Set up coordination topology for Claude Code
- `mcp__cortex-agent@alpha__agent_spawn` - Create cognitive patterns to guide Claude Code
- `mcp__cortex-agent@alpha__task_orchestrate` - Break down and coordinate complex tasks

### Monitoring Tools:

- `mcp__cortex-agent@alpha__swarm_status` - Monitor coordination effectiveness
- `mcp__cortex-agent@alpha__agent_list` - View active cognitive patterns
- `mcp__cortex-agent@alpha__agent_metrics` - Track coordination performance
- `mcp__cortex-agent@alpha__task_status` - Check workflow progress
- `mcp__cortex-agent@alpha__task_results` - Review coordination outcomes

### Memory & Neural Tools:

- `mcp__cortex-agent@alpha__memory_usage` - Persistent memory across sessions
- `mcp__cortex-agent@alpha__neural_status` - Neural pattern effectiveness
- `mcp__cortex-agent@alpha__neural_train` - Improve coordination patterns
- `mcp__cortex-agent@alpha__neural_patterns` - Analyze thinking approaches

### GitHub Integration Tools (NEW!):

- `mcp__cortex-agent@alpha__github_swarm` - Create specialized GitHub management swarms
- `mcp__cortex-agent@alpha__repo_analyze` - Deep repository analysis with AI
- `mcp__cortex-agent@alpha__pr_enhance` - AI-powered pull request improvements
- `mcp__cortex-agent@alpha__issue_triage` - Intelligent issue classification
- `mcp__cortex-agent@alpha__code_review` - Automated code review with swarms

### System Tools:

- `mcp__cortex-agent@alpha__benchmark_run` - Measure coordination efficiency
- `mcp__cortex-agent@alpha__features_detect` - Available capabilities
- `mcp__cortex-agent@alpha__swarm_monitor` - Real-time coordination tracking
...
- Tool: `mcp__cortex-agent@alpha__swarm_init`
- Parameters: `{"topology": "mesh", "maxAgents": 5, "strategy": "balanced"}`
- Result: Creates a mesh topology for comprehensive exploration

**Step 2:** Define research perspectives

- Tool: `mcp__cortex-agent@alpha__agent_spawn`
...
- Tool: `mcp__cortex-agent@alpha__task_orchestrate`
...
2. Each agent MUST use Cortex Agent hooks for coordination:
   - `npx cortex-agent@alpha hooks pre-task` before starting
   - `npx cortex-agent@alpha hooks post-edit` after each file operation
   - `npx cortex-agent@alpha hooks notify` to share decisions
...
4. Agents share progress through Cortex Agent memory
...
- Tool: `mcp__cortex-agent@alpha__github_swarm`
...
When using Cortex Agent coordination with Claude Code:
...
Cortex Agent includes powerful hooks that automate coordination:
...
4. **ALL AGENTS MUST USE COORDINATION TOOLS** - Every spawned agent MUST use cortex-agent@alpha hooks and memory

### 🎯 AGENT COUNT CONFIGURATION

**CRITICAL: Dynamic Agent Count Rules**

1. **Check CLI Arguments First**: If user runs `npx cortex-agent@alpha --agents 5`, use 5 agents
...
```javascript
// If CLI args provided: npx cortex-agent@alpha --agents 6
...
npx cortex-agent@alpha hooks pre-task --description "[agent task]" --auto-spawn-agents false
npx cortex-agent@alpha hooks session-restore --session-id "swarm-[id]" --load-memory true
```
...
npx cortex-agent@alpha hooks post-edit --file "[filepath]" --memory-key "swarm/[agent]/[step]"

# Store decisions and findings
npx cortex-agent@alpha hooks notify --message "[what was done]" --telemetry true

# Check coordination with other agents
npx cortex-agent@alpha hooks pre-search --query "[what to check]" --cache-results true
```
...
npx cortex-agent@alpha hooks post-task --task-id "[task]" --analyze-performance true
npx cortex-agent@alpha hooks session-end --export-metrics true --generate-summary true
```
...
1. START: Run `npx cortex-agent@alpha hooks pre-task --description "[your task]"`
2. DURING: After EVERY file operation, run `npx cortex-agent@alpha hooks post-edit --file "[file]" --memory-key "agent/[step]"`
3. MEMORY: Store ALL decisions using `npx cortex-agent@alpha hooks notify --message "[decision]"`
4. END: Run `npx cortex-agent@alpha hooks post-task --task-id "[task]" --analyze-performance true`
...
Message 1: [BatchTool]
  // MCP coordination setup
  - mcp__cortex-agent@alpha__swarm_init
  - mcp__cortex-agent@alpha__agent_spawn (researcher)
  - mcp__cortex-agent@alpha__agent_spawn (coder)
  - mcp__cortex-agent@alpha__agent_spawn (code-analyzer)
  - mcp__cortex-agent@alpha__agent_spawn (tester)
  - mcp__cortex-agent@alpha__agent_spawn (task-orchestrator)
...
[BatchTool]:
  ...
  - mcp__cortex-agent@alpha__agent_spawn { type: "system-architect", name: "System Designer" }
  - mcp__cortex-agent@alpha__agent_spawn { type: "coder", name: "API Developer" }
  - mcp__cortex-agent@alpha__agent_spawn { type: "coder", name: "Frontend Dev" }
  - mcp__cortex-agent@alpha__agent_spawn { type: "code-analyzer", name: "DB Designer" }
  - mcp__cortex-agent@alpha__agent_spawn { type: "tester", name: "QA Engineer" }
  - mcp__cortex-agent@alpha__agent_spawn { type: "researcher", name: "Tech Lead" }
  - mcp__cortex-agent@alpha__agent_spawn { type: "task-orchestrator", name: "PM" }
  - TodoWrite { todos: [multiple todos at once] }

STEP 2: PARALLEL TASK EXECUTION (Single Message!)
[BatchTool]:
  - mcp__cortex-agent@alpha__task_orchestrate { task: "main task", strategy: "parallel" }
  - mcp__cortex-agent@alpha__memory_usage { action: "store", key: "init", value: {...} }
...
[BatchTool - Message 1]:
  ...
  mcp__cortex-agent@alpha__swarm_init { topology: "hierarchical", maxAgents: 8, strategy: "parallel" }
  mcp__cortex-agent@alpha__agent_spawn { type: "system-architect", name: "System Designer" }
  mcp__cortex-agent@alpha__agent_spawn { type: "coder", name: "API Developer" }
  mcp__cortex-agent@alpha__agent_spawn { type: "coder", name: "Auth Expert" }
  mcp__cortex-agent@alpha__agent_spawn { type: "code-analyzer", name: "DB Designer" }
  mcp__cortex-agent@alpha__agent_spawn { type: "tester", name: "Test Engineer" }
  mcp__cortex-agent@alpha__agent_spawn { type: "task-orchestrator", name: "Lead" }
  ...
  // Start orchestration
  mcp__cortex-agent@alpha__task_orchestrate { task: "Build REST API", strategy: "parallel" }

  // Store initial memory
  mcp__cortex-agent@alpha__memory_usage { action: "store", key: "project/init", value: { started: Date.now() } }
...
Message 1: mcp__cortex-agent@alpha__swarm_init
Message 2: mcp__cortex-agent@alpha__agent_spawn (just one agent)
Message 3: mcp__cortex-agent@alpha__agent_spawn (another agent)
Message 4: TodoWrite (single todo)
...
mcp__cortex-agent@alpha__memory_usage
  action: "store"
...
4. **Monitor Progress**: Use mcp**cortex-agent@alpha**swarm_monitor for real-time tracking
...
## Cortex Agent v2.0.0 Features

Cortex Agent extends the base coordination with:
...
- Documentation: https://github.com/ruvnet/cortex-agent@alpha
- Issues: https://github.com/ruvnet/cortex-agent@alpha/issues
- Examples: https://github.com/ruvnet/cortex-agent@alpha/tree/main/examples

---

Remember: **Cortex Agent coordinates, Claude Code creates!** Start with `mcp__cortex-agent@alpha__swarm_init` to enhance your development workflow.
