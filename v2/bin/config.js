...
async function initConfig(subArgs, flags) {
  ...
  const configFile = 'cortex-agent.config.json';
  ...
  try {
    ...
    console.log('\nNext steps:');
    console.log('1. Review settings: cortex-agent config show');
    console.log('2. Customize values: cortex-agent config set <key> <value>');
    console.log('3. Validate config: cortex-agent config validate');
  } catch (err) {
...
async function showConfig(subArgs, flags) {
  ...
  try {
    ...
    if (format === 'json') {
      ...
      console.log(`   Path: ${config.memory?.path || './memory/cortex-agent-data.json'}`);
      console.log('\n🤖 Agents:');
    ...
    console.log('Run "cortex-agent config init" to create default configuration');
  }
}
...
async function getConfigValue(subArgs, flags) {
  ...
  if (!key) {
    ...
    console.log('  cortex-agent config get terminal.poolSize');
    console.log('  cortex-agent config get orchestrator.maxConcurrentTasks');
    return;
  }
  ...
  try {
    ...
    printError('Configuration file not found');
    console.log('Run "cortex-agent config init" to create configuration');
  }
}
...
async function setConfigValue(subArgs, flags) {
  ...
  if (!key || value === undefined) {
    ...
    console.log('Examples:');
    console.log('  cortex-agent config set terminal.poolSize 15');
    console.log('  cortex-agent config set orchestrator.taskTimeout 600000');
    return;
  }
...
async function validateConfig(subArgs, flags) {
  ...
  try {
    ...
    console.log('Run "cortex-agent config init" to create valid configuration');
  }
}
...
function showConfigHelp() {
  ...
  console.log();
  console.log('Examples:');
  console.log('  cortex-agent config init');
  console.log('  cortex-agent config set terminal.poolSize 15');
  console.log('  cortex-agent config get orchestrator.maxConcurrentTasks');
  console.log('  cortex-agent config validate');
}
