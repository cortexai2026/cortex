...
npx cortex-agent swarm "Build a complete REST API with authentication" --strategy development --monitor

# Research swarm with distributed coordination
npx cortex-agent swarm "Research and analyze blockchain technologies" --strategy research --distributed --ui

# Background optimization swarm (for long-running tasks)
npx cortex-agent swarm "Optimize application performance across all modules" --strategy optimization --background --monitor
\`\`\`

### Advanced Swarm Configuration:
\`\`\`bash
# Full-featured swarm with all enterprise features
npx cortex-agent swarm "Complex enterprise application development" \\
...
npx cortex-agent swarm "Your complex task" --dry-run --strategy development
\`\`\`
...
npx cortex-agent memory store "key" "value" --namespace swarm
npx cortex-agent memory query "search-term" --namespace swarm
\`\`\`
...
- Monitor progress with \`npx cortex-agent status\`
...
npx cortex-agent swarm "Build microservices e-commerce platform" \\
...
npx cortex-agent swarm "Comprehensive AI/ML market analysis" \\
  --strategy research --distributed --ui --verbose

# 2. Store findings for cross-agent access
npx cortex-agent memory store "research-findings" "key insights" --namespace swarm
\`\`\`

### Long-Running Optimization:
\`\`\`bash
# 1. Background optimization swarm
npx cortex-agent swarm "Optimize entire codebase performance" \\
...
### Common Issues:
1. **Swarm fails to start**: Check if Claude-Flow is initialized with \`npx cortex-agent init --sparc\`
...
5. **Dry Run Validation**
   ...
   npx cortex-agent swarm "${taskDescription}" --dry-run --strategy <selected-strategy>
   \`\`\`
...
6. **Launch Swarm**
   ...
   **For Standard Tasks (< 30 minutes):**
   \`\`\`bash
   npx cortex-agent swarm "${taskDescription}" --strategy <strategy> --monitor --verbose
   \`\`\`

   **For Complex Tasks (30-60 minutes):**
   \`\`\`bash
   npx cortex-agent swarm "${taskDescription}" --strategy <strategy> --parallel --monitor --review
   \`\`\`

   **For Long-Running Tasks (> 60 minutes):**
   \`\`\`bash
   npx cortex-agent swarm "${taskDescription}" --strategy <strategy> --background --monitor --testing
   \`\`\`

   **For Enterprise Tasks (Multiple hours):**
   \`\`\`bash
   npx cortex-agent swarm "${taskDescription}" \\
     --strategy <strategy> \\
     --mode distributed \\
     --max-agents 10 \\
     --background \\
...
8. **Cross-Agent Memory Sharing**
   ...
   npx cortex-agent memory store "${memoryNamespace}_findings" "key insights and decisions"
   
   # Store progress updates
   npx cortex-agent memory store "${memoryNamespace}_progress" "current completion status"
   
   # Store blockers or issues
   npx cortex-agent memory store "${memoryNamespace}_blockers" "any impediments or questions"
   
   # Query shared knowledge
   npx cortex-agent memory query "${memoryNamespace}" --limit 10
   \`\`\`
...
10. **Memory Documentation**
    \`\`\`bash
    # Store final results
    npx cortex-agent memory store "${memoryNamespace}_final" "completed task summary and results"
    
    # Store lessons learned
    npx cortex-agent memory store "${memoryNamespace}_lessons" "insights for future similar tasks"
    \`\`\`
