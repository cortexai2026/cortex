...
npx cortex-agent@alpha memory store api_key \
...
# Delete a pattern
npx cortex-agent@alpha memory delete api_key \
  --namespace backend --reasoningbank
...
npx cortex-agent@alpha memory query "API design" --reasoningbank
...
npx cortex-agent@alpha memory store api_pagination \
  "Cursor-based pagination: /api/items?limit=20&cursor=abc123" \
  --namespace api_patterns --reasoningbank

npx cortex-agent@alpha memory store api_error_format \
  'Consistent error format: {"error": true, "message": "...", "code": 400}' \
  --namespace api_patterns --reasoningbank

npx cortex-agent@alpha memory store api_rate_limiting \
  "100 requests/minute per IP using sliding window algorithm" \
  --namespace api_patterns --reasoningbank

# Query when designing new API
npx cortex-agent@alpha memory query "API response format" \
  --namespace api_patterns --reasoningbank
```
...
while IFS=',' read -r title content namespace; do
  npx cortex-agent@alpha memory store "$title" "$content" \
...
npx cortex-agent@alpha memory export --all > "$BACKUP_DIR/patterns.json"
