...
# Try it!
npx cortex-agent@alpha memory query "your question here" --reasoningbank
```
...
export CORTEX_AGENT_DB_PATH=./my-project/.swarm/memory.db

# Or use --db-path flag
npx cortex-agent@alpha memory query "test" --db-path ./my-project/.swarm/memory.db
```
...
npx cortex-agent@alpha memory query "API authentication" --namespace security

# High confidence only
npx cortex-agent@alpha memory query "database optimization" --min-confidence 0.8
...
npx cortex-agent@alpha memory query "authentication patterns" > context.json
