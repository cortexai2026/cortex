...
# 2. Copy to cortex-agent
cp memory.db ~/.swarm/memory.db

# 3. Try it
npx cortex-agent@alpha memory query "API optimization best practices"
```
...
**Example**:
```bash
npx cortex-agent@alpha memory query "JWT authentication" --reasoningbank
```
