...
# 2. Copy to your cortex-agent directory
cp memory.db ~/.swarm/memory.db

# 3. Test it!
npx cortex-agent@alpha memory query "API optimization" --reasoningbank
```
...
export CORTEX_AGENT_DB_PATH=./my-app/.swarm/memory.db
```
...
npx cortex-agent@alpha memory query "test" --reasoningbank --all-namespaces
```
