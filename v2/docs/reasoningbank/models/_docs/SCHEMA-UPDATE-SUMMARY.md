...
All pre-trained ReasoningBank models have been updated to include **all required cortex-agent memory tables**. The models now have **12 tables** instead of 4-8 tables.
...
✅ **Fully compatible** with all cortex-agent commands
...
npx cortex-agent@alpha memory store api_key "sk-123" --namespace config
...
# Store a memory
npx cortex-agent@alpha memory store test "Hello World" --namespace demo
...
# 3. Test it
npx cortex-agent@alpha memory query "test" --reasoningbank
```
...
**The models are now 100% compatible with all cortex-agent features!** 🎉
