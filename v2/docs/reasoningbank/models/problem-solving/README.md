...
```bash
npx cortex-agent@alpha memory query "How to approach a complex system outage with intermittent failures?" \
...
```bash
npx cortex-agent@alpha memory query "Explore multiple approaches to solve database scaling bottleneck" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
```bash
npx cortex-agent@alpha memory query "Traditional deployment process too slow, need breakthrough solution" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
```bash
npx cortex-agent@alpha memory query "Why does increasing marketing spend reduce lead quality?" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
```bash
npx cortex-agent@alpha memory query "Validate assumption that users want more features" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
npx cortex-agent@alpha memory query "debug production issue" \
  --reasoningbank \
  --filter "cognitive_type:convergent"

# Only divergent (creative) patterns
npx cortex-agent@alpha memory query "improve user engagement" \
  --reasoningbank \
...
```bash
# Technical domain
npx cortex-agent@alpha memory query "scale microservices" \
  --reasoningbank \
  --filter "domain:technical"

# Business domain
npx cortex-agent@alpha memory query "reduce customer churn" \
  --reasoningbank \
...
npx cortex-agent@alpha memory query "deploy safely" \
  --reasoningbank \
...
npx cortex-agent@alpha memory query "solve scaling problem" \
  --reasoningbank \
  --links "alternative" \
  --k 10

# Find prerequisite patterns
npx cortex-agent@alpha memory query "implement microservices" \
  --reasoningbank \
  --links "requires" \
  --k 5
```
...
npx cortex-agent@alpha memory query "complex system debugging" \
  --reasoningbank \
  --trajectory \
  --steps 5
```
...
```javascript
// Use with cortex-agent MCP server
{
  "action": "query",
...
Part of the cortex-agent ReasoningBank system. Pre-trained model available for use with cortex-agent and agentic-flow agents.
