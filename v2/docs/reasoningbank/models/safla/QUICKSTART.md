...
npx cortex-agent@alpha memory search "test" --namespace safla
...
npx cortex-agent@alpha memory search "optimize API performance" --namespace safla
...
npx cortex-agent@alpha memory search "API optimization" --namespace safla

# Get high-confidence REST patterns
npx cortex-agent@alpha memory retrieve "domain:self-learning AND tags:rest" --namespace safla
```
...
npx cortex-agent@alpha memory search "code review feedback" --namespace safla

# Get patterns that improve review quality
npx cortex-agent@alpha memory retrieve "domain:feedback-optimization AND confidence:>0.80" --namespace safla
```
...
npx cortex-agent@alpha memory search "error recovery adaptation" --namespace safla

# Get distilled failure patterns
npx cortex-agent@alpha memory retrieve "domain:distillation AND tags:error" --namespace safla
```
...
npx cortex-agent@alpha memory search "performance optimization" --namespace safla

# Get recursive improvement patterns
npx cortex-agent@alpha memory retrieve "domain:recursive-cycles AND success_rate:>0.90" --namespace safla
```
...
npx cortex-agent@alpha memory search "architecture refinement" --namespace safla

# Get high-confidence system design patterns
npx cortex-agent@alpha memory retrieve "domain:recursive-cycles AND tags:architecture" --namespace safla
```
...
npx cortex-agent@alpha memory search "API endpoint optimization" --namespace safla

# Database query learning
npx cortex-agent@alpha memory search "database query learning" --namespace safla

# Resource allocation patterns
npx cortex-agent@alpha memory search "resource allocation learning" --namespace safla
```
...
npx cortex-agent@alpha memory search "user interaction feedback" --namespace safla

# A/B testing integration
npx cortex-agent@alpha memory search "A/B test result integration" --namespace safla

# Performance monitoring
npx cortex-agent@alpha memory search "performance metric feedback" --namespace safla
```
...
```bash
# Bayesian updating
npx cortex-agent@alpha memory search "prior belief updating" --namespace safla

# Uncertainty quantification
npx cortex-agent@alpha memory search "uncertainty quantification" --namespace safla

# Multi-source evidence
npx cortex-agent@alpha memory search "multi-source evidence integration" --namespace safla
```
...
npx cortex-agent@alpha memory search "deployment success pattern extraction" --namespace safla

# Incident learning
npx cortex-agent@alpha memory search "incident post-mortem learning" --namespace safla

# Test failure analysis
npx cortex-agent@alpha memory search "test failure root cause" --namespace safla
```
...
npx cortex-agent@alpha memory search "meta-learning from optimization" --namespace safla

# Self-improving tests
npx cortex-agent@alpha memory search "self-improving test suite" --namespace safla

# Architecture refinement
npx cortex-agent@alpha memory search "architecture refinement cycles" --namespace safla
```

---

## 🛠️ Integration with Cortex Agent

### During Development

```bash
# Before starting a task
npx cortex-agent@alpha hooks pre-task --description "API optimization task"

# Query SAFLA for relevant patterns
npx cortex-agent@alpha memory search "API optimization microservices" --namespace safla

# After completing work
npx cortex-agent@alpha hooks post-task --task-id "api-opt-task"
```
...
npx cortex-agent@alpha agent spawn --type coder --context "Use SAFLA patterns"

# Share patterns in swarm memory
npx cortex-agent@alpha memory store "swarm/patterns/api" "$(npx cortex-agent@alpha memory search 'API' --namespace safla)" --namespace swarm
```
...
npx cortex-agent@alpha memory query "SELECT domain, COUNT(*) FROM patterns GROUP BY domain" --namespace safla

# Get confidence distribution
npx cortex-agent@alpha memory query "SELECT ROUND(confidence, 1) as conf, COUNT(*) FROM patterns GROUP BY conf" --namespace safla

# Get top technologies
npx cortex-agent@alpha memory query "SELECT tags FROM patterns LIMIT 100" --namespace safla | grep -o '"[^"]*"' | sort | uniq -c | sort -rn | head -20
```
...
4. Integrate with Cortex Agent hooks
...
```bash
# Check namespace
npx cortex-agent@alpha memory search "your query" --namespace safla

# Try broader search
npx cortex-agent@alpha memory search "optimization" --namespace safla
```
...
- **ReasoningBank CLI**: `npx cortex-agent@alpha memory --help`
