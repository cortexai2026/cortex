...
npx cortex-agent@alpha memory retrieve "confidence:>0.85" --namespace safla

# Find patterns by domain
npx cortex-agent@alpha memory retrieve "domain:feedback-optimization" --namespace safla
```
...
```bash
time npx cortex-agent@alpha memory search "optimize" --namespace safla
...
npx cortex-agent@alpha hooks pre-task --description "Using SAFLA model"
npx cortex-agent@alpha hooks session-restore --session-id "safla-session"
...
**Coordination**: Cortex Agent Hooks System
