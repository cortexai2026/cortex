...
```bash
# Store a pattern
npx cortex-agent@alpha memory store <key> <value> --reasoningbank

# Query semantically (finds related concepts)
npx cortex-agent@alpha memory query "<search query>" --reasoningbank
...
npx cortex-agent@alpha memory store memory_leak_fix \
...
npx cortex-agent@alpha memory query "memory leak" --reasoningbank
...
```bash
# Same query later
npx cortex-agent@alpha memory query "authentication" --reasoningbank
...
npx cortex-agent@alpha memory query "optimization strategies"

# Option 2: Code Reasoning (Programming)
cp docs/reasoningbank/models/code-reasoning/.swarm/memory.db ~/.swarm/memory.db
npx cortex-agent@alpha memory query "design patterns"
...
npx cortex-agent@alpha memory store bug_fix_001 \
...
npx cortex-agent@alpha memory store bug_fix_002 \
...
npx cortex-agent@alpha memory query "memory leak" \
  --namespace debugging --reasoningbank
```
...
**Full Details**: Read [Google Research Paper Analysis](./google-research.md) for implementation algorithms and integration with cortex-agent.
...
**With ReasoningBank:**
```bash
AI queries: npx cortex-agent@alpha memory query "CORS error fix"
...
Based on Google Research benchmarks and cortex-agent production usage:
...
npx cortex-agent@alpha memory store cache_redis \
...
npx cortex-agent@alpha memory store debug_strategy \
  "Use binary search to isolate bugs" \
  --cognitive-pattern convergent --reasoningbank

# Query by thinking style
npx cortex-agent@alpha memory query "problem solving" \
...
npx cortex-agent@alpha memory trajectory start api_build \
  "Building REST API" --reasoningbank

npx cortex-agent@alpha memory trajectory step api_build \
  "Designed database schema" --reasoningbank

npx cortex-agent@alpha memory trajectory step api_build \
  "Implemented endpoints" --reasoningbank

npx cortex-agent@alpha memory trajectory end api_build \
  --outcome success --reasoningbank

# Later, retrieve the workflow
npx cortex-agent@alpha memory trajectory get api_build --reasoningbank
```
...
```bash
# Backend pattern
npx cortex-agent@alpha memory store jwt_backend \
  "JWT signing with HMAC SHA256" \
  --namespace backend --reasoningbank

# Frontend pattern
npx cortex-agent@alpha memory store jwt_frontend \
  "Store JWT in httpOnly cookies" \
  --namespace frontend --reasoningbank

# Query finds both!
npx cortex-agent@alpha memory query "JWT security" --reasoningbank
...
npx cortex-agent@alpha memory store arch_microservices \
...
npx cortex-agent@alpha memory query "why microservices" \
  --namespace team_decisions --reasoningbank
...
```bash
# Store bug fix
npx cortex-agent@alpha memory store cors_fix \
  "CORS error: Add Access-Control-Allow-Origin in Express middleware" \
  --namespace debugging --reasoningbank

# Week later, similar error
npx cortex-agent@alpha memory query "CORS blocked" --reasoningbank
...
npx cortex-agent@alpha memory query "listing endpoints" \
  --namespace api_patterns --reasoningbank
```
...
npx cortex-agent@alpha memory store hello \
  "ReasoningBank learns automatically!" \
  --reasoningbank

# 3. Query semantically (2ms)
npx cortex-agent@alpha memory query "learning" --reasoningbank
...
- 💬 [GitHub Issues](https://github.com/ruvnet/cortex-agent/issues) - Get help or contribute
