        ...
        "hooks": [
          {
            ...
            "command": "cat | jq -r '.tool_input.command // empty' | tr '\\n' '\\0' | xargs -0 -I {} bash -c 'CMD=\"{}\"; CONTEXT=$(npx cortex-agent@alpha memory query \"$CMD\" --reasoningbank --limit 3 --format json 2>/dev/null || echo \"{}\"); echo \"📚 ReasoningBank Context: $CONTEXT\" >&2; npx cortex-agent@alpha hooks pre-command --command \"$CMD\" --validate-safety true --prepare-resources true --context \"$CONTEXT\"'"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            ...
            "command": "cat | jq -r '.tool_input.file_path // .tool_input.path // empty' | tr '\\n' '\\0' | xargs -0 -I {} bash -c 'FILE=\"{}\"; EXT=\"${FILE##*.}\"; CONTEXT=$(npx cortex-agent@alpha memory query \"$EXT file patterns\" --namespace code --reasoningbank --limit 2 --format json 2>/dev/null || echo \"{}\"); echo \"📚 Code Patterns: $CONTEXT\" >&2; npx cortex-agent@alpha hooks pre-edit --file \"$FILE\" --auto-assign-agents true --load-context true --reasoning-context \"$CONTEXT\"'"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            ...
            "command": "cat | jq -r '.tool_input.command // empty, .result.exit_code // \"unknown\"' | tr '\\n' '\\0' | xargs -0 bash -c 'set -- $0 $1; CMD=\"$1\"; EXIT=\"$2\"; if [ \"$EXIT\" = \"0\" ]; then DESC=\"Successful command: $CMD\"; npx cortex-agent@alpha memory store \"cmd_$(echo -n \"$CMD\" | md5sum | cut -d\\\" \\\" -f1)\" \"$DESC\" --namespace commands --reasoningbank --confidence 0.6 2>/dev/null; fi; npx cortex-agent@alpha hooks post-command --command \"$CMD\" --track-metrics true --store-results true --exit-code \"$EXIT\"'"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            ...
            "command": "cat | jq -r '.tool_input.file_path // .tool_input.path // empty, (.tool_input.new_string // .tool_input.content // empty | split(\"\\n\") | length | tostring)' | tr '\\n' '\\0' | xargs -0 bash -c 'set -- $0 $1; FILE=\"$1\"; LINES=\"$2\"; if [ -f \"$FILE\" ]; then EXT=\"${FILE##*.}\"; PATTERN=\"Edited $EXT file with $LINES lines\"; npx cortex-agent@alpha memory store \"edit_${EXT}_$(date +%s)\" \"$PATTERN\" --namespace code --reasoningbank --confidence 0.5 2>/dev/null; fi; npx cortex-agent@alpha hooks post-edit --file \"$FILE\" --format true --update-memory true'"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            ...
            "command": "/bin/bash -c 'TOP_PATTERNS=$(npx cortex-agent@alpha memory query \"important patterns\" --reasoningbank --limit 3 2>/dev/null || echo \"None\"); echo \"🔄 Auto-Compact with ReasoningBank:\"; echo \"📚 Top Patterns: $TOP_PATTERNS\"; echo \"✅ Auto-compact proceeding\"'"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            ...
            "command": "bash -c 'echo \"🧠 Consolidating ReasoningBank memory...\"; npx cortex-agent@alpha memory consolidate --reasoningbank --threshold 0.9 --prune-low-confidence 0.2 2>/dev/null || echo \"Consolidation skipped\"; npx cortex-agent@alpha hooks session-end --generate-summary true --persist-state true --export-metrics true; echo \"✅ Session ended with ReasoningBank update\"'"
          }
        ]
      }
    ]
        ...
        "hooks": [
          {
            ...
            "command": "cat | jq -r '.tool_input.file_path // .tool_input.path // empty' | xargs -I {} bash -c 'npx cortex-agent@alpha memory query \"{}\" --reasoningbank --limit 2 2>&1 | grep -q \"Found\" && echo \"📚 Found relevant patterns\" || echo \"📝 No patterns yet\"'"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            ...
            "command": "cat | jq -r '.tool_input.file_path // .tool_input.path // empty' | xargs -I {} bash -c 'FILE=\"{}\"; [ -f \"$FILE\" ] && npx cortex-agent@alpha memory store \"edit_$(basename $FILE)_$(date +%s)\" \"Edited: $FILE\" --namespace code --reasoningbank 2>/dev/null || true'"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            ...
            "command": "npx cortex-agent@alpha memory consolidate --reasoningbank 2>/dev/null || true"
          }
        ]
      }
    ]
...
# Store with metadata
npx cortex-agent@alpha memory store bug_fix_123 \
...
```bash
# Query semantically
npx cortex-agent@alpha memory query "authentication patterns" \
  --reasoningbank \
  --limit 3 \
  --format json

# Query with filters
npx cortex-agent@alpha memory query "bug fixes" \
  --namespace debugging \
  --reasoningbank \
  --min-confidence 0.7
```
...
```bash
# List all patterns
npx cortex-agent@alpha memory list --reasoningbank
...
npx cortex-agent@alpha memory stats --reasoningbank
```
...
1. PreToolUse Hook (Write tool)
   ...
   ├─ Query: npx cortex-agent memory query "server.js api patterns"
...
3. PostToolUse Hook (Write tool)
   ...
   ├─ Store: npx cortex-agent memory store "edit_server_1729123456"
...
```bash
if [ "$EXIT_CODE" = "0" ]; then
  npx cortex-agent@alpha memory store "pattern" "description" --reasoningbank
fi
```
...
```bash
npx cortex-agent@alpha memory query "pattern" --reasoningbank 2>/dev/null || echo "{}"
```
...
```bash
EXT="${FILE##*.}"
npx cortex-agent@alpha memory store "edit_${EXT}" "Pattern for $EXT files"
```
...
echo '{"tool_input": {"file_path": "test.js"}}' | \
  ...
  xargs -I {} npx cortex-agent@alpha memory query "{}"
```
...
echo '{"tool_input": {"file_path": "test.js"}, "result": {"exit_code": 0}}' | \
  jq -r '.tool_input.file_path' | \
  xargs -I {} npx cortex-agent@alpha memory store "edit_test_$(date +%s)" \
...
npx cortex-agent@alpha memory list --namespace code --reasoningbank
```
...
npx cortex-agent@alpha memory consolidate --reasoningbank --threshold 0.9
```
...
```bash
timeout 5s npx cortex-agent@alpha memory query "pattern" --limit 3
```
...
```bash
npx cortex-agent@alpha memory consolidate --reasoningbank --threshold 0.95
```
...
PATTERNS=$(npx cortex-agent@alpha memory query \
  "$EXT $FILENAME" \
  --namespace "$NAMESPACE" \
  --reasoningbank \
  --limit 5 \
...
4. Check stored patterns: `npx cortex-agent@alpha memory list --reasoningbank`
