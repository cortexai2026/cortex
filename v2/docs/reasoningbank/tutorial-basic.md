...
```bash
npx cortex-agent@alpha memory status --reasoningbank
```
...
```bash
npx cortex-agent@alpha memory store greeting \
...
```bash
npx cortex-agent@alpha memory list --namespace tutorial --reasoningbank
```
...
```bash
# Store a memory
npx cortex-agent@alpha memory store api_auth \
  "Use JWT tokens with 15-minute expiration" \
  --namespace backend --reasoningbank

# Query with exact keyword
npx cortex-agent@alpha memory query "JWT" \
  --namespace backend --reasoningbank
```
...
npx cortex-agent@alpha memory query "authentication" \
  --namespace backend --reasoningbank
```
...
npx cortex-agent@alpha memory store db_connection \
  "PostgreSQL with connection pooling (max 20 connections)" \
  --namespace backend --reasoningbank

npx cortex-agent@alpha memory store cache_strategy \
  "Redis for session storage with 24-hour TTL" \
  --namespace backend --reasoningbank

npx cortex-agent@alpha memory store rate_limiting \
  "100 requests per minute per IP using sliding window" \
  --namespace backend --reasoningbank

# Query with broad concept
npx cortex-agent@alpha memory query "performance optimization" \
  --namespace backend --reasoningbank
```
...
npx cortex-agent@alpha memory query "API design" \
  --namespace backend --reasoningbank
...
```bash
# Initial storage
npx cortex-agent@alpha memory store api_v1 \
...
```bash
# Store the solution
npx cortex-agent@alpha memory store react_rerender_fix \
...
npx cortex-agent@alpha memory query "React infinite loop" \
  --namespace debugging --reasoningbank
```
...
npx cortex-agent@alpha memory store arch_001_messaging \
...
npx cortex-agent@alpha memory query "event system" \
  --namespace team_decisions --reasoningbank
```
...
```bash
# ❌ Bad: Vague key
npx cortex-agent@alpha memory store config "JWT secret" --reasoningbank

# ✅ Good: Descriptive key
npx cortex-agent@alpha memory store jwt_secret_config \
...
```bash
# ❌ Bad: Too brief
npx cortex-agent@alpha memory store db "Postgres" --reasoningbank

# ✅ Good: Context included
npx cortex-agent@alpha memory store db_choice \
...
npx cortex-agent@alpha memory query "API patterns" --reasoningbank
...
```bash
# ❌ Bad: Store duplicate
npx cortex-agent@alpha memory store api_auth "JWT tokens" --reasoningbank
npx cortex-agent@alpha memory store api_auth_v2 "JWT with refresh" --reasoningbank

# ✅ Good: Store as evolution
npx cortex-agent@alpha memory store api_auth_basic \
  "Simple JWT tokens (deprecated)" --reasoningbank

npx cortex-agent@alpha memory store api_auth_production \
...
npx cortex-agent@alpha memory store tool_1 "..." --namespace tools --reasoningbank
npx cortex-agent@alpha memory store tool_2 "..." --namespace tools --reasoningbank
# ... 3 more

# Query to find relevant tool
npx cortex-agent@alpha memory query "testing" --namespace tools --reasoningbank
```
...
```bash
npx cortex-agent@alpha memory store bug_001 \
...
npx cortex-agent@alpha memory query "CORS" --namespace debugging --reasoningbank
```
...
```bash
npx cortex-agent@alpha memory store async_error \
...
```bash
npx cortex-agent@alpha memory query "foobar" --reasoningbank
# ❌ Found 0 results
```
...
```bash
npx cortex-agent@alpha memory query "configuration" --reasoningbank
# ✅ Found 3 results
```
...
```bash
npx cortex-agent@alpha memory query "security" --reasoningbank
...
```bash
npx cortex-agent@alpha memory query "authentication security" \
  --namespace backend --reasoningbank
```
...
```bash
npx cortex-agent@alpha memory store init "Initializing database" --reasoningbank
```
...
npx cortex-agent@alpha memory link api_auth requires secret_management

# Cognitive patterns
npx cortex-agent@alpha memory store problem_solving \
  "Use divergent thinking for creative solutions" \
  --cognitive-pattern divergent --reasoningbank

# Export/backup
npx cortex-agent@alpha memory export --namespace backend > backup.json
```
...
- Documentation: https://github.com/ruvnet/cortex-agent/tree/main/docs/reasoningbank
