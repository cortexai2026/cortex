...
When using `npx cortex-agent@alpha memory store`, the log shows:
```
...
export async function initialize() {
    ...
    console.log(`[ReasoningBank] Database: ${process.env.CORTEX_AGENT_DB_PATH || '.swarm/memory.db'}`);
    // ... rest of function
}
```
...
## Update cortex-agent After Publishing

Once agentic-flow@1.8.5 is published, update cortex-agent:
