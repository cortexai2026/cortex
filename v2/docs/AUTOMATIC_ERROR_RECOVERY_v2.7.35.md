...
Implemented comprehensive automatic error recovery system for `cortex-agent init` that handles the WSL better-sqlite3 ENOTEMPTY error **without manual intervention**.
...
$ rm -rf ~/.npm/_npx
$ npx cortex-agent@alpha init --force  # Try again
```
...
# Test command
CMD npx cortex-agent@alpha init --force
```
...
```bash
# Build test image
docker build -t cortex-agent-test -f Dockerfile.test .

# Run test
docker run -it cortex-agent-test

# Test with volume mount
docker run -it -v $(pwd):/workspace -w /workspace cortex-agent-test

# Simulate WSL environment
docker run -it -e SIMULATE_WSL=1 cortex-agent-test
```
