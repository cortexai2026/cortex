...
**Date:** 2025-10-25
**Versions:** cortex-agent@2.7.21 + agentic-flow@1.8.4
...
```bash
$ npx cortex-agent@alpha memory store "api-design" "REST with JWT auth"
...
### Step 2: Updated cortex-agent@2.7.21

**Changes made to cortex-agent:**
...
3. **Published to npm:**
   - Version: 2.7.21
   - Package: https://www.npmjs.com/package/cortex-agent
...
### Store Memory
```bash
npx cortex-agent@alpha memory store "api-pattern" "REST with JWT auth"
# ✅ Stored successfully in ReasoningBank
# 🧠 Memory ID: 998e10dc-db9a-4625-8f2d-458827dbb933
```
...
npx cortex-agent@2.7.20 memory export backup.json

# Import to SQLite (with new version)
npx cortex-agent@alpha memory import backup.json
```
...
```bash
npx cortex-agent@alpha memory store "my-key" "my-value"
...
```bash
npx cortex-agent@2.7.21 memory store "test" "value"
```
...
Full details: https://github.com/ruvnet/cortex-agent/issues/840
...
**Version:** cortex-agent@2.7.21 + agentic-flow@1.8.4
