...
```bash
npx cortex-agent@alpha memory store "api-design" "REST with JWT"
...
```bash
npx cortex-agent@alpha memory query "api"
...
# Or with npx:
npx -p @xenova/transformers -p cortex-agent@alpha cortex-agent memory query "api"
```
...
export CORTEX_AGENT_SEMANTIC_SEARCH=true
...
**File: `agentic-flow/package.json` or `cortex-agent/package.json`**
...
async function initializeEmbeddings(): Promise<boolean> {
  ...
  const semanticSearchEnabled = process.env.CORTEX_AGENT_SEMANTIC_SEARCH === 'true';
  if (!semanticSearchEnabled) {
    console.log('[Embeddings] Semantic search disabled (set CORTEX_AGENT_SEMANTIC_SEARCH=true to enable)');
    return false;
  }
  ...
  if (!transformersAvailable) {
    ...
    console.log('   export CORTEX_AGENT_SEMANTIC_SEARCH=true');
    console.log('');
    return false;
  }
