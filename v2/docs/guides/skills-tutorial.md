# 🎓 Cortex Agent Skills: Complete Introduction Tutorial

## Overview

We’re shifting how Cortex Agent evolves from here forward. This release marks the move from slash commands to a true skills-based system, our new foundation for intelligence, automation, and collaboration. Instead of memorizing /commands, you now just describe what you want. Claude reads the situation, identifies the right skills, and activates them automatically.
...
This is the direction forward: modular, intelligent, and adaptive. Skills turn Cortex Agent from a set of commands into a living system that learns how you work and grows with you.

## 🔄 What Changed: From Slash Commands to Skills

I'm transitioning cortex-agent from slash commands to Claude Code's native skills system. Here's why this matters and what I've learned.

### The Old Way: Slash Commands

Previously, cortex-agent used slash commands stored in `.claude/commands/`:
```bash
# Old approach
...
5. **Standards-Based**: Anthropic designed this. It works across Claude.ai, Claude Code CLI, and any other Claude implementation. Commands were cortex-agent specific.
...
# Do this:
npx cortex-agent@alpha init --force  # Install skills
...
Skills are the future of cortex-agent. They align with Anthropic's vision, work better with Claude Code, and solve real problems I had with commands. The migration takes effort, but it's worth it.
...
This release adds 4 new AgentDB skills that bring production-grade vector database capabilities to cortex-agent:
...
Cortex Agent includes 25 specialized skills that activate automatically based on your task description. Just describe what you want in natural language - no commands needed.
...
### What Does `npx cortex-agent@alpha init --force` Do?

This command initializes your project with Cortex Agent's complete skills system:
...
3. **Sets up configuration** - Creates `cortex-agent.json` with default settings
...
**When to Use It:**
- Starting a new project with Cortex Agent
...
```bash
# Full SPARC workflow
npx cortex-agent sparc tdd "user authentication system"

# Individual phases
npx cortex-agent sparc run spec-pseudocode "shopping cart feature"
npx cortex-agent sparc run architect "payment processing"
...
**What It Does:**
Comprehensive performance analysis, bottleneck detection, and optimization recommendations for Cortex Agent swarms.
...
1. **Install**: Run `npx cortex-agent@alpha init --force`
