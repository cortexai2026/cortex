...
Cortex Agent now includes **REAL** token tracking capabilities that capture actual Claude API usage, not simulated data. This guide shows how to enable and use token tracking with the Claude Code CLI.
...
- Initialize token tracking directory (`.cortex-agent/metrics/`)
...
CLAUDE_CODE_ENABLE_TELEMETRY=1 ./cortex-agent swarm "your task" --claude

# Option 2: Export environment variable
export CLAUDE_CODE_ENABLE_TELEMETRY=1
./cortex-agent swarm "your task" --claude

# Option 3: Use wrapper directly
./cortex-agent analysis claude-monitor  # Start monitoring in one terminal
./cortex-agent swarm "your task" --claude  # Run Claude in another terminal
```
...
./cortex-agent analysis claude-monitor [session-id]
```
...
3. **Token Extraction**: Token usage is extracted from:
   ...
   - `/cost` command output
4. **Persistent Storage**: Token data is stored in `.cortex-agent/metrics/token-usage.json`
...
./cortex-agent analysis claude-monitor current --interval 3000
```
...
```bash
# Detailed breakdown
./cortex-agent analysis token-usage --breakdown

# Agent-specific analysis
./cortex-agent analysis token-usage --agent coordinator --cost-analysis
```
...
2. Check token usage file exists:
   ```bash
   cat .cortex-agent/metrics/token-usage.json
   ```
...
1. Clear existing metrics:
   ```bash
   rm -rf .cortex-agent/metrics/token-usage.json
   ```
...
3. Run with explicit telemetry:
   ```bash
   CLAUDE_CODE_ENABLE_TELEMETRY=1 ./cortex-agent swarm "test" --claude
   ```
...
steps:
  - name: Setup telemetry
    run: ./cortex-agent analysis setup-telemetry
    
  - name: Run Claude task
    run: ./cortex-agent swarm "${{ inputs.task }}" --claude
    
  - name: Report costs
    run: ./cortex-agent analysis claude-cost
```

## Privacy & Security

- **No sensitive data**: Only token counts and costs are tracked
- **Local storage**: All data is stored locally in `.cortex-agent/metrics/`
...
| File | Purpose |
|------|---------|
| `.cortex-agent/metrics/token-usage.json` | Token usage data |
| `.cortex-agent/metrics/sessions/*.json` | Session metrics |
...
export CLAUDE_CODE_ENABLE_TELEMETRY=1
./cortex-agent swarm "Create a REST API with authentication" --claude

# 3. Check usage
./cortex-agent analysis token-usage --breakdown --cost-analysis

# 4. Monitor next session
./cortex-agent analysis claude-monitor &  # Run in background
./cortex-agent swarm "Add tests to the API" --claude
...
./cortex-agent analysis token-usage --agent coordinator

# Get optimization suggestions
./cortex-agent analysis bottleneck-detect --scope agent
```
...
Real token tracking in Cortex Agent provides transparency into API usage and costs. By following this guide, you can:
