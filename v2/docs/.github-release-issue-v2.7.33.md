...
This major release brings **three transformative feature sets** to Cortex Agent:
...
```bash
export CORTEX_AGENT_MCP2025=true
...
### CLI Status ✅
```bash
✅ npx cortex-agent --version → v2.7.33
✅ npx cortex-agent mcp status → Operational
✅ npx cortex-agent memory stats → ReasoningBank data
...
npx cortex-agent mcp call tools/search '{"query": "memory", "detailLevel": "basic"}'

# Tools load automatically on first invocation
npx cortex-agent mcp call memory_store '{"key": "test", "value": "data"}'
```
...
```bash
cp -r ~/.cortex-agent ~/.cortex-agent.backup
```
...
echo '{"mcp":{"features":{"enableMCP2025":true}}}' > ~/.cortex-agent/config.json
```
...
npm install -g cortex-agent@2.7.32

# Restore configuration
rm -rf ~/.cortex-agent
mv ~/.cortex-agent.backup ~/.cortex-agent
```
...
### Documentation
- Main docs: https://github.com/ruvnet/cortex-agent
