...
The `.env` file is **required** for ReasoningBank memory capabilities in cortex-agent. Without it, the system falls back to heuristic mode (simple regex pattern matching) with no actual learning.
...
cortex-agent agent run coder "task" --enable-memory
```
...
```bash
cortex-agent init --env                # Create new .env
cortex-agent init --env --force        # Overwrite existing .env
```
...
# Advanced memory
cortex-agent agent run coder "task" \
  --enable-memory \
  --memory-domain api/authentication \
  --memory-k 5 \
...
### Check Help
```bash
cortex-agent init --help               # See all init options
cortex-agent agent --help              # See all agent options
```
...
**Solution:** Run `cortex-agent init --env` and add API keys
