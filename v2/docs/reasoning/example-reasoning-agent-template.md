...
```bash
# Command
cortex-agent agent run [agent-name] "[task description]" \
  --enable-memory \
...
### With cortex-agent CLI:
```bash
cortex-agent agent run [agent-name] "[task]" \
  --enable-memory \
  --memory-domain [domain]
```
...
cortex-agent agent run [agent-name] "Task 1" --enable-memory
# Success rate: ~60%
...
cortex-agent agent memory list --domain [your-domain]
...
- **Available Agents**: `cortex-agent agent agents`
...
```bash
cortex-agent agent run adaptive-security-auditor \
...
cortex-agent agent run adaptive-security-auditor "Audit codebase" --enable-memory
