...
**Good news!** Starting with v2.7.35, cortex-agent includes **automatic error recovery** that handles this issue without manual intervention.
...
When running: `npx cortex-agent@alpha init --force` on Windows Subsystem for Linux (WSL)
...
echo "Command: npx cortex-agent@alpha init --force"
```
...
# Error details
npx cortex-agent@alpha init --force --verbose 2>&1 | tee error-log.txt
```

Then report at: https://github.com/ruvnet/cortex-agent/issues

### Alternative: Use Docker
If WSL issues persist, consider using Docker:

```bash
# Pull cortex-agent Docker image (if available)
docker pull ruvnet/cortex-agent:latest

# Run in container
docker run -it -v $(pwd):/workspace ruvnet/cortex-agent:latest init --force
```
