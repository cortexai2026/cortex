...
## Issue
`npx cortex-agent@alpha` was failing in remote environments (GitHub Codespaces, Docker containers) with:
...
### What Works Now:
✅ `npx cortex-agent@alpha --version` works without build tools
...
```bash
# Install cortex-agent locally (not via npx)
...
**If you previously ran `npx cortex-agent@alpha` and got the error:**
...
2. Test with v2.7.13:
   ```bash
   npx cortex-agent@2.7.13 --version
   ```
...
```bash
npm install cortex-agent@alpha agentdb
```
...
npm view cortex-agent@alpha dependencies
# Should NOT include: agentdb

npm view cortex-agent@alpha optionalDependencies
