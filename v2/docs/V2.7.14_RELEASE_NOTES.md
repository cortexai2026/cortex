...
**Problem:** `npx cortex-agent@alpha` was failing in GitHub Codespaces and minimal Docker environments with better-sqlite3 build errors.
...
npx cortex-agent@2.7.14 --version
...
npm view cortex-agent@alpha dependencies | grep agentdb
...
- **v2.7.10** - Removed version banner from bin/cortex-agent.js
...
The remote installation issue is now **fully resolved**. Users can run `npx cortex-agent@alpha` without build tools in GitHub Codespaces, minimal Docker containers, and any remote environment.
