...
The stream-chain command is included in Cortex Agent and registered in the command registry:

```bash
# Access stream-chain command
npx cortex-agent stream-chain help

# Or with the local CLI
./cortex-agent stream-chain help
```
...
./cortex-agent stream-chain demo --background
```
...
./cortex-agent stream-chain pipeline refactor --bg
...
**Example:**
```bash
$ ./cortex-agent stream-chain monitor

📊 Background Stream Chains
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🔗 stream_1755021020133
   Command: npx cortex-agent stream-chain demo
   PID: 366567
...
**Example:**
```bash
./cortex-agent stream-chain kill stream_1755021020133
```
...
- Process information stored in `.cortex-agent/stream-chains.json`
...
# Claude will execute:
./cortex-agent stream-chain pipeline analysis --background
...
```
.cortex-agent/stream-chains.json
```
...
```json
{
  ...
  "stream_9876543210": {
    "command": "npx cortex-agent stream-chain pipeline analysis",
    "pid": 67890,
...
watch -n 5 './cortex-agent stream-chain monitor'
```
...
### Combining with Other Cortex Agent Features

#### With Hive Mind
```bash
# Start hive mind coordination
npx cortex-agent hive-mind spawn "coordinator"
...
```bash
# Train agents first
./cortex-agent train-pipeline run
...
**Checks:**
1. Check process file exists: `ls -la .cortex-agent/stream-chains.json`
2. Verify process started: Check terminal output for process ID
3. Check system processes: `ps aux | grep cortex-agent`
...
MAX_CHAINS=3
CURRENT=$(./cortex-agent stream-chain monitor | grep "🟢 Running" | wc -l)
...
- GitHub Issues: [cortex-agent/issues](https://github.com/ruvnet/cortex-agent/issues)
- Documentation: [Stream Chaining Docs](https://github.com/ruvnet/cortex-agent/docs/stream-chaining.md)
- Wiki: [Cortex Agent Wiki](https://github.com/ruvnet/cortex-agent/wiki)

---

*Last updated: August 2025*
*Cortex Agent Version: Alpha 89*