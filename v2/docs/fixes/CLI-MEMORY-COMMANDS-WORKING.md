...
The issue was NOT missing functionality - it was the Node.js `--experimental-wasm-modules` flag requirement. This is now automatically included in `bin/cortex-agent`, so users don't need to worry about it.
...
2. **CLI Script**: Added `--experimental-wasm-modules` to `bin/cortex-agent`
...
```bash
$ ./bin/cortex-agent memory init --reasoningbank
...
```bash
$ ./bin/cortex-agent memory store test_pattern "A* pathfinding" --reasoningbank
...
```bash
$ ./bin/cortex-agent memory query "pathfinding" --reasoningbank
...
```bash
$ ./bin/cortex-agent memory status --reasoningbank
...
2. Added WASM flag to `bin/cortex-agent`:
   ```bash
