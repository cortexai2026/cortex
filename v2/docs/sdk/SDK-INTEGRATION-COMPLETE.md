...
Successfully integrated **100% real, SDK-powered features** into Cortex Agent with **zero breaking changes**.
...
| `src/sdk/cortex-agent-mcp-integration.ts` | 387 | Integration layer for MCP + SDK | ✅ Complete |
...
```bash
# Create checkpoint
npx cortex-agent checkpoint create <session-id> "Before deployment"

# List checkpoints
npx cortex-agent checkpoint list <session-id>

# Rollback
npx cortex-agent checkpoint rollback <checkpoint-id>

# Get checkpoint info
npx cortex-agent checkpoint info <checkpoint-id>
```
...
npx cortex-agent hive-mind pause -s <session-id>

# State saved to disk - can resume across restarts!
npx cortex-agent hive-mind resume -s <session-id>
```
...
- `src/sdk/cortex-agent-mcp-integration.ts`
...
- `src/mcp/cortex-agent-tools.ts` - **Added 7 new MCP tools**
