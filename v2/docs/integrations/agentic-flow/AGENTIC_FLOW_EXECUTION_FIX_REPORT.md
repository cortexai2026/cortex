...
✅ **Agent Listing**
```bash
cortex-agent agent agents  # Lists all 66+ available agents
```
...
cortex-agent agent run researcher "AI trends" --provider openrouter

# ONNX (local, free, private)
cortex-agent agent run reviewer "Code audit" --provider onnx

# Gemini (free tier)
cortex-agent agent run planner "Project plan" --provider gemini
```
...
$ cortex-agent agent run coder "Create a user authentication system" \
  --provider anthropic
...
$ cortex-agent agent run researcher \
...
$ cortex-agent agent run coder "Implement findings" --provider anthropic

# Step 3: Review with ONNX (local/private)
$ cortex-agent agent run reviewer "Security audit" --provider onnx
```
...
- ✅ Integration with existing cortex-agent features
