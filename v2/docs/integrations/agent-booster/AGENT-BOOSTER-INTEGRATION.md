...
cortex-agent agent booster edit src/app.js "Add try-catch error handling"

# Refactor to async/await
cortex-agent agent booster edit server.ts "Convert callbacks to async/await"

# Add JSDoc comments
cortex-agent agent booster edit utils.js "Add comprehensive JSDoc comments"

# Preview changes without applying (dry run)
cortex-agent agent booster edit app.js "Add logging" --dry-run

# Show performance comparison
cortex-agent agent booster edit app.js "Add logging" --benchmark
```
...
cortex-agent agent booster batch "src/**/*.ts" "Convert to arrow functions"

# Add logging to all JavaScript files
cortex-agent agent booster batch "*.js" "Add console.log for debugging"

# Update imports across project
cortex-agent agent booster batch "components/**/*.jsx" "Update React imports for v19"

# Preview batch changes (dry run)
cortex-agent agent booster batch "src/*.js" "Add comments" --dry-run
```
...
cortex-agent agent booster parse-markdown refactoring-plan.md

# Preview changes without applying
cortex-agent agent booster parse-markdown plan.md --dry-run
```
...
### `cortex-agent agent booster benchmark [options]`
...
cortex-agent agent booster benchmark

# Custom iteration count
cortex-agent agent booster benchmark --iterations 50

# Benchmark specific file
cortex-agent agent booster benchmark --file src/app.js --iterations 100
```
...
cortex-agent agent booster batch "src/**/*.js" "Convert to ES6 modules"
...
cortex-agent agent booster edit current-file.ts "Add type annotations"
...
cortex-agent agent booster batch "**/*.js" "Apply ESLint fixes"
...
cortex-agent agent booster batch "src/**/*.js" "Convert to TypeScript"
...
cortex-agent agent booster batch "src/**/*.jsx" "Update React 18 → React 19 APIs"
```
...
### Validate 352x Claim
```bash
cortex-agent agent booster benchmark --iterations 100
```
