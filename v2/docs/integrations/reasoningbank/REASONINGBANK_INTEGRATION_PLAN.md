# ReasoningBank Integration Plan for cortex-agent v2.7.0
...
This plan outlines comprehensive integration of agentic-flow's ReasoningBank learning memory system into cortex-agent as first-class CLI parameters and SDK features. Current integration is limited to agent execution; this extends it to include intelligent memory-based learning.
...
**Agent Execution:**
```bash
cortex-agent agent run coder "Create REST API"
  --provider openrouter
...
function showAgentHelp() {
  ...
  console.log('  cortex-agent agent run coder "Build REST API" --enable-memory');

  console.log('\n  # With domain filtering');
  console.log('  cortex-agent agent run coder "Add auth" --enable-memory --memory-domain web');

  console.log('\n  # Memory management');
  console.log('  cortex-agent agent memory init');
  console.log('  cortex-agent agent memory status');
  console.log('  cortex-agent agent memory list --sort confidence --limit 10');

  console.log('\n  # Retrieve without learning (read-only)');
  console.log('  cortex-agent agent run researcher "Research topic" --enable-memory --no-learning');
}
```
...
cortex-agent agent run coder "Build login form with CSRF protection" \
  --enable-memory \
  --memory-domain web \
  --provider openrouter
...
cortex-agent agent run coder "Add JWT authentication" \
  --enable-memory \
  --memory-domain api
...
# List top memories
cortex-agent agent memory list --sort confidence --limit 5
# Output:
...
cortex-agent agent run researcher "Research React 19 features" \
  --enable-memory \
  --no-learning \
  --memory-domain web
```
...
```bash
# Security domain
cortex-agent agent run security-auditor "Audit authentication" \
  --enable-memory \
  --memory-domain security

# Database domain
cortex-agent agent run database-architect "Design schema" \
  --enable-memory \
...
describe('ReasoningBank CLI Integration', () => {
  it('should execute with memory enabled via CLI', async () => {
    const { stdout } = await execAsync(
      'cortex-agent agent run coder "Test task" --enable-memory'
    );
...
cortex-agent agent run coder "Build feature" --enable-memory
```
