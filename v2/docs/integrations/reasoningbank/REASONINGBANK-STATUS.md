...
const rb = await createReasoningBank('cortex-agent-memory');
...
npx cortex-agent@alpha memory query "search term" --reasoningbank
```
...
- **Model Mapping**: cortex-agent memory → ReasoningBank pattern
...
3. **Integration**: cortex-agent now imports directly (no workarounds)
...
**cortex-agent@2.7.0-alpha.6 Adapter:**
- Imports from WASM API ✅
- Singleton instance management ✅
- LRU query caching ✅
- Fallback to category search ✅
- Model mapping cortex-agent ↔ ReasoningBank ✅
...
npx cortex-agent@alpha memory init --reasoningbank
npx cortex-agent@alpha memory store "key" "value" --reasoningbank
npx cortex-agent@alpha memory query "search" --reasoningbank
```
