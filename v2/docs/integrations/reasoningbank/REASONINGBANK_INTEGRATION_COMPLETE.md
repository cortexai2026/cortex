...
ReasoningBank from agentic-flow v1.4.11 is now fully integrated into cortex-agent, providing a closed-loop memory system that improves agent task success rates from 23% to 98% through experience-based learning.
...
```bash
# Install cortex-agent with ReasoningBank support
npm install cortex-agent@latest

# Initialize memory system
npx cortex-agent agent memory init

# Verify installation
npx cortex-agent agent memory status
```
...
npx cortex-agent@latest agent memory init

# Run agent with memory enabled
npx cortex-agent@latest agent run coder "Build REST API" --enable-memory

# Check learning progress
npx cortex-agent@latest agent memory status
```
...
npx cortex-agent agent memory demo
```
...
cortex-agent agent run coder "Build REST API with auth" --enable-memory

# Second execution (learns from first attempt)
cortex-agent agent run coder "Add JWT authentication" --enable-memory --memory-domain api

# Third execution (retrieves top 5 relevant memories)
cortex-agent agent run coder "Implement OAuth2 flow" --enable-memory --memory-k 5
```
...
cortex-agent agent memory list --domain api --limit 10
...
cortex-agent agent run coder "Build API" --enable-memory --provider anthropic

# OpenRouter (99% cost savings, still learns)
cortex-agent agent run coder "Add endpoints" --enable-memory --provider openrouter

# ONNX (free local, learns from local patterns)
cortex-agent agent run coder "Write tests" --enable-memory --provider onnx

# Gemini (free tier, learns efficiently)
cortex-agent agent run coder "Document code" --enable-memory --provider gemini
```
...
# 4. List memories
cortex-agent agent memory list --limit 10

# 5. Run agent with memory
cortex-agent agent run coder "Build calculator" --enable-memory --provider onnx

# 6. Verify memory was created
cortex-agent agent memory status  # Should show 1+ memories
```
...
```javascript
import { AgentExecutor } from 'cortex-agent';
...
export CORTEX_AGENT_DB_PATH=.swarm/memory.db
```
...
cortex-agent agent run coder "task" --enable-memory --provider onnx
```
...
cortex-agent agent run coder "task" --enable-memory --memory-learning true
...
cortex-agent agent run coder "API task" --enable-memory --memory-domain api

# 2. Increase k for complex tasks
cortex-agent agent run coder "Complex feature" --enable-memory --memory-k 10

# 3. Consolidate regularly (dedup + prune)
cortex-agent agent memory consolidate

# 4. Use appropriate provider for task
cortex-agent agent run coder "Quick fix" --enable-memory --provider onnx  # Fast
cortex-agent agent run coder "Critical bug" --enable-memory --provider anthropic  # Best
```
...
3. **Start using** memory in your workflows:
   ```bash
   cortex-agent agent run coder "Your task" --enable-memory
   ```
...
- **Claude-Flow Docs**: https://github.com/ruvnet/cortex-agent
...
Users can now run `cortex-agent agent run coder "task" --enable-memory` to leverage experience-based learning that improves success rates from 23% to 98%.
