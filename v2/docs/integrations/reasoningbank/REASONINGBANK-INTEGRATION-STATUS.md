...
npx cortex-agent memory store pattern "..." --reasoningbank
...
cortex-agent memory store api_pattern "Use environment variables for config"
...
- Bridge cortex-agent memory commands to ReasoningBank SDK
- Add migration tool: `memory migrate --to reasoningbank`

**Implementation Plan:**
1. Import agentic-flow's ReasoningBank SDK directly
2. Wrap SDK methods in cortex-agent memory commands
...
cortex-agent memory store api_best_practice "Always validate input"
