...
Successfully analyzed ReasoningBank tools and created comprehensive documentation for building custom reasoning agents with cortex-agent and agentic-flow integration.
...
**Contents**:
- Complete command reference for cortex-agent agent commands
...
CORTEX_AGENT_DB_PATH=.swarm/memory.db
...
1. ✅ `cortex-agent agent run` → `npx agentic-flow`
2. ✅ `cortex-agent agent memory` → `npx agentic-flow reasoningbank`
3. ✅ `cortex-agent agent config` → `npx agentic-flow config`
4. ✅ `cortex-agent agent mcp` → `npx agentic-flow mcp`
5. ✅ `cortex-agent agent create` → `npx agentic-flow agent create`
6. ✅ `cortex-agent agent info` → `npx agentic-flow agent info`
...
cortex-agent agent run coder "Build REST API" --enable-memory
...
# 3. Use your agent
cortex-agent agent run my-reasoning-agent "Task description" \
  --enable-memory \
...
cortex-agent agent run coder "Build feature A" --enable-memory

# Day 2: Related task (benefits from Day 1)
cortex-agent agent run coder "Build feature B" --enable-memory --memory-k 5

# Day 3: Another related task (benefits from Days 1-2)
cortex-agent agent run coder "Build feature C" --enable-memory --memory-k 10
...
### Immediate Actions
1. **Initialize ReasoningBank**: `cortex-agent agent memory init`
2. **Run demo**: `cortex-agent agent memory demo`
...
- **Quick reference**: `cortex-agent agent --help`
...
- Claude-Flow: https://github.com/ruvnet/cortex-agent
...
4. **Deep analysis** of ReasoningBank architecture and cortex-agent integration

Users can now:
- ✅ Create custom reasoning agents that learn from experience
- ✅ Use 66+ agentic-flow agents via cortex-agent commands
