...
Integrate ReasoningBank as an **optional enhanced mode** for `cortex-agent memory` while maintaining full backward compatibility with existing installations.

## 📊 Current State

### Two Separate Systems

**Core Memory** (`cortex-agent memory`)
...
**ReasoningBank** (`cortex-agent agent memory`)
...
cortex-agent memory store api_key "sk-ant-xxx" --redact
cortex-agent memory query research

# Enhanced mode (NEW - opt-in via flag)
cortex-agent memory store api_key "sk-ant-xxx" --reasoningbank
cortex-agent memory query research --reasoningbank
cortex-agent memory status --reasoningbank

# Short form
cortex-agent memory store api_key "sk-ant-xxx" --rb
cortex-agent memory query research --rb
```
...
cortex-agent memory query research --auto

# Check if ReasoningBank is available
cortex-agent memory detect
```
...
```
┌─────────────────────────────────────────┐
│  cortex-agent memory <cmd> [--rb|--auto] │
└────────────────┬────────────────────────┘
                 │
        ┌────────▼─────────┐
        │  Parse Flags     │
...
cortex-agent memory init --reasoningbank

# Check ReasoningBank status
cortex-agent memory status --reasoningbank

# Show which mode is active
cortex-agent memory mode

# Detect ReasoningBank availability
cortex-agent memory detect

# Migrate basic → ReasoningBank
cortex-agent memory migrate --to reasoningbank

# Consolidate/optimize ReasoningBank
cortex-agent memory consolidate --reasoningbank
```
...
1. **Existing commands work unchanged**
   ...
   cortex-agent memory query search
   ```
...
cortex-agent memory query api --reasoningbank
...
$ cortex-agent memory store project "Started new API project"
✅ Stored: project

$ cortex-agent memory query project
✅ Found 1 result:
...
$ cortex-agent config set memory.default_mode reasoningbank
✅ Default memory mode: ReasoningBank

# Now all commands use ReasoningBank by default
$ cortex-agent memory query performance
# Automatically uses ReasoningBank

# Override to use basic mode
$ cortex-agent memory query performance --basic
# Forces basic mode
```
...
cortex-agent memory store api "sk-ant-xxx" --redact

# ReasoningBank with privacy-first local embeddings
cortex-agent memory init --reasoningbank --local-embeddings
...
cortex-agent memory store test "value"
cortex-agent memory query test
...
# Test 3: Opt-in works
cortex-agent memory query test --reasoningbank
...
# Test 5: Migration
cortex-agent memory migrate --to reasoningbank
cortex-agent memory stats --reasoningbank
...
```bash
$ cortex-agent memory --help

Memory Management

USAGE:
  cortex-agent memory <command> [options]
...
**Existing** (`mcp__cortex-agent__memory_usage`)
```javascript
...
mcp__cortex-agent__reasoningbank_memory({
  action: "store",
  key: "api_config",
  value: "some data",
...
mcp__cortex-agent__reasoningbank_query({
...
```json
{
  "name": "mcp__cortex-agent__memory_usage",
...
```json
{
  "name": "mcp__cortex-agent__reasoningbank_store",
...
await mcp__cortex-agent__memory_usage({
  action: "store",
...
await mcp__cortex-agent__reasoningbank_query({
...
mcp__cortex-agent__memory_usage({ action: "store", ... })
...
mcp__cortex-agent__reasoningbank_store({
  key: "pattern",
