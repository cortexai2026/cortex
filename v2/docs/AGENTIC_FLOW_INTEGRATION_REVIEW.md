...
```json
{
  "dependencies": {
    ...
    "cortex-agent": "^2.0.0",     // Circular dependency (intentional)
    "fastmcp": "^3.19.0",
    "tiktoken": "^1.0.22",
    "zod": "^3.25.76"
  }
...
**Notable:**
- 🔄 **Circular dependency:** `agentic-flow` → `cortex-agent` → `agentic-flow`
- ✅ **Intentional design:** Each package extends the other
- ✅ **Version constraint:** `cortex-agent: "^2.0.0"` (cortex-agent is on 2.7.12)
...
- ✅ **Memory mapping** (cortex-agent model → ReasoningBank pattern model)
...
npm run validate:cortex-agent  # Claude-flow specific tests
...
### Agentic-Flow Documentation (in cortex-agent)
...
- **Issue:** cortex-agent ↔ agentic-flow circular dependency
...
npm run validate:cortex-agent  # If agentic-flow scripts are accessible
```
...
npx cortex-agent@alpha memory store "test" "value"
npx cortex-agent@alpha memory query "test"
```
...
- ℹ️ **better-sqlite3** - Native dependency (requires compilation)
  - **Mitigation:** Moved to optionalDependencies in cortex-agent
...
**Summary:**
The agentic-flow integration in cortex-agent is **well-designed and properly implemented**. The ReasoningBank adapter demonstrates best practices in error handling, caching, and graceful degradation. The only issue is running an outdated version (1.7.4 vs 1.8.3), which is easily resolved.
...
**Future-Proof:**
The wildcard dependency (`"*"`) ensures cortex-agent stays current with agentic-flow releases. Regular updates are recommended to maintain compatibility and access new features.
