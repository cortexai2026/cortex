# Docker Verification Report - cortex-agent@2.7.1
...
The cortex-agent@2.7.1 package has been successfully published to npm and verified in a clean Docker environment. All critical bug fixes are present and functional with no regressions detected.
...
Installation: npm install -g cortex-agent@2.7.1
Test Directory: /test
...
The cortex-agent@2.7.1 package:
...
docker build -t cortex-agent-test:2.7.1 tests/docker/
docker run --rm cortex-agent-test:2.7.1

# Run pattern verification
docker run --rm cortex-agent-test:2.7.1 /test/verify-patterns.sh
...
npm install cortex-agent@2.7.1

# Verify installation
cortex-agent --version
cortex-agent mcp tools | grep neural
```
