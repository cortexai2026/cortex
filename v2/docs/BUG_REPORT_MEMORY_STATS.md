...
### Issue
- **Command**: `npx cortex-agent@alpha memory stats`
...
# Force basic mode
$ npx cortex-agent@alpha memory stats --basic
...
3. **Test with data in JSON only**
   ```bash
   npx cortex-agent@alpha memory store "test" "value" --basic
   npx cortex-agent@alpha memory stats
...
4. **Test with data in both backends**
   ```bash
   npx cortex-agent@alpha memory store "json-key" "json-value" --basic
   npx cortex-agent@alpha memory store "rb-key" "rb-value" --reasoningbank
   npx cortex-agent@alpha memory stats
...
5. **Test with flags**
   ...
   npx cortex-agent@alpha memory stats --reasoningbank
   npx cortex-agent@alpha memory stats --basic
   npx cortex-agent@alpha memory stats --auto
...
### Reproduction Steps
1. Initialize ReasoningBank: `npx cortex-agent@alpha memory init --reasoningbank`
2. Store data: `npx cortex-agent@alpha memory store test-key "test value" --reasoningbank`
3. Verify storage: `npx cortex-agent@alpha memory list --reasoningbank` (shows data)
4. Run stats: `npx cortex-agent@alpha memory stats` (shows zeros ❌)
