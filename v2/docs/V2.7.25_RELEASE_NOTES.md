# cortex-agent v2.7.25 Release Notes
...
```bash
npx cortex-agent@alpha memory store "test" "Clean npx experience!"
...
```bash
npm install -g cortex-agent@alpha
cortex-agent memory store "api-design" "REST with JWT auth"
# ✅ Transformer model loads successfully

cortex-agent memory query "authentication patterns"
...
**For Users:**
1. Try npx for quick testing: `npx cortex-agent@alpha memory store "test" "value"`
2. Install globally for semantic search: `npm install -g cortex-agent@alpha`
...
- **Package:** https://www.npmjs.com/package/cortex-agent/v/2.7.25
