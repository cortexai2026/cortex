...
**Issue:** `npx cortex-agent@alpha memory` commands fail with dependency errors
...
```bash
$ npx cortex-agent@alpha memory store "api" "REST"
...
```bash
$ npx cortex-agent@alpha memory status
...
📚 Solutions:
  1. LOCAL INSTALL (Recommended):
     npm install && node_modules/.bin/cortex-agent memory store "key" "value"

  2. USE MCP TOOLS instead:
     mcp__cortex-agent__memory_usage({ action: "store", key: "test", value: "data" })

  3. USE JSON FALLBACK:
     npx cortex-agent@alpha memory store "key" "value" --basic
...
node_modules/.bin/cortex-agent memory stats
...
{
  "scripts": {
    "memory:stats": "cortex-agent memory stats",
    "memory:list": "cortex-agent memory list",
    "memory:store": "cortex-agent memory store",
    "memory:query": "cortex-agent memory query"
  }
}
```
...
# Store key-value
node_modules/.bin/cortex-agent memory store "key" "value"
node_modules/.bin/cortex-agent memory store "key" "value" --namespace "project"

# Query/Search
node_modules/.bin/cortex-agent memory query "search term"
node_modules/.bin/cortex-agent memory query "search" --namespace "sparc"

# List namespaces
node_modules/.bin/cortex-agent memory list

# Export/Import
node_modules/.bin/cortex-agent memory export backup.json
node_modules/.bin/cortex-agent memory import backup.json

# Clear namespace
node_modules/.bin/cortex-agent memory clear --namespace "temp"
```
...
$ node_modules/.bin/cortex-agent memory store "test" "value"
...
$ node_modules/.bin/cortex-agent memory query "test"
✅ Found 1 result(s):
...
alias cfmem="node_modules/.bin/cortex-agent memory"
...
{
  "scripts": {
    "memory:export": "cortex-agent memory export .memory-backup.json",
    "memory:import": "cortex-agent memory import .memory-backup.json",
    "memory:clear": "cortex-agent memory clear --namespace temp"
  }
}
```
...
# Or (best):
mcp__cortex-agent__memory_usage({ action: "retrieve" })  ✅
```
