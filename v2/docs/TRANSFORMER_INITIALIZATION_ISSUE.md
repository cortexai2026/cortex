...
**Versions:** cortex-agent@2.7.25, agentic-flow@1.8.9
...
**Affected Versions:** cortex-agent@2.7.24, agentic-flow@1.8.7
...
[Embeddings] For semantic search, install globally: npm install -g cortex-agent
✅ Stored successfully
```
...
**After fix (v2.7.25):**
```bash
npx cortex-agent@alpha memory store "test" "Clean experience!"
...
npx cortex-agent@alpha memory query "search"
...
```bash
# Store data
npx cortex-agent@alpha memory store "key1" "value1"
npx cortex-agent@alpha memory store "key2" "value2"
...
# Query data
npx cortex-agent@alpha memory query "key"
...
if (isNpx && !process.env.FORCE_TRANSFORMERS) {
  ...
  console.log('[Embeddings] For semantic search, install globally: npm install -g cortex-agent');
