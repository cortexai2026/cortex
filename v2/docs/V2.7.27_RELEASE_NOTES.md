...
### Issue
After v2.7.26 release, users experienced `ENOTEMPTY` errors when running `npx cortex-agent` commands:
...
#### 1. **Retry Logic with Exponential Backoff** (`bin/cortex-agent`)
```bash
execute_with_retry() {
...
❌ Failed after 3 attempts. Please try:
   ...
   2. Use global installation: npm install -g cortex-agent
   3. Report issue: https://github.com/ruvnet/cortex-agent/issues/856
```
...
```bash
# Build test image
docker build -f tests/docker/Dockerfile.npx-test -t cortex-agent-npx-test .

# Run concurrent execution tests
docker run --rm cortex-agent-npx-test /test/test-concurrent.sh

# Run cache cleanup tests
docker run --rm cortex-agent-npx-test /test/test-cache-cleanup.sh
```
...
npx cortex-agent@latest <command>

# Or use alpha tag
npx cortex-agent@alpha <command>
```
...
- **GitHub Issue**: [#856](https://github.com/ruvnet/cortex-agent/issues/856)
...
For CI/CD pipelines using `npx cortex-agent`:
...
**Full Changelog**: [v2.7.26...v2.7.27](https://github.com/ruvnet/cortex-agent/compare/v2.7.26...v2.7.27)
