...
The AgentDB v1.3.9 integration (PR #830) maintains **100% backward compatibility** with existing cortex-agent installations. No existing code will break.
...
- `cortex-agent memory list`
- `cortex-agent memory export`
- `cortex-agent memory import`
- `cortex-agent memory stats`
- All SPARC, hooks, and swarm commands unchanged

**New Commands (Optional):**
- `cortex-agent memory vector-search <query>` (requires AgentDB)
- `cortex-agent memory store-vector <key> <value>` (requires AgentDB)
- `cortex-agent memory agentdb-info` (informational only)
...
import { AgentDBMemoryAdapter } from 'cortex-agent/memory';
const memory = new AgentDBMemoryAdapter({ mode: 'hybrid' });

// 2. AgentDB backend (NEW - opt-in)
import { AgentDBBackend } from 'cortex-agent/memory';

// 3. Migration bridge (NEW - opt-in)
import { LegacyDataBridge } from 'cortex-agent/memory';
```
...
- `mcp__cortex-agent__memory_usage` - Works exactly as before
- `mcp__cortex-agent__memory_search` - Works exactly as before
- `mcp__cortex-agent__swarm_init` - Works exactly as before
- `mcp__cortex-agent__agent_spawn` - Works exactly as before
- `mcp__cortex-agent__task_orchestrate` - Works exactly as before
...
- **npm install cortex-agent** - Works exactly as before
- **npx cortex-agent@latest** - All commands work
- **Existing projects** - No code changes needed
- **CI/CD pipelines** - No workflow changes needed

### ✅ Existing Code
- **All imports** - `import { SharedMemory } from 'cortex-agent/memory'` works
...
**Bottom Line:**
> Existing cortex-agent installations will **NOT** break. AgentDB integration is an **optional enhancement** that existing users can adopt at their own pace. All legacy functionality is preserved and will continue to work exactly as before.
