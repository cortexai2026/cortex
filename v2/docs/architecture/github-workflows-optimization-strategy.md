...
- name: Real agent coordination test
  run: |
    ...
    ./bin/cortex-agent swarm init --topology mesh
    ./bin/cortex-agent agent spawn --type coder --count 2

    # Verify agents can communicate
    ./bin/cortex-agent task orchestrate --task "Simple coordination test"
```
