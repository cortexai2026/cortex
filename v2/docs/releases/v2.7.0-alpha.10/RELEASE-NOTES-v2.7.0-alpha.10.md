...
```bash
# Store memory
$ ./cortex-agent memory store test "validation data" --namespace semantic --reasoningbank
✅ Stored successfully in ReasoningBank
🔍 Semantic search: enabled

# Query memory (NOW WORKS!)
$ ./cortex-agent memory query "validation" --namespace semantic --reasoningbank
...
# List memories
$ ./cortex-agent memory list --namespace semantic --reasoningbank
...
```bash
# Store
$ ./cortex-agent memory store api_test "REST API configuration" --namespace semantic --reasoningbank
✅ Stored successfully

# Query immediately
$ ./cortex-agent memory query "REST API" --namespace semantic --reasoningbank
...
```bash
# Store a test memory
npx cortex-agent@alpha memory store test "semantic search validation" --namespace semantic --reasoningbank

# Query it back
npx cortex-agent@alpha memory query "semantic search" --namespace semantic --reasoningbank
...
Users should:
1. ✅ Update to alpha.10: `npm install -g cortex-agent@alpha`
...
**Recommendation**: Safe to deploy `cortex-agent@2.7.0-alpha.10` for production use.
