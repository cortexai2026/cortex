...
```bash
$ npx cortex-agent@alpha memory store test "data" --reasoningbank
...
```bash
$ ./cortex-agent memory store semantic_test "config data" --reasoningbank
✅ Stored successfully
[ReasoningBank] Database connection closed
$ echo $?  # Exit code: 0 ✅
```
...
### Before (alpha.8)
```bash
$ timeout 10 npx cortex-agent@alpha memory store test "data"
# Timed out after 10s ❌
```

### After (alpha.9)
```bash
$ timeout 5 node bin/cortex-agent.js memory store test "data" --reasoningbank
✅ Stored successfully in ReasoningBank
[ReasoningBank] Database connection closed
...
npx cortex-agent@alpha memory store test_fix "verification" --reasoningbank
...
Users should:
1. ✅ Update to alpha.9: `npm install -g cortex-agent@alpha`
...
**Recommendation**: Safe to deploy `cortex-agent@2.7.0-alpha.9` for production use.
