...
### Execute an Agent
```bash
./bin/cortex-agent agent execute coder "Write a REST API endpoint"
```

### With Custom Provider
```bash
# Using OpenRouter (99% cost savings)
./bin/cortex-agent agent execute coder "Optimize this algorithm" --provider openrouter

# Using Gemini (free tier)
./bin/cortex-agent agent execute researcher "Research Vue.js patterns" --provider gemini

# Using ONNX (local, free)
./bin/cortex-agent agent execute coder "Simple function" --provider onnx
```
...
**[View complete list with `./bin/cortex-agent agent agents`]**
...
./bin/cortex-agent agent execute coder "Task"
```

**OpenRouter (99% cost savings):**
```bash
export OPENROUTER_API_KEY="sk-or-..."
./bin/cortex-agent agent execute coder "Task" --provider openrouter
```

**Gemini (Free tier):**
```bash
export GOOGLE_GEMINI_API_KEY="..."
./bin/cortex-agent agent execute coder "Task" --provider gemini
```

**ONNX (Local, free):**
```bash
# First run downloads 4.9GB Phi-4 model
./bin/cortex-agent agent execute coder "Task" --provider onnx
```
