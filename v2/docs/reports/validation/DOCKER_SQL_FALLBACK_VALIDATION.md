...
Package: /app (mounted cortex-agent source)
```
...
npx cortex-agent memory query 'pathfinding' --reasoningbank --namespace test
```
...
**Status:** ✅ **CONFIRMED**
```bash
$ npx cortex-agent memory status --reasoningbank
