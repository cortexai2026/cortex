...
**Date**: 2025-10-12
**Test Scope**: All commands listed in `cortex-agent agent --help`
...
**Test**:
```bash
./bin/cortex-agent agent run --help
```
...
**Test**:
```bash
./bin/cortex-agent agent booster help
```
**Result**:
...
**Test**:
```bash
./bin/cortex-agent agent booster benchmark
```
**Result**:
...
**Test**:
```bash
./bin/cortex-agent agent memory status
```
**Result**:
...
**Test**:
```bash
./bin/cortex-agent agent memory list
```
**Result**:
...
**Test**:
```bash
./bin/cortex-agent agent config get provider
```
...
**Test**:
```bash
./bin/cortex-agent agent mcp status
```
**Result**:
...
**Test**:
```bash
./bin/cortex-agent agent spawn --type researcher --name "Test Agent"
```
**Result**:
...
- Type: general
- Status: active
- Saved to: `.cortex-agent/agents/general-1760280512706.json`
...
**Test**:
```bash
./bin/cortex-agent agent info coder-1758290254250
```
...
**Test**:
```bash
./bin/cortex-agent agent terminate coder-1758290254250
```
**Result**:
...
**Test**:
```bash
./bin/cortex-agent agent hierarchy
```
...
Examples:
  ...
  cortex-agent agent hierarchy show
```
...
**Test**:
```bash
./bin/cortex-agent agent ecosystem
```
...
- Main help (`cortex-agent --help`) shows Agent Booster
- Agent help (`cortex-agent agent --help`) shows all 18 commands
...
1. **Agent Info**: Queries agentic-flow agent registry, not internal agents
   - Internal agents tracked separately in `.cortex-agent/agents/`
...
✅ **All 18 commands in `cortex-agent agent --help` are functional**
