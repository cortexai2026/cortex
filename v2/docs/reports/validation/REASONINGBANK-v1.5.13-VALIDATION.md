...
**Package**: `cortex-agent@2.7.0-alpha.8`
**Date**: 2025-10-13
...
### Initialization
```bash
npx cortex-agent@alpha memory init --reasoningbank
...
### Memory Storage
```bash
npx cortex-agent@alpha memory store test_key "validation test data" --namespace test
...
npx cortex-agent@alpha memory store api-pattern "Use env vars for keys" --reasoningbank

# Query semantically
npx cortex-agent@alpha memory query "API configuration" --reasoningbank
...
npm install -g cortex-agent@2.7.0-alpha.7
...
**Recommendation**: Safe to deploy `cortex-agent@2.7.0-alpha.8` for production use.
