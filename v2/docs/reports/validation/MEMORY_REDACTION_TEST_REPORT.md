...
Added optional API key redaction to cortex-agent memory commands with two-level security:
...
**Command:**
```bash
./bin/cortex-agent memory store test_warning "ANTHROPIC_API_KEY=TEST_API_KEY_PLACEHOLDER" --namespace test
```
...
**Command:**
```bash
./bin/cortex-agent memory store test_redacted "ANTHROPIC_API_KEY=TEST_API_KEY_PLACEHOLDER" --namespace test --redact
```
...
**Command:**
```bash
./bin/cortex-agent memory query test --namespace test --redact
```
...
**Command:**
```bash
./bin/cortex-agent memory clear --namespace test
```
