...
export class MetricsReader {
  private metricsDir = '.cortex-agent/metrics';
  private sessionsDir = '.cortex-agent/sessions';
  ...
  async getMCPServerStatus(): Promise<MCPServerStatus> {
    try {
      ...
      const { stdout } = await execAsync('ps aux | grep -E "mcp-server\\.js|cortex-agent mcp start" | grep -v grep | wc -l');
