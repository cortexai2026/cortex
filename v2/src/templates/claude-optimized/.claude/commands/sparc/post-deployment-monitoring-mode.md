...
1. Run directly: `npx cortex-agent sparc run post-deployment-monitoring-mode "your task"`
2. Use in workflow: Include `post-deployment-monitoring-mode` in your SPARC workflow
3. Delegate tasks: Use `new_task` to assign work to this mode
4. Batch operations: `npx cortex-agent sparc run post-deployment-monitoring-mode --batch "monitor all services"`

## Example

```bash
# Standard monitoring setup
npx cortex-agent sparc run post-deployment-monitoring-mode "monitor authentication service"

# Batch monitoring across regions
npx cortex-agent sparc run post-deployment-monitoring-mode --all-regions "comprehensive health check"

# Parallel performance analysis
npx cortex-agent sparc run post-deployment-monitoring-mode --perf-analysis "analyze all service metrics"

# Concurrent log monitoring
npx cortex-agent sparc run post-deployment-monitoring-mode --log-analysis "real-time error detection"
```
