...
npx cortex-agent swarm "your complex task" --strategy <type> [options]

# Batch swarm operations
npx cortex-agent swarm batch --tasks "frontend:dev,backend:dev,database:architect" --parallel
```
...
npx cortex-agent swarm multi-strategy --config '{
...
npx cortex-agent swarm batch-dev --services '{
...
npx cortex-agent swarm batch-research --topics '{
...
npx cortex-agent swarm batch-test --suites '{
...
npx cortex-agent swarm pipeline --config '{
  "stages": [
...
npx cortex-agent swarm analyze-pipeline --stages '{
...
npx cortex-agent swarm deploy-multi --regions '{
...
npx cortex-agent swarm federated-train --nodes 20 --parallel --aggregate-method "secure"
```
...
npx cortex-agent swarm monitor-batch --metrics '{
...
npx cortex-agent swarm auto-manage --rules '{
...
npx cortex-agent swarm distribute --algorithm "weighted" --factors '{
  "agent-capacity": 0.4,
  "task-complexity": 0.3,
  "priority": 0.3
}'
...
npx cortex-agent swarm batch-execute --resilient '{
...
npx cortex-agent swarm batch --cache '{
...
npx cortex-agent swarm optimize-network --compression "gzip" --batching 100 --keepalive
```
...
npx cortex-agent swarm full-stack --project "e-commerce" --parallel-components '{
...
npx cortex-agent swarm data-pipeline --config '{
...
npx cortex-agent swarm security-audit --parallel-scans '{
