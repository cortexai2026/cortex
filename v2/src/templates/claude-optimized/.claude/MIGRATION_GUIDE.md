...
# Install or update cortex-agent
...
npx cortex-agent sparc run code "test batch operations" --non-interactive
```
...
npx cortex-agent memory export pre-migration-backup.json
...
npx cortex-agent sparc run architect "design user service"
# Wait...
npx cortex-agent sparc run architect "design auth service"
# Wait...
npx cortex-agent sparc run architect "design API gateway"
```
...
npx cortex-agent sparc run architect "design complete microservices architecture with user, auth, and gateway services"
...
npx cortex-agent sparc run code "create user controller"
npx cortex-agent sparc run code "create user service"
npx cortex-agent sparc run code "create user repository"
```
...
npx cortex-agent sparc run code "implement complete user management with controller, service, repository, and tests"
...
```bash
# Old
npx cortex-agent sparc run code "create user API"

# New with options
npx cortex-agent sparc run code "create user API" --non-interactive --parallel --batch-size=10
```
...
if npx cortex-agent sparc run code "test parallel" --non-interactive --dry-run 2>&1 | grep -q "parallel"; then
    echo "✅"
else
...
if npx cortex-agent memory stats > /dev/null 2>&1; then
    echo "✅"
else
...
time npx cortex-agent sparc run code "create test component" --non-interactive > /dev/null 2>&1
...
if [ -f "pre-migration-backup.json" ]; then
    npx cortex-agent memory import pre-migration-backup.json
fi
...
npx cortex-agent sparc run architect "test batch architecture" --non-interactive
npx cortex-agent sparc run tdd "test parallel testing" --non-interactive
npx cortex-agent sparc run code "test concurrent generation" --non-interactive
```
...
time npx cortex-agent sparc run code "create user CRUD" --legacy

echo "Testing new method..."
time npx cortex-agent sparc run code "create user CRUD" --non-interactive
```
...
```bash
# Test full workflow
npx cortex-agent sparc tdd "implement complete feature with batchtools"
```
...
const tests = [
  {
    ...
    old: 'npx cortex-agent sparc run code "create user CRUD" --legacy',
    new: 'npx cortex-agent sparc run code "create user CRUD with all operations"',
  },
  {
    name: 'Test Suite Creation',
    old: 'npx cortex-agent sparc run tdd "create auth tests" --legacy',
    new: 'npx cortex-agent sparc run tdd "create complete auth test suite"',
  },
  {
    name: 'Architecture Design',
    old: 'npx cortex-agent sparc run architect "design microservices" --legacy',
    new: 'npx cortex-agent sparc run architect "design complete microservices architecture"',
  },
];
...
batchtool orchestrate --prototypes \
  --idea-1 "npx cortex-agent sparc run code 'social feed prototype'" \
  --idea-2 "npx cortex-agent sparc run code 'marketplace prototype'" \
  --idea-3 "npx cortex-agent sparc run code 'subscription prototype'" \
  --test-all "npx cortex-agent sparc run tdd 'test all prototypes'" \
  --compare "npx cortex-agent sparc run architect 'analyze best approach'"
```
...
# Run with batchtools
npx cortex-agent sparc run <mode> "<task>" --non-interactive
