...
import { getCortexAgentRoot, getCortexAgentBin } from '../utils/paths.js';
...
Ensure your implementation is complete, well-structured, and follows best practices.`,
  ...
  private async executeTaskWithAgent(task: TaskDefinition, agent: AgentState): Promise<any> {
    ...
    try {
      // Use Cortex Agent executor for full SPARC system in non-interactive mode
      const { CortexAgentExecutor } = await import('./cortex-agent-executor.ts');
      const executor = new CortexAgentExecutor({
        logger: this.logger,
        claudeFlowPath: getCortexAgentBin(),
        enableSparc: true,
  ...
  private extractTargetDirectory(task: TaskDefinition): string | null {
    ...
    if (targetDir) {
      ...
      if (!targetDir.startsWith('/')) {
        targetDir = `${getCortexAgentRoot()}/${targetDir}`;
      }
    }
    ...
    try {
      ...
      const command = new Deno.Command('claude', {
        ...
        env: {
          ...
          CORTEX_AGENT_MEMORY_NAMESPACE: `swarm-${this.swarmId.id}`,
        },
        stdin: 'null',
        stdout: 'piped',
        stderr: 'piped',
  ...
  private async simulateTaskExecution(
    ...
    if (targetDir) {
      ...
      workDir = targetDir.startsWith('/') ? targetDir : `${getCortexAgentRoot()}/${targetDir}`;
...
// Generated by Cortex Agent Swarm

console.log('Hello, World!');
console.log('This application was created by the Cortex Agent Swarm system.');
...
`;
      ...
      const packageJson = {
        name: 'hello-world',
        version: '1.0.0',
        description: 'Hello World application created by Cortex Agent Swarm',
        main: 'index.js',
        scripts: {
          start: 'node index.js',
          test: 'node test.js',
        },
        keywords: ['hello-world', 'swarm', 'cortex-agent'],
        author: 'Cortex Agent Swarm',
        license: 'MIT',
      };
...
This application was created by the Cortex Agent Swarm system.
...
## Overview
This documentation was generated by the Cortex Agent Swarm system.
