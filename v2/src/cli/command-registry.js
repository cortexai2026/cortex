...
export function registerCoreCommands() {
  commandRegistry.set('init', {
    ...
    examples: [
      'npx cortex-agent@latest init --sparc  # Recommended: Full SPARC setup',
...
First-time users should run: npx cortex-agent@latest init --sparc`,
  });
...
Benefits:
  ...
  commandRegistry.set('maestro', {
    handler: () => {
      ...
      console.log('Please use: npx cortex-agent maestro help');
...
See 'cortex-agent help agent' for full documentation.`,
  });
...
Setup:
  ...
  2. cortex-agent agent config set OPENROUTER_API_KEY sk-or-xxx
  3. cortex-agent proxy start --daemon
...
Use with init command:
  cortex-agent init --batch-init project1,project2,project3
  cortex-agent init --config batch-config.json --parallel`,
  });
...
For more information: https://github.com/ruvnet/cortex-agent/issues/166`,
  });
...
export function showAllCommands() {
  ...
  console.log();
  console.log('Use "cortex-agent help <command>" for detailed usage information');
}
