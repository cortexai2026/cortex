...
async function showMigrationStatus(projectPath: string): Promise<void> {
  ...
  if (!analysis.hasOptimizedPrompts) {
    console.log(chalk.bold('\n💡 Recommendations:'));
    console.log('  • Run migration analysis: cortex-agent migrate analyze');
    console.log('  • Start with dry run: cortex-agent migrate --dry-run');
    console.log('  • Use selective strategy: cortex-agent migrate --strategy selective');
  }
