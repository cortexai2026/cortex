...
const HELP_TOPICS = [
  {
    ...
    tutorial: [
      ...
      '   cortex-agent config init',
      '',
      '2. Start the orchestration system:',
      '   cortex-agent start',
      '',
      '3. In another terminal, spawn your first agent:',
      '   cortex-agent agent spawn researcher --name "My Research Agent"',
      '',
      '4. Create a task for the agent:',
      '   cortex-agent task create research "Find information about AI trends"',
      '',
      '5. Monitor progress:',
      '   cortex-agent status',
      '',
      ...
      '   cortex-agent repl',
      '',
      'For more help, try: cortex-agent help <topic>',
    ],
  ...
  },
  {
    ...
    examples: [
      {
        ...
        command: 'cortex-agent agent spawn researcher --name "Research Assistant"',
        explanation:
          'Creates a new research agent with specialized capabilities for information gathering',
      },
      {
        description: 'List all active agents',
        command: 'cortex-agent agent list',
      ...
      },
      {
        ...
        command: 'cortex-agent agent info agent-001',
        explanation: 'Displays comprehensive information about a specific agent',
      },
      {
        description: 'Terminate an agent',
        command: 'cortex-agent agent terminate agent-001',
    ...
    tutorial: [
      ...
      '• Monitor agent performance with "cortex-agent status"',
  ...
  },
  {
    ...
    examples: [
      {
        ...
        command: 'cortex-agent claude spawn "implement user authentication" --research --parallel',
        explanation:
      ...
      },
      {
        ...
        command: 'cortex-agent claude spawn "fix payment bug" --no-permissions',
    ...
    tutorial: [
      ...
      '• CORTEX_AGENT_MODE: Development mode setting',
      '• CORTEX_AGENT_COVERAGE: Target test coverage percentage',
      '• CORTEX_AGENT_COMMIT: Commit frequency setting',
      '',
      'Common Use Cases:',
    ...
    examples: [
      {
        ...
        command: 'cortex-agent workflow run research-pipeline.json --watch',
  ...
  },
  {
    ...
    tutorial: [
      ...
      '',
      'Configuration Files:',
      '• Global: ~/.cortex-agent/config.json',
      '• Project: ./cortex-agent.config.json',
      '• Profiles: ~/.cortex-agent/profiles/',
      '',
      'Environment Variables:',
      '• CORTEX_AGENT_LOG_LEVEL: Override log level',
      '• CORTEX_AGENT_MAX_AGENTS: Override agent limit',
      '• CORTEX_AGENT_MCP_PORT: Override MCP port',
    ],
  ...
  },
  {
    ...
    examples: [
      {
        ...
        command: 'cortex-agent status --component all',
        explanation: 'Comprehensive health check of all components',
      },
      {
        description: 'Enable debug logging',
        command: 'cortex-agent start --log-level debug',
        explanation: 'Start with verbose logging for debugging',
      },
      {
        description: 'Validate configuration',
        command: 'cortex-agent config validate cortex-agent.config.json --strict',
        explanation: 'Check configuration file for errors',
      },
      {
        description: 'Reset to defaults',
        command: 'cortex-agent config reset --confirm',
    ...
    tutorial: [
      ...
      '• Solution: Ensure Claude-Flow is started with "cortex-agent start"',
      ...
      '',
      'Debug Commands:',
      '• cortex-agent status: System health check',
      '• cortex-agent config validate: Configuration check',
      '• cortex-agent --verbose: Enable detailed logging',
      '• cortex-agent monitor: Real-time diagnostics',
    ],
...
function showAllTopics(): void {
  ...
  console.log();
  console.log(chalk.gray('Use "cortex-agent help <topic>" for detailed information.'));
}
...
async function showTopicHelp(topicName: string, options: any): Promise<void> {
  ...
  if (!topic) {
    ...
    if (similar.length > 0) {
      ...
      console.log(chalk.gray('Use "cortex-agent help --all" to see all topics.'));
    }
    return;
  }
  ...
  if (options.tutorial && topic.tutorial) {
    ...
    for (const line of topic.tutorial) {
      if (line.trim().startsWith('cortex-agent')) {
  ...
  if (topic.related && topic.related.length > 0) {
    ...
    for (const related of topic.related) {
      console.log(chalk.cyan(`  cortex-agent help ${related}`));
    }
    console.log();
  }
}
