/**
 * Claude Swarm Mode - Self-orchestrating agent swarms using cortex-agent
 */
...
export async function swarmAction(ctx: CommandContext) {
  ...
  if (!objective) {
    ...
    console.log('\nExamples:');
    console.log('  cortex-agent swarm "Build a REST API"');
    console.log('  cortex-agent swarm "Research cloud architecture"');
    console.log('\nOptions:');
  ...
  try {
    ...
    if (options.background) {
      console.log(
        `Running in background mode. Check status with: cortex-agent swarm status ${swarmId}`,
      );
...
exit \${PIPESTATUS[0]}`;
      ...
      // For now, let's use the cortex-agent claude spawn command instead
      ...
      console.log(`    → Using: cortex-agent ${claudeFlowArgs.join(' ')}`);

      // Get the path to cortex-agent binary
      ...
      const claudeFlowBin = `${projectRoot}/bin/cortex-agent`;

      // Execute cortex-agent command
