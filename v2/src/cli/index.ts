...
const cli = new Command()
  ...
  // .meta() commented out - not available
  .option('-c, --config <path>', 'Path to configuration file', './cortex-agent.config.json')
...
async function handleError(error: unknown, options?: any): Promise<void> {
  ...
  if (process.env['CORTEX_AGENT_DEBUG'] === 'true' || options?.verbose) {
  ...
  if (!options?.quiet) {
    ...
    console.error(chalk.gray('Or use "cortex-agent help" to see available commands'));
  }
...
async function setupLogging(options: any): Promise<void> {
  ...
  try {
    if (options.config) {
      ...
      try {
        await configManager.load('./cortex-agent.config.json');
      } catch {
