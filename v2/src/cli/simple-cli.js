...
🎮 ENTERPRISE QUICK START:
  ...
  npx cortex-agent@2.0.0 init --sparc
...
async function main() {
  ...
  if (
    ...
    if (detailedHelp && !detailedHelp.includes('Help not available')) {
      ...
      console.log('\nRun "cortex-agent --help" to see available commands.');
    }
    return;
  }
  ...
  if (hasCommand(command)) {
    try {
      ...
      console.log(`\nRun "cortex-agent ${command} --help" for usage information.`);
      return;
    }
  }
...
`);
    ...
    config: async (key) => {
      try {
        const config = JSON.parse(await readTextFile('cortex-agent.config.json'));
        if (key) {
  ...
  function updatePrompt() {
    rl.setPrompt(
      replState.currentSession ? `cortex-agent:${replState.currentSession}> ` : 'cortex-agent> ',
    );
  }
