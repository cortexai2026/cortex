...
## Usage
\`\`\`bash
cortex-agent swarm "Research objective" --strategy research --mode distributed --parallel
\`\`\`
...
## Usage
\`\`\`bash
cortex-agent swarm "Build application" --strategy development --mode hierarchical --parallel
\`\`\`
...
## Usage
\`\`\`bash
cortex-agent swarm "Analyze data" --strategy analysis --parallel --max-agents 10
\`\`\`
...
## Usage
\`\`\`bash
cortex-agent swarm "Test application" --strategy testing --mode mesh --parallel
\`\`\`
...
## Usage
\`\`\`bash
cortex-agent swarm "Optimize performance" --strategy optimization --mode hybrid --parallel
\`\`\`
...
## Usage
\`\`\`bash
cortex-agent swarm "System maintenance" --strategy maintenance --mode centralized --monitor
\`\`\`
...
cortex-agent swarm "Research modern web frameworks" --strategy research --mode distributed --parallel --max-agents 6

# Market analysis with coordinated batch operations
cortex-agent swarm "Analyze AI market trends" --strategy research --parallel --monitor --timeout 120
\`\`\`

### Development Tasks with Batch Coordination
\`\`\`bash
# Hierarchical development with batch file operations
cortex-agent swarm "Build microservice API" --strategy development --mode hierarchical --parallel --max-agents 8

# React dashboard with coordinated component development
cortex-agent swarm "Create React dashboard" --strategy development --parallel --monitor --output sqlite
\`\`\`

### Analysis Tasks with Batch Processing
\`\`\`bash
# Mesh-coordinated data analysis
cortex-agent swarm "Analyze user behavior data" --strategy analysis --mode mesh --parallel --max-agents 10

# Performance analysis with monitoring
cortex-agent swarm "Performance analysis of application" --strategy analysis --monitor --output csv
\`\`\`

### Testing Tasks with Parallel Validation
\`\`\`bash
# Comprehensive parallel testing
cortex-agent swarm "Comprehensive testing suite" --strategy testing --parallel --max-agents 12

# Security testing with distributed coordination
cortex-agent swarm "Security testing analysis" --strategy testing --mode distributed --monitor
\`\`\`

### Optimization Tasks with Hybrid Coordination
\`\`\`bash
# Database optimization with hybrid approach
cortex-agent swarm "Optimize database queries" --strategy optimization --mode hybrid --parallel

# Frontend optimization with batch processing
cortex-agent swarm "Frontend performance optimization" --strategy optimization --monitor --max-agents 6
\`\`\`

### Maintenance Tasks with Centralized Control
\`\`\`bash
# Dependency updates with centralized coordination
cortex-agent swarm "Update dependencies safely" --strategy maintenance --mode centralized --monitor

# System health checks with batch operations
cortex-agent swarm "System health check" --strategy maintenance --parallel --output json
\`\`\`
