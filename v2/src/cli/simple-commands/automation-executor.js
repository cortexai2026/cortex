/**
 * Modular Automation Executor for Cortex Agent
 * 
...
1. **HOOKS INTEGRATION** (CRITICAL):
   - BEFORE starting: \`npx cortex-agent@alpha hooks pre-task --description "${task.description}"\`
   - AFTER each file operation: \`npx cortex-agent@alpha hooks post-edit --file "[filepath]"\`
   - WHEN complete: \`npx cortex-agent@alpha hooks post-task --task-id "${task.id}"\`

2. **MEMORY STORAGE** (CRITICAL):
   - Store findings: \`npx cortex-agent@alpha memory store "agent/${agent.id}/findings" "[your_findings]"\`
   - Store results: \`npx cortex-agent@alpha memory store "agent/${agent.id}/results" "[your_results]"\`
   - Check other agents: \`npx cortex-agent@alpha memory search "agent/*"\`
...
1. HOOKS: Use cortex-agent hooks for coordination:
   - Run "npx cortex-agent@alpha hooks pre-task --description '[your task]'" before starting
   - Run "npx cortex-agent@alpha hooks post-edit --file '[file]'" after each file operation  
   - Run "npx cortex-agent@alpha hooks post-task --task-id '${agent.id}'" when complete

2. MEMORY: Store all findings and results:
   - Use "npx cortex-agent@alpha memory store 'agent/${agent.id}/[key]' '[value]'" for important data
   - Check "npx cortex-agent@alpha memory search 'agent/*'" for coordination with other agents
...
- Prepare final deployment and documentation`
  ...
  async executeHook(hookType, params) {
    try {
      ...
      let hookCommand = `npx cortex-agent@alpha hooks ${hookType}`;
  ...
  async storeTaskResult(taskId, result) {
    try {
      ...
      execSync(`npx cortex-agent@alpha memory store "workflow/${this.executionId}/${taskId}" '${resultJson}'`, {
        stdio: 'pipe'
      });
