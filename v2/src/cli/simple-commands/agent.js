...
async function executeAgentTask(subArgs, flags) {
  ...
  if (!agentType || !task) {
    ...
    console.log('\nExamples:');
    console.log('  cortex-agent agent run coder "Create a REST API"');
    console.log('  cortex-agent agent run researcher "Research AI trends" --provider openrouter');
    console.log('  cortex-agent agent run reviewer "Review code for security" --provider onnx');
    return;
  }
...
async function listAgenticFlowAgents(subArgs, flags) {
  ...
  try {
    ...
    console.log(stdout);
    console.log('\nUsage:');
    console.log('  cortex-agent agent run <agent-type> "<task>"');
    console.log('\nExamples:');
    console.log('  cortex-agent agent run coder "Build a REST API with authentication"');
    console.log('  cortex-agent agent run security-auditor "Review this code for vulnerabilities"');
    console.log('  cortex-agent agent run full-stack-developer "Create a Next.js app"');
  } catch (error) {
...
async function createAgent(subArgs, flags) {
  ...
  try {
    ...
    console.log('\nNext steps:');
    console.log('  1. Run your agent: cortex-agent agent run <agent-name> "<task>"');
    console.log('  2. View agent info: cortex-agent agent info <agent-name>');
    console.log('  3. Check for conflicts: cortex-agent agent conflicts');
  } catch (error) {
...
async function getAgentInfo(subArgs, flags) {
  ...
  if (!agentName) {
    ...
    console.log('\nExample:');
    console.log('  cortex-agent agent info coder');
    console.log('  cortex-agent agent info security-auditor');
    return;
  }
  ...
  try {
    ...
    console.error(error.message);
    console.log('\nTip: List all agents with: cortex-agent agent agents');
    process.exit(1);
  }
}
...
async function initializeMemory(subArgs, flags) {
  ...
  try {
    ...
    console.log('  2. Check status: cortex-agent agent memory status');
    console.log('  3. View demo: cortex-agent agent memory demo');
  } catch (error) {
...
async function getMemoryStatus(subArgs, flags) {
  ...
  try {
    ...
    console.error(error.message);
    console.log('\nTip: Initialize first with: cortex-agent agent memory init');
    process.exit(1);
  }
}
...
function showMemoryHelp() {
  ...
  console.log('  cortex-agent agent memory init');
  console.log('  cortex-agent agent memory status');
  console.log('  cortex-agent agent memory list --domain web --limit 5');
  console.log('  cortex-agent agent memory consolidate');
  console.log('  cortex-agent agent memory demo');
}
...
async function agenticConfigSet(subArgs, flags) {
  ...
  if (!key || !value) {
    ...
    console.log('\nExample:');
    console.log('  cortex-agent agent config set ANTHROPIC_API_KEY sk-ant-xxx');
    console.log('  cortex-agent agent config set DEFAULT_MODEL claude-3-5-sonnet-20241022');
    return;
  }
...
async function agenticConfigGet(subArgs, flags) {
  ...
  if (!key) {
    ...
    console.log('\nExample:');
    console.log('  cortex-agent agent config get ANTHROPIC_API_KEY');
    return;
  }
...
async function agenticConfigReset(subArgs, flags) {
  if (!flags.force) {
    ...
    console.log('\nUse --force to confirm: cortex-agent agent config reset --force');
    return;
  }
...
function showAgenticConfigHelp() {
  ...
  console.log();
  console.log('Examples:');
  console.log('  cortex-agent agent config wizard');
  console.log('  cortex-agent agent config set ANTHROPIC_API_KEY sk-ant-xxx');
  console.log('  cortex-agent agent config list --show-secrets');
}
...
function showAgenticMcpHelp() {
  ...
  console.log();
  console.log('Examples:');
  console.log('  cortex-agent agent mcp start --daemon');
  console.log('  cortex-agent agent mcp status --detailed');
  console.log('  cortex-agent agent mcp list --server agent-booster');
  console.log('  cortex-agent agent mcp logs --lines 50 --follow');
}
...
async function spawnAgent(subArgs, flags) {
  ...
  const perfFile = '.cortex-agent/metrics/performance.json';
  try {
...
async function listAgents(subArgs, flags) {
  ...
  if (agents.length > 0) {
    ...
    console.log('\nTo create agents:');
    console.log('  cortex-agent agent spawn researcher --name "ResearchBot"');
    console.log('  cortex-agent agent spawn coder --name "CodeBot"');
    console.log('  cortex-agent agent spawn analyst --name "DataBot"');
  }
}
...
async function manageHierarchy(subArgs, flags) {
  ...
  switch (hierarchyCmd) {
    ...
    default:
      ...
      console.log('Examples:');
      console.log('  cortex-agent agent hierarchy create enterprise');
      console.log('  cortex-agent agent hierarchy show');
  }
}
...
function showAgentHelp() {
  ...
  console.log('                                   Run: cortex-agent init --env');
  ...
  console.log('  cortex-agent agent run coder "Build REST API with authentication"');
  console.log('  cortex-agent agent run researcher "Research React 19 features" --provider openrouter');
  console.log('  cortex-agent agent run security-auditor "Audit code" --provider onnx');
  console.log('  cortex-agent agent agents  # List all available agents');
  console.log('\n  # Model optimization (85-98% cost savings)');
  console.log('  cortex-agent agent run coder "Build API" --optimize');
  console.log('  cortex-agent agent run coder "Fix bug" --optimize --priority cost');
  console.log('  cortex-agent agent run coder "Critical fix" --optimize --priority quality --max-cost 0.50');
  console.log('\n  # Execute with ReasoningBank memory (learns from experience)');
  console.log('  cortex-agent agent run coder "Build API" --enable-memory');
  console.log('  cortex-agent agent run coder "Add auth" --enable-memory --memory-domain api');
  console.log('  cortex-agent agent run coder "Fix bug" --enable-memory --memory-k 5');
  console.log('\n  # Memory management');
  console.log('  cortex-agent agent memory init');
  console.log('  cortex-agent agent memory status');
  console.log('  cortex-agent agent memory list --domain api --limit 10');
  console.log('  cortex-agent agent memory consolidate');
  console.log('  cortex-agent agent memory demo  # See 70% → 88% success improvement');
  console.log('\n  # Configuration management');
  console.log('  cortex-agent agent config wizard  # Interactive setup');
  console.log('  cortex-agent agent config set ANTHROPIC_API_KEY sk-ant-xxx');
  console.log('  cortex-agent agent config list --show-secrets');
  console.log('  cortex-agent agent config get DEFAULT_MODEL');
  console.log('\n  # MCP server management');
  console.log('  cortex-agent agent mcp start --daemon');
  console.log('  cortex-agent agent mcp status --detailed');
  console.log('  cortex-agent agent mcp list --server agent-booster');
  console.log('  cortex-agent agent mcp logs --follow');
  console.log('\n  # Internal agent management');
  console.log('  cortex-agent agent spawn researcher --name "DataBot"');
  console.log('  cortex-agent agent list --verbose');
  console.log('  cortex-agent agent hierarchy create enterprise');
}
