 ...
 * Usage: cortex-agent proxy start [--port 8080]
 */
async function startProxy(subArgs, flags) {
  ...
  try {
    ...
    console.log('  4. Check status: cortex-agent proxy status');
    console.log('\n💰 Cost Savings:');
    ...
    if (error.killed && flags.daemon) {
      ...
      console.log('Check status: cortex-agent proxy status');
    } else {
 ...
 * Usage: cortex-agent proxy status
 */
async function getProxyStatus(subArgs, flags) {
  ...
  try {
    ...
    console.log('  cortex-agent proxy start');
    process.exit(1);
  }
}
 ...
 * Usage: cortex-agent proxy logs [--lines 100] [--follow]
 */
 ...
 * Usage: cortex-agent proxy restart
 */
 ...
 * Usage: cortex-agent proxy config
 */
async function configureProxy(subArgs, flags) {
  ...
  console.log('\n🚀 Quick Start:');
  console.log('  1. cortex-agent agent config set OPENROUTER_API_KEY sk-or-v1-...');
  console.log('  2. cortex-agent proxy start --daemon');
  ...
  if (flags.test) {
    ...
    try {
      ...
      printWarning('⚠️  Proxy not responding. Start it with: cortex-agent proxy start');
    }
  }
}
...
USAGE:
  cortex-agent proxy <command> [options]
...
EXAMPLES:
  # Start proxy server
  cortex-agent proxy start
  cortex-agent proxy start --port 8080 --daemon

  # Configure Claude Code to use proxy
  export ANTHROPIC_BASE_URL=http://localhost:8080

  # Check status
  cortex-agent proxy status
  cortex-agent proxy status --verbose

  # View logs
  cortex-agent proxy logs
  cortex-agent proxy logs --lines 50 --follow
  cortex-agent proxy logs --error

  # Stop/restart proxy
  cortex-agent proxy stop
  cortex-agent proxy restart

  # Configuration guide
  cortex-agent proxy config
  cortex-agent proxy config --test

SETUP GUIDE:
  1. Get OpenRouter API key: https://openrouter.ai/keys
  2. Set environment variable:
     cortex-agent agent config set OPENROUTER_API_KEY sk-or-v1-...

  3. Start proxy server:
     cortex-agent proxy start --daemon
