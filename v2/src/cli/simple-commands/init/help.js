...
export function showInitHelp() {
  ...
  console.log();
  console.log('Usage: cortex-agent init [options]');
  console.log();
  console.log('🚀 DEFAULT BEHAVIOR:');
  console.log('  cortex-agent init     Initialize with Cortex Agent v2.0.0 enhanced features');
  ...
  console.log('  cortex-agent init --env                      # Generate .env template');
  console.log('  cortex-agent init --env --force              # Overwrite existing .env');
  console.log();
  console.log('🌊 CLAUDE FLOW v2.0.0 (DEFAULT):');
  console.log('  cortex-agent init                    # ⭐ DEFAULT: Full Cortex Agent v2.0.0 setup');
  console.log('  cortex-agent init --force            # Overwrite existing configuration');
  console.log('  cortex-agent init --dry-run          # Preview what will be created');
  console.log('  cortex-agent init --monitoring       # Initialize with token tracking enabled');
  console.log();
  console.log('🚀 STANDARD INITIALIZATION:');
  console.log('  npx cortex-agent@latest init --sparc --force  # ⭐ RECOMMENDED: Optimized setup');
  console.log('  npx cortex-agent@latest init --sparc          # Standard SPARC setup');
  console.log(
    '  cortex-agent init --sparc --force             # Optimized setup (existing project)',
  );
  console.log('  cortex-agent init --sparc --modes architect,tdd,code  # Selective initialization');
  console.log('  cortex-agent init --dry-run --sparc          # Preview initialization');
  console.log('  cortex-agent init --minimal                  # Minimal setup');
  console.log();
  console.log('🧠 REASONING AGENTS WITH REASONINGBANK:');
  console.log('  cortex-agent init --agent reasoning          # Setup ReasoningBank agents');
  console.log('  cortex-agent init --agent reasoning --force  # Overwrite existing setup');
  ...
  console.log();
  console.log('🔍 VERIFICATION & PAIR PROGRAMMING:');
  console.log('  cortex-agent init --verify                   # Truth Verification System setup');
  console.log('  cortex-agent init --pair                     # Pair Programming mode setup');
  console.log('  cortex-agent init --verify --pair            # Both verification and pairing');
  console.log('  npx cortex-agent@alpha init --verify --pair  # Complete verification setup');
  console.log();
  console.log('🔄 VALIDATION & ROLLBACK:');
  console.log('  cortex-agent init --validate                 # Validate existing setup');
  console.log('  cortex-agent init --rollback --full          # Full system rollback');
  console.log('  cortex-agent init --rollback --partial --phase sparc-init  # Rollback SPARC only');
  console.log('  cortex-agent init --list-backups             # Show available backups');
  console.log();
  ...
  console.log('  • cortex-agent.config.json - Cortex Agent features and performance settings');
  ...
  console.log('  • ./cortex-agent local executable wrapper');
  ...
  console.log('  • /cortex-agent-help - Show all cortex-agent commands');
  console.log('  • /cortex-agent-memory - Interact with memory system');
  console.log('  • /cortex-agent-swarm - Coordinate multi-agent swarms');
  console.log();
