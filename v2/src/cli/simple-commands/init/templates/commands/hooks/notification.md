...
```bash
npx cortex-agent hook notification [options]
```
...
```bash
npx cortex-agent hook notification --message "Completed authentication module"
```

### Warning notification

```bash
npx cortex-agent hook notification -m "Potential security issue found" -l warning
```

### Broadcast to swarm

```bash
npx cortex-agent hook notification -m "API refactoring started" --broadcast
```

### Decision tracking

```bash
npx cortex-agent hook notification -m "Chose JWT over sessions for auth" --memory-store
```
...
```bash
# For coordination
npx cortex-agent hook notification --message "Starting database migration" --broadcast --memory-store
```
