...
```bash
npx cortex-agent hook session-restore [options]
```
...
```bash
npx cortex-agent hook session-restore --session-id "dev-session-2024"
```

### Full restoration

```bash
npx cortex-agent hook session-restore -s "feature-auth" --load-memory --restore-files --resume-tasks
```

### Selective restore

```bash
npx cortex-agent hook session-restore -s "bug-fix-123" --load-memory --resume-tasks
```

### Context merging

```bash
npx cortex-agent hook session-restore -s "refactor-api" --merge-context
```
...
```bash
# To restore context
npx cortex-agent hook session-restore --session-id "previous-session" --load-memory
```
