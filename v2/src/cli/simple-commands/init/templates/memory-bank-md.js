...
- **Primary**: JSON database (\`./memory/cortex-agent-data.json\`) with parallel access
...
### Batchtools Commands
- \`npx cortex-agent memory batch-store <entries-file>\`: Store multiple entries in parallel
- \`npx cortex-agent memory parallel-query <queries-file>\`: Execute multiple queries concurrently
- \`npx cortex-agent memory concurrent-export <namespaces>\`: Export multiple namespaces simultaneously
- \`npx cortex-agent memory batch-cleanup <retention-config>\`: Clean up multiple namespaces in parallel

## Configuration (Enhanced)
Memory settings are configured in \`cortex-agent.config.json\` with batchtools optimizations:
\`\`\`json
{
  "memory": {
    "backend": "json",
...
npx cortex-agent memory batch-store sparc-data.json --namespace sparc --parallel

# Concurrent query across multiple namespaces
npx cortex-agent memory parallel-query "authentication design" --namespaces arch,impl,test

# Batch export with parallel compression
npx cortex-agent memory concurrent-export project-backup --compress --parallel
\`\`\`

### Performance Monitoring
\`\`\`bash
# Monitor concurrent operations
npx cortex-agent memory stats --concurrent --verbose

# Analyze batch operation performance
npx cortex-agent memory performance-report --batchtools

# Check parallel indexing status
npx cortex-agent memory index-status --parallel
\`\`\`
...
npx cortex-agent memory debug --concurrent

# Analyze batch operation performance
npx cortex-agent memory analyze --batchtools --verbose

# Validate parallel index integrity
npx cortex-agent memory index-validate --parallel --repair
\`\`\`
