...
npx cortex-agent health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
npx cortex-agent metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
npx cortex-agent sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
npx cortex-agent alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
npx cortex-agent anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
npx cortex-agent notification-config --channels ["slack", "email", "webhook"]
```
