...
version: 1.0.0
description: Comprehensive performance analysis, bottleneck detection, and optimization recommendations for Cortex Agent swarms
category: monitoring
...
```bash
npx cortex-agent bottleneck detect
```

### Generate Performance Report
```bash
npx cortex-agent analysis performance-report --format html --include-metrics
```

### Analyze and Auto-Fix
```bash
npx cortex-agent bottleneck detect --fix --threshold 15
```
...
#### Command Syntax
```bash
npx cortex-agent bottleneck detect [options]
```
...
npx cortex-agent bottleneck detect --swarm-id swarm-123 -t 24h

# Export detailed analysis
npx cortex-agent bottleneck detect -t 24h -e bottlenecks.json

# Auto-fix detected issues
npx cortex-agent bottleneck detect --fix --threshold 15

# Low threshold for sensitive detection
npx cortex-agent bottleneck detect --threshold 10 --export critical-issues.json
```
...
mcp__cortex-agent__bottleneck_detect({
  timeRange: "1h",
  threshold: 20,
  autoFix: false
})
...
npx cortex-agent analysis performance-report --compare swarm-123 --format markdown

# Custom output with specific sections
npx cortex-agent analysis performance-report \
...
npx cortex-agent swarm monitor --interval 5
...
async function analyzePerformance() {
  ...
  const bottlenecks = await runCommand(
    'npx cortex-agent bottleneck detect --format json'
  );

  // Generate performance report
  const report = await runCommand(
    'npx cortex-agent analysis performance-report --format json'
  );
...
npx cortex-agent bottleneck detect --threshold 10

# Check cache performance
npx cortex-agent cache manage --action stats

# Review memory metrics
npx cortex-agent memory usage
```

**Slow Task Execution**
```bash
# Identify slow tasks
npx cortex-agent task status --detailed

# Analyze coordination overhead
npx cortex-agent bottleneck detect --time-range 1h
...
npx cortex-agent analysis performance-report --sections metrics

# Review cache strategy
npx cortex-agent cache manage --action analyze

# Enable cache warming
npx cortex-agent bottleneck detect --fix
```
...
- `npx cortex-agent swarm monitor` - Real-time monitoring
- `npx cortex-agent token usage` - Token optimization analysis
- `npx cortex-agent cache manage` - Cache optimization
- `npx cortex-agent agent metrics` - Agent performance metrics
- `npx cortex-agent task status` - Task execution analysis
