...
```bash
npx cortex-agent hive-mind init --topology hierarchical-mesh --consensus raft
```

### Spawn Queen
```bash
npx cortex-agent hive-mind spawn --role queen --name coordinator
```

### Check Consensus Status
```bash
npx cortex-agent hive-mind consensus --status
```

### View Sessions
```bash
npx cortex-agent hive-mind sessions --active
```
