...
### Review Code
```bash
npx cortex-agent github review --pr 123
```

### Manage Issues
```bash
npx cortex-agent github issues list --state open
npx cortex-agent github issues create --title "Bug: ..."
```

### Setup Workflow
```bash
npx cortex-agent workflow create --template ci
```

### Release Management
```bash
npx cortex-agent deployment release --version 1.0.0
```
