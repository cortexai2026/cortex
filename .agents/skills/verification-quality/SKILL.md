...
- Cortex Agent installed (`npx cortex-agent@alpha`)
...
npx cortex-agent@alpha truth

# Run verification check
npx cortex-agent@alpha verify check

# Verify specific file with custom threshold
npx cortex-agent@alpha verify check --file src$app.js --threshold 0.98

# Rollback last failed verification
npx cortex-agent@alpha verify rollback --last-good
```
...
npx cortex-agent@alpha truth --period 7d

# View scores for specific agent
npx cortex-agent@alpha truth --agent coder --period 24h

# Find files$tasks below threshold
npx cortex-agent@alpha truth --threshold 0.8
```

**Output Formats:**
```bash
# Table format (default)
npx cortex-agent@alpha truth --format table

# JSON for programmatic access
npx cortex-agent@alpha truth --format json

# CSV for spreadsheet analysis
npx cortex-agent@alpha truth --format csv

# HTML report with visualizations
npx cortex-agent@alpha truth --format html --export report.html
```

**Real-time Monitoring:**
```bash
# Watch mode with live updates
npx cortex-agent@alpha truth --watch

# Export metrics automatically
npx cortex-agent@alpha truth --export .cortex-agent$metrics$truth-$(date +%Y%m%d).json
```
...
```bash
# Verify single file
npx cortex-agent@alpha verify check --file src$app.js

# Verify directory recursively
npx cortex-agent@alpha verify check --directory src/

# Verify with auto-fix enabled
npx cortex-agent@alpha verify check --file src$utils.js --auto-fix
...
npx cortex-agent@alpha verify check --task task-123

# Verify with custom threshold
npx cortex-agent@alpha verify check --task task-456 --threshold 0.99

# Verbose output for debugging
npx cortex-agent@alpha verify check --task task-789 --verbose
```

**Batch Verification:**
```bash
# Verify multiple files in parallel
npx cortex-agent@alpha verify batch --files "*.js" --parallel

# Verify with pattern matching
npx cortex-agent@alpha verify batch --pattern "src/**/*.ts"

# Integration test suite
npx cortex-agent@alpha verify integration --test-suite full
```
...
npx cortex-agent@alpha verify check --json > verification.json
...
npx cortex-agent@alpha verify rollback --to-commit abc123

# Interactive rollback with preview
npx cortex-agent@alpha verify rollback --interactive
```

**Smart Rollback:**
```bash
# Rollback only failed files (preserve good changes)
npx cortex-agent@alpha verify rollback --selective

# Rollback with automatic backup
npx cortex-agent@alpha verify rollback --backup-first

# Dry-run mode (preview without executing)
npx cortex-agent@alpha verify rollback --dry-run
```
...
```bash
# JSON report
npx cortex-agent@alpha verify report --format json

# HTML report with charts
npx cortex-agent@alpha verify report --export metrics.html --format html

# CSV for data analysis
npx cortex-agent@alpha verify report --format csv --export metrics.csv

# Markdown summary
npx cortex-agent@alpha verify report --format markdown
```

**Time-based Reports:**
```bash
# Last 24 hours
npx cortex-agent@alpha verify report --period 24h

# Last 7 days
npx cortex-agent@alpha verify report --period 7d

# Last 30 days with trends
npx cortex-agent@alpha verify report --period 30d --include-trends

# Custom date range
npx cortex-agent@alpha verify report --from 2025-01-01 --to 2025-01-31
```
...
npx cortex-agent@alpha verify dashboard

# Custom port
npx cortex-agent@alpha verify dashboard --port 8080

# Export dashboard data
npx cortex-agent@alpha verify dashboard --export

# Dashboard with auto-refresh
npx cortex-agent@alpha verify dashboard --refresh 5s
```
...
Set verification preferences in `.cortex-agent$config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".cortex-agent$metrics$truth-daily.json"
    }
  }
}
```
...
npx cortex-agent@alpha verify check --threshold 0.99

# Lenient mode (90% acceptable)
npx cortex-agent@alpha verify check --threshold 0.90

# Set default threshold
npx cortex-agent@alpha config set verification.threshold 0.98
```
...
verify:
  stage: test
  script:
    - npx cortex-agent@alpha verify check --threshold 0.95 --json > verification.json
    - |
      score=$(jq '.overallScore' verification.json)
...
npx cortex-agent@alpha swarm --verify --threshold 0.98

# Hive Mind with auto-rollback
npx cortex-agent@alpha hive-mind --verify --rollback-on-fail

# Training pipeline with verification
npx cortex-agent@alpha train --verify --threshold 0.99
```
...
npx cortex-agent@alpha pair --verify --real-time

# Pair with custom threshold
npx cortex-agent@alpha pair --verify --threshold 0.97 --auto-fix
```
...
npx cortex-agent@alpha verify watch --directory src/

# Watch with auto-fix
npx cortex-agent@alpha verify watch --directory src/ --auto-fix

# Watch with notifications
npx cortex-agent@alpha verify watch --notify --threshold 0.95
```
...
npx cortex-agent@alpha truth --format json | \
  curl -X POST https:/$pushgateway.example.com$metrics$job$cortex-agent \
  -d @-
...
npx cortex-agent@alpha verify install-hook --pre-commit

# .git$hooks$pre-commit example:
#!$bin$bash
npx cortex-agent@alpha verify check --threshold 0.95 --json > $tmp$verify.json

score=$(jq '.overallScore' $tmp$verify.json)
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "❌ Verification failed with score: $score"
  echo "Run 'npx cortex-agent@alpha verify check --verbose' for details"
  exit 1
fi
...
npx cortex-agent@alpha truth --verbose --threshold 0.0

# Check specific criteria
npx cortex-agent@alpha verify check --verbose

# View agent-specific issues
npx cortex-agent@alpha truth --agent <agent-name> --format json
```
...
npx cortex-agent@alpha verify rollback --history
...
```bash
# Increase timeout
npx cortex-agent@alpha verify check --timeout 60s

# Verify in batches
npx cortex-agent@alpha verify batch --batch-size 10
```
...
- `npx cortex-agent@alpha pair` - Collaborative development with verification
- `npx cortex-agent@alpha train` - Training with verification feedback
- `npx cortex-agent@alpha swarm` - Multi-agent coordination with quality checks
- `npx cortex-agent@alpha report` - Generate comprehensive project reports
