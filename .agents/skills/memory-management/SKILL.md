...
**Example:**
```bash
npx @cortex-agent/cli memory store --key "auth-jwt-pattern" --value "JWT validation with refresh tokens" --namespace patterns
```
...
**Example:**
```bash
npx @cortex-agent/cli memory search --query "authentication best practices" --limit 5
```
...
```bash
npx @cortex-agent/cli memory get --key "[key]" --namespace [namespace]
```

**Example:**
```bash
npx @cortex-agent/cli memory get --key "auth-jwt-pattern" --namespace patterns
```
...
```bash
npx @cortex-agent/cli memory list --namespace [namespace]
```

**Example:**
```bash
npx @cortex-agent/cli memory list --namespace patterns --limit 20
```
...
```bash
npx @cortex-agent/cli memory delete --key "[key]" --namespace [namespace]
```
...
```bash
npx @cortex-agent/cli memory init --enable-hnsw
```
...
```bash
npx @cortex-agent/cli memory export --output memory-backup.json
```
