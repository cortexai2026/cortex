...
Tools prefixed with backend name (e.g., \`cortex-agent__agent_spawn\`). Always use the full prefixed name.
...
- **guidance** — Get help on tool groups or specific tool usage.
  ...
  \`{"topic": "overview"}\` or \`{"topic": "tool", "tool_name": "cortex-agent__agent_spawn"}\`
...
7. **Code task** → \`ruvector__hooks_route\` + \`cortex-agent__agent_spawn\`
8. **Remember something** → \`cortex-agent__memory_store\` / \`cortex-agent__memory_search\`
...
async function handleAutopilot(req, res, provider, body) {
  ...
  const priorityPrefixes = ['ruvector__hooks', 'ruvector__memory', 'cortex-agent__memory', 'cortex-agent__agent'];
