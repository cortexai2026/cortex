#!/usr/bin/env node
// Cortex Agent CLI - thin wrapper around @cortex-agent/cli with cortex-agent branding
...
// Walk up from cortex-agent/bin/ to find @cortex-agent/cli in node_modules
function findCliPath() {
  let dir = resolve(__dirname, '..');
  for (let i = 0; i < 10; i++) {
    const candidate = join(dir, 'node_modules', '@cortex-agent', 'cli', 'bin', 'cli.js');
...
const cliBase = pkgDir
  ? join(pkgDir, 'node_modules', '@cortex-agent', 'cli')
  : resolve(__dirname, '../../v3/@cortex-agent/cli');
...
if (isMCPMode) {
  ...
  // CLI mode: use cortex-agent branding
  const { CLI } = await import(toImportURL(join(cliBase, 'dist', 'src', 'index.js')));
  const cli = new CLI({
    name: 'cortex-agent',
    description: 'Cortex Agent - AI Agent Orchestration Platform',
  });
  cli.run().catch((error) => {
