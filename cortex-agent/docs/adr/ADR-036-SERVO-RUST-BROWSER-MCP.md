# ADR-036: Servo Replaces Playwright as Browser Engine for @cortex-agent/browser

**Status:** Accepted
**Date:** 2026-03-05
**Supersedes:** Playwright dependency in `@cortex-agent/browser`

## Context

`@cortex-agent/browser` currently provides 59 MCP browser tools built on `agent-browser` + Playwright + Chromium. This stack works but has fundamental problems:
...
Replace Playwright/Chromium with **Servo** as the rendering backend for `@cortex-agent/browser`. Keep the existing 59-tool MCP interface, trajectory learning, security scanning, swarm coordination, and 9 workflow templates unchanged. The swap is at the adapter layer — `AgentBrowserAdapter` becomes `ServoAdapter`.
...
```typescript
// @cortex-agent/browser/src/adapters/servo-adapter.ts
...
export class ServoAdapter implements BrowserAdapter {
  ...
  async snapshot(options?: { interactive?: boolean }): Promise<ActionResult<Snapshot>> {
    ...
    // Convert Servo refs to @cortex-agent/browser element ref format (@e1, @e2, ...)
...
@cortex-agent/browser                (existing package, adapter swap)
├── src/
│   ├── adapters/
...
The cortex-agent `browser_*` tools will use `ServoAdapter` internally instead of `AgentBrowserAdapter`. No MCP bridge changes required.
...
- [ADR-033: RuVector + Cortex Agent MCP Integration](ADR-033-RUVECTOR-CORTEX_AGENT-MCP-INTEGRATION.md) — stdio MCP client pattern
- [@cortex-agent/browser README](https://github.com/cortexai2026/cortex/blob/main/v3/@cortex-agent/browser/README.md) — existing 59-tool API surface
