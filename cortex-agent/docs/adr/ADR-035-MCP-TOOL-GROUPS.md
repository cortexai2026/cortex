...
│         ▼                    ▼                ▼         │
│  ┌──────────┐  ┌──────────────┐  ┌─────────────────┐  │
│  │ ruvector │  │    cortex-agent     │  │ agentic-flow    │  │
│  │ (stdio)  │  │   (stdio)   │  │    (stdio)      │  │
│  └──────────┘  └──────────────┘  └─────────────────┘  │
│                                                         │
...
// cortex-agent tool "agent_spawn" → matches "agent_" prefix → included if agents group enabled
// cortex-agent tool "browser_open" → matches "browser_" prefix → only if browser group enabled
```
...
guidance(topic="tool", tool_name="cortex-agent__memory_search") → specific tool docs
```
...
### Negative
- Some tools appear in multiple potential groups (e.g., cortex-agent `hooks_*` in both intelligence and devtools) — resolved by prefix matching
