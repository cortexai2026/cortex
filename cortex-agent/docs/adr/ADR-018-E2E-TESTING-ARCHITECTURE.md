...
- **Browser Automation**: `@cortex-agent/browser` (Playwright-based)
- **Test Framework**: `@cortex-agent/testing` (assertion library + reporters)
...
2. **@cortex-agent/browser API** for AI-optimized element references

This design was chosen because `@cortex-agent/browser`'s `createBrowserService()` requires the `agent-browser` CLI which doesn't launch browsers directly in containerized environments.
...
async function createBrowser(options, runId) {
  ...
  // Return @cortex-agent/browser compatible API
  return {
    async open(url) { /* ... */ },
    async snapshot(opts) { /* ... */ },
...
- [@cortex-agent/browser Package](https://www.npmjs.com/package/@cortex-agent/browser)
- [@cortex-agent/testing Package](https://www.npmjs.com/package/@cortex-agent/testing)
