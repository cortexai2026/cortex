...
ADR-033 added ruvector (61 tools) and cortex-agent (215 tools) as default MCP backends. Users also want access to additional AI agent capabilities:
...
Add three optional MCP backends that can be enabled via environment variables. Unlike ruvector/cortex-agent (enabled by default), these are **disabled by default** and require explicit API keys.
...
| cortex-agent | `ENABLE_CORTEX_AGENT` | None | `npx cortex-agent mcp start` | **enabled** |
...
cortex-agent__agent_spawn         → cortex-agent MCP
...
- [ADR-033: RuVector + Cortex Agent MCP Integration](ADR-033-RUVECTOR-CORTEX_AGENT-MCP-INTEGRATION.md)
