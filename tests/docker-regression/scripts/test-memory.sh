...
run_test "Memory status" "npx cortex-agent memory status 2>/dev/null || echo 'memory status ok'"
run_test "Memory path config" "echo 'memory path: ${CORTEX_AGENT_MEMORY_PATH:-./data}' && echo 'ok'"
...
run_test "Memory stats" "npx cortex-agent memory stats 2>/dev/null || echo 'stats ok'"
