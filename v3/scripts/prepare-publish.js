...
function updatePackageJson(pkgName) {
  const pkgPath = path.join(v3Root, '@cortex-agent', pkgName, 'package.json');
...
function updateMainPackage() {
  const pkgPath = path.join(v3Root, 'cortex-agent', 'package.json');
  const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
  ...
  fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + '\n');
  console.log(`✅ cortex-agent: Updated for publishing`);
}

console.log('📦 Preparing V3 packages for npm publishing...\n');

// Update all @cortex-agent packages
for (const pkg of packages) {
  updatePackageJson(pkg);
}

// Update main cortex-agent package
updateMainPackage();

console.log('\n✅ All packages prepared for publishing!');
console.log(`\n📋 Publish order (${publishOrder.length} packages + main):`);
publishOrder.forEach((pkg, i) => console.log(`   ${i + 1}. @cortex-agent/${pkg}`));
console.log(`   ${publishOrder.length + 1}. cortex-agent`);

console.log('\n🚀 To publish, run:');
console.log('   npm login');
console.log('   cd v3 && npm run build');
for (const pkg of publishOrder) {
  console.log(`   cd @cortex-agent/${pkg} && npm publish --tag v3alpha && cd ../..`);
}
console.log('   cd cortex-agent && npm publish --tag v3alpha');
