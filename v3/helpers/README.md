# Cortex Agent V3 Helper System

The V3 Helper System provides cross-platform automation and development tools for cortex-agent v3 users. These helpers enable automatic progress tracking, checkpointing, GitHub integration, and development workflow automation.

## 🚀 Quick Start

### Installation
```bash
# Copy helpers to your cortex-agent v3 project
...
```bash
# Linux/Mac
./.claude/helpers/cortex-agent-v3.sh init
./.claude/helpers/cortex-agent-v3.sh status
./.claude/helpers/cortex-agent-v3.sh update domain 3

# Windows (PowerShell)
.\.claude\helpers\cortex-agent-v3.ps1 init
.\.claude\helpers\cortex-agent-v3.ps1 status
.\.claude\helpers\cortex-agent-v3.ps1 update domain 3
```

## 🛠️ Available Helpers

### 🎛️ Master Control Interface
- **`cortex-agent-v3.sh`** (Linux/Mac) / **`cortex-agent-v3.ps1`** (Windows)
...
```json
{
  "helpers": {
    ...
    "scripts": {
      "master": ".claude/helpers/cortex-agent-v3",
  ...
  update-progress:
    ...
    steps:
      ...
      - name: Update V3 Progress
        run: |
          ./.claude/helpers/cortex-agent-v3.sh update-from-ci
  ...
  "tasks": [
    {
      "label": "V3 Status",
      "type": "shell",
      "command": "./.claude/helpers/cortex-agent-v3.sh status",
      "group": "build",
      "presentation": {
        "echo": true,
        "reveal": "always",
...
*Cortex Agent V3 Helper System - Enabling cross-platform development automation*