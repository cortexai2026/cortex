...
This document describes how the Quality Engineering (agentic-qe) plugin integrates with Cortex Agent V3's existing domains: Security, Core, Memory, Integration, and Coordination.
...
│   │   Translates between agentic-qe and cortex-agent V3 domains              │   │
│   └─────────────────────────────────────────────────────────────────────────┘   │
│                                        │                                         │
│     ┌──────────────┬──────────────────┼──────────────────┬──────────────┐       │
│     │              │                  │                  │              │       │
...
import type { IMemoryService, MemoryEntry } from '@cortex-agent/memory';
...
The QE domain reuses V3's `@cortex-agent/embeddings` service instead of running separate `@xenova/transformers` instances:
...
import type { AgentService, TaskService, WorkflowService } from '@cortex-agent/core';
...
import type { HiveMindService, ConsensusResult } from '@cortex-agent/coordination';
...
import type { MCPServer, ToolDefinition } from '@cortex-agent/mcp';
