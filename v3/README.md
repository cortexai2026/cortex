...
[![Version](https://img.shields.io/badge/version-3.0.0--alpha.1-blue.svg)](https://github.com/ruvnet/cortex-agent)
...
Cortex Agent V3 is a next-generation AI agent coordination system built on 10 Architecture Decision Records (ADRs). It provides a modular, security-first, high-performance platform for orchestrating multi-agent swarms with hierarchical mesh topology.
...
│   ├── cortex-agent-v3.sh            # Master helper (Linux/macOS)
│   ├── cortex-agent-v3.ps1           # Master helper (Windows)
...
```typescript
import { PasswordHasher, validateInput, sanitizePath } from '@cortex-agent/security';
...
```typescript
import { HybridMemoryRepository, HNSWIndex } from '@cortex-agent/memory';
...
```typescript
import { BenchmarkRunner, formatTime } from '@cortex-agent/performance';
...
```typescript
import { SONAAdapter } from '@cortex-agent/neural';
...
```bash
npx @cortex-agent/cli swarm init --topology hierarchical-mesh
npx @cortex-agent/cli agent spawn --type queen-coordinator
npx @cortex-agent/cli memory search "knowledge"
```
...
```typescript
import { createMockAgent, createTestFixture } from '@cortex-agent/testing';
...
```typescript
import { EventBus, Result, success, failure } from '@cortex-agent/shared';
import type { AgentId, TaskStatus } from '@cortex-agent/shared/types';
```
...
```typescript
import { initializeV3Swarm } from '@cortex-agent/v3';
...
```typescript
import { createMCPServer } from '@cortex-agent/v3/mcp';
...
```bash
# Linux/macOS
./helpers/cortex-agent-v3.sh init
./helpers/cortex-agent-v3.sh status
./helpers/cortex-agent-v3.sh update domain 3

# Windows (PowerShell)
.\helpers\cortex-agent-v3.ps1 init
.\helpers\cortex-agent-v3.ps1 status
.\helpers\cortex-agent-v3.ps1 update domain 3
```
...
### Modules
- [@cortex-agent/security](./@cortex-agent/security/)
...
- [@cortex-agent/performance](./@cortex-agent/performance/)
- [@cortex-agent/neural](./@cortex-agent/neural/)
- [@cortex-agent/cli](./@cortex-agent/cli/)
- [@cortex-agent/testing](./@cortex-agent/testing/)
- [@cortex-agent/shared](./@cortex-agent/shared/)
- [@cortex-agent/deployment](./@cortex-agent/deployment/)

### Examples
- [AgentDB Example](./@cortex-agent/memory/examples/agentdb-example.ts)
- [Cross-Platform Usage](./@cortex-agent/memory/examples/cross-platform-usage.ts)
