...
This plugin adds 58 AI agents to Cortex Agent that handle all aspects of software quality:
...
**Via Cortex Agent CLI (recommended):**
```bash
npx cortex-agent plugins install --name @cortex-agent/plugin-agentic-qe
```
...
```bash
npx cortex-agent@v3alpha mcp call aqe/generate-tests \
...
```bash
npx cortex-agent@v3alpha mcp call aqe/tdd-cycle \
...
```bash
npx cortex-agent@v3alpha mcp call aqe/security-scan \
  --targetPath ./src \
  --scanType sast \
...
```bash
npx cortex-agent@v3alpha mcp call aqe/predict-defects \
...
npx cortex-agent@v3alpha mcp call aqe/chaos-inject \
