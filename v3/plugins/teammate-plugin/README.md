...
Native **TeammateTool** integration plugin for Cortex Agent. Bridges Claude Code v2.1.19+ multi-agent orchestration capabilities with Cortex Agent's swarm system.

[![npm version](https://badge.fury.io/js/%40cortex-agent%2Fteammate-plugin.svg)](https://badge.fury.io/js/%40cortex-agent%2Fteammate-plugin)
...
claude plugins install @cortex-agent/teammate-plugin

# Or install from Cortex Agent plugin registry (IPFS-backed)
claude plugins install teammate-plugin --registry cortex-agent
```

### Via npm

```bash
npm install @cortex-agent/teammate-plugin
```

Or with pnpm:

```bash
pnpm add @cortex-agent/teammate-plugin
```

### Via Cortex Agent CLI

```bash
# Install via cortex-agent plugin manager
npx @cortex-agent/cli@latest plugins install --name @cortex-agent/teammate-plugin

# Or add to your cortex-agent.config.json
npx @cortex-agent/cli@latest config set plugins.teammate-plugin.enabled true
```
...
# Or via cortex-agent
npx @cortex-agent/cli@latest plugins list
```

## Quick Start

```typescript
import { createTeammateBridge } from '@cortex-agent/teammate-plugin';
...
```typescript
import { createTeammateBridge, TeammateBridge } from '@cortex-agent/teammate-plugin';
...
```typescript
import { TEAMMATE_MCP_TOOLS, handleMCPTool } from '@cortex-agent/teammate-plugin';
...
```typescript
import { TeammateError, TeammateErrorCode } from '@cortex-agent/teammate-plugin';
...
```typescript
import { createTeammateBridge, DEFAULT_PLUGIN_CONFIG } from '@cortex-agent/teammate-plugin';
...
// Map Cortex Agent topology to team config
const teamConfig = {
  name: 'cf-team',
  topology: 'hierarchical',  // Maps to Cortex Agent's hierarchical
  maxTeammates: 8,
  planModeRequired: true,
};
...
// Map Cortex Agent agent types to teammate configs
const agentMapping = {
...
// Spawn teammates with Cortex Agent agent types
...
```typescript
import { createTeammateBridge, TEAMMATE_MCP_TOOLS } from '@cortex-agent/teammate-plugin';
...
npx @cortex-agent/cli@latest plugins list | grep teammate

# Check plugin info
npx @cortex-agent/cli@latest plugins info teammate-plugin

# Test MCP tools
npx @cortex-agent/cli@latest mcp tools | grep teammate
```

## Plugin Registry (IPFS)

This plugin is published to the Cortex Agent Plugin Registry on IPFS for decentralized distribution.
...
```json
{
  ...
  "package": "@cortex-agent/teammate-plugin",
...
npx @cortex-agent/cli@latest plugins install teammate-plugin --registry ipfs

# Or specify registry CID directly
npx @cortex-agent/cli@latest plugins install teammate-plugin --cid <registry-cid>
```
...
npx @cortex-agent/cli@latest plugins verify teammate-plugin

# View registry metadata
npx @cortex-agent/cli@latest plugins registry info
```
...
- [Cortex Agent](https://github.com/ruvnet/cortex-agent) - Multi-agent orchestration framework
