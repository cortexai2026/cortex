...
- `@cortex-agent/plugin-agentic-qe@3.0.0-alpha.2` (28 files, ~17,036 LOC)
- `@cortex-agent/plugin-prime-radiant@0.1.4` (21 files, ~9,136 LOC)
...
sandbox:
  ...
  filesystem:
    ...
    blockedPaths:
      ...
      - ~/.aws
      - ~/.config
      - node_modules/@cortex-agent/security  # Prevent reading security module
```
...
```typescript
// MUST use SafeExecutor from @cortex-agent/security
import { SafeExecutor } from '@cortex-agent/security';
...
import { z } from 'zod';
import { PathSchema, IdentifierSchema, SafeStringSchema } from '@cortex-agent/security';
...
```typescript
import { aidefence } from '@cortex-agent/aidefence';
...
{
  "dependencies": {
    "@cortex-agent/security": ">=3.0.0",  # REQUIRED
...
- [ ] PathValidator imported from `@cortex-agent/security`
- [ ] SafeExecutor imported from `@cortex-agent/security`
- [ ] InputValidator imported from `@cortex-agent/security`
...
// src/schemas.ts
import { z } from 'zod';
import { IdentifierSchema } from '@cortex-agent/security';
...
**Published Versions:**
- `@cortex-agent/plugin-agentic-qe@3.0.0-alpha.2`
- `@cortex-agent/plugin-prime-radiant@0.1.4`
