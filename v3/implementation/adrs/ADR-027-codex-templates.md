...
> **Branding Note**: This package is published as `@cortex-agent/codex` and is the first step in transitioning to the `coflow` brand. The future umbrella package will be `npm/npx coflow`.
...
| CLI Command | `npx @cortex-agent/codex init` |
| Integration | Works with `@cortex-agent/cli` via `--codex` flag |
...
### Storing Patterns
```bash
npx cortex-agent@v3alpha memory store \
  --key "pattern-name" \
...
### Searching Memory
```bash
npx cortex-agent@v3alpha memory search \
  --query "search terms" \
  --namespace patterns
```
...
Cortex Agent exposes tools via MCP:

```bash
# Start MCP server
npx cortex-agent@v3alpha mcp start
```
...
Cortex Agent uses hooks for automation:
...
### Example
```bash
npx cortex-agent@v3alpha hooks pre-task \
...
### 1. Initialize Swarm
```bash
npx cortex-agent@v3alpha swarm init \
...
### 2. Route Task
```bash
npx cortex-agent@v3alpha hooks route --task "[task description]"
```

### 3. Monitor Status
```bash
npx cortex-agent@v3alpha swarm status
```
...
### Before Starting
```bash
npx cortex-agent@v3alpha memory search --query "[task keywords]"
```
...
```bash
npx cortex-agent@v3alpha memory retrieve \
  --key "unique-key" \
  --namespace patterns
```
...
### List All Entries
```bash
npx cortex-agent@v3alpha memory list \
  --namespace patterns \
  --limit 50
```
...
```bash
npx cortex-agent@v3alpha hooks route --task "specification: [requirements]"
```
...
```bash
npx cortex-agent@v3alpha hooks route --task "pseudocode: [algorithm design]"
```
...
```bash
npx cortex-agent@v3alpha hooks route --task "architecture: [system design]"
```
...
```bash
npx cortex-agent@v3alpha hooks route --task "refinement: [improvements]"
```
...
```bash
npx cortex-agent@v3alpha hooks route --task "completion: [validation]"
```
...
### 1. Input Validation
```bash
npx cortex-agent@v3alpha security scan --check input-validation
```

### 2. Path Security
```bash
npx cortex-agent@v3alpha security scan --check path-traversal
```

### 3. Command Injection
```bash
npx cortex-agent@v3alpha security scan --check command-injection
```
...
```toml
# Cortex Agent V3 - Codex Configuration
# Generated by: cortex-agent init --codex
# Documentation: https://github.com/ruvnet/cortex-agent
...
```toml
# Cortex Agent V3 - Minimal Codex Configuration
...
```toml
# Cortex Agent V3 - CI/CD Pipeline Configuration
...
└── .cortex-agent/                      # Runtime data (shared)
    ├── config.yaml
    ├── data/
    └── logs/
```
...
dependencies:
  tools:
    - type: "mcp"
      value: "cortex-agent"
      description: "Cortex Agent MCP server for swarm coordination"
      transport: "stdio"
      command: "npx"
      args: ["-y", "@cortex-agent/cli@latest"]
```
...
```typescript
// Usage in cortex-agent CLI or standalone
// Package: @cortex-agent/codex (first step toward coflow rebranding)
...
import {
  ...
  generateConfigToml,
  CodexInitializer
} from '@cortex-agent/codex';

// Or via the CLI
// npx @cortex-agent/codex init
// npx @cortex-agent/codex generate-skill --name my-skill
...
const config = await generateConfigToml({
  ...
  mcpServers: [
    { name: 'cortex-agent', command: 'npx', args: ['-y', '@cortex-agent/cli@latest'] }
  ],
  skills: [
...
The `@cortex-agent/codex` package provides the following commands:
...
npx @cortex-agent/codex init --template minimal

# Generate a new skill
npx @cortex-agent/codex generate-skill --name custom-skill

# Migrate from Claude Code to Codex
npx @cortex-agent/codex migrate --from claude.md

# Validate AGENTS.md and skills
npx @cortex-agent/codex validate

# Generate dual-platform setup (Claude Code + Codex)
npx @cortex-agent/codex init --dual
```

## Integration with @cortex-agent/cli

When using the main CLI, Codex support is available via:

```bash
# Initialize with Codex support
npx cortex-agent@v3alpha init --codex

# Initialize with dual-platform support
npx cortex-agent@v3alpha init --dual
