...
Cortex Agent V3 requires comprehensive quality engineering (QE) capabilities for:
...
The current V3 architecture provides agent coordination (`@cortex-agent/plugins`), memory management (`@cortex-agent/memory`), and security primitives (`@cortex-agent/security`), but lacks specialized QE capabilities.
...
| **Browser Automation** | @cortex-agent/browser integration | Full Playwright |
...
| Dependency | agentic-qe | cortex-agent V3 | Strategy |
|------------|------------|----------------|----------|
...
| `hnswlib-node` | Vector search | @cortex-agent/memory | **Share** index |
| `better-sqlite3` | Persistence | sql.js (WASM) | **Separate** DBs |
| `@xenova/transformers` | Embeddings | @cortex-agent/embeddings | **Share** model |

---

## Decision

Integrate `agentic-qe` as a **first-class plugin** for Cortex Agent V3 using the `@cortex-agent/plugins` SDK with clear bounded context mapping, shared infrastructure coordination, and security isolation.
...
import type { IMemoryService } from '@cortex-agent/memory';
import type { SecurityModule } from '@cortex-agent/security';
import type { EmbeddingsService } from '@cortex-agent/embeddings';
...
import type { MCPTool } from '@cortex-agent/plugins';
...
import type { EnhancedModelRouter, EnhancedRouteResult } from '@cortex-agent/cli/ruvector';
...
import type { HiveMindService } from '@cortex-agent/coordination';

/**
 * Bridge between agentic-qe Queen Coordinator and cortex-agent Hive Mind
 */
...
**Deliverables:**
- Plugin registers with `@cortex-agent/plugins` SDK
...
```json
{
  ...
  "dependencies": {
    "agentic-qe": "^3.2.3",
    "@cortex-agent/plugins": "^3.0.0",
    "@cortex-agent/memory": "^3.0.0",
    "@cortex-agent/security": "^3.0.0",
    "@cortex-agent/embeddings": "^3.0.0",
    "zod": "^3.23.0"
  },
  "devDependencies": {
    "vitest": "^2.0.0",
