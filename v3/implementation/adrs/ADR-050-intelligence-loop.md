...
The memory system has three powerful modules — AutoMemoryBridge (storage), MemoryGraph (PageRank + community detection), LearningBridge (confidence tracking) — all shipped and tested (219 tests, published as `@cortex-agent/memory@3.0.0-alpha.8`). But they are not wired into the hook system that runs during Claude Code sessions.
...
Hooks are short-lived Node.js processes invoked by Claude Code. `hook-handler.cjs` uses `require()`. ESM dynamic `import()` is async and adds ~50ms overhead per invocation. The memory package (`@cortex-agent/memory`) is ESM-only. The intelligence layer must be CJS for synchronous, fast loading.
...
2. **ESM import of @cortex-agent/memory** — Use the full memory package in hooks. Rejected: CJS hooks can't synchronously import ESM.
