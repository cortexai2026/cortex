...
### 4. auto-memory-hook.mjs fails to resolve @cortex-agent/memory (#1287)
...
### 10. Zero swarms always: `cortex-agent spawn hive-mind --claude` (#1279)
...
- **Impact:** Some users get ECOMPROMISED errors on `npx cortex-agent`. Related to the removed preinstall script.
...
### 23. ADR-058: Self-Contained cortex-agent.rvf Appliance (#1245)
...
| — | Daemon branding | "Worker Daemon" → "Cortex Agent Daemon" in 3 status displays |

### v3.5.5 (2026-03-05) — Branding Sweep

All "Cortex Agent V3" → "Cortex Agent V3" across 30+ files (CLI source, helpers, statusline).
