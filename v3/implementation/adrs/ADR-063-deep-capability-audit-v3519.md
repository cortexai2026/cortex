...
**Context:** Comprehensive 4-agent parallel audit of all 98 CLI capabilities in Docker environment (cortex-agent@latest). Covers CLI commands, memory/neural, hooks/sessions, MCP/hive-mind, and known platform limitations.
...
- **Environment:** Docker (multi-stage build from cortex-agent@latest on npm)
...
- **Files:** `v3/@cortex-agent/cli/src/parser.ts` (added `buildScopedAliases()`, `getScopedBooleanFlags()`)
...
- **File:** `v3/@cortex-agent/cli/src/commands/neural.ts`
...
- **File:** `v3/@cortex-agent/cli/src/memory/intelligence.ts`
...
- **File:** `v3/@cortex-agent/cli/src/commands/hooks.ts`
...
- **File:** `v3/@cortex-agent/cli/src/commands/agent.ts`
...
- **Mitigation:** Lowered default similarity threshold to 0.1 for hash-fallback (C-3 fix). For production, install `@cortex-agent/embeddings` with ONNX model.
...
- **Context:** `@cortex-agent/embeddings` package is pruned from Docker image
- **Impact:** `embeddings init` reports success but uses hash-fallback backend
- **Mitigation:** Hash-fallback provides basic functionality; full embeddings available via `plugins install @cortex-agent/embeddings`
