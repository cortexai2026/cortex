...
As of v3.5.2, cortex-agent has 30 open issues spanning security, platform stability, CLI correctness, MCP protocol compliance, and Chat UI runtime bugs. This ADR triages every open issue into a priority matrix to guide engineering effort.

Note: `@cortex-agent/memory` (AgentDB) is now published at `@latest`, which affects several wiring issues below.
...
- **Impact:** Supply-chain trust — the `preinstall` script in `package.json` silently deletes npm cache entries for `cortex-agent` and `cortex-agent`. This resembles malicious behavior and will trigger security scanners (Socket, Snyk, npm audit).
...
### 7. auto-memory-hook.mjs fails to resolve @cortex-agent/memory (#1287)
...
- **Note:** `@cortex-agent/memory` (AgentDB) is now published at `@latest`. The export is available but CLI init doesn't wire it.
- **Fix:** Update CLI to `import { ControllerRegistry } from '@cortex-agent/memory'` (now on @latest); wire into init sequence.
...
### 13. MCP server and statusline report 'cortex-agent' v3.0.0 branding (#1280)
- **Impact:** Confusing branding — MCP server still identifies as `cortex-agent` v3.0.0 instead of `cortex-agent` v3.5.x.
- **Fix:** Update MCP server metadata, version string, and statusline template.
- **Status:** ✅ **FIXED** in PR #1298. MCP `system-tools.ts` now reads version from `package.json` at runtime via `getPackageVersion()`. Branding updated to "Cortex Agent" across 20+ CLI files. Statusline.cjs updated. Issue #1280 closed.

### 14. Statusline shows 'Cortex Agent V3' instead of 'Cortex Agent V3' (#1254)
- **Impact:** Branding inconsistency in IDE status bar.
- **Fix:** Update statusline configuration defaults.
- **Status:** ✅ **FIXED** in PR #1298. Updated `statusline.cjs` (lines 3, 552, 619) from "Cortex Agent V3" to "Cortex Agent V3". Updated `settings.json` version to 3.5.2. Issue #1254 closed.
...
### 18. Zero swarms always: `cortex-agent spawn hive-mind --claude` (#1279)
...
### 28. ADR-058: Self-Contained cortex-agent.rvf Appliance (#1245)
...
### 30. "How to use cortex-agent" (#1251)
...
| 14 | #1254 — Statusline branding | #1298 | `statusline.cjs` updated to "Cortex Agent V3" |
