...
Successfully implemented MCP-first API design for Cortex Agent V3. CLI commands now call MCP tools rather than implementing functionality directly, following the principle:
...
Updated `/workspaces/cortex-agent/v3/mcp/server.ts`:
...
1. `/workspaces/cortex-agent/v3/mcp/tools/agent-tools.ts` (463 lines)
2. `/workspaces/cortex-agent/v3/mcp/tools/swarm-tools.ts` (489 lines)
3. `/workspaces/cortex-agent/v3/mcp/tools/memory-tools.ts` (575 lines)
4. `/workspaces/cortex-agent/v3/mcp/tools/config-tools.ts` (568 lines)
5. `/workspaces/cortex-agent/v3/mcp/tools/index.ts` (300 lines)
6. `/workspaces/cortex-agent/v3/mcp/tools/README.md` (405 lines)

## Files Modified

1. `/workspaces/cortex-agent/v3/mcp/server.ts` (updated `registerBuiltInTools()`)
...
Added hooks-related MCP tools in `@cortex-agent/cli/src/mcp-tools/hooks-tools.ts`:
...
All MCP tools now exposed via CLI commands in `@cortex-agent/cli@3.0.0-alpha.7`:
...
4. **Mac Settings Validation** (alpha.89) - Fixed Claude Code settings.json validation errors on macOS
   ...
   - Fix: Removed `PermissionRequest` hook block; changed patterns from `*` to `:*` (e.g., `Bash(npx cortex-agent:*)`)
...
**Published**: `@cortex-agent/cli@3.0.0-alpha.90` with `v3alpha` tag (latest)
...
This ensures `npx cortex-agent@alpha` always gets the latest version.

**Published**: `@cortex-agent/cli@3.0.0-alpha.95`, `cortex-agent@3.0.0-alpha.46`
