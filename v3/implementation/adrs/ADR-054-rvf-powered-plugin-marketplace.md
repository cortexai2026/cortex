...
The Cortex Agent plugin ecosystem currently comprises 20 plugins distributed via IPFS (Pinata), discovered through a static JSON registry (`QmXbfEAaR7D2Ujm4GAkbwcGZQMHqAMpwDoje4583uNP834`), and searched via keyword substring matching. While functional, this approach has critical limitations as the ecosystem grows:

1. **Search is keyword-only** — `searchPlugins()` in `search.ts` does case-insensitive substring matching across name, description, tags, keywords. A query like "I need permission management" won't find `@cortex-agent/claims` unless the user knows the exact term "claims."
...
5. **No behavioral context** — The system cannot observe that a developer working on security tasks should be shown security plugins, or that users who install `@cortex-agent/security` almost always also install `@cortex-agent/claims`.
...
**Impact:** "permission management" finds `@cortex-agent/claims`. "machine learning" finds `@cortex-agent/neural`. "code quality" finds `@cortex-agent/plugin-agentic-qe`. Zero change to plugin commands — the search function is transparently upgraded.
...
npx cortex-agent plugins deps @cortex-agent/security --transitive

# Find plugin ecosystems (community detection)
npx cortex-agent plugins ecosystems

# Show hub plugins (highest PageRank)
npx cortex-agent plugins hubs

# Check for conflicts
npx cortex-agent plugins conflicts @cortex-agent/plugin-a @cortex-agent/plugin-b
```
...
**New CLI Features:**
```bash
npx cortex-agent plugins recommend          # Based on installed plugins + context
npx cortex-agent plugins trending           # Real-time trending (not static)
npx cortex-agent plugins why @cortex-agent/X # "Why is this recommended?"
```
...
```bash
npx cortex-agent plugins find-for "validate user input with schemas"
# → @cortex-agent/security (InputValidator — Zod-based validation)
# → @cortex-agent/claims (claims-based authorization)

npx cortex-agent plugins find-for "train neural patterns from code"
# → @cortex-agent/neural (SONA, MoE, EWC++)
# → @cortex-agent/plugin-neural-coordinator
```
...
**Deployment Options:**
- **Self-hosted**: `npx cortex-agent marketplace start --port 3001`
...
If `@cortex-agent/memory` or AgentDB is unavailable, every feature falls back to the existing implementation. Zero breaking changes.
...
|---|---|---|
| Embedding model download (100MB+) blocks first search | High latency on first use | Pre-warm in `daemon start`; cache model in `.cortex-agent/models/` |
...
- Plugin Store Types: `v3/@cortex-agent/cli/src/plugins/store/types.ts`
- Plugin Search: `v3/@cortex-agent/cli/src/plugins/store/search.ts`
- Plugin Discovery: `v3/@cortex-agent/cli/src/plugins/store/discovery.ts`
- ControllerRegistry: `v3/@cortex-agent/memory/src/controller-registry.ts`
