...
The `@cortex-agent/cli` package requires integration with `ruvector` for advanced code intelligence features:
...
These features are unique to ruvector and complement cortex-agent's existing capabilities without duplicating functionality already present in `@cortex-agent/embeddings` or `@cortex-agent/memory`.

## Decision

Implement ruvector as an **OPTIONAL dependency** with graceful fallback, following the existing patterns in `@cortex-agent/cli`.
...
```typescript
// v3/@cortex-agent/cli/src/ruvector/availability.ts
...
```typescript
// v3/@cortex-agent/cli/src/ruvector/adapters/router-adapter.ts
...
```typescript
// v3/@cortex-agent/cli/src/ruvector/adapters/ast-adapter.ts
...
```typescript
// v3/@cortex-agent/cli/src/ruvector/adapters/diff-adapter.ts
...
```typescript
// v3/@cortex-agent/cli/src/ruvector/adapters/graph-adapter.ts
...
```typescript
// v3/@cortex-agent/cli/src/commands/route.ts
...
export const routeCommand: Command = {
  ...
  examples: [
    {
      command: 'cortex-agent route -t "Implement user authentication" --q-learning',
      description: 'Route with Q-Learning model'
    },
    {
      command: 'cortex-agent route -t "Fix login bug" --coverage-aware',
...
```typescript
// v3/@cortex-agent/cli/src/commands/analyze.ts
...
const astSubcommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent analyze ast -p src/', description: 'Analyze src directory' },
    { command: 'cortex-agent analyze ast -p src/api.ts --complexity', description: 'Get complexity for file' }
  ],
  action: astAction
};
...
const diffSubcommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent analyze diff --risk', description: 'Analyze current diff with risk' },
    { command: 'cortex-agent analyze diff --base main --target feature', description: 'Compare branches' },
    { command: 'git diff | cortex-agent analyze diff --stdin --risk', description: 'Pipe diff from git' }
  ],
  action: diffAction
};
...
const boundariesSubcommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent analyze boundaries -p src/', description: 'Detect boundaries in src' },
    { command: 'cortex-agent analyze boundaries -a louvain', description: 'Use Louvain algorithm' }
  ],
...
```typescript
// v3/@cortex-agent/cli/src/ruvector/errors.ts
...
```typescript
// v3/@cortex-agent/cli/src/mcp-tools/ruvector-tools.ts
...
```typescript
// v3/@cortex-agent/cli/src/ruvector/index.ts
...
```typescript
// v3/@cortex-agent/cli/src/commands/index.ts
...
```typescript
// v3/@cortex-agent/cli/tests/ruvector/availability.test.ts
...
```typescript
// v3/@cortex-agent/cli/tests/commands/route.test.ts
...
All cache utilities exported from `@cortex-agent/cli/ruvector`:

```typescript
// Diff caches
import { clearDiffCache, clearAllDiffCaches } from '@cortex-agent/cli/ruvector';

// Graph caches
import { clearGraphCaches, getGraphCacheStats } from '@cortex-agent/cli/ruvector';

// Coverage caches
import { clearCoverageCache, getCoverageCacheStats } from '@cortex-agent/cli/ruvector';
```
...
**Changes Made:**
- `@cortex-agent/cli/src/commands/index.ts` - Refactored to use dynamic imports
...
**Changes Made:**
- `@cortex-agent/memory/src/agentdb-adapter.ts` - Optimized bulk methods
...
|---------|--------|-------|
| `cortex-agent` | 3.0.0-alpha.17 | 3.0.0-alpha.18 |
| `@cortex-agent/cli` | 3.0.0-alpha.24 | 3.0.0-alpha.25 |
| `@cortex-agent/mcp` | 3.0.0-alpha.7 | 3.0.0-alpha.8 |

### Usage Examples

**Lazy Command Loading:**
```typescript
import { getCommandAsync, loadAllCommands } from '@cortex-agent/cli';
...
```typescript
import { UnifiedMemoryService } from '@cortex-agent/memory';
...
- `cortex-agent@3.0.0-alpha.18`
- `@cortex-agent/cli@3.0.0-alpha.25`
- `@cortex-agent/mcp@3.0.0-alpha.8`
- `@cortex-agent/memory@3.0.0-alpha.2`

### Performance Validation

All builds pass successfully:
```
✓ @cortex-agent/cli build passed
✓ @cortex-agent/mcp build passed
✓ @cortex-agent/memory build passed
```
