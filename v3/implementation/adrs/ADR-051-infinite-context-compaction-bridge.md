...
|  |    -> .cortex-agent/data/transcript-archive.db              |   |
...
|  |                                                            |   |
|  |  Tier 3: AgentDB + HNSW  (if @cortex-agent/memory built)   |   |
|  |    -> 150x-12,500x faster semantic search                  |   |
|  |    -> Vector-indexed retrieval                             |   |
|  |                                                            |   |
|  |  Tier 4: JsonFileBackend                                   |   |
|  |    -> .cortex-agent/data/transcript-archive.json            |   |
...
State is persisted to `.cortex-agent/data/autopilot-state.json`:
...
| Environment Variable | Default | Description |
|---------------------|---------|-------------|
| `CORTEX_AGENT_COMPACT_RESTORE_BUDGET` | `4000` | Max chars for restored context in SessionStart |
| `CORTEX_AGENT_COMPACT_INSTRUCTION_BUDGET` | `2000` | Max chars for custom compact instructions |
| `CORTEX_AGENT_AUTO_OPTIMIZE` | `true` | Enable importance ranking, pruning, RuVector sync |
| `CORTEX_AGENT_RETENTION_DAYS` | `30` | Auto-prune never-accessed entries older than N days |
| `CORTEX_AGENT_CONTEXT_AUTOPILOT` | `true` | Enable Context Autopilot tracking |
| `CORTEX_AGENT_CONTEXT_WINDOW` | `200000` | Context window size in tokens |
| `CORTEX_AGENT_AUTOPILOT_WARN` | `0.70` | Warning threshold (70%) |
| `CORTEX_AGENT_AUTOPILOT_PRUNE` | `0.85` | Critical threshold (85%) — session rotation advised |
...
2. **Local storage default**: SQLite writes to `.cortex-agent/data/` which is
...
- `resolveBackend()` checks for `@cortex-agent/memory` dist at Tier 3
...
- Transparent upgrade when `@cortex-agent/memory` package is built
...
When `CORTEX_AGENT_AUTO_OPTIMIZE` is not `false` (default: enabled), the system
...
- **Default retention**: 30 days (configurable via `CORTEX_AGENT_RETENTION_DAYS`)
...
|-------|-------|---------|----------|
| `SQLiteBackend` | 57-272 | `.cortex-agent/data/transcript-archive.db` | WAL mode, indexed queries, prepared statements, importance-ranked queries, access tracking, stale pruning |
| `JsonFileBackend` | 278-355 | `.cortex-agent/data/transcript-archive.json` | Zero dependencies, Map-based in-memory with JSON persist |
