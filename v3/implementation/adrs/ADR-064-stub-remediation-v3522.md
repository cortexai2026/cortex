...
| **Workflow persistence** | Saves to `.cortex-agent/workflows/store.json` | `workflow-tools.ts:52-78` — `loadWorkflowStore()`/`saveWorkflowStore()` |
...
| **P2** | Intelligence stats → neural bridge | Stats counter disconnected | Read from ReasoningBank `.cortex-agent/neural/patterns.json` |
...
- Read pattern count from `.cortex-agent/neural/patterns.json`
- Read trajectory count from `.cortex-agent/neural/stats.json`
...
| `@cortex-agent/memory` | 3.0.0-alpha.12 | AgentDB + ControllerRegistry |
