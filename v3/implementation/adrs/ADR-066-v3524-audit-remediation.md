...
| M-2 | Swarm tools returning mock data | `swarm-tools.ts` | Complete rewrite: 4 tools now use file-based state persistence at `.cortex-agent/swarm/swarm-state.json` with real CRUD operations |
...
State file: `.cortex-agent/swarm/swarm-state.json` (restrictive 0o700 permissions)
