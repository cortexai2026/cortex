# ADR-043: Gas Town Bridge Plugin for Cortex Agent V3
...
Cortex Agent V3 would benefit from:
...
Create `@cortex-agent/plugin-gastown-bridge` with a **WASM-centric hybrid architecture**:
...
6. **GUPP Adapter**: Translate GUPP hooks to Cortex Agent session persistence
...
```json
{
  ...
  "version": "0.1.0",
  "description": "Gas Town orchestrator integration for Cortex Agent V3",
...
```typescript
import { AgentDB } from '@cortex-agent/agentdb';
...
│   │   └── ...                # From @cortex-agent/plugin-micro-hnsw
│   └── ruvector-learning-wasm/ # SONA patterns (shared)
│       └── ...                # From @cortex-agent/plugin-ruvector-learning
├── tests/
│   ├── bridges.test.ts
│   ├── formula.test.ts
...
export interface GasTownConfig {
  ...
  // Enable convoy tracking in Cortex Agent
  enableConvoys: boolean;

  // Auto-create beads from Cortex Agent tasks
...
- Convoy tracking with Cortex Agent tasks
- 3 Convoy MCP tools
...
- Integration with Cortex Agent daemon
...
{
  "dependencies": {
    "@cortex-agent/plugin-micro-hnsw": "^0.1.0",
    "@cortex-agent/plugin-ruvector-gnn": "^0.1.0",
    "@cortex-agent/plugin-ruvector-learning": "^0.1.0"
  },
  "devDependencies": {
