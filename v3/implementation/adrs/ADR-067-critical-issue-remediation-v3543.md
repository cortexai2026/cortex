...
| **P2 — Medium** | AgentDB bridge unavailable | #1399 | CLI bundles `@cortex-agent/memory@alpha.11` (missing `ControllerRegistry`); runtime patch targets v1.x paths |
...
| **P2 — Medium** | Hive-mind uses native tools instead of MCP | #1422 | No tool preference enforcement; Claude defaults to native tools over Cortex Agent MCP |
...
File: `v3/@cortex-agent/cli/src/daemon/headless-worker-executor.ts`
...
1. On `daemon start`, check `$PROJECT/.cortex-agent/daemon.pid`
...
File: `v3/@cortex-agent/cli/src/commands/swarm.ts` (line ~645)
...
File: `v3/@cortex-agent/cli/src/commands/memory.ts` (init handler)
...
1. Update `@cortex-agent/cli` dependency on `@cortex-agent/memory` to `>=3.0.0-alpha.12`
...
Audit all `type: 'array'` properties and add appropriate `items` schema. This is partially addressed in PR #73 (cortex-agent repo).

**4.2 — Enforce Cortex Agent MCP tool preference in hive-mind**

Options:
- **A**: Add `--allowedTools` constraint when spawning hive-mind sessions to prefer Cortex Agent MCP tools
- **B**: Add system prompt injection that instructs Claude to use Cortex Agent MCP tools for orchestration
...
2. Integration test: `npx cortex-agent daemon start` → workers execute successfully
3. Integration test: `npx cortex-agent memory init` → process exits cleanly
