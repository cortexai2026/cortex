...
export const HEADLESS_WORKERS: HeadlessWorkerConfig[] = [
  ...
  },
  {
    ...
    headless: {
      ...
      model: 'haiku',
      outputFormat: 'json',
      contextPatterns: ['.cortex-agent/metrics/*.json'],
    },
  },
];
```
...
npx cortex-agent@v3alpha daemon start --headless

# Start with specific sandbox mode for all workers
npx cortex-agent@v3alpha daemon start --sandbox strict

# Trigger headless worker manually
npx cortex-agent@v3alpha daemon trigger -w audit --headless

# Show worker modes
npx cortex-agent@v3alpha daemon status --show-modes
```
  ...
  async executeWorkerInContainer(
    ...
    try {
      ...
      const result = await container.exec([
        'npx', 'cortex-agent@v3alpha', 'daemon', 'trigger',
        '-w', worker.type,
        '--headless',
...
services:
  worker-pool:
    ...
    volumes:
      ...
      - cortex-agent-state:/root/.cortex-agent
