...
Cortex Agent V3 uses embeddings extensively for:
...
function getMigrationSteps(target: string) {
  ...
  if (target === 'all' || target === 'embeddings') {
    steps.push({
      ...
      dest: '.cortex-agent/models/',
      execute: async () => {
        const { downloadEmbeddingModel } = await import('@cortex-agent/embeddings');
        await downloadEmbeddingModel('all-MiniLM-L6-v2', '.cortex-agent/models/');
      }
    });
  }
}
...
npx cortex-agent@v3alpha hooks pretrain \
...
npx cortex-agent@v3alpha embeddings init [options]
...
npx cortex-agent@v3alpha embeddings status
...
npx cortex-agent@v3alpha embeddings download <model-id>

# Example
npx cortex-agent@v3alpha embeddings download all-mpnet-base-v2
...
async function initializeEmbeddings(options: InitOptions): Promise<void> {
  ...
  const configDir = path.join(options.targetDir, '.cortex-agent');
...
async function migrateEmbeddings(ctx: CommandContext): Promise<void> {
  ...
  const v2CachePath = path.join(ctx.cwd, '.cortex-agent', 'cache', 'embeddings.db');
  const v2Exists = fs.existsSync(v2CachePath);

  // 2. Download V3 model
  const { downloadEmbeddingModel, listEmbeddingModels } = await import('@cortex-agent/embeddings');
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (ctx.flags.embeddings) {
      ...
      const { downloadEmbeddingModel, createEmbeddingService } =
        await import('@cortex-agent/embeddings');
      ...
      const embedder = createEmbeddingService({
        provider: 'agentic-flow',
        modelPath: '.cortex-agent/models/all-MiniLM-L6-v2',
      });
...
const initSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      await downloadEmbeddingModel(model, '.cortex-agent/models/', (p) => {
      ...
      const service = createEmbeddingService({
        provider: 'agentic-flow',
        modelPath: `.cortex-agent/models/${model}`,
      });
...
const downloadSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    await downloadEmbeddingModel(modelId, '.cortex-agent/models/', (p) => {
...
- Requires `@cortex-agent/embeddings` package
