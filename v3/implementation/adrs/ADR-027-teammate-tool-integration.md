# ADR-027: Native TeammateTool Integration for Cortex Agent
...
The `@cortex-agent/teammate-plugin` package has been fully implemented with:
...
```
v3/@cortex-agent/teammate-plugin/
...
This ADR defines the architecture for deep integration between Cortex Agent and Claude Code's native **TeammateTool** multi-agent orchestration system. By leveraging TeammateTool's built-in capabilities for team management, inter-agent communication, and plan approval workflows, Cortex Agent can eliminate redundant coordination code and provide seamless native multi-agent experiences.
...
Cortex Agent currently implements its own multi-agent orchestration via:
...
**Implement a Cortex Agent plugin that acts as a bridge to TeammateTool**, providing:

1. **Native Team Management** - Use TeammateTool for spawning instead of MCP
2. **Mailbox Integration** - Bridge teammate_mailbox to Cortex Agent's memory system
3. **Plan Mode Orchestration** - Leverage launchSwarm for coordinated execution
4. **Hybrid Topology** - Combine Cortex Agent's advanced topologies with native spawning
...
| **Transparent Bridge** | Cortex Agent APIs unchanged, backend swapped |
...
/**
 * Bridge between Cortex Agent and Claude Code's TeammateTool
 *
...
### 3.4 Cortex Agent Integration Layer

```typescript
// cortex-agent-integration.ts
...
/**
 * Integration layer between Cortex Agent's swarm system
 * and Claude Code's native TeammateTool
 */
export class CortexAgentTeammateIntegration {
  ...
  /**
   * Map Cortex Agent topology to team configuration
   */
  ...
  /**
   * Map Cortex Agent agent type to teammate spawn config
   */
  mapAgentToTeammateConfig(
    ...
    // Map common Cortex Agent agent types to roles
...
```typescript
// mcp-tools.ts - Complete implementation in v3/@cortex-agent/teammate-plugin/src/mcp-tools.ts
...
mcp__cortex-agent__teammate_spawn_team({
...
### Phase 2: Cortex Agent Integration ✅ COMPLETE
...
### From Cortex Agent MCP-only to Hybrid
...
const integration = new CortexAgentTeammateIntegration();
...
**Status:** Implemented ✅
**Package:** `@cortex-agent/teammate-plugin` (v1.0.0-alpha.1)
**Location:** `v3/@cortex-agent/teammate-plugin/`
...
4. **Phase 6: Memory Bridge** - Integrate with Cortex Agent's HNSW memory system
5. **Phase 7: Consensus Integration** - Bridge TeammateTool approval with Cortex Agent consensus protocols
