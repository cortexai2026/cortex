...
### 1. Create `@cortex-agent/providers` Package

A dedicated package for LLM provider implementations:

```
v3/@cortex-agent/providers/
├── src/
...
Add LLM-specific hooks to `@cortex-agent/hooks`:
...
### Neutral
- Integration with existing `@cortex-agent/integration` multi-model-router
...
7. Add LLM hooks to @cortex-agent/hooks
...
- V3 Multi-Model Router: `v3/@cortex-agent/integration/src/multi-model-router.ts`
