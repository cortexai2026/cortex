...
cortex-agent issues list --available

# Claim an issue (as current user/agent)
cortex-agent issues claim 123

# Release a claim
cortex-agent issues release 123

# Request handoff to specific agent/human
cortex-agent issues handoff 123 --to agent:coder-1
cortex-agent issues handoff 123 --to human:alice

# Update status
cortex-agent issues status 123 --blocked "Waiting for API spec"
cortex-agent issues status 123 --review-requested

# View who's working on what
cortex-agent issues board

# Work stealing commands
cortex-agent issues stealable              # List stealable issues
cortex-agent issues steal 123              # Steal an issue
cortex-agent issues mark-stealable 123     # Mark your claim as stealable
cortex-agent issues contest 123            # Contest a steal

# Load balancing
cortex-agent issues load                   # View agent load distribution
cortex-agent issues load --agent coder-1   # View specific agent load
cortex-agent issues rebalance              # Trigger swarm rebalancing
cortex-agent issues rebalance --dry-run    # Preview rebalancing without applying
```
...
- [x] Claim service implemented (`@cortex-agent/cli/src/services/claim-service.ts`)
...
- [x] CLI commands functional (`@cortex-agent/cli/src/commands/issues.ts`)
...
|------|-------|-------------|
| `@cortex-agent/cli/src/services/claim-service.ts` | ~600 | Full claims service with work stealing |
| `@cortex-agent/cli/src/commands/issues.ts` | ~450 | CLI commands for issue claims |

### CLI Commands Implemented

```bash
# Issue claim commands (10 subcommands)
cortex-agent issues list          # List all claims
cortex-agent issues claim <id>    # Claim an issue
cortex-agent issues release <id>  # Release a claim
cortex-agent issues handoff       # Request handoff
cortex-agent issues status <id>   # Get claim status
cortex-agent issues stealable     # List stealable issues
cortex-agent issues steal <id>    # Steal an issue
cortex-agent issues load          # View agent load
cortex-agent issues rebalance     # Rebalance swarm
cortex-agent issues board         # Visual claim board
```
...
6. **Persistence**: File-based storage in `.cortex-agent/claims/claims.json`
