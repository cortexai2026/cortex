...
The `@cortex-agent/mcp` package implements the Model Context Protocol (MCP) 2025-11-25 specification. A security audit identified several vulnerabilities and missing features that needed to be addressed to ensure production readiness.
...
### Schema Validation
```typescript
import { validateSchema, formatValidationErrors } from '@cortex-agent/mcp';
...
### Rate Limiting
```typescript
import { createRateLimiter, rateLimitMiddleware } from '@cortex-agent/mcp';
...
### Sampling
```typescript
import { createSamplingManager, createAnthropicProvider } from '@cortex-agent/mcp';
...
### OAuth 2.1
```typescript
import { createOAuthManager, createGitHubOAuthConfig } from '@cortex-agent/mcp';
