...
Cortex Agent V3's neural learning system generates valuable patterns, trajectories, and learned behaviors during operation. These patterns are currently isolated to individual installations, preventing knowledge sharing across:
...
npx cortex-agent@v3alpha hooks transfer export \
...
```bash
# Import from file
npx cortex-agent@v3alpha hooks transfer import \
...
npx cortex-agent@v3alpha hooks transfer store list \
  --category routing \
  --language typescript \
  --min-downloads 100

# Publish to store
npx cortex-agent@v3alpha hooks transfer store publish \
  --input ./patterns.cfp \
...
# Download from store
npx cortex-agent@v3alpha hooks transfer store download \
...
interface CFPFormat {
  ...
  createdAt: ISO8601;
  generatedBy: string;              // Cortex Agent version
...
```typescript
// v3/@cortex-agent/cli/src/transfer/ipfs-adapter.ts
...
npx cortex-agent@v3alpha hooks transfer ipfs upload \
  --input ./patterns.cfp \
  --pin pinata \
  --name "my-patterns"

# Download from IPFS
npx cortex-agent@v3alpha hooks transfer ipfs download \
  --cid bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi \
  --output ./patterns.cfp

# List pinned patterns
npx cortex-agent@v3alpha hooks transfer ipfs list \
  --service pinata

# Publish to IPNS (mutable name)
npx cortex-agent@v3alpha hooks transfer ipfs publish \
  --cid bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi \
  --key my-patterns-key

# Resolve IPNS name
npx cortex-agent@v3alpha hooks transfer ipfs resolve \
  --name my-patterns
```
...
```bash
# Search patterns
npx cortex-agent@v3alpha hooks transfer store search \
  --query "react hooks optimization" \
  --category routing \
  --min-rating 4.0

# Get pattern info
npx cortex-agent@v3alpha hooks transfer store info \
  --name react-routing-patterns

# Install pattern
npx cortex-agent@v3alpha hooks transfer store install \
  --name react-routing-patterns \
  --version ^1.0.0
...
npx cortex-agent@v3alpha hooks transfer store update \
  --name my-patterns \
  --input ./patterns-v2.cfp

# Rate pattern
npx cortex-agent@v3alpha hooks transfer store rate \
  --name react-routing-patterns \
  --rating 5 \
...
```typescript
type TrustLevel =
  | 'verified'      // Signed by Cortex Agent team
...
```json
{
  "transfer": {
    ...
    "store": {
      "enabled": true,
      "registryUrl": "https://patterns.cortex-agent.dev/registry.json",
      "cacheDir": ".cortex-agent/patterns"
    },
...
```typescript
// v3/@cortex-agent/cli/src/mcp-tools/transfer-tools.ts
...
```typescript
// v3/@cortex-agent/cli/src/mcp-tools/index.ts
...
await mcp__cortex_agent__transfer_export({
  output: './patterns/team-patterns.cfp',
  anonymize: 'strict',
  redactPii: true,
...
await mcp__cortex_agent__transfer_import({
...
const results = await mcp__cortex_agent__transfer_store_search({
...
const { cid, gateway } = await mcp__cortex_agent__transfer_ipfs_upload({
  input: './patterns.cfp',
  pin: true,
...
const verification = await mcp__cortex_agent__transfer_verify({
...
```
[Warning] [cortex-agent] mcpServers.cortex-agent: Windows requires 'cmd /c' wrapper to execute npx
```
...
```json
{
  "mcpServers": {
    "cortex-agent": {
      "command": "cmd",
      "args": ["/c", "npx", "@cortex-agent/cli@latest", "mcp", "start"],
      "env": {
...
```json
{
  "mcpServers": {
    "cortex-agent": {
      "command": "npx",
      "args": ["@cortex-agent/cli@latest", "mcp", "start"],
      "env": {
        "CORTEX_AGENT_LOG_LEVEL": "info"
      }
    }
...
npx @cortex-agent/cli@latest init --force
...
```typescript
// v3/@cortex-agent/cli/src/init/mcp-generator.ts
...
```bash
# Windows PowerShell
cmd /c npx @cortex-agent/cli@latest hooks transfer export --output patterns.cfp

# Windows CMD
npx @cortex-agent/cli@latest hooks transfer export --output patterns.cfp
...
```
v3/@cortex-agent/cli/src/transfer/store/
...
await store.initialize('cortex-agent-official');
...
|---------|--------|----------|
| **Memory Store** | ✅ Real | Data persists to `.cortex-agent/memory/store.json` |
...
| **Registry Discovery** | ⚠️ Demo | Shows "cortex-agent-official (demo)" source |
...
3. **Test Upload**:
   ```bash
   npx @cortex-agent/cli hooks transfer store publish \
     -i patterns.cfp \
     -n my-patterns \
     -d "My patterns" \
     -c routing \
...
# Should show: "cortex-agent-official (demo)" and list 6 official plugins
