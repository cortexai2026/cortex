...
`npx cortex-agent@latest` installs **1.3GB** across 914 packages with a 35-second cold start in Docker. The Docker optimization work (Dockerfile.lite with `--omit=optional` + aggressive pruning) reduced this to 324MB, but the **core dependency chain** still carries unnecessary weight:

```
cortex-agent (5KB wrapper)
  └─ @cortex-agent/cli (9MB)
       ├─ @cortex-agent/shared (11MB) ← depends on sql.js (18MB WASM)
       ├─ @cortex-agent/mcp (650KB)
       ├─ semver (tiny)
       └─ @noble/ed25519 (tiny)
```
...
|----------|------|---------|-------|
| `@cortex-agent/shared` | `events/event-store.ts` | Append-only event sourcing log | 589 |
| `@cortex-agent/memory` | `sqljs-backend.ts` | Memory entries + brute-force vector search | 767 |
| `@cortex-agent/embeddings` | `persistent-cache.ts` | LRU embedding cache with TTL | 411 |
...
RVF (RuVector Format) is a binary container format already used in the Cortex Agent ecosystem (ADR-054). It provides everything sql.js does **plus native HNSW indexing** in a fraction of the footprint:
...
**Replace sql.js with RVF as the native storage backend** across `@cortex-agent/shared`, `@cortex-agent/memory`, and `@cortex-agent/embeddings`. Provide automatic and manual migration paths for existing SQLite (`.db`) and JSON (`.json`) data files with full backward compatibility.
...
cortex-agent migrate status --storage

# Dry-run migration (report what would change, don't modify)
cortex-agent migrate run --storage --dry-run

# Migrate specific file
cortex-agent migrate run --storage --file ./data/memory/memory.db

# Migrate all storage files in project
cortex-agent migrate run --storage --all

# Force re-migration (even if .rvf already exists)
cortex-agent migrate run --storage --force

# Rollback: restore from .bak files
cortex-agent migrate rollback --storage

# Validate migrated data integrity
cortex-agent migrate validate --storage
```
...
4. **`cortex-agent migrate rollback --storage`** restores `.bak` → original and removes `.rvf`.
...
cortex-agent memory init --backend sqljs

# Force JSON backend
cortex-agent memory init --backend json

# Default (RVF)
cortex-agent memory init
```
...
```typescript
// v3/@cortex-agent/embeddings/src/types.ts
...
The existing `RuVectorProvider` in `@cortex-agent/providers` gains RVF-backed persistence:
...
|------|---------|-------------|
| P1.1 | `@cortex-agent/memory` | Create `RvfBackend` implementing `IMemoryBackend` interface |
| P1.2 | `@cortex-agent/memory` | Map `KV_SEG` to memory entry CRUD operations |
| P1.3 | `@cortex-agent/memory` | Map `VEC_SEG` to embedding storage with typed quantization |
| P1.4 | `@cortex-agent/memory` | Map `INDEX_SEG` to HNSW search (replace brute-force cosine) |
| P1.5 | `@cortex-agent/memory` | Add `RvfBackend` to `DatabaseProvider` selection chain |

### Phase 2: Event Store Migration (Week 2-3)

| Task | Package | Description |
|------|---------|-------------|
| P2.1 | `@cortex-agent/shared` | Create `RvfEventLog` implementing `IEventStore` interface |
| P2.2 | `@cortex-agent/shared` | Map `LOG_SEG` to append-only event operations |
| P2.3 | `@cortex-agent/shared` | Map `SNAP_SEG` to snapshot save/load |
| P2.4 | `@cortex-agent/shared` | Move `sql.js` from `dependencies` to `optionalDependencies` |

### Phase 3: Embedding Cache Migration (Week 3)

| Task | Package | Description |
|------|---------|-------------|
| P3.1 | `@cortex-agent/embeddings` | Create `RvfEmbeddingCache` implementing `IPersistentCache` |
| P3.2 | `@cortex-agent/embeddings` | LRU eviction via RVF metadata (no SQL DELETE needed) |
| P3.3 | `@cortex-agent/embeddings` | TTL via RVF expiry flags (segment-level) |
| P3.4 | `@cortex-agent/embeddings` | Move `sql.js` from `dependencies` to `optionalDependencies` |

### Phase 4: Migration Tooling (Week 3-4)

| Task | Package | Description |
|------|---------|-------------|
| P4.1 | `@cortex-agent/cli` | `cortex-agent migrate status --storage` — detect formats, report state |
| P4.2 | `@cortex-agent/cli` | `cortex-agent migrate run --storage` — batch migration with progress |
| P4.3 | `@cortex-agent/cli` | `cortex-agent migrate rollback --storage` — restore from `.bak` |
| P4.4 | `@cortex-agent/cli` | `cortex-agent migrate validate --storage` — integrity verification |
| P4.5 | `@cortex-agent/memory` | Automatic migration in `DatabaseProvider.openStorage()` |

### Phase 5: RVF Embedding Provider (Week 4)

| Task | Package | Description |
|------|---------|-------------|
| P5.1 | `@cortex-agent/embeddings` | Add `'rvf'` to `EmbeddingProvider` type union |
| P5.2 | `@cortex-agent/embeddings` | Implement `RvfEmbeddingService` with hash-based embeddings |
| P5.3 | `@cortex-agent/embeddings` | Update `createEmbeddingServiceAsync` auto-select: `rvf > agentic-flow > transformers > mock` |
| P5.4 | `@cortex-agent/embeddings` | Add `RvfEmbeddingConfig` interface |
| P5.5 | `@cortex-agent/embeddings` | Tests: RVF provider passes `IEmbeddingService` test suite |

### Phase 6: ruvLLM Learning Persistence (Week 4-5)

| Task | Package | Description |
|------|---------|-------------|
| P6.1 | `@cortex-agent/memory` | Create `RvfLearningStore` class (VEC + KV + LOG segments for SONA) |
| P6.2 | `@cortex-agent/memory` | Implement `savePatterns` / `loadPatterns` for ReasoningBank persistence |
| P6.3 | `@cortex-agent/memory` | Implement LoRA adapter serialization to RVF OVERLAY segment |
| P6.4 | `@cortex-agent/memory` | Implement EWC++ Fisher diagonal persistence to META_SEG |
| P6.5 | `@cortex-agent/providers` | Extend `RuVectorProvider` with RVF-backed `searchMemory()` |
| P6.6 | `@cortex-agent/memory` | Create `PersistentSonaCoordinator` wrapping `SonaCoordinator` |

### Phase 7: Progressive Download System (Week 5-6)

| Task | Package | Description |
|------|---------|-------------|
| P7.1 | `@cortex-agent/cli` | Implement `ProgressiveDownloader` class |
| P7.2 | `@cortex-agent/cli` | Create capability manifest schema and seed registry |
| P7.3 | `@cortex-agent/cli` | `cortex-agent capabilities status/install/remove/list/prefetch` CLI commands |
| P7.4 | `@cortex-agent/embeddings` | Integrate progressive download into `createEmbeddingServiceAsync` |
| P7.5 | `@cortex-agent/providers` | Integrate progressive download into `RuVectorProvider` for LLM models |
| P7.6 | `@cortex-agent/cli` | Package Phase 1-2 capabilities as .rvf files on CDN/IPFS |

### Phase 8: Dependency Cleanup (Week 6-7)

| Task | Package | Description |
|------|---------|-------------|
| P8.1 | `@cortex-agent/shared` | Remove `sql.js` from hard dependencies |
| P8.2 | `@cortex-agent/memory` | Remove `sql.js` from hard dependencies |
| P8.3 | `@cortex-agent/embeddings` | Remove `sql.js` from hard dependencies |
...
|---------|-----------|---------------------|
| `@cortex-agent/shared` | sql.js (18MB) | ~30MB |
| `@cortex-agent/memory` | sql.js (18MB, deduped) | ~5MB own |
| `@cortex-agent/embeddings` | sql.js (18MB, deduped) | ~3MB own |
...
| Package | Hard Deps | Total Install Weight |
|---------|-----------|---------------------|
| `@cortex-agent/shared` | `@ruvector/rvf` (WASM: 52KB, native: ~2MB) | ~13MB (−17MB) |
| `@cortex-agent/memory` | (uses shared's rvf) | ~5MB (no change) |
| `@cortex-agent/embeddings` | (uses shared's rvf) | ~3MB (no change) |
...
| `@ruvector/rvf` npm availability | Low | High | Vendor WASM binary into `@cortex-agent/shared` as fallback |
...
- **Loss of SQL tooling** — can't `sqlite3 memory.db` to inspect data (mitigated by `cortex-agent memory list`)
...
- `npx cortex-agent@latest` installs 1.3GB (all optional deps)
...
Phase 0: Core CLI (always installed)
  cortex-agent (5KB) → @cortex-agent/cli (9MB) → @cortex-agent/shared (~13MB with RVF)
...
Phase 1: Lightweight Embeddings (downloaded on first use)
  ...
  Downloaded to: ~/.cortex-agent/capabilities/rvf-wasm.rvf

Phase 2: Neural Embeddings (downloaded on demand)
  all-MiniLM-L6-v2 ONNX model (~22MB)
  Stored as: ~/.cortex-agent/capabilities/models/minilm-l6-v2.rvf
...
Phase 3: Local LLM Inference (downloaded on demand)
  ...
  Stored as: ~/.cortex-agent/capabilities/models/<model>.rvf
...
Phase 4: Advanced Intelligence (downloaded on demand)
  ...
  Stored as: ~/.cortex-agent/capabilities/kernels/<kernel>.rvf
...
export class ProgressiveDownloader {
  ...
  constructor(cortex-agentHome = '~/.cortex-agent') {
    this.manifestPath = `${cortex-agentHome}/capabilities/manifest.json`;
    this.capabilitiesDir = `${cortex-agentHome}/capabilities`;
  }
  ...
  async ensure(capabilityId: string): Promise<string> {
    ...
    console.info(`[cortex-agent] Downloading ${entry.name} (${this.formatSize(entry.size)})...`);
    ...
    console.info(`[cortex-agent] ✓ ${entry.name} ready`);
    return rvfPath;
  }
...
cortex-agent capabilities status

# Download specific capability
cortex-agent capabilities install neural-embeddings

# Download all capabilities up to phase N
cortex-agent capabilities prefetch --phase 2

# Remove a capability
cortex-agent capabilities remove local-llm-qwen

# List all available models/kernels
cortex-agent capabilities list --phase 3
cortex-agent capabilities list --type model
cortex-agent capabilities list --type kernel
```
