# ADR-045: Guidance System Integration — Cortex Agent v3.1

**Status:** Accepted
**Date:** 2026-02-02
**Author:** Cortex Agent Architecture
**Version:** 3.1.0-alpha.1

## Context

The `@cortex-agent/guidance` package (published as `3.0.0-alpha.1`) provides a governance control plane for Claude Code sessions:
...
Currently, `@cortex-agent/guidance` is:
1. A standalone package at `v3/@cortex-agent/guidance/`
2. Used by `@cortex-agent/cli` via dynamic `import()` — but NOT declared as a dependency
3. Not included in the umbrella `cortex-agent` package's `files` array
4. Not available to end users running `npx cortex-agent@alpha`

This means the `guidance` CLI commands silently fail at runtime when installed from npm.

## Decision

Integrate `@cortex-agent/guidance` as a **first-class dependency** in both `@cortex-agent/cli` and the `cortex-agent` umbrella package, making it a core component of Cortex Agent v3.1.

### 1. Dependency Graph

```
cortex-agent (umbrella v3.1.0-alpha.1)
  └── @cortex-agent/cli (v3.1.0-alpha.1)
        ├── @cortex-agent/guidance (v3.0.0-alpha.1)  ← NEW
        ├── @cortex-agent/shared
        ├── @cortex-agent/mcp
...
{
  "dependencies": {
    "@cortex-agent/guidance": "^3.0.0-alpha.1"
  }
}
```

**`cortex-agent/package.json` (umbrella)**:
```json
{
  "files": [
    "v3/@cortex-agent/guidance/dist/**/*.js",
    "v3/@cortex-agent/guidance/dist/**/*.d.ts",
    "v3/@cortex-agent/guidance/package.json"
  ]
}
```
...
|---------|---------|------|
| `cortex-agent` (umbrella) | 3.0.0-alpha.185 | 3.1.0-alpha.1 |
| `@cortex-agent/cli` | 3.0.0-alpha.185 | 3.1.0-alpha.1 |
| `@cortex-agent/guidance` | 3.0.0-alpha.1 | 3.0.0-alpha.1 (unchanged) |
...
1. Add `@cortex-agent/guidance` to CLI's `dependencies`
