# ADR-042: Gas Town & Beads Analysis - Lessons for Cortex Agent V3
...
Steve Yegge released [Gas Town](https://github.com/steveyegge/gastown) on January 1, 2026, a multi-agent orchestration system built on top of [Beads](https://github.com/steveyegge/beads), his git-backed issue tracker. This ADR analyzes these systems and identifies lessons applicable to Cortex Agent V3.
...
| Concept | Gas Town | Cortex Agent V3 |
|---------|----------|----------------|
...
| Role | Function | Cortex Agent Equivalent |
|------|----------|----------------------|
...
## Comparison: Gas Town vs Cortex Agent V3

### Similarities

| Feature | Gas Town | Cortex Agent V3 |
|---------|----------|----------------|
...
| Aspect | Gas Town | Cortex Agent V3 |
|--------|----------|----------------|
...
```bash
npx cortex-agent tmux attach --mayor
npx cortex-agent tmux crew cycle
```
...
```bash
npx cortex-agent formulas search "release"
npx cortex-agent formulas install @community/release-workflow
```
...
Cortex Agent uses AgentDB for vectors, but could add Git-backed audit logs for critical state.
...
| 8 | Building orchestrator | Gas Town, Cortex Agent |

**Cortex Agent V3 targets Stage 7-8 users.**
...
Cortex Agent V3 has advantages in:
- **Intelligence**: Vector search, neural learning
- **Ecosystem**: MCP, plugins, multi-model
- **Accessibility**: No tmux requirement

The ideal system would combine Gas Town's durability guarantees with Cortex Agent's intelligence features.
