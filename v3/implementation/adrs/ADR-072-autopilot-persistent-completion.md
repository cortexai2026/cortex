...
- **Related**: ADR-037 (Autopilot Chat Mode in Cortex Agent UI), ADR-071 (Guidance MCP Tools)
...
Integrate agentic-flow's **Autopilot Persistent Completion System** (ADR-058) into the `@cortex-agent/cli` package at three layers:

1. **CLI commands** — 9 subcommands under `npx cortex-agent autopilot`
...
| **Swarm Tasks** | `.cortex-agent/swarm-tasks.json` | agentic-flow swarm state | High |
| **Checklist Files** | `.cortex-agent/data/checklist.json` | Manual task checklists | Normal |
...
4. **Manual disable**: User runs `npx cortex-agent autopilot disable` or calls `autopilot_disable` MCP tool
...
### Phase 1: CLI Command (npx cortex-agent autopilot)

**File**: `v3/@cortex-agent/cli/src/commands/autopilot.ts`
...
**File**: `v3/@cortex-agent/cli/src/mcp-tools/autopilot-tools.ts`
...
**File**: `v3/@cortex-agent/cli/src/hooks/autopilot-stop-hook.ts`
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx cortex-agent@latest hooks autopilot-check"
          }
        ]
      }
    ]
...
**File**: `v3/@cortex-agent/cli/src/init/executor.ts`

When `npx cortex-agent init` runs, inject autopilot behavioral instructions into the generated CLAUDE.md:
...
### Autopilot Commands
- `npx cortex-agent autopilot status` — Check current progress
- `npx cortex-agent autopilot enable` — Enable persistent completion
- `npx cortex-agent autopilot disable` — Disable (allow early stop)
- `npx cortex-agent autopilot predict` — Get AI-recommended next action
```
...
**Location**: `.cortex-agent/data/autopilot-state.json`
...
**Location**: `.cortex-agent/data/autopilot-log.json`
...
```json
{
  "claudeFlow": {
    "autopilot": {
      ...
      "logFile": ".cortex-agent/data/autopilot-log.json"
    }
  }
}
```
...
1. `npx cortex-agent autopilot status` returns current state (enabled, iterations, progress)
2. `npx cortex-agent autopilot enable/disable` toggles persistent completion
3. `npx cortex-agent autopilot config --max-iterations 100` persists to settings
...
9. `npx cortex-agent autopilot predict` returns actionable recommendations
10. `npx cortex-agent init` includes autopilot configuration in generated settings
...
| File | Purpose |
|------|---------|
| `v3/@cortex-agent/cli/src/commands/autopilot.ts` | CLI command with 9 subcommands |
| `v3/@cortex-agent/cli/src/mcp-tools/autopilot-tools.ts` | 10 MCP tools |
| `v3/@cortex-agent/cli/src/hooks/autopilot-stop-hook.ts` | Stop hook coordinator |
| `v3/@cortex-agent/cli/__tests__/autopilot.test.ts` | Unit tests |

### Modified Files

| File | Change |
|------|--------|
| `v3/@cortex-agent/cli/src/commands/index.ts` | Register autopilot command |
| `v3/@cortex-agent/cli/src/mcp-tools/index.ts` | Export autopilotTools |
| `v3/@cortex-agent/cli/src/mcp-client.ts` | Register autopilot tools in registerTools() |
| `v3/@cortex-agent/cli/src/init/executor.ts` | Inject autopilot config in CLAUDE.md + settings |
