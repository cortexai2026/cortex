...
| ADR | Title | Status | File |
|-----|-------|--------|------|
| ADR-046 | Dual Umbrella: cortex-agent + cortex-agent | Accepted | [ADR-046-cortex-agent-rebrand.md](./ADR-046-cortex-agent-rebrand.md) |
...
#### Release: @cortex-agent/cli@3.0.0-alpha.84 (Beta Ready)
...
```bash
npx cortex-agent update check      # Check for updates
npx cortex-agent update all        # Update all packages
npx cortex-agent update history    # View update history
npx cortex-agent update rollback   # Rollback last update
```
...
#### Release: @cortex-agent/cli@3.0.0-alpha.15 (Latest)
...
npx cortex-agent@v3alpha doctor

# Auto-install Claude Code CLI if missing
npx cortex-agent@v3alpha doctor --install

# Check only Claude Code CLI
npx cortex-agent@v3alpha doctor -c claude
```

**Package Resolution Fix**: Fixed Windows module resolution issue where `@cortex-agent/cli` exports pointed to wrong paths (`dist/index.js` → `dist/src/index.js`).

#### Release: @cortex-agent/cli@3.0.0-alpha.7
...
- **File Persistence**: All MCP tools use file-based persistence in `.cortex-agent/` directories
...
#### Install
```bash
npx @cortex-agent/cli@v3alpha --help
```

### Release: @cortex-agent/cli@3.0.0-alpha.11 (2026-01-07)
...
cortex-agent completions bash > ~/.bash_completion.d/cortex-agent

# Install zsh completions
cortex-agent completions zsh > ~/.zfunc/_cortex-agent

# Install fish completions
cortex-agent completions fish > ~/.config/fish/completions/cortex-agent.fish

# Install PowerShell completions
cortex-agent completions powershell >> $PROFILE
```
...
| 🟢 P2 | Add interactive shell/REPL mode | Pending | `cortex-agent shell` for interactive command execution |
...
cortex-agent doctor              # Full system check
cortex-agent doctor --fix        # Auto-fix issues where possible
cortex-agent doctor --component mcp  # Check specific component
...
cortex-agent logs                # All logs
cortex-agent logs --follow       # Tail logs
cortex-agent logs --component daemon
cortex-agent logs --level error
```

**P2 - Nice to Have**
```bash
# Self-update
cortex-agent upgrade             # Upgrade to latest
cortex-agent upgrade --check     # Check for updates
cortex-agent upgrade --version 3.1.0

# Interactive shell
cortex-agent shell               # Enter REPL
> swarm init mesh
> agent spawn coder
...
## agentic-flow vs cortex-agent Feature Comparison

### Feature Matrix

| Feature | agentic-flow | cortex-agent | Integration Value |
|---------|--------------|-------------|-------------------|
| **Core Agent System** | | | |
...
```bash
# Integration target:
cortex-agent transport quic --port 4433
cortex-agent swarm start --transport quic  # 50-70% faster agent comms
```

**2. Federation Hub (Ephemeral Agents)**
```bash
# Integration target:
cortex-agent federation start --port 9443
cortex-agent federation spawn --tenant acme --lifetime 600
```
...
```bash
# Integration target:
cortex-agent agent spawn -t coder --optimize --priority cost
cortex-agent providers optimize --task "Build API" --budget 0.01
```

**4. Provider Fallback (Enterprise resilience)**
```bash
# Integration target:
cortex-agent providers fallback configure --primary anthropic --fallback openrouter,onnx
```

**5. ReasoningBank (WASM Learning Memory)**
```bash
# Integration target:
cortex-agent reasoningbank store "pattern" --reasoning "..."
cortex-agent reasoningbank search "authentication patterns"
```
...
cortex-agent agent spawn -t coder --optimize --priority cost
cortex-agent providers fallback configure --primary anthropic --fallback openrouter,onnx
cortex-agent embeddings download all-MiniLM-L6-v2
```
...
# After Phase 2:
cortex-agent federation start --port 9443
cortex-agent federation spawn --lifetime 300 --task "Quick analysis"
cortex-agent proxy start --provider openrouter
cortex-agent daemon dispatch security-audit
```
...
```bash
# After Phase 3:
cortex-agent swarm start --transport quic  # 50-70% faster
cortex-agent reasoningbank store "pattern" --reasoning "..."
cortex-agent agent spawn -t security-analyst  # One of 66 types
```
...
- agentic-flow/src/transport/quic.ts → cortex-agent/src/transport/
- agentic-flow/src/federation/ → cortex-agent/src/federation/
- agentic-flow/src/reasoningbank/ → cortex-agent/src/reasoningbank/
```

**Option C: Unified Package (Long-term)**
```bash
@cortex-agent/core      # Shared primitives
@cortex-agent/cli       # CLI (current)
@cortex-agent/agents    # From agentic-flow's 66 agents
@cortex-agent/transport # QUIC + HTTP + WebSocket
```
...
**cortex-agent ALREADY HAS** (via @cortex-agent/embeddings):
| Feature | cortex-agent | ruvector | Status |
|---------|-------------|----------|--------|
| ONNX Embeddings | ✅ agentic-flow (~3ms) | ✅ @ruvector/core | **Equivalent** |
| Local Embeddings | ✅ all-MiniLM-L6-v2 | ✅ all-MiniLM-L6-v2 | **Equivalent** |
| HNSW Indexing | ✅ @cortex-agent/memory | ✅ @ruvector/core | **Equivalent** |
| Persistent Cache | ✅ SQLite + LRU | ✅ Memory cache | **Equivalent** |
| Hyperbolic Embeddings | ✅ Poincaré ball | ❌ | **cortex-agent ahead** |
| Document Chunking | ✅ 4 strategies | ❌ | **cortex-agent ahead** |
| Normalization | ✅ L2, L1, min-max, z-score | ❌ | **cortex-agent ahead** |
| Neural Substrate | ✅ Drift, memory physics | ❌ | **cortex-agent ahead** |

### Unique ruvector Features (Integration Candidates)

#### 🔴 Tier 1: High Value (cortex-agent lacks these)
...
*Note: Flash Attention, SONA Learning, HNSW, and ONNX embeddings are already in cortex-agent via agentic-flow.*
...
**Already in cortex-agent (skip):**
- `hooks_rag_context` — Use @cortex-agent/memory instead
- `hooks_attention_info` — Use @cortex-agent/neural instead
- Embeddings tools — Use @cortex-agent/embeddings instead

### Integration Approach

**Recommended: Option A - Add as Optional Dependency (for unique features only)**
```json
// @cortex-agent/cli/package.json
{
...
cortex-agent route "task" --q-learning          # Uses hooks_route

# AST analysis (unique to ruvector)
cortex-agent analyze ast src/                   # Uses hooks_ast_analyze

# Diff classification (unique to ruvector)
cortex-agent analyze diff --risk                # Uses hooks_diff_analyze

# Coverage-aware routing (unique to ruvector)
cortex-agent route "task" --coverage-aware      # Uses hooks_coverage_route
```

**Already in cortex-agent (DO NOT import from ruvector):**
```bash
cortex-agent embeddings generate --local        # Uses @cortex-agent/embeddings (ONNX)
cortex-agent memory search --semantic "query"   # Uses @cortex-agent/memory (HNSW)
```
...
*Note: SONA, Flash Attention, HNSW already in cortex-agent - no need to import.*
