...
Add 5 new MCP tools to the `@cortex-agent/cli` package:
...
**PID reuse guard**: `isProcessRunning()` now verifies the process is actually `node`/`cortex-agent`/`npx` by inspecting `/proc/{pid}/cmdline` (Linux) or `ps -p` (macOS). Falls back to `kill -0` on platforms where this isn't available.

**Legacy cleanup**: `removePidFile()` now also removes `.cortex-agent/mcp-server.pid` from older versions that wrote to a different path than the current `/tmp/cortex-agent-mcp.pid`.
...
### Negative
- `hooks` fields were removed from agent frontmatter (they contained Cortex Agent shell scripts, not Claude Code hook references — functionality preserved in CLI hooks system)
- Guidance tools use a static catalog that must be updated when new capabilities are added

## Files Changed

- `v3/@cortex-agent/cli/src/mcp-tools/guidance-tools.ts` — new (5 tools)
- `v3/@cortex-agent/cli/src/mcp-tools/index.ts` — export guidance tools
- `v3/@cortex-agent/cli/src/mcp-client.ts` — register guidance tools
- `v3/@cortex-agent/cli/src/mcp-server.ts` — PID self-detection + reuse guard + legacy cleanup
- `v3/@cortex-agent/cli/src/init/executor.ts` — stop counting .yaml files
