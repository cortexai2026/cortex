...
`npx cortex-agent init`, and real-time data feeds from 8+ subsystems.
...
2. **Generated statusline** (`.claude/helpers/statusline.cjs`) — CommonJS script
   created by `npx cortex-agent init`, comprehensive with 12+ metric panels
...
5. **Generator** (`v3/@cortex-agent/cli/src/init/statusline-generator.ts`) — TypeScript
...
│  │  ▊ Cortex Agent V3  ● user  │  ⎇ branch  │  Model            │  │
│  │  ─────────────────────────────────────────                   │  │
...
|-------------|-----------|---------|------------|
| `.cortex-agent/data/autopilot-state.json` | Context Autopilot (ADR-051) | Token %, token count, prune cycles, growth trend | `context-persistence-hook.mjs` on every `UserPromptSubmit` |
| `.cortex-agent/metrics/v3-progress.json` | DDD Architecture | Domain count, DDD progress %, active agents | `init` command, manual updates |
| `.cortex-agent/security/audit-status.json` | Security | CVE count, audit status (CLEAN/PENDING) | `security scan` command |
| `.cortex-agent/metrics/performance.json` | Performance | Flash Attention speedup | `performance benchmark` command |
| `.cortex-agent/metrics/learning.json` | Intelligence | Score (0-100), routing accuracy, SONA status | `hooks post-task`, neural training |
| `.cortex-agent/learning/patterns.db` | Pattern DB (SQLite) | Short/long-term pattern counts, avg quality | `hooks intelligence`, neural training |
| `.cortex-agent/data/transcript-archive.db` | Context Archive (SQLite) | Entry count, session count | `context-persistence-hook.mjs` |
| `.cortex-agent/metrics/swarm-activity.json` | Swarm Monitor | Active agent count, swarm state | Swarm monitor daemon |
...
```
Line 0: ▊ Cortex Agent V3 ● user  │  ⎇ branch  │  Model
Line -: ─────────────────────────────────────────
...
Created by `npx cortex-agent init`. CommonJS for ES module project compatibility.
...
#### Generator — `v3/@cortex-agent/cli/src/init/statusline-generator.ts` (1,317 lines)

Produces the `.cjs` script during `npx cortex-agent init`:
...
#### Settings Generator — `v3/@cortex-agent/cli/src/init/settings-generator.ts`
...
#### Executor — `v3/@cortex-agent/cli/src/init/executor.ts`
...
#### Types — `v3/@cortex-agent/cli/src/init/types.ts`
...
| `v3/@cortex-agent/cli/src/init/statusline-generator.ts` | 1,317 | TypeScript | Generator for `.cjs` during init |
| `v3/@cortex-agent/cli/src/init/settings-generator.ts` | ~20 | TypeScript | Wires statusLine into settings.json |
| `v3/@cortex-agent/cli/src/init/executor.ts` | ~60 | TypeScript | Copy/generate during init |
| `v3/@cortex-agent/cli/src/init/types.ts` | ~20 | TypeScript | `StatuslineConfig` interface |
