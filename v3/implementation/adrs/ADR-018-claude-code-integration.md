...
The `@anthropic-ai/claude-code` package (v2.1.1) provides the official CLI for Claude AI. Deep integration with Claude Code enables enhanced developer experience for cortex-agent users. This ADR documents **undocumented integration points** discovered through source code analysis that are not covered in official documentation.
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "hooks": ["npx cortex-agent@v3alpha hooks modify-bash"]
      },
      {
        "matcher": "Write|Edit",
        "hooks": ["npx cortex-agent@v3alpha hooks modify-file"]
      }
    ],
    "PostToolUse": [
      {
        "matcher": ".*",
        "hooks": ["npx cortex-agent@v3alpha hooks post-command"]
      }
    ],
    "UserPromptSubmit": [
      {
        "hooks": ["npx cortex-agent@v3alpha hooks route --task \"$PROMPT\""]
      }
    ]
  }
}
...
```json
{
  "mcpServers": {
    "cortex-agent": {
      "command": "npx",
      "args": ["cortex-agent@v3alpha", "mcp", "start"],
      "allowlist": [
        "swarm_init",
        "agent_spawn",
        "memory_*",
...
export async function configureIntegration(options: {
  ...
  if (options.enableMcp && status.configPath) {
    await exec(`claude mcp add ${options.mcpServerName || 'cortex-agent'} npx cortex-agent@v3alpha mcp start`);
  }
}
```
...
/**
 * Install cortex-agent hooks into Claude Code settings
 */
...
npx @cortex-agent/cli@latest init --start-all

# Equivalent to running:
# 1. npx @cortex-agent/cli@latest init
# 2. npx @cortex-agent/cli@latest memory init
# 3. npx @cortex-agent/cli@latest daemon start
# 4. npx @cortex-agent/cli@latest swarm init --topology hierarchical
```
...
**CLI Version:** `@cortex-agent/cli@3.0.0-alpha.56`
