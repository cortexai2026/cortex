...
- Package: `@cortex-agent/codex` (npm)
- Future umbrella: `coflow` (npm/npx coflow)
- Current umbrella: `cortex-agent` (maintained for compatibility)
...
We will create a **parallel Codex integration** in cortex-agent that:
...
5. **Maps cortex-agent concepts** to Codex equivalents
6. **Supports dual-mode** projects (both Claude Code and Codex)

### Architecture

```
cortex-agent init --codex
...
└── .cortex-agent/                # Runtime (shared between both)
    ├── config.yaml
    └── data/
```
...
| cortex-agent Concept | Claude Code Output | Codex Output |
|---------------------|-------------------|--------------|
| Project instructions | `CLAUDE.md` | `AGENTS.md` |
...
### 1. Initialize Swarm
```bash
npx cortex-agent@v3alpha swarm init --topology hierarchical
```
...
```bash
npx cortex-agent@v3alpha memory store --key "[pattern]" --value "[learned]"
```
```
...
1. Create `@cortex-agent/codex` package in `v3/@cortex-agent/codex/`
2. Implement AGENTS.md generator
3. Implement SKILL.md generator
4. Implement config.toml generator
5. Set up npm publishing for `@cortex-agent/codex`
...
5. Wire up `@cortex-agent/codex` as dependency
...
4. Publish skills as part of `@cortex-agent/codex`

### Phase 4: Automation Integration (Week 5)
1. Map cortex-agent hooks to Codex Automations
...
3. Maintain `cortex-agent` as alias for compatibility
...
### AGENTS.md Sections (Cortex Agent Template)
...
| Security | From @cortex-agent/security patterns |
...
These features were discovered through binary analysis and can be leveraged for deep cortex-agent integration.
...
**Integration opportunity**: Map to cortex-agent swarm coordination.
...
**Integration opportunity**: Register cortex-agent tools dynamically.
...
```toml
# Cortex Agent V3 - Codex Configuration (Enhanced)
