...
Upgrade `agentic-flow` from `^2.0.7` to `^3.0.0-alpha.1` in `@cortex-agent/cli`, preserving all lazy-import patterns and fallback behavior.
...
| `cortex-agent-sdk` | SDK | Cortex Agent SDK server |
...
| `src/mcp-tools/neural-tools.ts` | `import('agentic-flow/reasoningbank')`, `import('@cortex-agent/embeddings')` | Tier 1: ReasoningBank WASM, Tier 2: embeddings, Tier 3: mock |
...
| I4 | `neural-tools.ts` | Added Tier 1: ReasoningBank WASM embeddings before @cortex-agent/embeddings |
...
| Embedding service | Compatible | 3-tier: ReasoningBank → @cortex-agent/embeddings → mock |
...
|---------|-----------|-----------------|
| `@cortex-agent/cli` | `agentic-flow` | `^3.0.0-alpha.1` (updated) |
| `@cortex-agent/cli` | `agentdb` | `^3.0.0-alpha.10` (updated) |
...
cd v3/@cortex-agent/cli && npm test  # → 445 passed
