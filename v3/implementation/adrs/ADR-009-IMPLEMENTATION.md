...
```typescript
import { HybridBackend } from '@cortex-agent/memory';
...
```typescript
import { HybridBackend, AgentDBAdapter } from '@cortex-agent/memory';
...
```bash
cd /workspaces/cortex-agent/v3/@cortex-agent/memory
...
This provides Cortex Agent V3 with a **flexible, performant, and reliable** memory system that adapts to different query patterns while maintaining consistency across both backends.
