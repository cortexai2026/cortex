# ADR-019: @cortex-agent/headless Runtime Package
...
Create `@cortex-agent/headless` package providing:
...
export interface DockerConfig {
  ...
  image: string;  // e.g., 'ghcr.io/ruvnet/cortex-agent-headless:latest'
...
npx @cortex-agent/headless serve --port 3001 --sandbox strict

# Execute single prompt
npx @cortex-agent/headless exec "Fix the bug in auth.ts" --cwd ./project

# Execute from file
npx @cortex-agent/headless exec --file tasks.txt --parallel 3

# Queue management
npx @cortex-agent/headless queue add "Refactor utils" --priority high
npx @cortex-agent/headless queue list
npx @cortex-agent/headless queue cancel <id>

# Docker mode
npx @cortex-agent/headless docker start --containers 3
npx @cortex-agent/headless docker exec "Run tests" --isolated
npx @cortex-agent/headless docker scale 5

# Monitoring
npx @cortex-agent/headless status
npx @cortex-agent/headless metrics --prometheus
```
  ...
  review:
    ...
    steps:
      ...
      - name: Run Claude Review
        ...
        run: |
          npx @cortex-agent/headless exec \
...
```typescript
import { HeadlessExecutor, TaskQueue } from '@cortex-agent/headless';
...
spec:
  template:
    spec:
      containers:
      - name: claude
        image: ghcr.io/ruvnet/cortex-agent-headless:latest
        env:
...
```typescript
import { ContainerExecutor } from '@cortex-agent/headless';
...
```json
{
  "name": "@cortex-agent/headless",
  "version": "3.0.0-alpha.1",
  "dependencies": {
    "@cortex-agent/shared": "^3.0.0-alpha.1",
...
2. **Standalone** - Can be used without other cortex-agent packages
