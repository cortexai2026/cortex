...
The `aidefence` npm package (v2.1.1) provides a production-ready AI Manipulation Defense System (AIMDS) with capabilities that complement and enhance Cortex Agent V3's security architecture:
...
| aidefence Feature | Cortex Agent V3 Equivalent | Synergy |
|-------------------|---------------------------|---------|
| AgentDB integration | `@cortex-agent/memory` with AgentDB | **Direct compatibility** - both use AgentDB for vector search |
...
The current `@cortex-agent/security` module addresses CVE-2, CVE-3, HIGH-1, HIGH-2 but lacks:
...
Integrate `aidefence` as a security enhancement layer within Cortex Agent V3 using a **bounded context** approach with clear domain boundaries.

### 1. Domain-Driven Design Architecture

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        Cortex Agent V3 Security Domain                        │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌────────────────────────────┐    ┌────────────────────────────────────┐   │
│  │  @cortex-agent/security     │    │  @cortex-agent/aidefence            │   │
...
#### 2.1 Core Security Context (`@cortex-agent/security`)
...
#### 2.2 AI Defense Context (`@cortex-agent/aidefence`) - NEW
...
Translate between aidefence and cortex-agent domains:

```typescript
// v3/@cortex-agent/aidefence/src/infrastructure/aidefence-adapter.ts

import { DefenseResult as AIDefenseResult } from 'aidefence';
import { ThreatDetectionResult } from '@cortex-agent/security';
...
```typescript
// v3/@cortex-agent/mcp/src/tools/aidefence-tools.ts
...
```typescript
// v3/@cortex-agent/cli/src/commands/security.ts (extension)
...
```typescript
// v3/@cortex-agent/cli/src/hooks/aidefence-hooks.ts
...
```yaml
# v3/@cortex-agent/cli/.claude/skills/aidefence.yaml
...
```yaml
# v3/@cortex-agent/cli/.claude/agents/v3/security-architect.yaml (enhancement)
...
hooks:
  pre: |
    ...
    ATTACK_PATTERNS=$(npx cortex-agent@v3alpha security defend --input "$TASK" --mode thorough --json)
  ...
  post: |
    ...
    npx cortex-agent@v3alpha security behavior --agent "security-architect-$(date +%s)" --record-action "$TASK"
```
...
```typescript
// v3/@cortex-agent/memory/src/config/security-namespaces.ts

export const securityNamespaces: NamespaceConfig[] = [
  {
    name: 'security_threats',
    description: 'Shared threat pattern storage (aidefence + cortex-agent)',
    vectorDimension: 384,
    hnswConfig: {
      m: 16,
      efConstruction: 200,
    ...
    schema: {
      ...
      source: { type: 'string', enum: ['aidefence', 'cortex-agent', 'manual'] }
    }
  },
  {
...
```typescript
// v3/@cortex-agent/aidefence/src/infrastructure/metrics.ts
...
```json
{
  "name": "@cortex-agent/aidefence",
  "version": "3.0.0-alpha.1",
  "dependencies": {
    "aidefence": "^2.1.1",
    "@cortex-agent/security": "workspace:*",
    "@cortex-agent/memory": "workspace:*",
    "@cortex-agent/core": "workspace:*",
...
- Create `@cortex-agent/aidefence` package
