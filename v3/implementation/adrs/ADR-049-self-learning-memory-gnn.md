...
1. **No learning pipeline**: The `@cortex-agent/neural` package has a fully implemented `NeuralLearningSystem` with SONA, ReasoningBank (4-step RETRIEVE/JUDGE/DISTILL/CONSOLIDATE pipeline), and PatternLearner — but these are completely disconnected from the memory bridge.
...
- **Optional dependency**: `@cortex-agent/neural` is loaded dynamically; when unavailable, all learning operations degrade to no-ops (confidence remains static).
...
│           │                                                   │
│  ┌────────▼─────────────────┐                                │
│  │  @cortex-agent/neural     │  (optional peer dependency)    │
...
- **Graceful degradation**: Tests verify no-op behavior when `@cortex-agent/neural` unavailable
...
- `@cortex-agent/neural` becomes an optional peer dependency
