...
Complete command reference for the cortex-agent V3 CLI including hooks, workflow execution, hive-mind coordination, process management, and all subcommands with SONA/MoE/HNSW integration.
...
```bash
# Hooks are available through the main cortex-agent CLI
npm install -g @cortex-agent/cli

# Or via npx
npx cortex-agent hooks --help
```
...
```bash
npx cortex-agent hooks pre-edit <filePath> [options]
...
Examples:
  npx cortex-agent hooks pre-edit src/auth.ts
  npx cortex-agent hooks pre-edit src/new-file.ts --operation create
  npx cortex-agent hooks pre-edit src/legacy.ts --no-suggestions --format json
```
...
```bash
npx cortex-agent hooks post-edit <filePath> [options]
...
Examples:
  npx cortex-agent hooks post-edit src/auth.ts --success true
  npx cortex-agent hooks post-edit src/auth.ts --success false --outcome "Type error on line 42"

  # V2 compatibility
  npx cortex-agent hooks post-edit --file src/auth.ts --success true --memory-key "swarm/coder/edit1"
```
...
```bash
npx cortex-agent hooks pre-command "<command>" [options]
...
Examples:
  npx cortex-agent hooks pre-command "npm test"
  npx cortex-agent hooks pre-command "rm -rf ./dist" --working-dir /project
  npx cortex-agent hooks pre-command "docker compose up" --format json
```
...
```bash
npx cortex-agent hooks post-command "<command>" [options]
...
Examples:
  npx cortex-agent hooks post-command "npm test" --success true --time 5230
  npx cortex-agent hooks post-command "npm build" --success false --exit-code 1 --error "Module not found"
```
...
```bash
npx cortex-agent hooks pre-task [options]
...
Examples:
  npx cortex-agent hooks pre-task --description "Implement OAuth2 flow"
  npx cortex-agent hooks pre-task -d "Fix login bug" --agent debugger --task-id task-123
```
...
```bash
npx cortex-agent hooks post-task [options]
...
Examples:
  npx cortex-agent hooks post-task --task-id task-123 --success true
  npx cortex-agent hooks post-task --task-id task-123 --success false --result "Blocked by dependency"
```
...
```bash
npx cortex-agent hooks session-restore [options]
...
Examples:
  npx cortex-agent hooks session-restore --session-id swarm-abc123
  npx cortex-agent hooks session-restore --session-id previous --include-memory false
```
...
```bash
npx cortex-agent hooks session-end [options]
...
Examples:
  ...
  npx cortex-agent hooks session-end --export-metrics true --summary
```
...
```bash
npx cortex-agent hooks route "<task>" [options]
...
Examples:
  npx cortex-agent hooks route "Implement user authentication"
  npx cortex-agent hooks route "Fix CSS bug" --prefer "coder,reviewer"
  npx cortex-agent hooks route "Research API options" --context "REST vs GraphQL" --format json
```
...
```bash
npx cortex-agent hooks explain "<task>" [options]
...
Examples:
  npx cortex-agent hooks explain "Implement OAuth2 authentication"
  npx cortex-agent hooks explain "Security audit" --verbose
```
...
```bash
npx cortex-agent hooks pretrain [options]
...
Examples:
  ...
  npx cortex-agent hooks pretrain --path /project --max-patterns 5000
  npx cortex-agent hooks pretrain --force --no-include-git
```
...
```bash
npx cortex-agent hooks build-agents [options]
...
Examples:
  npx cortex-agent hooks build-agents --focus security
  npx cortex-agent hooks build-agents --output agents.json
  npx cortex-agent hooks build-agents --v3-mode --focus performance
```
...
```bash
npx cortex-agent hooks transfer <sourceProject> [options]
...
Examples:
  npx cortex-agent hooks transfer /other-project
  npx cortex-agent hooks transfer ../shared-project --filter "security/*"
  npx cortex-agent hooks transfer /template --dry-run
```
...
```bash
npx cortex-agent hooks metrics [options]
...
Examples:
  ...
  npx cortex-agent hooks metrics --category routing --time-range week
  npx cortex-agent hooks metrics --detailed --format json
  npx cortex-agent hooks metrics --v3-dashboard
```
...
```bash
npx cortex-agent hooks list [options]
...
Examples:
  npx cortex-agent hooks list
  npx cortex-agent hooks list --category routing
  npx cortex-agent hooks list --include-disabled --format json
```
...
```bash
npx cortex-agent hooks notify [options]
...
Examples:
  npx cortex-agent hooks notify --message "Task completed successfully"
  npx cortex-agent hooks notify -m "Build failed" --level error
```
...
CORTEX_AGENT_HOOK_TIMEOUT=5000

# Enable/disable ReasoningBank integration
CORTEX_AGENT_REASONINGBANK_ENABLED=true

# Learning namespace
CORTEX_AGENT_HOOKS_NAMESPACE=hooks-learning

# Logging level
CORTEX_AGENT_HOOKS_LOG_LEVEL=info
```
...
npx cortex-agent hooks session-restore --session-id "swarm-[id]"
npx cortex-agent hooks post-edit --file "[file]" --memory-key "swarm/[agent]/[step]"
npx cortex-agent hooks notify --message "[what was done]"
npx cortex-agent hooks post-task --task-id "[task]"
...
```
$ npx cortex-agent hooks route "Implement authentication"
...
```bash
$ npx cortex-agent hooks route "Implement authentication" --format json
```
...
```
$ npx cortex-agent hooks metrics --format table
...
```bash
npx cortex-agent hooks intelligence [options]
```
...
```bash
npx cortex-agent workflow run <name|file> [options]
```
...
**Example:**
```bash
npx cortex-agent workflow run development --input '{"feature": "auth"}' --parallel 6
```
...
```bash
npx cortex-agent workflow validate <file> [options]
```
...
```bash
npx cortex-agent workflow list [options]
```
...
```bash
npx cortex-agent workflow status [options]
```
...
```bash
npx cortex-agent workflow stop [options]
```
...
```bash
# List templates
npx cortex-agent workflow template list

# Show template details
npx cortex-agent workflow template show <name>

# Create new template
npx cortex-agent workflow template create --name <name> --output <file>
```
...
**Example:**
```bash
npx cortex-agent hive-mind init --topology hierarchical-mesh --consensus byzantine --workers 15
```
...
```bash
npx cortex-agent hive-mind spawn [options]
```
...
```bash
npx cortex-agent hive-mind status [options]
```
...
```bash
npx cortex-agent hive-mind task [options]
```
...
```bash
npx cortex-agent hive-mind optimize-memory [options]
```
...
```bash
npx cortex-agent hive-mind shutdown [options]
```
...
```bash
npx cortex-agent agent list [options]
```
...
```bash
npx cortex-agent agent pool [options]
```
...
```bash
npx cortex-agent agent health [options]
```
...
```bash
npx cortex-agent agent logs [options]
```
...
```bash
npx cortex-agent memory store --key <key> --value <value> [options]
```
...
```bash
npx cortex-agent memory retrieve --key <key> [options]
```
...
```bash
npx cortex-agent memory search --query "<query>" [options]
```
...
```bash
npx cortex-agent memory stats [options]
```
...
```bash
npx cortex-agent memory cleanup [options]
```
...
```bash
npx cortex-agent memory compress [options]
```
...
```bash
# Export memory
npx cortex-agent memory export --output <file> [options]

# Import memory
npx cortex-agent memory import --input <file> [options]
```
...
```bash
npx cortex-agent process daemon --action <action> [options]
```
...
| `--pid-file` | string | `.cortex-agent/daemon.pid` | PID file location |
| `--log-file` | string | `.cortex-agent/daemon.log` | Log file location |
...
```bash
npx cortex-agent process monitor [options]
```
...
```
╔══════════════════════════════════════════════════════════════╗
║            🖥️  CORTEX-AGENT PROCESS MONITOR                    ║
╠══════════════════════════════════════════════════════════════╣
║  SYSTEM                                                      ║
║  CPU:    [████████░░░░░░░░░░░░] 38.5%                        ║
║  Memory: [██████████████░░░░░░] 512MB/2048MB                 ║
...
```bash
npx cortex-agent process workers --action <action> [options]
```
...
```bash
npx cortex-agent process signals --target <target> --signal <signal> [options]
```
...
```bash
npx cortex-agent process logs [options]
```
...
```bash
npx cortex-agent swarm status [options]
```
...
```bash
npx cortex-agent swarm task --description "<task>" [options]
```
...
|----------|-------------|---------|
| `CORTEX_AGENT_CONFIG` | Config file path | `.cortex-agent/config.json` |
| `CORTEX_AGENT_LOG_LEVEL` | Log level | `info` |
| `CORTEX_AGENT_MEMORY_BACKEND` | Memory backend | `agentdb` |
| `CORTEX_AGENT_EMBEDDING_PROVIDER` | Embedding provider | `transformers` |
| `CORTEX_AGENT_HOOK_TIMEOUT` | Hook timeout (ms) | `5000` |
| `CORTEX_AGENT_REASONINGBANK_ENABLED` | ReasoningBank integration | `true` |
