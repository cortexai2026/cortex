...
│  └─> Reads from:                                                         │
│       ├─ .cortex-agent/metrics.db      (SQLite, primary)                 │
│       ├─ .cortex-agent/hooks.db        (ReasoningBank patterns)          │
│       └─ .cortex-agent/metrics/*.json  (exported, legacy compat)         │
├─────────────────────────────────────────────────────────────────────────┤
│  SessionEnd Hook                                                         │
...
if [ -f ".cortex-agent/metrics.db" ]; then
  METRICS=$(node -e "
...
# Format output
printf "▊ Cortex Agent V3 ● agentic-flow@alpha  │  ⎇ v3\n"
printf "─────────────────────────────────────────────────────\n"
        ...
        "hooks": [
          {
            "type": "command",
            "timeout": 5000,
            "command": "/workspaces/cortex-agent/.claude/helpers/daemon-manager.sh start 3 30 60"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "type": "command",
            "timeout": 3000,
            "command": "/workspaces/cortex-agent/.claude/helpers/daemon-manager.sh stop"
          }
        ]
      }
    ],
...
### V3 Project Settings (`.cortex-agent/config.json`)
...
```json
{
  "daemons": {
    ...
    "metricsSync": {
      "enabled": true,
      "interval": 30000,
      "database": ".cortex-agent/metrics.db"
    },
    "hooksLearning": {
      "enabled": true,
      "interval": 60000,
      "database": ".cortex-agent/hooks.db"
    }
  },
  "statusline": {
    "enabled": true,
...
SessionStart Hook Triggered
    ...
    │   ├─> Load patterns from .cortex-agent/hooks.db
...
# View logs
tail -f .cortex-agent/logs/daemon.log

# Check for stale PID files
ls -la .cortex-agent/pids/

# Manual cleanup and restart
rm .cortex-agent/pids/*.pid
.claude/helpers/daemon-manager.sh start
```
...
sqlite3 .cortex-agent/metrics.db "SELECT * FROM hooks_metrics"
