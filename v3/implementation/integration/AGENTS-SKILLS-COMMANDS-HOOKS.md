...
```json
{
  ...
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "commands": [
          "npx cortex-agent hooks pre-tool --tool=$TOOL_NAME --command=\"$BASH_COMMAND\""
        ],
        "timeout": 5000,
        "failOnError": false
      },
      {
        "matcher": "Write|Edit",
        "commands": [
          "npx cortex-agent hooks pre-edit --file=$FILE_PATH"
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        "matcher": "*",
        "commands": [
          "npx cortex-agent hooks post-tool --tool=$TOOL_NAME --success=$SUCCESS"
        ]
      },
      {
        "matcher": "Write|Edit",
        "commands": [
          "npx cortex-agent hooks post-edit --file=$FILE_PATH --memory-key=\"edits/$FILE_PATH\""
        ]
      }
    ],
    ...
    "PreCompact": [
      {
        "commands": [
          "npx cortex-agent hooks pre-compact --session=$SESSION_ID"
        ]
      }
    ],
