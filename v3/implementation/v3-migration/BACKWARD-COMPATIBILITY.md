...
```typescript
// v3/@cortex-agent/shared/src/compat/v2-aliases.ts
...
export { UnifiedSwarmCoordinator as HiveMind } from '@cortex-agent/swarm';
export { UnifiedSwarmCoordinator as SwarmCoordinator } from '@cortex-agent/swarm';
export { UnifiedMemoryService as MemoryManager } from '@cortex-agent/memory';
```
...
```typescript
// v3/@cortex-agent/cli/src/compat/v2-commands.ts
...
```typescript
// v3/@cortex-agent/cli/src/commands/migrate.ts
export async function migrateConfig(v2ConfigPath: string): Promise<void> {
  ...
  const v3Config = {
    ...
    memory: {
      backend: 'hybrid',
      sqlite: {
        path: v2Config.memory?.path || './.cortex-agent/memory.db'
      },
      agentdb: {
        enableHNSW: true,
        dimensions: 384
  ...
  await saveYaml('.cortex-agent/config.yaml', v3Config);
}
...
```typescript
// v3/@cortex-agent/memory/src/migration.ts
