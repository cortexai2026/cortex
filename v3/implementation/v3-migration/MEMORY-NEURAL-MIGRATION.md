...
import { MemoryManager } from 'cortex-agent/memory';
const memory = new MemoryManager({
  backend: 'sqlite',
  path: './.cortex-agent/memory.db'
});
...
import { migrateMemoryData } from '@cortex-agent/memory/migration';
...
await migrateMemoryData({
  ...
  target: {
    type: 'v3-hybrid',
    sqlitePath: './.cortex-agent/v3-memory.db',
    agentdbPath: './.cortex-agent/v3-vectors'
  },
  options: {
...
import { NeuralDomainMapper } from 'cortex-agent/neural';
import { registerNeuralHooks } from 'cortex-agent/neural/integration';

const mapper = new NeuralDomainMapper();
registerNeuralHooks(mapper);

// V3: Neural learning system
import { NeuralLearningSystem } from '@cortex-agent/neural';
...
import { SONAManager, LearningMode } from '@cortex-agent/neural';
...
import { createPPO, createDQN, createA2C } from '@cortex-agent/neural/algorithms';
...
### Memory Migration
- [ ] Update imports to `@cortex-agent/memory`
...
### Neural Migration
- [ ] Update imports to `@cortex-agent/neural`
