...
Complete guide for upgrading from Cortex Agent v2 to v3.0.0-alpha.1
...
### What's Changed
Cortex Agent v3 is a complete architectural overhaul based on 10 Architecture Decision Records (ADRs). The migration involves:

- **Code reduction**: 15,000+ lines → <5,000 lines
- **Module architecture**: Monolith → 10 @cortex-agent modules
...
cp -r ~/.cortex-agent ~/.cortex-agent.v2.backup
...
cat ~/.cortex-agent/config.json > v2-config-backup.json

# List installed agents
npx agentic-flow --list > v2-agents-list.txt

# Export environment variables
env | grep CORTEX_AGENT > v2-env-backup.txt
```
...
+ # v3: Unified AgentDB
+ npx @cortex-agent/memory unify --backend agentdb
```
...
+ import { SwarmCoordinator } from '@cortex-agent/swarm'
```
...
+ # v3: Module imports
+ import { SecurityModule } from '@cortex-agent/security';
...
# 3. Install required @cortex-agent modules
npm install @cortex-agent/security@latest
npm install @cortex-agent/memory@latest
npm install @cortex-agent/integration@latest
npm install @cortex-agent/performance@latest
npm install @cortex-agent/swarm@latest
npm install @cortex-agent/cli@latest
...
# Edit ~/.cortex-agent/config.json with your v2 settings
# Follow new schema from v3/config/schema.json

# 3. Set environment variables
export CORTEX_AGENT_VERSION=3
export CORTEX_AGENT_MODE=production
export CORTEX_AGENT_MEMORY_BACKEND=agentdb
```

#### Windows Configuration
```powershell
# PowerShell
setx CORTEX_AGENT_VERSION "3"
setx CORTEX_AGENT_MODE "production"
setx CORTEX_AGENT_MEMORY_BACKEND "agentdb"

# Update config path
$env:CORTEX_AGENT_CONFIG = "$env:APPDATA\cortex-agent\config.json"
```
...
# Update config path
export CORTEX_AGENT_CONFIG="$HOME/.cortex-agent/config.json"
...
npx @cortex-agent/memory init --backend agentdb

# 3. Import v2 memory into v3
npx @cortex-agent/memory import ./v2-memory.json --format v2
...
npx @cortex-agent/memory optimize --hnsw
```
...
npx @cortex-agent/security audit --strict

# 2. Fix any CVEs automatically
npx @cortex-agent/security fix --auto

# 3. Validate credentials
npx @cortex-agent/security validate-credentials

# 4. Check path security
npx @cortex-agent/security check-paths

# 5. Review security report
cat ~/.cortex-agent/security-report.json
```
...
npx @cortex-agent/performance benchmark

# 2. Compare with v2 baseline
npx @cortex-agent/performance compare --baseline v2
...
npx @cortex-agent/performance profile --memory

# 5. Analyze bottlenecks
npx @cortex-agent/performance analyze
```
...
npx @cortex-agent/integration test --agentic-flow-version alpha
...
npx @cortex-agent/swarm test --agents 15
```
...
### @cortex-agent/security Migration
...
### @cortex-agent/performance Migration
...
#### After (v3)
```typescript
import { PerformanceModule } from '@cortex-agent/performance';
...
```bash
CORTEX_AGENT_VERSION=2
CORTEX_AGENT_MEMORY_PATH=./memory
CORTEX_AGENT_COORDINATOR=hierarchical
```

#### v3 Environment Variables
```bash
# Core
CORTEX_AGENT_VERSION=3
CORTEX_AGENT_MODE=production
CORTEX_AGENT_CONFIG=~/.cortex-agent/config.json

# Memory
CORTEX_AGENT_MEMORY_BACKEND=agentdb
CORTEX_AGENT_MEMORY_PATH=./data
CORTEX_AGENT_AGENTDB_HNSW=true

# Security
CORTEX_AGENT_SECURITY_STRICT=true
CORTEX_AGENT_SECURITY_MODE=strict

# Performance
CORTEX_AGENT_FLASH_ATTENTION=true
CORTEX_AGENT_SONA_LEARNING=true

# Platform-specific (Windows)
APPDATA=C:\Users\YourName\AppData\Roaming
CORTEX_AGENT_CONFIG=%APPDATA%\cortex-agent\config.json

# Platform-specific (macOS/Linux)
HOME=/home/yourname
CORTEX_AGENT_CONFIG=$HOME/.cortex-agent/config.json
```
...
// ✅ v3 Pattern
import { SwarmCoordinator } from '@cortex-agent/swarm';
import { AttentionCoordinator } from '@cortex-agent/swarm/attention';
...
import { SecurityError, MemoryError } from '@cortex-agent/shared';
...
cp v2-config-backup.json ~/.cortex-agent/config.json
...
npm uninstall agentic-flow
npm uninstall @cortex-agent/security
npm uninstall @cortex-agent/memory
npm uninstall @cortex-agent/swarm
npm uninstall @cortex-agent/integration
npm uninstall @cortex-agent/performance
npm uninstall agentdb
npm uninstall @ruvector/attention
npm uninstall @ruvector/sona

# 2. Restore entire v2 environment
cp -r ~/.cortex-agent.v2.backup ~/.cortex-agent
rm -rf node_modules
cp package.json.v2.backup package.json
cp package-lock.json.v2.backup package-lock.json
...
**Symptoms**:
```
Error: Cortex Agent v3 requires Node.js 20.x or higher
...
npx @cortex-agent/memory init --backend agentdb --force

# 2. Create data directory
mkdir -p ./data/agentdb

# 3. Import v2 memory with verbose logging
npx @cortex-agent/memory import ./v2-memory-backup.json \
  --format v2 \
  --verbose \
  --continue-on-error
...
cat ~/.cortex-agent/config.json | grep -A 10 security

# 2. Update allowedDirectories
npx @cortex-agent/security configure \
  --allowed-dirs "./src/,./tests/,./data/" \
  --blocked-patterns "../,~/,/etc/,/tmp/"

# 3. Validate paths
npx @cortex-agent/security check-paths --fix

# 4. Re-run with strict mode disabled (temporary)
export CORTEX_AGENT_SECURITY_STRICT=false
```
...
**Symptoms**:
```
Error: Cannot find module '@cortex-agent/security'
Module not found
```
...
# 4. Verify modules
npm list @cortex-agent/security
npm list @cortex-agent/memory
```
...
spctl --add --label "Cortex Agent" /path/to/npx
```
...
- [ ] All 10 @cortex-agent modules installed
...
- **ADR Reference**: /workspaces/cortex-agent/v3/docs/adr/
- **Examples**: /workspaces/cortex-agent/v3/examples/
...
1. **Collect diagnostic information**:
   ...
   npx @cortex-agent/security audit --report security-report.json
   ```
...
3. **Try safe mode**:
   ```bash
   export CORTEX_AGENT_SAFE_MODE=true
...
**Happy Migrating! Welcome to Cortex Agent v3!** 🚀
