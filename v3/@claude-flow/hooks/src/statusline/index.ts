 ...
 * ▊ Cortex Agent V3 ● ruvnet  │  ⎇ v3  │  Opus 4.5
 * ─────────────────────────────────────────────────────
...
export class StatuslineGenerator {
  ...
  private getV3Progress(): StatuslineData['v3Progress'] {
    ...
    const metricsPath = join(this.projectRoot, '.cortex-agent', 'metrics', 'v3-progress.json');
    try {
  ...
  private getSecurityStatus(): StatuslineData['security'] {
    ...
    const auditPath = join(this.projectRoot, '.cortex-agent', 'security', 'audit-status.json');
    try {
  ...
  private getSwarmActivity(): StatuslineData['swarm'] {
    ...
    const activityPath = join(this.projectRoot, '.cortex-agent', 'metrics', 'swarm-activity.json');
    try {
  ...
  private getSystemMetrics(): ExtendedStatuslineData['system'] {
    ...
    const patternsPath = join(this.projectRoot, '.cortex-agent', 'learning', 'patterns.db');
    try {
