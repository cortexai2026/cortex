 ...
 * - ONNX embeddings via @cortex-agent/embeddings (MiniLM-L6 384-dim)
 * - AgentDB backend for persistence
 * - Pattern promotion from short-term to long-term memory
 *
 * @module @cortex-agent/hooks/reasoningbank
 */
...
export class ReasoningBank extends EventEmitter {
  ...
  private async loadDependencies(): Promise<void> {
    ...
    const memoryModule = await dynamicImport('@cortex-agent/memory');
    if (memoryModule) {
      AgentDBAdapter = memoryModule.AgentDBAdapter;
      HNSWIndex = memoryModule.HNSWIndex;
    }

    const embeddingsModule = await dynamicImport('@cortex-agent/embeddings');
    if (embeddingsModule) {
...
/**
 * Real embedding service using @cortex-agent/embeddings
 */
