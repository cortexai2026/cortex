...
hooks:
  pre: |
    ...
    mcp__cortex-agent__memory_search --pattern="ddd:*" --namespace="architecture" --limit=10
    # Load domain context
    mcp__cortex-agent__memory_usage --action="retrieve" --namespace="architecture" --key="domain:model"
  post: |
    echo "✅ Domain model analysis complete"
    # Store domain patterns
    mcp__cortex-agent__memory_usage --action="store" --namespace="architecture" --key="ddd:analysis:$(date +%s)" --value="$DOMAIN_SUMMARY"
---
...
## Cortex Agent V3 Bounded Contexts
...
npx cortex-agent@v3alpha ddd analyze --path ./src

# Generate bounded context map
npx cortex-agent@v3alpha ddd context-map

# Validate aggregate design
npx cortex-agent@v3alpha ddd validate-aggregates

# Check ubiquitous language consistency
npx cortex-agent@v3alpha ddd language-check
```
...
mcp__cortex-agent__memory_search --pattern="ddd:aggregate:*" --namespace="architecture"
```
