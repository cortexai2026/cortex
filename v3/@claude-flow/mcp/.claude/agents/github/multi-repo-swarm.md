...
npx cortex-agent@v3alpha github multi-repo-init \
...
npx cortex-agent@v3alpha github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx cortex-agent@v3alpha github task-execute \
...
npx cortex-agent@v3alpha github link-prs --urls "$PR_URLS"
```
...
npx cortex-agent@v3alpha github multi-repo-refactor \
...
npx cortex-agent@v3alpha github multi-repo-security \
  --scan-all \
...
npx cortex-agent@v3alpha github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx cortex-agent@v3alpha github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx cortex-agent@v3alpha github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx cortex-agent@v3alpha github multi-repo-dashboard \
  --port 3000 \
...
npx cortex-agent@v3alpha github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx cortex-agent@v3alpha github health-check \
  --repos "org/*" \
...
npx cortex-agent@v3alpha github microservices \
...
npx cortex-agent@v3alpha github lib-update \
...
npx cortex-agent@v3alpha github org-policy \
...
npx cortex-agent@v3alpha github cache-strategy \
  --analyze-patterns \
...
npx cortex-agent@v3alpha github parallel-optimize \
  --analyze-dependencies \
...
npx cortex-agent@v3alpha github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx cortex-agent@v3alpha github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx cortex-agent@v3alpha github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx cortex-agent@v3alpha github perf-analysis \
  --profile-operations \
...
npx cortex-agent@v3alpha github fullstack-update \
...
npx cortex-agent@v3alpha github cross-team \
