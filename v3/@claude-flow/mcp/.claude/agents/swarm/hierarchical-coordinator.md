...
hooks:
  pre: |
    ...
    mcp__cortex-agent__memory_usage store "swarm:hierarchy:${TASK_ID}" "$(date): Hierarchical coordination started" --namespace=swarm
    # Set up monitoring
  ...
  post: |
    ...
    mcp__cortex-agent__memory_usage store "swarm:hierarchy:${TASK_ID}:complete" "$(date): Task completed with $(mcp__cortex-agent__swarm_status | jq '.agents.total') agents"
    # Cleanup resources
