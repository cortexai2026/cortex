...
```bash
# Check status
cortex-agent pair --status

# View history
cortex-agent pair --history
...
# End session
cortex-agent pair --end [--save] [--report]
```
...
```bash
cortex-agent pair --start --mode driver
```
...
```bash
cortex-agent pair --start --mode navigator
```
...
cortex-agent pair --start --mode switch

# 5-minute intervals (rapid)
cortex-agent pair --start --mode switch --interval 5m

# 15-minute intervals (deep focus)
cortex-agent pair --start --mode switch --interval 15m
```
...
#### Basic Configuration
Create `.cortex-agent/pair-config.json`:
...
cortex-agent pair --template refactor
...
# Feature template
cortex-agent pair --template feature
...
# Debug template
cortex-agent pair --template debug
...
# Learning template
cortex-agent pair --template learn
# Mode: Mentor
# Pace: Slow
# Explanations: Detailed
# Examples: Many
...
```bash
# Save session
cortex-agent pair --save [--name <name>]

# Load session
cortex-agent pair --load <session-id>

# Export session
cortex-agent pair --export <session-id> [--format json|md]

# Generate report
cortex-agent pair --report <session-id>
```
...
```bash
# Start in background
cortex-agent pair --start --background

# Monitor background session
cortex-agent pair --monitor

# Attach to background session
cortex-agent pair --attach <session-id>

# End background session
cortex-agent pair --end <session-id>
```
...
cortex-agent pair --start --record

# Replay session
cortex-agent pair --replay <session-id>

# Session analytics
cortex-agent pair --analytics <session-id>
```
...
**With Git:**
```bash
cortex-agent pair --start --git --auto-commit
```

**With CI/CD:**
```bash
cortex-agent pair --start --ci --non-interactive
```

**With IDE:**
```bash
cortex-agent pair --start --ide vscode
```
...
- Run `cortex-agent pair config validate`
...
- `cortex-agent pair --help` - Show help
- `cortex-agent pair config` - Manage configuration
- `cortex-agent pair profile` - Manage profiles
- `cortex-agent pair templates` - List templates
- `cortex-agent pair agents` - List available agents
