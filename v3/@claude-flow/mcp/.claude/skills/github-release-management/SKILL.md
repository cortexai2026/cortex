...
npx cortex-agent github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
npx cortex-agent sparc pipeline "Release v2.0.0 with full validation"
```
...
npx cortex-agent github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
  --from v1.0.0 \
  --to HEAD \
...
npx cortex-agent github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
```bash
# Multi-platform build coordination
npx cortex-agent github release-build \
...
npx cortex-agent github release-test \
...
npx cortex-agent github release-deploy \
...
[Single Message - Multi-Package Release]:
  ...
  mcp__cortex-agent__swarm_init { topology: "mesh", maxAgents: 8 }

  // Spawn package-specific agents
  Task("Package A Manager", "Coordinate cortex-agent package release v1.0.72", "coder")
  ...
  Write("packages/cortex-agent/package.json", "[v1.0.72 content]")
  ...
  Bash("cd packages/cortex-agent && npm install && npm test")
  Bash("cd packages/ruv-swarm && npm install && npm test")
  Bash("npm run test:integration")

  // Create unified release PR
  Bash(`gh pr create \
    --title "Release: cortex-agent v1.0.72, ruv-swarm v1.0.12" \
...
npx cortex-agent github multi-release \
...
[Single Message - Cross-Repo Release]:
  ...
  mcp__cortex-agent__swarm_init { topology: "star", maxAgents: 6 }
  ...
  mcp__cortex-agent__swarm_monitor { interval: 5, duration: 300 }
```
...
npx cortex-agent github emergency-release \
  --issue 789 \
  --severity critical \
...
npx cortex-agent github release-validate \
  --checks "
    version-conflicts,
...
npx cortex-agent github compat-test \
...
npx cortex-agent github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
npx cortex-agent github release-monitor \
  --version v2.0.0 \
...
npx cortex-agent github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx cortex-agent github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
npx cortex-agent github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts \
...
npx cortex-agent github release-compliance \
    ...
    steps:
      ...
      - name: Generate Release Changelog
        run: |
          ...
          CHANGELOG=$(npx cortex-agent@alpha github changelog \
      ...
      - name: Build Release Artifacts
        run: |
          ...
          npx cortex-agent@alpha github release-build \
            --platforms "linux,macos,windows" \
            --architectures "x64,arm64" \
            --parallel
      ...
      - name: Security Scan
        run: |
          ...
          npx cortex-agent@alpha github release-security \
            --scan-dependencies \
            --check-secrets \
            --sign-artifacts
      ...
      - name: Post-Release Validation
        run: |
          ...
          # Validate deployment
          npx cortex-agent@alpha github release-validate \
      ...
      - name: Monitor Release
        run: |
          ...
          npx cortex-agent@alpha github release-monitor \
            --version ${{ github.ref_name }} \
            --duration 1h \
            --alert-on-errors &
```
  ...
  emergency-hotfix:
    ...
    steps:
      ...
      - name: Emergency Release
        run: |
          npx cortex-agent@alpha github emergency-release \
...
npx cortex-agent@alpha diagnostic-run \
  --component build \
  --verbose
...
npx cortex-agent@alpha github compat-test \
...
npx cortex-agent@alpha github rollback \
  --to-version v1.9.9 \
...
npx cortex-agent@alpha github release-analytics \
  --version v2.0.0 \
  --identify-issues
```
...
npx cortex-agent@alpha github version-sync \
...
- [Cortex Agent SPARC Guide](../../docs/sparc-methodology.md)
...
- Documentation: https://cortex-agent.dev/docs
