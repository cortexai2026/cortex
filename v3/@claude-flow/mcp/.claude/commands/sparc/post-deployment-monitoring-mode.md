...
npx cortex-agent sparc run post-deployment-monitoring-mode "monitor production metrics"

# For alpha features
npx cortex-agent@alpha sparc run post-deployment-monitoring-mode "monitor production metrics"

# With namespace
npx cortex-agent sparc run post-deployment-monitoring-mode "your task" --namespace post-deployment-monitoring-mode

# Non-interactive mode
npx cortex-agent sparc run post-deployment-monitoring-mode "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If cortex-agent is installed locally
./cortex-agent sparc run post-deployment-monitoring-mode "monitor production metrics"
```
...
```bash
# Store mode-specific context
npx cortex-agent memory store "post-deployment-monitoring-mode_context" "important decisions" --namespace post-deployment-monitoring-mode

# Query previous work
npx cortex-agent memory query "post-deployment-monitoring-mode" --limit 5
```
