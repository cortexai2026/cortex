...
npx cortex-agent sparc run integration "connect payment service"

# For alpha features
npx cortex-agent@alpha sparc run integration "connect payment service"

# With namespace
npx cortex-agent sparc run integration "your task" --namespace integration

# Non-interactive mode
npx cortex-agent sparc run integration "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If cortex-agent is installed locally
./cortex-agent sparc run integration "connect payment service"
```
...
```bash
# Store mode-specific context
npx cortex-agent memory store "integration_context" "important decisions" --namespace integration

# Query previous work
npx cortex-agent memory query "integration" --limit 5
```
