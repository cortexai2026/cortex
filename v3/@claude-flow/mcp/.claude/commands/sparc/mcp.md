...
npx cortex-agent sparc run mcp "integrate with external API"

# For alpha features
npx cortex-agent@alpha sparc run mcp "integrate with external API"

# With namespace
npx cortex-agent sparc run mcp "your task" --namespace mcp

# Non-interactive mode
npx cortex-agent sparc run mcp "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If cortex-agent is installed locally
./cortex-agent sparc run mcp "integrate with external API"
```
...
```bash
# Store mode-specific context
npx cortex-agent memory store "mcp_context" "important decisions" --namespace mcp

# Query previous work
npx cortex-agent memory query "mcp" --limit 5
```
