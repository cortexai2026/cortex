...
npx cortex-agent sparc run debug "fix memory leak in service"

# For alpha features
npx cortex-agent@alpha sparc run debug "fix memory leak in service"

# With namespace
npx cortex-agent sparc run debug "your task" --namespace debug

# Non-interactive mode
npx cortex-agent sparc run debug "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If cortex-agent is installed locally
./cortex-agent sparc run debug "fix memory leak in service"
```
...
```bash
# Store mode-specific context
npx cortex-agent memory store "debug_context" "important decisions" --namespace debug

# Query previous work
npx cortex-agent memory query "debug" --limit 5
```
