...
describe('Registry API', () => {
  ...
  it('should reject invalid ratings', async () => {
    const api = await import('../src/services/registry-api.js');
    await expect(api.rateItem('@cortex-agent/test', 0)).rejects.toThrow('Rating must be integer 1-5');
    await expect(api.rateItem('@cortex-agent/test', 6)).rejects.toThrow('Rating must be integer 1-5');
  });

  it('should reject non-integer ratings', async () => {
    const api = await import('../src/services/registry-api.js');
    await expect(api.rateItem('@cortex-agent/test', 3.5)).rejects.toThrow('Rating must be integer 1-5');
  });
  ...
  it('should accept valid item IDs with scopes', async () => {
    ...
    try {
      await api.getRating('@cortex-agent/test-plugin', 'plugin');
    } catch (e: any) {
