# 🌊 Cortex Agent v3.5: Enterprise AI Orchestration Platform
...
> **Why Cortex Agent?** Cortex Agent is now Cortex Agent — named by Ruv, who loves Rust, flow states, and building things that feel inevitable. The "Ru" is the Ruv. The "flo" is the flow. Underneath, WASM kernels written in Rust power the policy engine, embeddings, and proof system. 5,900+ commits later, the alpha is over. This is v3.5.

## Getting into the Flow

Cortex Agent is a comprehensive AI agent orchestration framework that transforms Claude Code into a powerful multi-agent development platform. It enables teams to deploy, coordinate, and optimize specialized AI agents working together on complex software engineering tasks.

### Self-Learning/Self-Optimizing Agent Architecture

```
User → Cortex Agent (CLI/MCP) → Router → Swarm → Agents → Memory → LLM Providers
                       ↑                          ↓
                       └──── Learning Loop ←──────┘
```
...
**RuVector Components** (included with Cortex Agent):
...
```bash
# Use RuVector via Cortex Agent
npx cortex-agent@latest hooks intelligence --status
```
...
⚡ **Plugs Into Claude Code** - Native integration via MCP (Model Context Protocol). Use cortex-agent commands directly in your Claude Code sessions with full tool access.
...
Complex swarms can drift from their original goals. Cortex Agent V3 includes anti-drift defaults that prevent agents from going off-task.
...
### Claude Code: With vs Without Cortex Agent

| Capability | Claude Code Alone | Claude Code + Cortex Agent |
|------------|-------------------|---------------------------|
...
npm install -g cortex-agent@latest --omit=optional
```

<details>
<summary>🤖 <strong>OpenAI Codex CLI Support</strong> — Full Codex integration with self-learning</summary>

Cortex Agent supports both **Claude Code** and **OpenAI Codex CLI** via the [@cortex-agent/codex](https://www.npmjs.com/package/@cortex-agent/codex) package, following the [Agentics Foundation](https://agentics.org) standard.

### Quick Start for Codex

```bash
# Initialize for Codex CLI (creates AGENTS.md instead of CLAUDE.md)
npx cortex-agent@latest init --codex

# Full Codex setup with all 137+ skills
npx cortex-agent@latest init --codex --full

# Initialize for both platforms (dual mode)
npx cortex-agent@latest init --dual
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  CORTEX-AGENT = ORCHESTRATOR (tracks state, stores memory)       │
...
npx @cortex-agent/codex dual run --template feature --task "Add user auth"

# Run security audit swarm (scanner → analyzer → fixer)
npx @cortex-agent/codex dual run --template security --task "src/auth/"

# Run refactoring swarm (analyzer → planner → refactorer → validator)
npx @cortex-agent/codex dual run --template refactor --task "src/legacy/"
```
...
codex mcp add cortex-agent -- npx cortex-agent mcp start
```
...
npx cortex-agent@latest mcp start

# Spawn a coding agent
npx cortex-agent@latest agent spawn -t coder --name my-coder

# Launch a hive-mind swarm with an objective
npx cortex-agent@latest hive-mind spawn "Implement user authentication"
...
npx cortex-agent@latest init upgrade

# Update AND add any missing skills/agents/commands
npx cortex-agent@latest init upgrade --add-missing
```
...
Add cortex-agent as an MCP server for seamless integration:

```bash
# Add cortex-agent MCP server to Claude Code
...
Once added, Claude Code can use all 259 cortex-agent MCP tools directly:
...
<details>
<summary>🆚 <strong>Why Cortex Agent v3?</strong></summary>

Cortex Agent v3 introduces **self-learning neural capabilities** that no other agent orchestration framework offers. While competitors require manual agent configuration and static routing, Cortex Agent learns from every task execution, prevents catastrophic forgetting of successful patterns, and intelligently routes work to specialized experts.

#### 🧠 Neural & Learning

| Feature | Cortex Agent v3 | CrewAI | LangGraph | AutoGen | Manus |
|---------|----------------|--------|-----------|---------|-------|
...
What makes Cortex Agent different from other agent frameworks? These 10 capabilities work together to create a system that learns from experience, runs efficiently on any hardware, and keeps working even when things go wrong.
...
Not every task needs the most powerful (and expensive) model. Cortex Agent analyzes each request and automatically routes it to the cheapest handler that can do the job well. Simple code transforms skip the LLM entirely using WebAssembly. Medium tasks use faster, cheaper models. Only complex architecture decisions use Opus.
...
Cortex Agent V3 integrates AgentDB v3 (3.0.0-alpha.10) providing 20+ memory controllers accessible via MCP tools and the CLI.
...
Connect Cortex Agent to your development environment.

<details>
<summary>🔌 <strong>MCP Setup</strong> — Connect Cortex Agent to Any AI Environment</summary>

Cortex Agent runs as an MCP (Model Context Protocol) server, allowing you to connect it to any MCP-compatible AI client. This means you can use Cortex Agent's 60+ agents, swarm coordination, and self-learning capabilities from Claude Desktop, VS Code, Cursor, Windsurf, ChatGPT, and more.

### Quick Add Command

```bash
# Start Cortex Agent MCP server in any environment
npx cortex-agent@latest mcp start
```
...
```json
{
  "mcpServers": {
    "cortex-agent": {
      "command": "npx",
      "args": ["cortex-agent@latest", "mcp", "start"],
      "env": {
...
For ChatGPT, you need a remote MCP server (not local stdio). Deploy cortex-agent to a server with HTTP transport:

```bash
# Start with HTTP transport
npx cortex-agent@latest mcp start --transport http --port 3000
```
...
2. Configure your cortex-agent MCP server
...
| `CORTEX_AGENT_LOG_LEVEL` | Logging level (debug, info, warn, error) | Optional |
| `CORTEX_AGENT_TOOL_GROUPS` | MCP tool groups to enable (comma-separated) | Optional |
| `CORTEX_AGENT_TOOL_MODE` | Preset tool mode (develop, pr-review, devops, etc.) | Optional |
...
export CORTEX_AGENT_TOOL_GROUPS=implement,test,fix,memory

# Or use a preset mode
export CORTEX_AGENT_TOOL_MODE=develop
```
...
<details>
<summary>🛡️ <strong>@cortex-agent/guidance</strong> — Long-horizon governance control plane for Claude Code agents</summary>

### Overview

`@cortex-agent/guidance` turns `CLAUDE.md` into a runtime governance system with enforcement gates, cryptographic proofs, and feedback loops. Agents that normally drift after 30 minutes can now operate for days — rules are enforced mechanically at every step, not remembered by the model.
...
| Import Path | Purpose |
|-------------|---------|
| `@cortex-agent/guidance` | Main entry — GuidanceControlPlane |
| `@cortex-agent/guidance/compiler` | CLAUDE.md → PolicyBundle compiler |
| `@cortex-agent/guidance/retriever` | Intent classification + shard retrieval |
| `@cortex-agent/guidance/gates` | 4 enforcement gates |
| `@cortex-agent/guidance/ledger` | Run event logging + evaluators |
| `@cortex-agent/guidance/proof` | HMAC-SHA256 proof chain |
| `@cortex-agent/guidance/adversarial` | Threat, collusion, memory quorum |
| `@cortex-agent/guidance/trust` | Trust accumulation + privilege tiers |
| `@cortex-agent/guidance/authority` | Human authority + irreversibility classification |
| `@cortex-agent/guidance/wasm-kernel` | WASM-accelerated security-critical paths |
| `@cortex-agent/guidance/analyzer` | CLAUDE.md quality analysis + A/B benchmarking |
| `@cortex-agent/guidance/conformance-kit` | Headless conformance test runner |
...
- [Architecture Overview](v3/@cortex-agent/guidance/docs/guides/architecture-overview.md)
- [Getting Started](v3/@cortex-agent/guidance/docs/guides/getting-started.md)
- [Enforcement Gates Tutorial](v3/@cortex-agent/guidance/docs/tutorials/enforcement-gates.md)
- [Proof Audit Trail](v3/@cortex-agent/guidance/docs/tutorials/proof-audit-trail.md)
- [Multi-Agent Security](v3/@cortex-agent/guidance/docs/guides/multi-agent-security.md)
- [API Quick Reference](v3/@cortex-agent/guidance/docs/reference/api-quick-reference.md)
- [Full README](v3/@cortex-agent/guidance/README.md)
...
**CLI Commands:**
```bash
npx cortex-agent hive-mind init                    # Initialize hive mind
npx cortex-agent hive-mind spawn "Build API"       # Spawn with objective
npx cortex-agent hive-mind spawn "..." --queen-type strategic --consensus byzantine
npx cortex-agent hive-mind status                  # Check status
npx cortex-agent hive-mind metrics                 # Performance metrics
npx cortex-agent hive-mind memory                  # Collective memory stats
npx cortex-agent hive-mind sessions                # List active sessions
```
...
**Enable Agent Teams:**
```bash
# Automatically enabled with cortex-agent init
npx cortex-agent@latest init
...
npx cortex-agent@latest hooks teammate-idle --auto-assign true

# Handle task completion
npx cortex-agent@latest hooks task-completed --task-id <id> --train-patterns
```
...
Install these optional plugins to extend Cortex Agent capabilities:

| Plugin | Version | Description | Install Command |
|--------|---------|-------------|-----------------|
| **@cortex-agent/plugin-agentic-qe** | 3.0.0-alpha.2 | Quality Engineering with 58 AI agents across 12 DDD contexts. TDD, coverage analysis, security scanning, chaos engineering, accessibility testing. | `npm install @cortex-agent/plugin-agentic-qe` |
| **@cortex-agent/plugin-prime-radiant** | 0.1.4 | Mathematical AI interpretability with 6 engines: sheaf cohomology, spectral analysis, causal inference, quantum topology, category theory, HoTT proofs. | `npm install @cortex-agent/plugin-prime-radiant` |
| **@cortex-agent/plugin-gastown-bridge** | 0.1.0 | Gas Town orchestrator integration with WASM-accelerated formula parsing (352x faster), Beads sync, convoy management, and graph analysis. 20 MCP tools. | `npx cortex-agent@latest plugins install -n @cortex-agent/plugin-gastown-bridge` |
| **@cortex-agent/teammate-plugin** | 1.0.0-alpha.1 | Native TeammateTool integration for Claude Code v2.1.19+. BMSSP WASM acceleration, rate limiting, circuit breaker, semantic routing. 21 MCP tools. | `npx cortex-agent@latest plugins install -n @cortex-agent/teammate-plugin` |

#### 🏥 Domain-Specific Plugins

| Plugin | Version | Description | Install Command |
|--------|---------|-------------|-----------------|
| **@cortex-agent/plugin-healthcare-clinical** | 0.1.0 | HIPAA-compliant clinical decision support with FHIR/HL7 integration. Symptom analysis, drug interactions, treatment recommendations. | `npm install @cortex-agent/plugin-healthcare-clinical` |
| **@cortex-agent/plugin-financial-risk** | 0.1.0 | PCI-DSS/SOX compliant financial risk analysis. Portfolio optimization, fraud detection, regulatory compliance, market simulation. | `npm install @cortex-agent/plugin-financial-risk` |
| **@cortex-agent/plugin-legal-contracts** | 0.1.0 | Attorney-client privilege protected contract analysis. Risk identification, clause extraction, compliance verification. | `npm install @cortex-agent/plugin-legal-contracts` |

#### 💻 Development Intelligence Plugins

| Plugin | Version | Description | Install Command |
|--------|---------|-------------|-----------------|
| **@cortex-agent/plugin-code-intelligence** | 0.1.0 | Advanced code analysis with GNN-based pattern recognition. Security vulnerability detection, refactoring suggestions, architecture analysis. | `npm install @cortex-agent/plugin-code-intelligence` |
| **@cortex-agent/plugin-test-intelligence** | 0.1.0 | AI-powered test generation and optimization. Coverage analysis, mutation testing, test prioritization, flaky test detection. | `npm install @cortex-agent/plugin-test-intelligence` |
| **@cortex-agent/plugin-perf-optimizer** | 0.1.0 | Performance profiling and optimization. Memory leak detection, CPU bottleneck analysis, I/O optimization, caching strategies. | `npm install @cortex-agent/plugin-perf-optimizer` |

#### 🧠 Advanced AI/Reasoning Plugins

| Plugin | Version | Description | Install Command |
|--------|---------|-------------|-----------------|
| **@cortex-agent/plugin-neural-coordination** | 0.1.0 | Multi-agent neural coordination with SONA learning. Agent specialization, knowledge transfer, collective decision making. | `npm install @cortex-agent/plugin-neural-coordination` |
| **@cortex-agent/plugin-cognitive-kernel** | 0.1.0 | Cognitive computing kernel for working memory, attention control, meta-cognition, and task scaffolding. Miller's Law (7±2) compliance. | `npm install @cortex-agent/plugin-cognitive-kernel` |
| **@cortex-agent/plugin-quantum-optimizer** | 0.1.0 | Quantum-inspired optimization (QAOA, VQE, quantum annealing). Combinatorial optimization, Grover search, tensor networks. | `npm install @cortex-agent/plugin-quantum-optimizer` |
| **@cortex-agent/plugin-hyperbolic-reasoning** | 0.1.0 | Hyperbolic geometry for hierarchical reasoning. Poincaré embeddings, tree-like structure analysis, taxonomic inference. | `npm install @cortex-agent/plugin-hyperbolic-reasoning` |
...
npm install @cortex-agent/plugin-agentic-qe

# Install AI Interpretability plugin
npm install @cortex-agent/plugin-prime-radiant
...
npm install @cortex-agent/plugin-healthcare-clinical
npm install @cortex-agent/plugin-financial-risk
npm install @cortex-agent/plugin-legal-contracts

# Install development intelligence plugins
npm install @cortex-agent/plugin-code-intelligence
npm install @cortex-agent/plugin-test-intelligence
npm install @cortex-agent/plugin-perf-optimizer

# Install advanced AI/reasoning plugins
npm install @cortex-agent/plugin-neural-coordination
npm install @cortex-agent/plugin-cognitive-kernel
npm install @cortex-agent/plugin-quantum-optimizer
npm install @cortex-agent/plugin-hyperbolic-reasoning

# List all installed plugins
npx cortex-agent plugins list --installed
```
...
```typescript
import { createRuVectorBridge } from '@cortex-agent/plugins';
...
```typescript
import { createHyperbolicSpace } from '@cortex-agent/plugins';
...
```typescript
import { createSelfLearningSystem } from '@cortex-agent/plugins';
...
```bash
npx cortex-agent@latest worker dispatch --trigger audit --context "./src"
npx cortex-agent@latest worker status
```
...
| **Auto-Install** | `cortex-agent embeddings init` or `createEmbeddingServiceAsync()` | Zero config |
...
cortex-agent embeddings init

# Use larger model for higher quality
cortex-agent embeddings init --model all-mpnet-base-v2

# Semantic search
cortex-agent embeddings search -q "authentication patterns"
```
...
cortex-agent ruvector init --database mydb --user admin

# Check connection and schema status
cortex-agent ruvector status --verbose

# Run pending migrations
cortex-agent ruvector migrate --up

# Performance benchmark
cortex-agent ruvector benchmark --iterations 1000

# Optimize indices and vacuum
cortex-agent ruvector optimize --analyze

# Backup vector data
cortex-agent ruvector backup --output ./backup.sql
```
...
**Quick Commands:**
```bash
npx cortex-agent hive-mind init                                    # Initialize
npx cortex-agent hive-mind spawn "Build API" --queen-type tactical # Spawn swarm
npx cortex-agent hive-mind spawn "Research AI" --consensus byzantine --claude
npx cortex-agent hive-mind status                                  # Check status
```

**Cortex Agent Skill:** `/hive-mind-advanced` — Full hive mind orchestration
...
**Output Format:**
```
▊ Cortex Agent V3 ● ruvnet  │  ⎇ main  │  Opus 4.6  | ●42% ctx  | $0.15
🏗️ DDD [●●●●○] 4/5  ⚡ HNSW 150x  🤖 ◉ [12/8]  👥 3  🟢 CVE 3/3  💾 512MB  🧠 15%  📦 AgentDB ●1.2K vectors
```

| Indicator | Description | Source |
|-----------|-------------|--------|
| `▊ Cortex Agent V3` | Project header | Always shown |
...
| `[●●●●○]` | DDD domain progress bar | `.cortex-agent/metrics/v3-progress.json` |
...
| `🟢 CVE 3/3` | Security CVE remediation | `.cortex-agent/security/audit-status.json` |
...
Run `npx cortex-agent@latest init` — this generates `.claude/settings.json` with the correct statusline config and creates the helper script at `.claude/helpers/statusline.cjs`.
...
```bash
npx cortex-agent@latest init --update --settings
```
...
- `.cortex-agent/metrics/v3-progress.json` — DDD domain progress
- `.cortex-agent/metrics/swarm-activity.json` — Active agent counts
- `.cortex-agent/security/audit-status.json` — CVE remediation status
...
npx cortex-agent@latest daemon status

# Manually trigger a worker
npx cortex-agent@latest daemon trigger map

# Enable/disable workers
npx cortex-agent@latest daemon enable map audit optimize

# Stop daemon
npx cortex-agent@latest daemon stop
```
...
Complete command-line interface for all Cortex Agent operations.
...
npx cortex-agent@latest swarm init --v3-mode

# Search memory (HNSW-indexed, 150x faster)
npx cortex-agent@latest memory search -q "authentication patterns"

# Run security scan
npx cortex-agent@latest security scan --depth full

# Performance benchmark
npx cortex-agent@latest performance benchmark --suite all
```
...
Run `npx cortex-agent@latest doctor` to diagnose and fix common issues.
...
npx cortex-agent@latest doctor

# Run diagnostics with auto-fix
npx cortex-agent@latest doctor --fix

# Check specific component
npx cortex-agent@latest doctor --component memory

# Verbose output
npx cortex-agent@latest doctor --verbose
```

**Output Example:**

```
🩺 Cortex Agent Doctor v3.5

✅ Node.js      20.11.0 (required: 20+)
✅ npm          10.2.4 (required: 9+)
✅ Git          2.43.0
✅ Config       Valid cortex-agent.config.json
...
npx cortex-agent@latest embeddings init

# Generate embedding for text
npx cortex-agent@latest embeddings embed "authentication patterns"

# Batch embed multiple texts
npx cortex-agent@latest embeddings batch --file texts.txt

# Search with semantic similarity
npx cortex-agent@latest embeddings search "login flow" --top-k 5
```

**Programmatic:**

```typescript
import { createEmbeddingServiceAsync } from '@cortex-agent/embeddings';
...
|----------|----------------|--------------|
| **Code Review** | Get thorough reviews with security, performance, and style checks | `npx cortex-agent@latest agent spawn -t reviewer --name pr-review` |
| **Test Generation** | Auto-generate unit, integration, and e2e tests for existing code | `npx cortex-agent@latest agent spawn -t tester --name test-gen` |
| **Refactoring** | Safely restructure code while maintaining behavior | `npx cortex-agent@latest hive-mind spawn "Refactor user service to repository pattern"` |
| **Bug Fixing** | Diagnose and fix bugs with full context analysis | `npx cortex-agent@latest hive-mind spawn "Fix race condition in checkout flow"` |

### 🔒 Security & Compliance

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Security Audit** | Find vulnerabilities before attackers do | `npx cortex-agent@latest security scan --depth full` |
| **Dependency Scan** | Identify vulnerable packages and suggest upgrades | `npx cortex-agent@latest security cve --check` |
| **Compliance Check** | Ensure code meets security standards | `npx cortex-agent@latest security audit` |

### 🐝 Multi-Agent Swarms

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Feature Development** | Coordinate multiple agents on complex features | `npx cortex-agent@latest swarm init --topology hierarchical && npx cortex-agent@latest task orchestrate "Build user dashboard"` |
| **Large Refactors** | Parallel refactoring across many files without conflicts | `npx cortex-agent@latest swarm init --topology mesh --max-agents 8` |
| **Codebase Migration** | Migrate frameworks, languages, or patterns systematically | `npx cortex-agent@latest task orchestrate "Migrate from Express to Fastify" --strategy adaptive` |

### 📊 Performance & Optimization

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Performance Profiling** | Find and fix bottlenecks in your application | `npx cortex-agent@latest performance profile --target src/` |
| **Query Optimization** | Speed up slow database queries | `npx cortex-agent@latest performance benchmark --suite all` |
| **Memory Analysis** | Reduce memory usage and fix leaks | `npx cortex-agent@latest performance metrics` |

### 🔄 GitHub & DevOps

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **PR Management** | Review, approve, and merge PRs efficiently | `npx cortex-agent@latest hive-mind spawn "Review open PRs"` |
| **Issue Triage** | Categorize, prioritize, and assign issues automatically | `npx cortex-agent@latest hive-mind spawn "Triage new issues"` |
| **Release Management** | Coordinate releases with changelogs and versioning | `npx cortex-agent@latest hive-mind spawn "Prepare v2.0 release"` |
| **CI/CD Optimization** | Speed up pipelines and reduce flaky tests | `npx cortex-agent@latest hive-mind spawn "Optimize GitHub Actions workflow"` |

### 📋 Spec-Driven Development

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Generate Specs** | Create complete specifications before coding | `npx cortex-agent@latest hive-mind spawn "Create ADR for authentication system"` |
| **Validate Implementation** | Ensure code matches specifications | `npx cortex-agent@latest hooks progress --detailed` |
| **Track Compliance** | Monitor spec adherence across the team | `npx cortex-agent@latest progress sync` |

### 🧠 Learning & Intelligence

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Bootstrap Intelligence** | Train the system on your codebase patterns | `npx cortex-agent@latest hooks pretrain --depth deep` |
| **Optimize Routing** | Improve task-to-agent matching over time | `npx cortex-agent@latest hooks route "<task>" --include-explanation` |
| **Transfer Learning** | Apply patterns learned from other projects | `npx cortex-agent@latest hooks transfer <sourceProject>` |
...
Cortex Agent eliminates Claude Code's context window ceiling with a real-time memory management system that archives, optimizes, and restores conversation context automatically.
...
Cortex Agent intercepts the compaction lifecycle with three hooks that make context loss invisible:
...
|------|---------|---------|----------|
| 1 | **SQLite** (default) | `.cortex-agent/data/transcript-archive.db` | WAL mode, indexed queries, ACID, importance ranking |
...
| 4 | **JSON** (fallback) | `.cortex-agent/data/transcript-archive.json` | Zero dependencies, always works |

### Configuration

```bash
# Context Autopilot (all have sensible defaults)
CORTEX_AGENT_CONTEXT_AUTOPILOT=true        # Enable/disable autopilot (default: true)
CORTEX_AGENT_CONTEXT_WINDOW=200000         # Context window size in tokens
CORTEX_AGENT_AUTOPILOT_WARN=0.70           # Warning threshold (70%)
CORTEX_AGENT_AUTOPILOT_PRUNE=0.85          # Optimization threshold (85%)
CORTEX_AGENT_COMPACT_RESTORE_BUDGET=4000   # Max chars restored after compaction
CORTEX_AGENT_RETENTION_DAYS=30             # Auto-prune never-accessed entries
CORTEX_AGENT_AUTO_OPTIMIZE=true            # Importance ranking + pruning + sync
```
...
sqlite3 .cortex-agent/data/transcript-archive.db \
...
Cortex Agent uses RVF — a compact binary storage format that replaces the 18MB sql.js WASM dependency with pure TypeScript. No native compilation, no WASM downloads, works everywhere Node.js runs.
...
```typescript
import { RvfBackend } from '@cortex-agent/memory';
...
```typescript
import { RvfMigrator } from '@cortex-agent/memory';
...
```typescript
import { PersistentSonaCoordinator } from '@cortex-agent/memory';
...
```bash
# Environment variables
CORTEX_AGENT_MEMORY_BACKEND=hybrid   # auto-selects RVF
CORTEX_AGENT_MEMORY_PATH=./data/memory

# Or via CLI
cortex-agent memory init --force
cortex-agent config set memory.backend hybrid
```
...
**Example Hook Output:**
```bash
$ npx cortex-agent@latest hooks pre-task --description "convert var to const in utils.ts"
...
npx cortex-agent@latest hooks pre-edit ./src/auth.ts
npx cortex-agent@latest hooks post-edit ./src/auth.ts --success true --train-patterns
```
...
npx cortex-agent@latest hooks route "refactor authentication to use JWT" --include-explanation

# Bootstrap intelligence from your codebase
npx cortex-agent@latest hooks pretrain --depth deep --model-type moe
```
...
npx cortex-agent@latest hooks session-start --session-id "feature-auth" --start-daemon

# End session and export learnings
npx cortex-agent@latest hooks session-end --export-metrics --persist-patterns
```
...
npx cortex-agent@latest hooks intelligence trajectory-start --task "implement OAuth2"

# Record successful action
npx cortex-agent@latest hooks intelligence trajectory-step --action "created token service" --quality 0.9

# End trajectory and trigger learning
npx cortex-agent@latest hooks intelligence trajectory-end --success true
...
```bash
# List all workers
npx cortex-agent@latest hooks worker list

# Manually dispatch security audit
npx cortex-agent@latest hooks worker dispatch --trigger audit --context "./src/auth"

# Check worker status
npx cortex-agent@latest hooks worker status
```
...
npx cortex-agent@latest hooks model-route --task "fix typo in README"
# → Recommends: haiku (simple task, low complexity)

npx cortex-agent@latest hooks model-route --task "design distributed consensus system"
...
npx cortex-agent@latest hooks route "<task>" --include-explanation

# Start/end session with learning
npx cortex-agent@latest hooks session-start --start-daemon
npx cortex-agent@latest hooks session-end --persist-patterns

# View what the system has learned
npx cortex-agent@latest hooks metrics
npx cortex-agent@latest hooks intelligence stats
...
npx cortex-agent@latest hooks pretrain --depth deep

# Dispatch background worker
npx cortex-agent@latest hooks worker dispatch --trigger audit
```
...
npx cortex-agent@latest memory export --format json --output ./patterns.json

# Export specific namespace
npx cortex-agent@latest memory export --namespace "security" --output ./security-patterns.json

# Export with embeddings (larger file, faster import)
npx cortex-agent@latest memory export --include-embeddings --output ./full-export.json

# Export agent configurations
npx cortex-agent@latest config export --scope project --output ./agent-configs.json

# Export session state
npx cortex-agent@latest session export --session-id "my-session" --output ./session.json
```
...
npx cortex-agent@latest memory import --input ./patterns.json

# Import and merge with existing (don't overwrite)
npx cortex-agent@latest memory import --input ./patterns.json --merge

# Import from another project
npx cortex-agent@latest hooks transfer --source-path ../other-project

# Import agent configurations
npx cortex-agent@latest config import --input ./agent-configs.json --scope project

# Restore session
npx cortex-agent@latest session restore --session-id "my-session"
```
...
npx cortex-agent@latest transfer-store search --query "authentication" --min-rating 4.0

# Download a pattern
npx cortex-agent@latest transfer-store download --id "auth-jwt-patterns-v2" --verify

# Publish your patterns
npx cortex-agent@latest transfer-store publish --input ./my-patterns.json --category "security"
```
...
npx cortex-agent@latest plugins list

# Filter by type
npx cortex-agent@latest plugins list --type integration

# Rate a plugin
npx cortex-agent@latest plugins rate --name @cortex-agent/embeddings --rating 5

# Search for MCP tool plugins
npx cortex-agent@latest transfer plugin-search --type "mcp-tool" --verified

# Get plugin info
npx cortex-agent@latest transfer plugin-info --name "semantic-code-search"

# List official plugins
npx cortex-agent@latest transfer plugin-official
```
...
npx cortex-agent@latest transfer ipfs-resolve --name "/ipns/patterns.cortex-agent.io"

# Detect PII before publishing
npx cortex-agent@latest transfer detect-pii --content "$(cat ./patterns.json)"
```
...
curl -X POST "https://api.pinata.cloud/pinning/pinJSONToIPFS" \
  ...
  -d '{
    ...
    "pinataMetadata": {"name": "cortex-agent-learning-pattern"}
  }'
...
# Import all models
npx cortex-agent@latest transfer import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Import specific category
npx cortex-agent@latest neural import --model security-review-patterns --source ipfs

# Use patterns in routing
npx cortex-agent@latest hooks route --task "review authentication code" --use-patterns
```
...
npx cortex-agent@latest transfer-store download --id "security-essentials" --apply
```
...
npx cortex-agent@latest neural list

# List models by category
npx cortex-agent@latest neural list --category security

# Train with WASM acceleration
npx cortex-agent@latest neural train -p coordination -e 100 --wasm --flash --contrastive

# Train security patterns
npx cortex-agent@latest neural train -p security --wasm --contrastive

# Benchmark WASM performance
npx cortex-agent@latest neural benchmark -d 256 -i 1000

# Import pre-trained models
npx cortex-agent@latest neural import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Export trained patterns to IPFS
npx cortex-agent@latest neural export --ipfs --sign
```
...
# Via CLI
npx cortex-agent@latest skill run github-code-review
npx cortex-agent@latest skill list
npx cortex-agent@latest skill info sparc-methodology
```
...
|---------|--------------|---------|
| `issues list` | See all issues and their status | `npx cortex-agent@latest issues list` |
| `issues claim` | Claim an issue for yourself/agent | `npx cortex-agent@latest issues claim #123 --as coder-1` |
| `issues release` | Release your claim | `npx cortex-agent@latest issues release #123` |
| `issues handoff` | Hand off to another worker | `npx cortex-agent@latest issues handoff #123 --to reviewer` |
| `issues status` | Update progress on claimed work | `npx cortex-agent@latest issues status #123 --progress 75` |
| `issues stealable` | List abandoned/stuck issues | `npx cortex-agent@latest issues stealable` |
| `issues steal` | Take over stealable issue | `npx cortex-agent@latest issues steal #123` |
| `issues load` | View agent workloads | `npx cortex-agent@latest issues load` |
| `issues rebalance` | Redistribute work evenly | `npx cortex-agent@latest issues rebalance --dry-run` |
| `issues board` | Visual board view | `npx cortex-agent@latest issues board` |

### Visual Board View

```bash
npx cortex-agent@latest issues board
```
...
npx cortex-agent@latest issues handoff #123 \
  --to security-architect \
  --reason "Needs security review" \
  --progress 80

# 2. Target accepts handoff
npx cortex-agent@latest issues accept #123 --as security-architect
...
```bash
# View current load
npx cortex-agent@latest issues load
...
# Auto-rebalance
npx cortex-agent@latest issues rebalance
```
...
| Command | What It Does | Example |
|---------|--------------|---------|
| `route task` | Get agent recommendation | `npx cortex-agent@latest route task "implement OAuth2"` |
| `route explain` | Understand routing decision | `npx cortex-agent@latest route explain "task"` |
| `route coverage` | Route based on test coverage | `npx cortex-agent@latest route coverage` |

### Example: Route a Task

```bash
npx cortex-agent@latest route task "refactor authentication to use JWT"
...
```bash
npx cortex-agent@latest route coverage
...
npx cortex-agent@latest hooks route "implement caching layer" --include-explanation

# Record outcome for learning
npx cortex-agent@latest hooks post-task --task-id "task-123" --success true --agent coder
```
...
Use Cortex Agent packages directly in your applications.

<details>
<summary>💻 <strong>Programmatic SDK</strong> — Use Cortex Agent in Your Code</summary>

Use Cortex Agent packages directly in your TypeScript/JavaScript applications.

### Installation

```bash
# Install specific packages
npm install @cortex-agent/cli @cortex-agent/memory @cortex-agent/swarm

# Or install everything
npm install cortex-agent@latest
```
...
```typescript
import { AgentDB } from '@cortex-agent/memory';
...
npx cortex-agent@latest memory init --force

# Store patterns
npx cortex-agent@latest memory store --key "pattern-auth" --value "JWT authentication with refresh tokens"
npx cortex-agent@latest memory store --key "pattern-cache" --value "Redis caching for API responses"

# Build HNSW index for 150x-12,500x faster search
npx cortex-agent@latest memory search --query "authentication" --build-hnsw

# Semantic search (uses HNSW if built)
npx cortex-agent@latest memory search --query "how to cache data" --limit 5

# List and manage entries
npx cortex-agent@latest memory list --namespace patterns
npx cortex-agent@latest memory stats
```
...
```typescript
import { isSafe, checkThreats, createAIDefence } from '@cortex-agent/aidefence';
...
```typescript
import { chunkText, estimateTokens } from '@cortex-agent/embeddings';
...
```typescript
import { l2Normalize, l1Normalize, minMaxNormalize, zScoreNormalize } from '@cortex-agent/embeddings';
...
```typescript
import { createNeuralService, isNeuralAvailable } from '@cortex-agent/embeddings';
...
```typescript
import { PersistentEmbeddingCache } from '@cortex-agent/embeddings';
...
# Neural operations
cortex-agent embeddings neural drift --baseline "context" --input "check"
...
```typescript
import { HooksService } from '@cortex-agent/hooks';
...
|---------|---------|--------------|
| `@cortex-agent/memory` | Vector storage, HNSW, self-learning graph | `AgentDB`, `AutoMemoryBridge`, `LearningBridge`, `MemoryGraph` |
| `@cortex-agent/swarm` | Agent coordination | `createSwarm`, `Swarm` |
| `@cortex-agent/aidefence` | Threat detection | `isSafe`, `checkThreats`, `createAIDefence` |
| `@cortex-agent/embeddings` | Vector embeddings | `createEmbeddingService` |
| `@cortex-agent/hooks` | Event hooks, learning | `HooksService`, `ReasoningBank` |
| `@cortex-agent/security` | Input validation | `InputValidator`, `PathValidator` |
| `@cortex-agent/neural` | SONA learning | `SONAAdapter`, `MoERouter` |
| `@cortex-agent/providers` | LLM providers | `ProviderRegistry`, `createProvider` |
| `@cortex-agent/plugins` | Plugin SDK | `PluginBuilder`, `createPlugin` |
...
Core infrastructure packages powering Cortex Agent's intelligence layer.
...
Cortex Agent v3 is built on top of **[agentic-flow](https://github.com/ruvnet/agentic-flow)**, a production-ready AI agent orchestration platform. This deep integration provides 352x faster code transformations, learning memory, and geometric intelligence.
...
Cortex Agent automatically leverages agentic-flow for:
...
```typescript
// Cortex Agent automatically uses agentic-flow optimizations
import { getTokenOptimizer } from '@cortex-agent/integration';
...
Cortex Agent includes a dedicated `/agentic-jujutsu` skill for AI-powered version control:
...
**RuVector** is a high-performance distributed vector database combining vector search, graph queries, and self-learning neural networks. Written in Rust with Node.js/WASM bindings, it powers Cortex Agent's intelligence layer with native speed.
...
npx cortex-agent ruvector setup --output ./my-ruvector
...
docker run -d \
  ...
  -e POSTGRES_PASSWORD=cortex-agent-test \
  -e POSTGRES_DB=cortex_agent \
...
Cortex Agent automatically uses RuVector when available:

```typescript
// Cortex Agent detects and uses native ruvector
import { getVectorStore } from '@cortex-agent/memory';
...
npx cortex-agent ruvector setup                    # Output to ./ruvector-postgres
npx cortex-agent ruvector setup --output ./mydir   # Custom directory
npx cortex-agent ruvector setup --print            # Preview files

# Import from sql.js/JSON to PostgreSQL
npx cortex-agent ruvector import --input data.json              # Direct import
npx cortex-agent ruvector import --input data.json --output sql # Dry-run (generate SQL)

# Other RuVector commands
npx cortex-agent ruvector status --verbose         # Check connection
npx cortex-agent ruvector benchmark --vectors 10000 # Performance test
npx cortex-agent ruvector optimize --analyze       # Optimization suggestions
npx cortex-agent ruvector backup --output backup.sql # Backup data
...
Flow Nexus is a **cloud platform** for deploying and scaling Cortex Agent beyond your local machine.

### What Flow Nexus Provides

| Feature | Local Cortex Agent | + Flow Nexus |
|---------|-------------------|--------------|
...
# Or via CLI
npx cortex-agent@latest nexus swarm deploy \
  --topology hierarchical \
  --max-agents 50 \
  --region us-east-1
```
...
```bash
# Create sandbox
npx cortex-agent@latest nexus sandbox create --language python

# Execute code safely
npx cortex-agent@latest nexus sandbox exec --code "print('Hello')"

# Cleanup
npx cortex-agent@latest nexus sandbox destroy
```
...
# 2. Get API key
# 3. Configure
npx cortex-agent@latest nexus configure --api-key <key>
...
# Define pipeline
npx cortex-agent@latest stream-chain create \
...
```bash
# Run the pipeline
npx cortex-agent@latest stream-chain run feature-pipeline \
  --input '{"requirements": "Add user dashboard with analytics"}'

# Monitor progress
npx cortex-agent@latest stream-chain status feature-pipeline
```
...
# Via CLI
npx cortex-agent@latest pair start --mode navigator
```
...
npx cortex-agent@latest pair switch

# Get AI explanation
npx cortex-agent@latest pair explain

# Run tests
npx cortex-agent@latest pair test

# End session with summary
npx cortex-agent@latest pair end
```
...
```bash
# Basic threat scan
npx cortex-agent@latest security defend -i "ignore previous instructions"

# Scan a file
npx cortex-agent@latest security defend -f ./user-prompts.txt

# Quick scan (faster)
npx cortex-agent@latest security defend -i "some text" --quick

# JSON output
npx cortex-agent@latest security defend -i "test" -o json

# View statistics
npx cortex-agent@latest security defend --stats
...
```json
{
  "hooks": {
    "pre-agent-input": {
      "command": "node -e \"const { isSafe } = require('@cortex-agent/aidefence'); if (!isSafe(process.env.AGENT_INPUT)) { process.exit(1); }\"",
      "timeout": 5000
    }
  }
}
...
|--------|---------|--------------|
| `@cortex-agent/hooks` | Event-driven lifecycle | ReasoningBank, 27 hooks, pattern learning |
| `@cortex-agent/memory` | Unified vector storage | AgentDB, RVF binary format, HnswLite, RvfMigrator, SONA persistence, LearningBridge, MemoryGraph |
| `@cortex-agent/security` | CVE remediation | Input validation, path security, AIDefence |
| `@cortex-agent/swarm` | Multi-agent coordination | 6 topologies, Byzantine consensus, auto-scaling |
| `@cortex-agent/plugins` | WASM extensions | RuVector plugins, semantic search, intent routing |
| `@cortex-agent/cli` | Command interface | 26 commands, 140+ subcommands, shell completions |
| `@cortex-agent/neural` | Self-learning | SONA, 9 RL algorithms, EWC++ memory preservation |
| `@cortex-agent/testing` | Quality assurance | London School TDD, Vitest, fixtures, mocks |
| `@cortex-agent/deployment` | Release automation | Versioning, changelogs, NPM publishing |
| `@cortex-agent/shared` | Common utilities | Types, validation schemas, RvfEventLog, constants |
| `@cortex-agent/browser` | Browser automation | 59 MCP tools, element refs, trajectory learning |
...
<details>
<summary><strong>🌐 Browser Automation — @cortex-agent/browser</strong></summary>

[![npm version](https://img.shields.io/npm/v/@cortex-agent/browser?color=blue&label=npm)](https://www.npmjs.com/package/@cortex-agent/browser)

AI-optimized browser automation integrating [agent-browser](https://github.com/AugmentCode/agent-browser) with cortex-agent for intelligent web automation, trajectory learning, and multi-agent browser coordination.
...
```typescript
import { listWorkflows, getWorkflow } from '@cortex-agent/browser';
...
📖 [Full Documentation](./v3/@cortex-agent/browser/README.md)
...
<details>
<summary>📦 <strong>Release Management</strong> — @cortex-agent/deployment</summary>

Automated release management, versioning, and CI/CD for Cortex Agent packages.
...
```typescript
import { prepareRelease, publishToNpm, validate } from '@cortex-agent/deployment';
...
# Dry run (no changes)
npx @cortex-agent/deployment release --version 2.0.0 --dry-run
```
...
<details>
<summary>📊 <strong>Performance Benchmarking</strong> — @cortex-agent/performance</summary>
...
# Performance report
npx cortex-agent@latest performance report

# Benchmark specific suite
npx cortex-agent@latest performance benchmark --suite memory
```
...
<details>
<summary>🧪 <strong>Testing Framework</strong> — @cortex-agent/testing</summary>
...
```typescript
import { createPerformanceTestHelper, TEST_CONFIG } from '@cortex-agent/testing';
...
```powershell
npx @cortex-agent/security@latest audit --platform windows
$env:CORTEX_AGENT_MODE = "integration"
```

### macOS (Bash/Zsh)

```bash
npx @cortex-agent/security@latest audit --platform darwin
export CORTEX_AGENT_SECURITY_MODE="strict"
```

### Linux (Bash)

```bash
npx @cortex-agent/security@latest audit --platform linux
export CORTEX_AGENT_MEMORY_PATH="./data"
```
...
|----------|-------------|---------|
| `CORTEX_AGENT_MODE` | Operation mode (`development`, `production`, `integration`) | `development` |
| `CORTEX_AGENT_ENV` | Environment name for test/dev isolation | - |
| `CORTEX_AGENT_DATA_DIR` | Root data directory | `./data` |
| `CORTEX_AGENT_MEMORY_PATH` | Directory for persistent memory storage | `./data` |
| `CORTEX_AGENT_MEMORY_TYPE` | Memory backend type (`json`, `sqlite`, `agentdb`, `hybrid`) | `hybrid` |
| `CORTEX_AGENT_SECURITY_MODE` | Security level (`strict`, `standard`, `permissive`) | `standard` |
| `CORTEX_AGENT_LOG_LEVEL` | Logging verbosity (`debug`, `info`, `warn`, `error`) | `info` |
| `CORTEX_AGENT_CONFIG` | Path to configuration file | `./cortex-agent.config.json` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `CORTEX_AGENT_MAX_AGENTS` | Default concurrent agent limit | `15` |
| `CORTEX_AGENT_TOPOLOGY` | Default swarm topology (`hierarchical`, `mesh`, `ring`, `star`) | `hierarchical` |
| `CORTEX_AGENT_HEADLESS` | Run in headless mode (no interactive prompts) | `false` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `CORTEX_AGENT_MCP_PORT` | MCP server port | `3000` |
| `CORTEX_AGENT_MCP_HOST` | MCP server host | `localhost` |
| `CORTEX_AGENT_MCP_TRANSPORT` | Transport type (`stdio`, `http`, `websocket`) | `stdio` |

### Vector Search (HNSW)

| Variable | Description | Default |
|----------|-------------|---------|
| `CORTEX_AGENT_HNSW_M` | HNSW index M parameter (connectivity, higher = more accurate) | `16` |
| `CORTEX_AGENT_HNSW_EF` | HNSW search ef parameter (accuracy, higher = slower) | `200` |
| `CORTEX_AGENT_EMBEDDING_DIM` | Vector embedding dimensions | `384` |
...
| `GCS_PREFIX` | Prefix for stored files | `cortex-agent-patterns` |

### Auto-Update System

| Variable | Description | Default |
|----------|-------------|---------|
| `CORTEX_AGENT_AUTO_UPDATE` | Enable/disable auto-updates | `true` |
| `CORTEX_AGENT_FORCE_UPDATE` | Force update check | `false` |
...
| `CORTEX_AGENT_TOKEN` | Internal authentication token | Optional |
...
# Memory
CORTEX_AGENT_MEMORY_TYPE=hybrid
CORTEX_AGENT_MEMORY_PATH=./data

# Vector Search
CORTEX_AGENT_HNSW_M=16
CORTEX_AGENT_HNSW_EF=200
...
Cortex Agent looks for configuration in this order:
1. `./cortex-agent.config.json` (project root)
2. `~/.config/cortex-agent/config.json` (user config)
...
```json
{
  ...
  "logging": {
    ...
    "format": "json",
    "destination": "console",
    "filePath": "./logs/cortex-agent.log",
    "maxFileSize": "100MB",
    "maxFiles": 10
  },
...
```json
{
  ...
  "memory": {
    "type": "hybrid",
    "path": "/var/lib/cortex-agent/data",
  ...
  "logging": {
    ...
    "format": "json",
    "destination": "file",
    "filePath": "/var/log/cortex-agent/production.log"
  },
...
npx cortex-agent@latest config list

# Get specific value
npx cortex-agent@latest config get --key memory.type

# Set configuration value
npx cortex-agent@latest config set --key swarm.maxAgents --value 10

# Export configuration
npx cortex-agent@latest config export > my-config.json

# Import configuration
npx cortex-agent@latest config import --file my-config.json

# Reset to defaults
npx cortex-agent@latest config reset --key swarm
...
export CORTEX_AGENT_MAX_AGENTS=5
```
...
npx cortex-agent@latest hooks pretrain
```

**Windows path issues**
```powershell
# Use forward slashes or escape backslashes
$env:CORTEX_AGENT_MEMORY_PATH = "./data"
# Or use absolute path
$env:CORTEX_AGENT_MEMORY_PATH = "C:/Users/name/cortex-agent/data"
```
...
node --expose-gc node_modules/.bin/cortex-agent
# Reduce HNSW parameters for lower memory
export CORTEX_AGENT_HNSW_M=8
export CORTEX_AGENT_HNSW_EF=100
```
...
│ Modular Architecture  │ 18 @cortex-agent/* packages          │
...
| Change | V2 | V3 | Impact |
|--------|----|----|--------|
| **Package Structure** | `cortex-agent` | `@cortex-agent/*` (scoped) | Update imports |
...
| **Config Format** | `.cortex-agent/config.json` | `cortex-agent.config.json` | Update path |
...
cp -r ./.cortex-agent ./.cortex-agent-backup-v2

# STEP 2: Check migration status
npx cortex-agent@latest migrate status

# STEP 3: Run migration with dry-run first
npx cortex-agent@latest migrate run --dry-run

# STEP 4: Execute migration
npx cortex-agent@latest migrate run --from v2

# STEP 5: Verify migration
npx cortex-agent@latest migrate verify
...
|------------|------------|-------|
| `cortex-agent start` | `cortex-agent mcp start` | MCP is explicit |
| `cortex-agent init` | `cortex-agent init --wizard` | Interactive mode |
| `cortex-agent spawn <type>` | `cortex-agent agent spawn -t <type>` | Nested under `agent` |
| `cortex-agent swarm create` | `cortex-agent swarm init --topology mesh` | Explicit topology |
...
**V2 Config (`.cortex-agent/config.json`)**:
```json
{
  "mode": "basic",
...
**V3 Config (`cortex-agent.config.json`)**:
```json
{
  "version": "3.0.0",
  "memory": {
...
```typescript
// V2 (deprecated)
import { CortexAgent, Agent, Memory } from 'cortex-agent';

// V3 (new)
import { CortexAgentClient } from '@cortex-agent/cli';
import { AgentDB } from '@cortex-agent/memory';
import { ThreatDetector } from '@cortex-agent/security';
import { HNSWIndex } from '@cortex-agent/embeddings';
```
...
npx cortex-agent@latest migrate rollback --list

# Rollback to V2
npx cortex-agent@latest migrate rollback --to v2
...
- [ ] Verify all agents spawn correctly: `npx cortex-agent@latest agent list`
- [ ] Check memory search works: `npx cortex-agent@latest memory search -q "test"`
- [ ] Confirm MCP server starts: `npx cortex-agent@latest mcp start`
- [ ] Run doctor diagnostics: `npx cortex-agent@latest doctor`
- [ ] Test a simple swarm: `npx cortex-agent@latest swarm init --topology mesh`
- [ ] Bootstrap learning: `npx cortex-agent@latest hooks pretrain`

### Common Migration Issues

| Issue | Cause | Solution |
|-------|-------|----------|
| `MODULE_NOT_FOUND` | Old package references | Update imports to `@cortex-agent/*` |
| `Config not found` | Path change | Rename to `cortex-agent.config.json` |
...
|--------|-------------|------|
| `@cortex-agent/plugins` | Plugin SDK with workers, hooks, providers, security | [README](./v3/@cortex-agent/plugins/README.md) |
| `@cortex-agent/hooks` | Event-driven lifecycle hooks + ReasoningBank | [Source](./v3/@cortex-agent/hooks/) |
| `@cortex-agent/memory` | AgentDB unification with HNSW indexing | [Source](./v3/@cortex-agent/memory/) |
| `@cortex-agent/security` | CVE remediation & security patterns | [Source](./v3/@cortex-agent/security/) |
| `@cortex-agent/swarm` | 15-agent coordination engine | [Source](./v3/@cortex-agent/swarm/) |
| `@cortex-agent/cli` | CLI modernization | [Source](./v3/@cortex-agent/cli/) |
| `@cortex-agent/neural` | SONA learning integration | [Source](./v3/@cortex-agent/neural/) |
| `@cortex-agent/testing` | TDD London School framework | [Source](./v3/@cortex-agent/testing/) |
| `@cortex-agent/mcp` | MCP server & tools | [Source](./v3/@cortex-agent/mcp/) |
| `@cortex-agent/embeddings` | Vector embedding providers | [Source](./v3/@cortex-agent/embeddings/) |
| `@cortex-agent/providers` | LLM provider integrations | [Source](./v3/@cortex-agent/providers/) |
| `@cortex-agent/integration` | agentic-flow@alpha integration | [Source](./v3/@cortex-agent/integration/) |
| `@cortex-agent/performance` | Benchmarking & optimization | [Source](./v3/@cortex-agent/performance/) |
| `@cortex-agent/deployment` | Release & CI/CD | [Source](./v3/@cortex-agent/deployment/) |
| `@cortex-agent/shared` | Shared utilities, types & V3ProgressService | [Source](./v3/@cortex-agent/shared/) |
| `@cortex-agent/browser` | AI-optimized browser automation with agent-browser | [README](./v3/@cortex-agent/browser/README.md) |
