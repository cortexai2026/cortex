/**
 * Cortex Agent Plugin Registry Cloud Function
 *
 ...
 *     --service-account=plugin-registry-publisher@cortex-agent.iam.gserviceaccount.com
 */
...
const PROJECT_ID = process.env.GCP_PROJECT || 'cortex-agent';
const BUCKET_NAME = 'cortex-agent-plugin-registry';
...
async function exportModel(modelData, jwt) {
  const result = await pinToIPFS(modelData, `cortex-agent-model-${Date.now()}`, jwt);
  return {
    cid: result.IpfsHash,
    size: result.PinSize,
    gateways: [
  ...
  try {
    ...
    switch (action) {
      case 'publish': {
        ...
        // Pin to IPFS
        const pinResult = await pinToIPFS(registry, 'cortex-agent-plugin-registry', pinataJwt);
