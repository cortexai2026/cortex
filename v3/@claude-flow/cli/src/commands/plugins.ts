...
const listCommand: Command = {
  ...
  options: [
    ...
    { name: 'registry', short: 'r', type: 'string', description: 'Registry to use (default: cortex-agent-official)' },
  ],
  examples: [
    { command: 'cortex-agent plugins list', description: 'List all plugins from registry' },
    { command: 'cortex-agent plugins list --installed', description: 'List installed only' },
    { command: 'cortex-agent plugins list --official', description: 'List official plugins' },
    { command: 'cortex-agent plugins list --category security', description: 'List security plugins' },
  ],
    ...
    if (installedOnly) {
      ...
      try {
        ...
        if (installed.length === 0) {
          ...
          output.writeln();
          output.writeln(output.dim('Run "cortex-agent plugins list" to see available plugins'));
          output.writeln(output.dim('Run "cortex-agent plugins install -n <plugin>" to install'));
          return { success: true };
        }
...
const installCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins install -n community-analytics', description: 'Install plugin from IPFS' },
    { command: 'cortex-agent plugins install -n ./my-plugin --dev', description: 'Install local plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (existingPlugin) {
        ...
        output.writeln();
        output.writeln(output.dim('Use "cortex-agent plugins upgrade -n ' + name + '" to update'));
        return { success: false, exitCode: 1 };
      }
...
const uninstallCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins uninstall -n community-analytics', description: 'Uninstall plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const toggleCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins toggle -n analytics --enable', description: 'Enable plugin' },
    { command: 'cortex-agent plugins toggle -n analytics --disable', description: 'Disable plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const infoCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins info -n @cortex-agent/neural', description: 'Show plugin info' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
    const registryName = ctx.flags.registry as string;
...
const createCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins create -n my-plugin', description: 'Create basic plugin' },
    { command: 'cortex-agent plugins create -n my-plugin -t hooks', description: 'Create hooks plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const upgradeCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins upgrade -n @cortex-agent/neural', description: 'Upgrade to latest' },
    { command: 'cortex-agent plugins upgrade -n @cortex-agent/neural -v 3.1.0', description: 'Upgrade to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins search -q neural', description: 'Search for neural plugins' },
    { command: 'cortex-agent plugins search -q security --verified', description: 'Search verified security plugins' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const rateCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins rate -n @cortex-agent/embeddings -r 5', description: 'Rate 5 stars' },
    { command: 'cortex-agent plugins rate -n my-plugin -r 4', description: 'Rate 4 stars' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const pluginsCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent plugins list', description: 'List plugins from IPFS registry' },
    { command: 'cortex-agent plugins search -q neural', description: 'Search for plugins' },
    { command: 'cortex-agent plugins install -n community-analytics', description: 'Install from IPFS' },
    { command: 'cortex-agent plugins create -n my-plugin', description: 'Create new plugin' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Cortex Agent Plugin System'));
    ...
    output.printList([
      '@cortex-agent/neural              - Neural patterns and inference (WASM SIMD)',
      '@cortex-agent/security            - Security scanning and CVE detection',
      '@cortex-agent/embeddings          - Vector embeddings with hyperbolic support',
      '@cortex-agent/claims              - Claims-based authorization',
      '@cortex-agent/performance         - Performance profiling and benchmarks',
      '@cortex-agent/plugin-gastown-bridge - Gas Town orchestrator integration (WASM-accelerated)',
    ]);
    output.writeln();
    output.writeln(output.dim('Run "cortex-agent plugins list --official" to see all official plugins'));
