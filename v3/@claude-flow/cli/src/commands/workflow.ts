...
const runCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent workflow run -t development --task "Build auth system"', description: 'Run development workflow' },
    { command: 'cortex-agent workflow run -f ./workflow.yaml', description: 'Run from file' },
    { command: 'cortex-agent workflow run -t sparc --dry-run', description: 'Validate SPARC workflow' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!dryRun) {
        output.writeln();
        output.printInfo(`Track progress: cortex-agent workflow status ${result.workflowId}`);
      }
...
const validateCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent workflow validate -f ./workflow.yaml', description: 'Validate workflow file' },
    { command: 'cortex-agent workflow validate -f ./workflow.json --strict', description: 'Strict validation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const templateCommand: Command = {
  ...
  subcommands: [
    ...
    },
    {
      ...
      action: async (ctx: CommandContext): Promise<CommandResult> => {
        ...
        output.writeln(output.dim('  Use with: cortex-agent workflow run -t ' + name));
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: cortex-agent workflow template <subcommand>');
    output.writeln();
...
export const workflowCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'cortex-agent workflow run -t development --task "Build feature"', description: 'Run workflow' },
    { command: 'cortex-agent workflow validate -f ./workflow.yaml', description: 'Validate workflow' },
    { command: 'cortex-agent workflow list', description: 'List workflows' }
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Workflow Commands'));
    output.writeln();
    output.writeln('Usage: cortex-agent workflow <subcommand> [options]');
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
    ...
    output.writeln();
    output.writeln('Run "cortex-agent workflow <subcommand> --help" for more info');
