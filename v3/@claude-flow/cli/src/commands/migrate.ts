...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Check v2 config: cortex-agent.config.json with version "2" or missing version
    const v2ConfigPath = path.join(cwd, 'cortex-agent.config.json');
    const v3ConfigDir = path.join(cwd, '.cortex-agent');
    ...
    const migrationStatePath = path.join(cwd, '.cortex-agent', 'migration-state.json');
    ...
    output.writeln();
    if (needsMigration) {
      output.printInfo('V2 artifacts detected. Run "cortex-agent migrate run" to migrate.');
    } else {
...
const runCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const v3Dir = path.join(cwd, '.cortex-agent');
    ...
    // Ensure .cortex-agent directory exists
    if (!dryRun) {
      ...
      if (fs.existsSync(v2ConfigPath)) {
        ...
        if (parsed.version === '2' || parsed.version === 2 || !parsed.version) {
          if (dryRun) {
            ...
            // Backup
            if (!skipBackup) {
              fs.copyFileSync(v2ConfigPath, path.join(backupDir, 'cortex-agent.config.json'));
            }
      ...
      if (fs.existsSync(v2MemoryDir)) {
        ...
        if (jsonFiles.length > 0 || hasDb) {
          if (dryRun) {
            ...
            output.printInfo('Run "cortex-agent memory init --force" to import v2 memory into v3 AgentDB.');
          }
    ...
    if (dryRun) {
      ...
      output.printInfo('Run "cortex-agent migrate verify" to validate the migration.');
    } else {
...
const verifyCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (allPassed) {
      ...
      output.printInfo('Run "cortex-agent migrate run" to re-run the migration, or "migrate rollback" to restore from backup.');
    }
...
const rollbackCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      // Restore config
      const backupConfig = path.join(backupPath, 'cortex-agent.config.json');
      if (fs.existsSync(backupConfig)) {
        const destConfig = path.join(cwd, 'cortex-agent.config.json');
...
const breakingCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const changes = [
      {
        ...
        changes: [
          { change: 'Config file renamed', from: 'cortex-agent.json', to: 'cortex-agent.config.json' },
      ...
      },
      {
        ...
        changes: [
          ...
          { change: 'Storage path changed', from: '.cortex-agent/memory', to: 'data/memory' }
        ]
      },
      {
        category: 'CLI',
    ...
    output.printInfo('Run "cortex-agent migrate run" to automatically handle these changes');
...
export const migrateCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'cortex-agent migrate status', description: 'Check migration status' },
    { command: 'cortex-agent migrate run --dry-run', description: 'Preview migration' },
    { command: 'cortex-agent migrate run -t all', description: 'Run full migration' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('V2 to V3 Migration Tools'));
    output.writeln();
    output.writeln('Usage: cortex-agent migrate <subcommand> [options]');
    output.writeln();
...
function getMigrationSteps(target: string): Array<{ name: string; description: string; source: string; dest: string }> {
  const allSteps = [
    { name: 'Configuration Files', description: 'Migrate config schema to V3 format', source: './cortex-agent.json', dest: './cortex-agent.config.json' },
    { name: 'Memory Backend', description: 'Upgrade to hybrid backend with AgentDB', source: './.cortex-agent/memory', dest: './data/memory' },
    { name: 'Agent Definitions', description: 'Convert agent configs to V3 format', source: './.cortex-agent/agents', dest: './v3/agents' },
    { name: 'Hook Registry', description: 'Migrate hooks to V3 hook system', source: './src/hooks', dest: './v3/hooks' },
    { name: 'Workflow Definitions', description: 'Convert workflows to event-sourced format', source: './.cortex-agent/workflows', dest: './data/workflows' },
    { name: 'Embeddings System', description: 'Migrate to ONNX with hyperbolic (Poincaré ball)', source: 'OpenAI/TF.js embeddings', dest: '.cortex-agent/embeddings.json' }
  ];
