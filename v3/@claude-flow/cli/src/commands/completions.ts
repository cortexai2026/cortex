...
function generateBashCompletion(): string {
  return `# cortex-agent bash completion
# Generated by cortex-agent completions bash
# Add this to ~/.bashrc or ~/.bash_completion

_cortex_agent_completions() {
...
complete -F _cortex_agent_completions cortex-agent
complete -F _cortex_agent_completions npx\\ @cortex-agent/cli@v3alpha
`;
}
...
# cortex-agent zsh completion
# Generated by cortex-agent completions zsh
# Add to ~/.zfunc/_cortex-agent or ~/.zsh/completions/_cortex-agent
...
_cortex_agent() {
    ...
    case $state in
        command)
            _describe -t commands 'cortex-agent commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
function generateFishCompletion(): string {
  return `# cortex-agent fish completion
# Generated by cortex-agent completions fish
# Save to ~/.config/fish/completions/cortex-agent.fish

# Disable file completion by default
complete -c cortex-agent -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c cortex-agent -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c cortex-agent -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c cortex-agent -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c cortex-agent -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c cortex-agent -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c cortex-agent -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c cortex-agent -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c cortex-agent -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# cortex-agent PowerShell completion
# Generated by cortex-agent completions powershell
# Add to $PROFILE or save to a separate file and dot-source it

$script:CortexAgentCommands = @(
...
Register-ArgumentCompleter -Native -CommandName cortex-agent -ScriptBlock {
    ...
    if ($tokens.Count -eq 2) {
        ...
        $script:CortexAgentCommands | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'cortex-agent completions bash > ~/.bash_completion.d/cortex-agent', description: 'Install bash completions' },
    { command: 'cortex-agent completions zsh > ~/.zfunc/_cortex-agent', description: 'Install zsh completions' },
    { command: 'cortex-agent completions fish > ~/.config/fish/completions/cortex-agent.fish', description: 'Install fish completions' },
    { command: 'cortex-agent completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  cortex-agent completions bash > ~/.bash_completion.d/cortex-agent'));
    output.writeln(output.dim('  source ~/.bash_completion.d/cortex-agent'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  cortex-agent completions zsh > ~/.zfunc/_cortex-agent'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  cortex-agent completions fish > ~/.config/fish/completions/cortex-agent.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  cortex-agent completions powershell >> $PROFILE'));
