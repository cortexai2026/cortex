...
const storeCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory store -k "api/auth" -v "JWT implementation"', description: 'Store text' },
    { command: 'cortex-agent memory store -k "pattern/singleton" --vector', description: 'Store vector' },
    { command: 'cortex-agent memory store -k "pattern" -v "updated" --upsert', description: 'Update existing' }
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory search -q "authentication patterns"', description: 'Semantic search' },
    { command: 'cortex-agent memory search -q "JWT" -t keyword', description: 'Keyword search' },
    { command: 'cortex-agent memory search -q "test" --build-hnsw', description: 'Build HNSW index and search' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (results.length === 0) {
        ...
        output.writeln(output.dim('Try: cortex-agent memory store -k "key" --value "data"'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        output.printInfo('Store data: cortex-agent memory store -k "key" --value "data"');
        return { success: true, data: [] };
      }
...
const deleteCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory delete -k "mykey"', description: 'Delete entry with default namespace' },
    { command: 'cortex-agent memory delete -k "lesson" -n "lessons"', description: 'Delete entry from specific namespace' },
    { command: 'cortex-agent memory delete mykey -f', description: 'Delete without confirmation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const cleanupCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory cleanup --dry-run', description: 'Preview cleanup' },
    { command: 'cortex-agent memory cleanup --older-than 30d', description: 'Delete entries older than 30 days' },
    { command: 'cortex-agent memory cleanup --expired-only', description: 'Clean expired entries' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const compressCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory compress', description: 'Balanced compression' },
    { command: 'cortex-agent memory compress --quantize --bits 4', description: '4-bit quantization (32x reduction)' },
    { command: 'cortex-agent memory compress -l max -t vectors', description: 'Max compression on vectors' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory export -o ./backup.json', description: 'Export all to JSON' },
    { command: 'cortex-agent memory export -o ./data.csv -f csv', description: 'Export to CSV' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory import -i ./backup.json', description: 'Import from file' },
    { command: 'cortex-agent memory import -i ./data.json -n archive', description: 'Import to namespace' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const initMemoryCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent memory init', description: 'Initialize hybrid backend with all features' },
    { command: 'cortex-agent memory init -b agentdb', description: 'Initialize AgentDB backend' },
    { command: 'cortex-agent memory init -p ./data/memory.db --force', description: 'Reinitialize at custom path' },
    { command: 'cortex-agent memory init --verbose --verify', description: 'Initialize with full verification' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printList([
        `Store data: ${output.highlight('cortex-agent memory store -k "key" --value "data"')}`,
        `Search: ${output.highlight('cortex-agent memory search -q "query"')}`,
        `Train patterns: ${output.highlight('cortex-agent neural train -p coordination')}`,
        `View stats: ${output.highlight('cortex-agent memory stats')}`
      ]);
...
export const memoryCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'cortex-agent memory store -k "key" -v "value"', description: 'Store data' },
    { command: 'cortex-agent memory search -q "auth patterns"', description: 'Search memory' },
    { command: 'cortex-agent memory stats', description: 'Show statistics' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Memory Management Commands'));
    output.writeln();
    output.writeln('Usage: cortex-agent memory <subcommand> [options]');
    output.writeln();
