...
export const wasmStatusCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'cortex-agent agent wasm-status', description: 'Check WASM agent runtime status' },
  ],
...
export const wasmCreateCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent agent wasm-create', description: 'Create a default WASM agent' },
    { command: 'cortex-agent agent wasm-create -t coder', description: 'Create from gallery template' },
    { command: 'cortex-agent agent wasm-create -m "anthropic:claude-sonnet-4-20250514" -i "You are a security auditor"', description: 'Create with custom config' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const wasm = await loadWasm();
...
export const wasmPromptCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent agent wasm-prompt -a wasm-agent-1-abc -i "Write a hello world"', description: 'Send prompt to WASM agent' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const wasmGalleryCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent agent wasm-gallery', description: 'List all gallery templates' },
    { command: 'cortex-agent agent wasm-gallery -s coder', description: 'Search gallery templates' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const wasm = await loadWasm();
