...
export const initCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent ruvector init -d mydb', description: 'Initialize with database name' },
    { command: 'cortex-agent ruvector init -d mydb -h db.example.com --ssl', description: 'Remote with SSL' },
    { command: 'cortex-agent ruvector init -d mydb --force', description: 'Force re-initialization' },
    { command: 'cortex-agent ruvector init -d mydb --dimensions 768', description: 'Custom vector dimensions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox([
        ...
        '',
        'Next steps:',
        '  1. Run migrations: cortex-agent ruvector migrate --up',
        '  2. Check status: cortex-agent ruvector status --verbose',
        '  3. Run benchmark: cortex-agent ruvector benchmark',
