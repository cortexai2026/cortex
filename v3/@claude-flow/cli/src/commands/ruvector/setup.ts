 ...
 *   npx cortex-agent ruvector setup --output /path/to/dir
 *   npx cortex-agent ruvector setup --print      # Print to stdout only
 *
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: cortex-agent-test
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U claude -d cortex_agent"]
      interval: 5s
      timeout: 5s
      retries: 10
    command: >
  ...
  pgadmin:
    ...
    environment:
      PGADMIN_DEFAULT_EMAIL: admin@cortex-agent.local
...
-- Create the cortex_agent schema
CREATE SCHEMA IF NOT EXISTS cortex_agent;

-- Grant permissions
GRANT ALL ON SCHEMA cortex_agent TO claude;

-- Set search path
SET search_path TO cortex_agent, public;
...
CREATE TABLE IF NOT EXISTS cortex_agent.embeddings (
...
CREATE TABLE IF NOT EXISTS cortex_agent.patterns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS cortex_agent.agents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS cortex_agent.trajectories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS cortex_agent.memory_entries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS cortex_agent.hyperbolic_embeddings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    content TEXT NOT NULL,
    ...
    parent_id UUID REFERENCES cortex_agent.hyperbolic_embeddings(id),
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Graph nodes for GNN operations
CREATE TABLE IF NOT EXISTS cortex_agent.graph_nodes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS cortex_agent.graph_edges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_id UUID REFERENCES cortex_agent.graph_nodes(id),
    target_id UUID REFERENCES cortex_agent.graph_nodes(id),
...
ON cortex_agent.graph_nodes
USING hnsw (embedding ruvector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Additional indices for common queries
CREATE INDEX IF NOT EXISTS idx_embeddings_namespace ON cortex_agent.embeddings(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_namespace ON cortex_agent.memory_entries(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_key ON cortex_agent.memory_entries(key);
...
CREATE OR REPLACE FUNCTION cortex_agent.search_similar(
...
BEGIN
    ...
    SELECT
        ...
        e.metadata
    FROM cortex_agent.embeddings e
...
CREATE OR REPLACE FUNCTION cortex_agent.search_memory(
    query_embedding ruvector(384),
...
BEGIN
    ...
    SELECT
        ...
        m.metadata
    FROM cortex_agent.memory_entries m
...
CREATE OR REPLACE FUNCTION cortex_agent.search_patterns(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION cortex_agent.find_agents(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION cortex_agent.to_poincare(
    euclidean real[],
    curvature FLOAT DEFAULT -1.0
)
RETURNS real[] AS $$
...
CREATE OR REPLACE FUNCTION cortex_agent.poincare_distance(
    x real[],
    y real[],
    curvature FLOAT DEFAULT -1.0
)
...
CREATE OR REPLACE FUNCTION cortex_agent.hyperbolic_search(
    query ruvector(384),
    limit_count INT DEFAULT 10,
    curvature FLOAT DEFAULT -1.0
)
...
BEGIN
    ...
    query_poincare := cortex_agent.to_poincare(query_arr, curvature);
    ...
    SELECT
        ...
        he.hierarchy_level,
        he.metadata
    FROM cortex_agent.hyperbolic_embeddings he
...
CREATE OR REPLACE FUNCTION cortex_agent.ruvector_info()
RETURNS TABLE (
    version TEXT,
    simd_info TEXT
) AS $$
...
CREATE OR REPLACE FUNCTION cortex_agent.cosine_similarity(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- L2 distance helper
CREATE OR REPLACE FUNCTION cortex_agent.l2_distance(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- Upsert memory entry
CREATE OR REPLACE FUNCTION cortex_agent.upsert_memory(
    p_key VARCHAR(255),
    p_value TEXT,
...
DECLARE
    v_id UUID;
BEGIN
    INSERT INTO cortex_agent.memory_entries (key, value, embedding, namespace, metadata, ttl, updated_at)
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Schema: cortex_agent';
    ...
    RAISE NOTICE '  - cortex_agent.search_similar(embedding, limit, min_sim)';
    RAISE NOTICE '  - cortex_agent.search_memory(embedding, namespace, limit)';
    RAISE NOTICE '  - cortex_agent.search_patterns(embedding, type, limit)';
    RAISE NOTICE '  - cortex_agent.find_agents(embedding, type, limit)';
    RAISE NOTICE '  - cortex_agent.hyperbolic_search(embedding, limit, curvature)';
    RAISE NOTICE '  - cortex_agent.upsert_memory(key, value, embedding, namespace)';
    RAISE NOTICE '';
...
docker exec ruvector-postgres psql -U claude -d cortex_agent -c "SELECT ruvector_version();"
\`\`\`
...
| Host | localhost |
| Port | 5432 |
| Database | cortex_agent |
| Username | claude |
| Password | cortex-agent-test |
| Schema | cortex_agent |
...
npx cortex-agent memory list --format json > memory-export.json
...
- Email: admin@cortex-agent.local
- Password: admin
...
- [Claude-Flow Documentation](https://github.com/ruvnet/cortex-agent)
`;
...
export const setupCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent ruvector setup', description: 'Output files to ./ruvector-postgres/' },
    { command: 'cortex-agent ruvector setup --output /path/to/dir', description: 'Output to custom directory' },
    { command: 'cortex-agent ruvector setup --print', description: 'Print files to stdout' },
    { command: 'cortex-agent ruvector setup --force', description: 'Overwrite existing files' },
  ],
