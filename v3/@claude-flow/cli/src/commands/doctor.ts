...
async function checkConfigFile(): Promise<HealthCheck> {
  ...
  return { name: 'Config File', status: 'warn', message: 'No config file (using defaults)', fix: 'cortex-agent config init' };
}

// Check daemon status
async function checkDaemonStatus(): Promise<HealthCheck> {
  try {
    const pidFile = '.cortex-agent/daemon.pid';
    if (existsSync(pidFile)) {
      ...
      try {
        ...
        return { name: 'Daemon Status', status: 'warn', message: 'Stale PID file', fix: 'rm .cortex-agent/daemon.pid && cortex-agent daemon start' };
      }
    }
    return { name: 'Daemon Status', status: 'warn', message: 'Not running', fix: 'cortex-agent daemon start' };
  } catch {
    return { name: 'Daemon Status', status: 'warn', message: 'Unable to check', fix: 'cortex-agent daemon status' };
  }
}
...
async function checkMemoryDatabase(): Promise<HealthCheck> {
  ...
  return { name: 'Memory Database', status: 'warn', message: 'Not initialized', fix: 'cortex-agent memory configure --backend hybrid' };
}
...
async function checkMcpServers(): Promise<HealthCheck> {
  ...
  for (const configPath of mcpConfigPaths) {
    if (existsSync(configPath)) {
      try {
        ...
        const hasCortexAgent = 'cortex-agent' in servers || 'cortex-agent_alpha' in servers || 'cortex-agent' in servers || 'cortex-agent_alpha' in servers;
        if (hasCortexAgent) {
          return { name: 'MCP Servers', status: 'pass', message: `${count} servers (cortex-agent configured)` };
        } else {
          return { name: 'MCP Servers', status: 'warn', message: `${count} servers (cortex-agent not found)`, fix: 'claude mcp add cortex-agent -- npx -y cortex-agent@latest mcp start' };
        }
      } catch {
  ...
  return { name: 'MCP Servers', status: 'warn', message: 'No MCP config found', fix: 'claude mcp add cortex-agent npx @cortex-agent/cli@v3alpha mcp start' };
}
...
async function checkVersionFreshness(): Promise<HealthCheck> {
  try {
    ...
    try {
      ...
      // package.json that belongs to @cortex-agent/cli (or cortex-agent/cli).
        ...
        try {
          if (existsSync(candidate)) {
            ...
            if (
              ...
              (pkg.name === '@cortex-agent/cli' || pkg.name === 'cortex-agent' || pkg.name === 'cortex-agent')
            ) {
    ...
    try {
      const npmInfo = await runCommand('npm view @cortex-agent/cli@alpha version', 5000);
    ...
    if (isOutdated) {
      const fix = isNpx
        ? 'rm -rf ~/.npm/_npx/* && npx -y @cortex-agent/cli@latest'
        : 'npm update @cortex-agent/cli';
...
export const doctorCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent doctor', description: 'Run full health check' },
    { command: 'cortex-agent doctor --fix', description: 'Show fixes for issues' },
    { command: 'cortex-agent doctor --install', description: 'Auto-install missing dependencies' },
    { command: 'cortex-agent doctor -c version', description: 'Check for stale npx cache' },
    { command: 'cortex-agent doctor -c claude', description: 'Check Claude Code CLI only' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Cortex Agent Doctor'));
