 ...
 * Session management for Cortex Agent
 */
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (result.sessions.length === 0) {
        ...
        output.printInfo('Run "cortex-agent session save" to create a session');
        return { success: true, data: result };
      }
...
const saveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printInfo(`Restore with: cortex-agent session restore ${result.sessionId}`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (error instanceof MCPClientError) {
        ...
        output.printInfo('Start a session with "cortex-agent start"');
...
export const sessionCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'cortex-agent session list', description: 'List all sessions' },
    { command: 'cortex-agent session save -n "checkpoint-1"', description: 'Save current session' },
    { command: 'cortex-agent session restore session-123', description: 'Restore a session' },
    { command: 'cortex-agent session delete session-123', description: 'Delete a session' },
    { command: 'cortex-agent session export -o backup.json', description: 'Export session to file' },
    { command: 'cortex-agent session import backup.json', description: 'Import session from file' },
    { command: 'cortex-agent session current', description: 'Show current session' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: cortex-agent session <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "cortex-agent session <subcommand> --help" for subcommand help');
