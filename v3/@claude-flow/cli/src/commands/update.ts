...
const checkCommand: Command = {
  name: 'check',
  description: 'Check for available @cortex-agent package updates',
  options: [
  ...
  async action(ctx: CommandContext): Promise<CommandResult> {
    ...
    try {
      ...
      if (results.length === 0) {
        output.printSuccess('All @cortex-agent packages are up to date!');
        return { success: true };
      }
      ...
      if (manualUpdates.length > 0) {
        ...
        output.writeln('  cortex-agent update all');
      }

      return { success: true };
    } finally {
      delete process.env.CORTEX_AGENT_FORCE_UPDATE;
    }
  },
};
...
const allCommand: Command = {
  name: 'all',
  description: 'Update all @cortex-agent packages',
  options: [
...
const updateCommand: Command = {
  name: 'update',
  description: 'Manage @cortex-agent package updates (ADR-025)',
  ...
  async action(): Promise<CommandResult> {
    ...
    output.writeln();
    output.writeln('Manage @cortex-agent package updates with auto-update support.');
    output.writeln();
    ...
    output.printList([
      `${output.dim('CORTEX_AGENT_AUTO_UPDATE=false')}  - Disable auto-update`,
      `${output.dim('CORTEX_AGENT_FORCE_UPDATE=true')} - Force update check`,
    ]);
    output.writeln();
    output.writeln('Run "cortex-agent update <subcommand> --help" for subcommand help');
