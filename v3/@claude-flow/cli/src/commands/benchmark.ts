...
const pretrainCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent benchmark pretrain', description: 'Run pre-training benchmarks' },
    { command: 'cortex-agent benchmark pretrain -i 500 --save results.json', description: 'Extended benchmark with results saved' },
    { command: 'cortex-agent benchmark pretrain -o json', description: 'Output results as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (saveFile) {
        const resultsDir = join(process.cwd(), '.cortex-agent', 'benchmarks');
...
const neuralCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent benchmark neural', description: 'Run neural benchmarks' },
    { command: 'cortex-agent benchmark neural -d 768 -n 5000', description: 'Higher dimension, more vectors' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const iterations = parseInt(ctx.flags.iterations as string || '100', 10);
...
const memoryCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent benchmark memory', description: 'Run memory benchmarks' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const iterations = parseInt(ctx.flags.iterations as string || '100', 10);
    const outputFormat = ctx.flags.output as string || 'text';
...
const allCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent benchmark all', description: 'Run all benchmarks' },
    { command: 'cortex-agent benchmark all --save full-results.json', description: 'Run all and save results' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold(output.highlight('═'.repeat(65))));
    output.writeln(output.bold('  Cortex Agent V3 - Full Benchmark Suite'));
    output.writeln(output.bold(output.highlight('═'.repeat(65))));
...
export const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'cortex-agent benchmark pretrain', description: 'Benchmark pre-training system' },
    { command: 'cortex-agent benchmark neural', description: 'Benchmark neural operations' },
    { command: 'cortex-agent benchmark memory', description: 'Benchmark memory operations' },
    { command: 'cortex-agent benchmark all', description: 'Run all benchmarks' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Cortex Agent V3 Benchmark Suite'));
    ...
    output.writeln('  cortex-agent benchmark pretrain -i 200');
    output.writeln('  cortex-agent benchmark all --save results.json');
    output.writeln();
