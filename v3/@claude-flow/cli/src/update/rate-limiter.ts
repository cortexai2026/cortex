...
const STATE_FILE = path.join(os.homedir(), '.cortex-agent', 'update-state.json');
...
export function shouldCheckForUpdates(
  ...
  if (process.env.CORTEX_AGENT_AUTO_UPDATE === 'false') {
    return { allowed: false, reason: 'Auto-update disabled via environment' };
  }

  // Force update if requested
  if (process.env.CORTEX_AGENT_FORCE_UPDATE === 'true') {
    return { allowed: true };
  }
