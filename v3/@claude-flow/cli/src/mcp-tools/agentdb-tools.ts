...
export const agentdbControllers: MCPTool = {
  ...
  handler: async () => {
    try {
      ...
      if (!controllers) return { available: false, controllers: [], error: 'AgentDB bridge not available — @cortex-agent/memory not installed or missing controller-registry. Use memory_store/memory_search tools instead.' };
      return {
        available: true,
        controllers,
