...
export async function autoInstallPackage(
  ...
  if (!validPackageName.test(packageName)) {
    if (!silent) {
      console.error(`[cortex-agent] Invalid package name: ${packageName}`);
    }
    return false;
  }
  ...
  try {
    if (!silent) {
      console.error(`[cortex-agent] Auto-installing ${packageName}...`);
    }
    ...
    if (!silent) {
      console.error(`[cortex-agent] Successfully installed ${packageName}`);
    }
    return true;
  } catch (error) {
    if (!silent) {
      console.error(`[cortex-agent] Failed to auto-install ${packageName}: ${error}`);
    }
    return false;
  }
}
  ...
  try {
    ...
    if (installed) {
      try {
        ...
        console.error(`[cortex-agent] ${packageName} installed but failed to load. Restart MCP server.`);
        return null;
      }
    }
    return null;
