...
type AIDefenceInstance = ReturnType<typeof import('@cortex-agent/aidefence').createAIDefence>;
...
async function getAIDefence(): Promise<AIDefenceInstance> {
  ...
  const packageName = '@cortex-agent/aidefence';
  ...
  if (installAttempted) {
    throw new Error('AIDefence package not available. Install with: npm install @cortex-agent/aidefence');
  }
  installAttempted = true;

  // Second attempt - auto-install and retry
  console.error(`[cortex-agent] ${packageName} not found, attempting auto-install...`);
  ...
  try {
    ...
    aidefenceInstance = instance;
    console.error(`[cortex-agent] ${packageName} loaded successfully after install`);
    return instance;
  } catch (retryError) {
...
const aidefenceIsSafeTool: MCPTool = {
  ...
  handler: async (args: Record<string, unknown>): Promise<MCPToolResult> => {
    ...
    try {
      const { isSafe } = await import('@cortex-agent/aidefence');
