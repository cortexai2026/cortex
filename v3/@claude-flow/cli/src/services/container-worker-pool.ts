...
const DEFAULT_CONFIG: ContainerPoolConfig = {
  ...
  statePath: '.cortex-agent/container-pool',
...
export class ContainerWorkerPool extends EventEmitter {
  ...
  private async createContainer(): Promise<ContainerInfo | null> {
    ...
    const name = `cortex-agent-worker-${id}`;
    ...
    try {
      ...
      const args = [
        ...
        '-v', `${join(this.projectRoot, this.config.statePath)}:/root/.cortex-agent`,
  ...
  private buildWorkerCommand(options: ContainerExecutionOptions): string[] {
    // Use npx to run cortex-agent daemon trigger
    return [
      'npx', 'cortex-agent@v3alpha',
      'daemon', 'trigger',
