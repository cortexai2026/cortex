 ...
 *
 * Creates a single binary containing kernel, runtime, Cortex Agent CLI, models/keys,
...
const CORTEX_AGENT_COMMANDS = 'init agent swarm memory mcp task session config status start workflow hooks hive-mind daemon neural security performance providers plugins deployment embeddings claims migrate process doctor completions'.split(' ');
...
type SectionId = 'kernel' | 'runtime' | 'cortex-agent' | 'models' | 'data' | 'verify';
...
export class RvfaBuilder {
  ...
  constructor(options: BuildOptions) {
    this.opts = {
      ...
      cortex-agentVersion: options.cortex-agentVersion || detectCortex AgentVersion(),
  ...
  async build(): Promise<BuildResult> {
    ...
    const stages: { id: SectionId; raw: Buffer; label: string }[] = [
      ...
      { id: 'cortex-agent',   raw: this.buildCortex AgentSection(),   label: 'Cortex Agent CLI' },
  ...
  private buildKernelSection(): Buffer {
    return jsonBuf({
      ...
      init: '/sbin/init -> cortex-agent-init (PID 1)',
  ...
  private buildCortex AgentSection(): Buffer {
    let packageMeta: Record<string, unknown> | null = null;
    try {
      const raw = execSync('npm pack cortex-agent@latest --dry-run --json 2>/dev/null', { encoding: 'utf-8', timeout: 15_000 });
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) packageMeta = parsed[0];
    } catch { /* manifest-only fallback */ }

    return jsonBuf({
      type: 'cortex-agent', version: this.opts.cortex-agentVersion,
      package: packageMeta ?? { name: 'cortex-agent', version: this.opts.cortex-agentVersion },
      commands: CORTEX_AGENT_COMMANDS, commandCount: CORTEX_AGENT_COMMANDS.length,
  ...
  private buildVerifySection(): Buffer {
    ...
    if (existsSync(scriptPath)) {
      ...
      script = Buffer.from([
        '#!/bin/sh', 'set -e', 'CORTEX_AGENT_CMD="${CORTEX_AGENT_CMD:-cortex-agent}"',
        'echo "Running basic verification..."',
        '$CORTEX_AGENT_CMD --version && echo "  OK: CLI" || echo "  FAIL: CLI"',
        '$CORTEX_AGENT_CMD doctor && echo "  OK: Doctor" || echo "  FAIL: Doctor"',
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    ...
    const boot: RvfaBootConfig = {
      entrypoint: '/opt/cortex-agent/bin/cli.js',
      args: ['--profile', this.opts.profile],
      env: { NODE_ENV: 'production', CORTEX_AGENT_MEMORY_BACKEND: 'hybrid', CORTEX_AGENT_LOG_LEVEL: 'info' },
    ...
    return {
      magic: 'RVFA', version: 1, name: 'cortex-agent-appliance', appVersion: this.opts.cortex-agentVersion,
...
function detectCortex AgentVersion(): string {
  try {
