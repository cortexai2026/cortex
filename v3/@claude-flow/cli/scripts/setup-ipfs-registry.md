...
```bash
# Create new project
gcloud projects create cortex-agent-registry --name="Cortex Agent Registry"

# Set as active project
gcloud config set project cortex-agent-registry

# Enable billing (required for Cloud Functions)
gcloud beta billing projects link cortex-agent-registry \
...
gsutil iam ch allUsers:objectViewer gs://cortex-agent-plugin-registry
```
...
# Grant permissions
gcloud projects add-iam-policy-binding cortex-agent-registry \
  --member="serviceAccount:plugin-registry-publisher@cortex-agent-registry.iam.gserviceaccount.com" \
...
async function getSecret(name) {
  const [version] = await secretManager.accessSecretVersion({
    name: `projects/cortex-agent-registry/secrets/${name}/versions/latest`,
  });
...
gcloud functions deploy publish-registry \
  ...
  --service-account=plugin-registry-publisher@cortex-agent-registry.iam.gserviceaccount.com \
  --set-env-vars=GCP_PROJECT=cortex-agent-registry
```
...
# Upload to GCS
gsutil cp registry.json gs://cortex-agent-plugin-registry/registry.json
```
...
steps:
  ...
  - name: 'node:20'
    ...
    dir: 'v3/@cortex-agent/cli'

  # Upload to GCS
  - name: 'gcr.io/cloud-builders/gsutil'
    args: ['cp', 'registry.json', 'gs://cortex-agent-plugin-registry/registry.json']
...
substitutions:
  _FUNCTION_URL: 'https://us-central1-cortex-agent-registry.cloudfunctions.net/publish-registry'
...
gcloud scheduler jobs create http publish-registry-daily \
  ...
  --schedule="0 2 * * *" \
  --uri="https://us-central1-cortex-agent-registry.cloudfunctions.net/publish-registry" \
  --http-method=POST
```

## Step 6: Update Cortex Agent CLI
