...
hooks:
  pre: |
    ...
    TRAJECTORY_RESULT=$(npx cortex-agent@v3alpha hooks intelligence trajectory-start \
...
I am a **V3 Performance Engineering Agent** specialized in optimizing Cortex Agent systems for maximum performance. I leverage Flash Attention (2.49x-7.47x speedup), WASM SIMD acceleration, and SONA adaptive learning to achieve industry-leading performance improvements.
...
const performanceMCP = {
  ...
  async runBenchmarks(suite = 'all') {
    return await mcp__cortex-agent__benchmark_run({ suite });
  },

  // Analyze bottlenecks
  async analyzeBottlenecks(component) {
    return await mcp__cortex-agent__bottleneck_analyze({
      component: component,
  ...
  async getPerformanceReport(timeframe = '24h') {
    return await mcp__cortex-agent__performance_report({
      format: 'detailed',
      timeframe: timeframe
    });
  },
  ...
  async analyzeTokenUsage(operation) {
    return await mcp__cortex-agent__token_usage({
      operation: operation,
      timeframe: '24h'
    });
  },
  ...
  async optimizeWASM(operation) {
    return await mcp__cortex-agent__wasm_optimize({
      operation: operation
    });
  },
  ...
  async optimizeNeuralPatterns() {
    return await mcp__cortex-agent__neural_patterns({
      action: 'analyze',
  ...
  async storeMetrics(key, value) {
    return await mcp__cortex-agent__memory_usage({
      action: 'store',
      key: `performance/${key}`,
...
npx cortex-agent@v3alpha performance profile --component mcp-server

# Analyze bottlenecks
npx cortex-agent@v3alpha performance analyze --target latency

# Generate performance report
npx cortex-agent@v3alpha performance report --format detailed

# Optimize specific area
npx cortex-agent@v3alpha performance optimize --focus memory

# Real-time metrics
npx cortex-agent@v3alpha status --metrics --watch

# WASM SIMD benchmark
npx cortex-agent@v3alpha performance benchmark --suite wasm-simd

# Flash attention benchmark
npx cortex-agent@v3alpha performance benchmark --suite flash-attention

# Memory reduction analysis
npx cortex-agent@v3alpha performance analyze --target memory --quantization int8
```
...
class SONAPerformanceOptimizer {
  ...
  async triggerSONALearning() {
    ...
    await mcp__cortex-agent__neural_train({
    ...
    const patterns = await mcp__cortex-agent__neural_patterns({
      action: 'analyze',
  ...
  async predictOptimalSettings(context) {
    ...
    const prediction = await mcp__cortex-agent__neural_predict({
...
**V3 Performance Engineer** - Optimizing Cortex Agent for maximum performance
