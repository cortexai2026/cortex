...
```bash
npx cortex-agent agent logs <agent-id> [options]
```
...
```bash
# View last 50 lines
npx cortex-agent agent logs coder-lx7m9k2

# View last 100 lines
npx cortex-agent agent logs coder-lx7m9k2 -n 100

# Follow logs (live)
npx cortex-agent agent logs coder-lx7m9k2 --follow

# Filter by level
npx cortex-agent agent logs coder-lx7m9k2 -l error

# Logs since time
npx cortex-agent agent logs coder-lx7m9k2 --since "1h"
npx cortex-agent agent logs coder-lx7m9k2 --since "2026-01-08T10:00:00"

# JSON output
npx cortex-agent agent logs coder-lx7m9k2 --format json
```
...
# Last 30 minutes
npx cortex-agent agent logs coder-lx7m9k2 --since "30m"

# Specific timestamp
npx cortex-agent agent logs coder-lx7m9k2 --since "2026-01-08T09:00:00"

# Today's logs
npx cortex-agent agent logs coder-lx7m9k2 --since "today"
```
...
- `npx cortex-agent agent status` - Current agent status
- `npx cortex-agent agent health` - Health monitoring
- `npx cortex-agent agent metrics` - Performance metrics
