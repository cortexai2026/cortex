...
```bash
npx cortex-agent agent metrics [agent-id] [options]
```
...
npx cortex-agent agent metrics coder-lx7m9k2

# Last hour
npx cortex-agent agent metrics -p 1h

# Last 7 days
npx cortex-agent agent metrics --period 7d

# JSON output
npx cortex-agent agent metrics --format json
```
...
- `npx cortex-agent agent status` - Individual agent metrics
- `npx cortex-agent performance benchmark` - Full performance suite
- `npx cortex-agent status` - System-wide status
