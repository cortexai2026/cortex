...
List all active agents in the Cortex Agent system with filtering options.

## Usage

```bash
npx cortex-agent agent list [options]
npx cortex-agent agent ls [options]  # Alias
```
...
npx cortex-agent agent list --all

# Filter by type
npx cortex-agent agent list -t coder

# Filter by status
npx cortex-agent agent list -s idle

# JSON output for scripting
npx cortex-agent agent list --format json

# Combined filters
npx cortex-agent agent list -t researcher -s active
```
