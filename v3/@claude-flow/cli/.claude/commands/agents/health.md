...
```bash
npx cortex-agent agent health [agent-id] [options]
```
...
npx cortex-agent agent health coder-lx7m9k2

# Continuous monitoring
npx cortex-agent agent health --watch

# Custom interval
npx cortex-agent agent health -w -i 10

# JSON output
npx cortex-agent agent health --format json
```
...
- `npx cortex-agent agent status` - Detailed agent info
- `npx cortex-agent agent metrics` - Performance metrics
- `npx cortex-agent doctor` - System-wide health
