...
```bash
npx cortex-agent auto agent [options]
```
...
```bash
npx cortex-agent auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx cortex-agent auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx cortex-agent auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx cortex-agent auto agent -t "Fix bug in login" -s minimal
```
