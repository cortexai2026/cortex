...
```bash
cortex-agent pair [--start] [options]
```
...
cortex-agent pair --start --agent senior-dev

# Start with verification
cortex-agent pair --start --verify --threshold 0.98
```
...
```bash
cortex-agent pair --start --mode switch --interval 10m
```
...
cortex-agent pair --end --save

# End and generate report
cortex-agent pair --end --report
```
...
cortex-agent pair --start --git --review-commit
```

### With Testing Frameworks
```bash
# Jest integration
cortex-agent pair --start --test-framework jest

# Pytest integration
cortex-agent pair --start --test-framework pytest
```
...
Configure pair programming in `.cortex-agent/config.json`:
