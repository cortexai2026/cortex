...
```bash
npx cortex-agent github swarm --repository owner/repo
```

### Maintenance-focused swarm

```bash
npx cortex-agent github swarm -r owner/repo -f maintenance --issue-labels
```

### Development swarm with PR automation

```bash
npx cortex-agent github swarm -r owner/repo -f development --auto-pr --code-review
```

### Full-featured triage swarm

```bash
npx cortex-agent github swarm -r owner/repo -a 8 -f triage --issue-labels --auto-pr
```
...
```javascript
mcp__cortex-agent__github_swarm {
