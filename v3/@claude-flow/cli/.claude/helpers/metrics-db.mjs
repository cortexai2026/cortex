#!/usr/bin/env node
/**
 * Cortex Agent V3 - Metrics Database Manager
...
const DB_PATH = join(PROJECT_ROOT, '.cortex-agent', 'metrics.db');
...
function checkSecurityFile(filename, minLines = 100) {
  const filePath = join(V3_DIR, '@cortex-agent/security/src', filename);
...
async function syncMetrics() {
  ...
  if (existsSync(modulesDir)) {
    ...
    for (const entry of entries) {
      // Skip hidden directories (like .agentic-flow, .cortex-agent)
...
function exportToJSON() {
  ...
  const securityDir = join(PROJECT_ROOT, '.cortex-agent/security');
