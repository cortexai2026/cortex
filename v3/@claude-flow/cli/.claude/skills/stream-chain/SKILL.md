...
```bash
cortex-agent stream-chain run <prompt1> <prompt2> [...] [options]
```
...
```bash
cortex-agent stream-chain pipeline refactor --timeout 60 --verbose
```

#### Debug Test Generation

```bash
cortex-agent stream-chain pipeline test --debug
```

#### Comprehensive Optimization

```bash
cortex-agent stream-chain pipeline optimize --timeout 90 --verbose
```
...
Define reusable pipelines in `.cortex-agent/config.json`:
...
cortex-agent stream-chain pipeline documentation
```
...
cortex-agent swarm init --topology mesh
...
# Results stored in .cortex-agent/memory/stream-chain/
```
...
```bash
cortex-agent stream-chain run "complex task" --timeout 120
```
...
```bash
cortex-agent stream-chain run "step 1" "step 2" --debug
```
...
cat .cortex-agent/config.json | grep -A 10 "streamChain"
```
