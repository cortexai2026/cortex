...
The Hive Mind system represents the pinnacle of multi-agent coordination in Cortex Agent, implementing a queen-led hierarchical architecture where a strategic queen coordinator directs specialized worker agents through collective decision-making and shared memory.
...
npx cortex-agent hive-mind init --config hive-config.json
```
...
npx cortex-agent hive-mind spawn "Build microservices architecture"

# Strategic queen type
npx cortex-agent hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
npx cortex-agent hive-mind spawn "Implement API" --queen-type tactical --max-workers 12

# Adaptive queen with consensus
npx cortex-agent hive-mind spawn "Optimize system" --queen-type adaptive --consensus byzantine

# Generate Claude Code commands
npx cortex-agent hive-mind spawn "Build full-stack app" --claude
```
...
npx cortex-agent hive-mind metrics

# Monitor collective memory
npx cortex-agent hive-mind memory
```
...
npx cortex-agent hive-mind sessions

# Pause a session
npx cortex-agent hive-mind pause <session-id>

# Resume a paused session
npx cortex-agent hive-mind resume <session-id>

# Stop a running session
npx cortex-agent hive-mind stop <session-id>
```
...
```bash
npx cortex-agent hive-mind spawn "Build REST API" --claude
```
...
npx cortex-agent sparc tdd "User authentication" --hive-mind
...
npx cortex-agent hive-mind spawn "Analyze repo quality" --objective "owner/repo"

# PR review coordination
npx cortex-agent hive-mind spawn "Review PR #123" --queen-type tactical
```
...
Hive Mind integrates with Cortex Agent hooks for automation:
...
```bash
npx cortex-agent hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution
```bash
npx cortex-agent hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks
```bash
npx cortex-agent hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
npx cortex-agent hive-mind memory --gc

# Optimize database
npx cortex-agent hive-mind memory --optimize

# Export and clear
npx cortex-agent hive-mind memory --export --clear
```
...
npx cortex-agent hive-mind spawn "..." --consensus weighted

# Or use simple majority
npx cortex-agent hive-mind spawn "..." --consensus majority
```
...
```bash
# Frontend hive
npx cortex-agent hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
npx cortex-agent hive-mind spawn "Build API" --name backend-hive
...
npx cortex-agent hive-mind export <session-id> --output backup.json

# Import session
npx cortex-agent hive-mind import backup.json
```
...
npx cortex-agent hive-mind spawn "Build e-commerce platform" \
...
```bash
# Spawn research hive
npx cortex-agent hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
npx cortex-agent hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
...
- [Hive Mind Documentation](https://github.com/ruvnet/cortex-agent/docs/hive-mind)
- [Collective Intelligence Patterns](https://github.com/ruvnet/cortex-agent/docs/patterns)
- [Byzantine Consensus](https://github.com/ruvnet/cortex-agent/docs/consensus)
- [Memory Optimization](https://github.com/ruvnet/cortex-agent/docs/memory)

---

**Skill Version**: 1.0.0
**Last Updated**: 2025-10-19
**Maintained By**: Cortex Agent Team
**License**: MIT
