...
```ts
import { createCapabilityAlgebra } from '@cortex-agent/guidance/capabilities';
...
```ts
import { getTrustBasedRateLimit } from '@cortex-agent/guidance/trust';
...
```ts
import { createAuthorityGate, createIrreversibilityClassifier } from '@cortex-agent/guidance/authority';
