...
The existing Cortex Agent V3 infrastructure provides hooks (`pre-edit`, `pre-command`, `post-task`), a memory subsystem (AgentDB + HNSW), and a headless execution mode (`claude -p --output-format json`). A control plane can leverage all three without modifying Claude Code itself.

## Decision

Build a separate package, `@cortex-agent/guidance`, that runs **beside** Claude Code as a parallel control plane. The control plane has five components:
...
- `v3/@cortex-agent/guidance/src/index.ts` -- `GuidanceControlPlane` orchestrator class
- `v3/@cortex-agent/guidance/src/compiler.ts` -- `GuidanceCompiler`
- `v3/@cortex-agent/guidance/src/retriever.ts` -- `ShardRetriever`
- `v3/@cortex-agent/guidance/src/gates.ts` -- `EnforcementGates`
- `v3/@cortex-agent/guidance/src/ledger.ts` -- `RunLedger`
- `v3/@cortex-agent/guidance/src/optimizer.ts` -- `OptimizerLoop`
- `v3/@cortex-agent/guidance/src/headless.ts` -- `HeadlessRunner`
- `v3/@cortex-agent/guidance/src/types.ts` -- All type definitions
