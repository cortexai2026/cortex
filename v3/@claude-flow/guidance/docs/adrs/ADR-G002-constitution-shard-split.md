...
- `v3/@cortex-agent/guidance/src/types.ts` -- `Constitution`, `RuleShard`, `PolicyBundle` type definitions
- `v3/@cortex-agent/guidance/src/compiler.ts` -- `buildConstitution()`, `buildShards()`, `CONSTITUTION_MARKERS`
- `v3/@cortex-agent/guidance/src/retriever.ts` -- `ShardRetriever.retrieve()`, `selectWithContradictionCheck()`
