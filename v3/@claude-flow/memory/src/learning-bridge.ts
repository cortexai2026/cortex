 ...
 *
 * @module @cortex-agent/memory/learning-bridge
 */
...
export interface LearningBridgeConfig {
   ...
   * When provided, this replaces the default dynamic import of @cortex-agent/neural.
...
export class LearningBridge extends EventEmitter {
  ...
  private async loadNeural(): Promise<void> {
    try {
      ...
      const mod = await import('@cortex-agent/neural' as string);
      ...
      // @cortex-agent/neural not installed or failed to initialize.
