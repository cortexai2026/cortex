...
This implementation adds **complete Windows cross-platform support** to the `@cortex-agent/memory` module using sql.js as a WASM-based SQLite fallback when native compilation fails.
...
```typescript
import { getPlatformInfo, getAvailableProviders } from '@cortex-agent/memory';
...
RUN npm install @cortex-agent/memory
