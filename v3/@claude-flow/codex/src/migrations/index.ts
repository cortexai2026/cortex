...
const WARNING_PATTERNS: Array<{ pattern: RegExp | string; message: string }> = [
  ...
  { pattern: /mcp__cortex-agent__/g, message: 'MCP tool calls need migration to Codex MCP configuration' },
...
export function generateAgentsMdFromParsed(parsed: ParsedClaudeMd): string {
  ...
  lines.push('');
  lines.push('> Migrated from CLAUDE.md by @cortex-agent/codex');
  lines.push('');
...
export function convertSettingsToToml(settings: Record<string, unknown>): string {
  ...
  lines.push('# Generated by @cortex-agent/codex');
  lines.push('');
  ...
  if (settings.mcpServers && typeof settings.mcpServers === 'object') {
    ...
    // Add default cortex-agent server
    lines.push('[mcp_servers.cortex-agent]');
    lines.push('command = "npx"');
    lines.push('args = ["-y", "@cortex-agent/cli@latest"]');
    lines.push('enabled = true');
    lines.push('');
  }
...
export function generateConfigTomlFromParsed(parsed: ParsedClaudeMd): string {
  ...
  if (parsed.mcpServers.length > 0) {
    ...
    // Default cortex-agent server
    lines.push('[mcp_servers.cortex_agent]');
    lines.push('command = "npx"');
    lines.push('args = ["-y", "@cortex-agent/cli@latest"]');
    lines.push('enabled = true');
    lines.push('');
