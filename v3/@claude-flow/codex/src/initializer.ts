/**
 * @cortex-agent/codex - CodexInitializer
 *
...
export class CodexInitializer {
  ...
  async initialize(options: CodexInitOptions): Promise<CodexInitResult> {
    ...
    try {
      ...
      if (mcpResult.registered) {
        filesCreated.push('MCP server (cortex-agent) registered');
      }
  ...
  /**
   * Register cortex-agent as MCP server with Codex
   */
  private async registerMCPServer(): Promise<{ registered: boolean; warning?: string }> {
    try {
      ...
      try {
        ...
        return {
          registered: false,
          warning: 'Codex CLI not found. Run: codex mcp add cortex-agent -- npx cortex-agent mcp start',
        };
      }
      ...
      try {
        execSync('codex mcp add cortex-agent -- npx cortex-agent mcp start', {
          stdio: 'pipe',
          timeout: 10000,
        });
        ...
        return {
          registered: false,
          warning: `Failed to register MCP server: ${errorMessage}. Run manually: codex mcp add cortex-agent -- npx cortex-agent mcp start`,
        };
      }
    } catch {
      return {
        registered: false,
        warning: 'Could not register MCP server. Run manually: codex mcp add cortex-agent -- npx cortex-agent mcp start',
      };
    }
  }
...
- Cortex Agent: https://github.com/ruvnet/cortex-agent
`;
  }
...
*Generated by @cortex-agent/codex - Dual platform mode*
`;
