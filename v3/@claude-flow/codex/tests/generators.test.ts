/**
 * @cortex-agent/codex - Generator Tests
 *
...
describe('generateAgentsMd', () => {
  ...
  describe('default template', () => {
    ...
    it('should include commit message format', async () => {
      ...
      expect(result).toContain('Co-Authored-By: cortex-agent');
...
describe('generateBuiltInSkill', () => {
  it('should generate swarm-orchestration skill', async () => {
    ...
    expect(result.skillMd).toContain('npx @cortex-agent/cli swarm init');
  });
...
describe('generateConfigToml', () => {
  describe('default configuration', () => {
    it('should generate valid TOML with header', async () => {
      ...
      expect(result).toContain('# Cortex Agent V3 - Codex Configuration');
      expect(result).toContain('# Generated by: @cortex-agent/codex');
      expect(result).toContain('# Documentation: https://github.com/ruvnet/cortex-agent');
    });
    ...
    it('should include default cortex-agent MCP server', async () => {
      const result = await generateConfigToml();

      expect(result).toContain('[mcp_servers.cortex-agent]');
      expect(result).toContain('command = "npx"');
      expect(result).toContain('args = ["-y", "@cortex-agent/cli@latest"]');
  ...
  describe('MCP servers configuration', () => {
    ...
    it('should not duplicate cortex-agent server if already specified', async () => {
      const options: ConfigTomlOptions = {
        mcpServers: [
          {
            name: 'cortex-agent',
            command: 'custom-cortex-agent',
            args: ['--custom'],
          },
        ],
      };
      ...
      const matches = result.match(/\[mcp_servers\.cortex-agent\]/g);
      expect(matches).toHaveLength(1);
      expect(result).toContain('command = "custom-cortex-agent"');
    });
  });
...
describe('generateMinimalConfigToml', () => {
  it('should generate minimal config', async () => {
    ...
    expect(result).toContain('# Cortex Agent V3 - Minimal Codex Configuration');
    expect(result).toContain('model = "gpt-5.3-codex"');
    expect(result).toContain('approval_policy = "on-request"');
    expect(result).toContain('sandbox_mode = "workspace-write"');
    expect(result).toContain('[mcp_servers.cortex-agent]');
...
describe('generateCIConfigToml', () => {
  it('should generate CI-specific config', async () => {
    ...
    expect(result).toContain('# Cortex Agent V3 - CI/CD Pipeline Configuration');
    expect(result).toContain('approval_policy = "never"');
