...
function generateEnhancedMarkdown(report: FullValidationReport, options: ReportOptions): string {
  ...
  lines.push('|-----------|-----------|');
  lines.push('| cortex-agent/hive-mind | @cortex-agent/swarm |');
  lines.push('| cortex-agent/swarm | @cortex-agent/swarm |');
  lines.push('| cortex-agent/memory | @cortex-agent/memory |');
  lines.push('| cortex-agent/agents | @cortex-agent/agent-lifecycle |');
  lines.push('| cortex-agent/tasks | @cortex-agent/task-execution |');
  lines.push('');
...
function generateExamplesSection(): string[] {
  ...
  lines.push('npx cortex-agent hive-mind status');
  lines.push('');
  lines.push('# V3 (recommended)');
  lines.push('npx @cortex-agent/cli swarm init');
  lines.push('npx @cortex-agent/cli swarm status');
  lines.push('```');
  lines.push('');
  ...
  lines.push("import { MemoryManager } from 'cortex-agent/memory';");
  lines.push('');
  lines.push('// V3 imports with aliases');
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@cortex-agent/swarm';");
  lines.push("import { UnifiedMemoryService as MemoryManager } from '@cortex-agent/memory';");
  lines.push('');
...
function generateMigrationScriptsSection(): string[] {
  ...
  lines.push('npx @cortex-agent/cli migrate --from v2 --to v3');
  lines.push('');
  lines.push('# Migrate configuration');
  lines.push('npx @cortex-agent/cli migrate config --input .cortex-agent/config.yaml');
  lines.push('');
  lines.push('# Migrate memory database');
  lines.push('npx @cortex-agent/cli migrate memory --input .cortex-agent/memory.db');
  lines.push('```');
  lines.push('');
  ...
  lines.push('');
  lines.push('```yaml');
  lines.push('# V2 Configuration (.cortex-agent/config.yaml)');
  lines.push('orchestrator:');
  lines.push('  maxAgents: 10');
  lines.push('  defaultStrategy: balanced');
  lines.push('memory:');
  lines.push('  backend: sqlite');
  lines.push('  path: ./.cortex-agent/memory.db');
  lines.push('coordination:');
  lines.push('  topology: hierarchical');
  lines.push('');
  lines.push('# V3 Configuration (.cortex-agent/config.yaml)');
  lines.push('swarm:');
