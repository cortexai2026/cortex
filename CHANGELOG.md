...
All notable changes to the Cortex Agent project (formerly Cortex Agent) are documented here.
...
### Cortex Agent v3.5 — First Major Stable Release

This release marks the official rebranding from **Cortex Agent** to **Cortex Agent** and represents the first major stable release after 5,800+ commits, 55 alpha iterations, and 10 months of development.

### Highlights

- **Rebranding**: Cortex Agent → Cortex Agent across all packages (`@cortex-agent/cli`, `cortex-agent`, `cortex-agent`)
...
- Tiered embedding resolution: ReasoningBank WASM (Tier 1) → @cortex-agent/embeddings (Tier 2) → mock fallback (Tier 3)
...
### Cortex Agent Branding Fix

- Fixed CLI branding: show 'cortex-agent' instead of 'cortex-agent' when run via `npx cortex-agent`
...
- **@cortex-agent/teammate-plugin**: MCP tools for Agent Teams coordination
...
### v3.0.0-alpha.100 — @cortex-agent/guidance (2025-11)

- **@cortex-agent/guidance Control Plane**: Governance, compliance, and policy enforcement
...
- Auto-update system for @cortex-agent packages (ADR-025)
- Replaced all mock implementations with real functionality

### v3.0.0-alpha.1 — Foundation (2025-10)

- Complete V3 monorepo structure (`@cortex-agent/cli`, `shared`, `memory`, `hooks`, `security`)
...
| **Cortex Agent v3.5** | **v3.5.0** | **2026-02-27** | **First stable release, rebranding** |
